const RAW_AIRPORT_CODES = {
  AAA: ["Anaa Airport"],
  AAB: ["Arrabury Airport"],
  AAC: ["El Arish International Airport", "El Arish"],
  AAD: ["Adado Airport", "Adado"],
  AAE: ["Rabah Bitat Airport", "Annaba"],
  AAF: ["Apalachicola Regional Airport", "Apalachicola"],
  AAG: ["Arapoti Airport", "Arapoti"],
  AAH: ["Aachen-Merzbr\u00fcck Airport", "Aachen"],
  AAI: ["Arraias Airport", "Arraias"],
  AAJ: ["Cayana Airstrip", "Awaradam"],
  AAK: ["Buariki Airport", "Buariki"],
  AAL: ["Aalborg Airport", "Aalborg"],
  AAM: ["Malamala Airport", "Malamala"],
  AAN: ["Al Ain International Airport", "Al Ain"],
  AAO: ["Anaco Airport", "Anaco"],
  AAP: ["Andrau Airpark", "Houston"],
  AAQ: ["Anapa Vityazevo Airport", "Anapa"],
  AAR: ["Aarhus Airport", "Aarhus"],
  AAS: ["Apalapsili Airport", "Apalapsili"],
  AAT: ["Altay Air Base", "Altay"],
  AAU: ["Asau Airport", "Asau"],
  AAV: ["Allah Valley Airport", "Surallah"],
  AAX: ["Romeu Zema Airport", "Arax\u00e1"],
  AAY: ["Al Ghaidah International Airport"],
  AAZ: ["Quezaltenango Airport", "Quezaltenango"],
  ABA: ["Abakan Airport", "Abakan"],
  ABB: ["Asaba International Airport", "Asaba"],
  ABC: ["Albacete-Los Llanos Airport", "Albacete"],
  ABD: ["Abadan Airport", "Abadan"],
  ABE: ["Lehigh Valley International Airport", "Allentown"],
  ABF: ["Abaiang Airport", "Abaiang"],
  ABG: ["Abingdon Downs Airport"],
  ABH: ["Alpha Airport", "Alpha"],
  ABI: ["Abilene Regional Airport", "Abilene"],
  ABJ: ["Port Bouet Airport", "Abidjan"],
  ABK: ["Kabri Dehar Airport", "Kabri Dehar"],
  ABL: ["Ambler Airport", "Ambler"],
  ABM: ["Northern Peninsula Airport", "Bamaga"],
  ABN: ["Albina Airport", "Albina"],
  ABO: ["Aboisso Airport", "Aboisso"],
  ABP: ["Atkamba Airport", "Atkamba Mission"],
  ABQ: ["Albuquerque International Sunport", "Albuquerque"],
  ABR: ["Aberdeen Regional Airport", "Aberdeen"],
  ABS: ["Abu Simbel Airport", "Abu Simbel"],
  ABT: ["Al Baha Airport"],
  ABU: ["Haliwen Airport", "Atambua-Timor Island"],
  ABV: ["Nnamdi Azikiwe International Airport", "Abuja"],
  ABW: ["Abau Airport", "Abau"],
  ABX: ["Albury Airport", "Albury"],
  ABY: ["Southwest Georgia Regional Airport", "Albany"],
  ABZ: ["Aberdeen Dyce Airport", "Aberdeen"],
  ACA: ["General Juan N Alvarez International Airport", "Acapulco"],
  ACB: ["Antrim County Airport", "Bellaire"],
  ACC: ["Kotoka International Airport", "Accra"],
  ACD: ["Alcides Fern\u00e1ndez Airport", "Acand\u00ed"],
  ACE: ["Lanzarote Airport", "Lanzarote Island"],
  ACF: ["Brisbane Archerfield Airport", "Brisbane"],
  ACH: ["St Gallen Altenrhein Airport", "Altenrhein"],
  ACI: ["Alderney Airport", "Saint Anne"],
  ACJ: ["Anuradhapura Air Force Base", "Anuradhapura"],
  ACK: ["Nantucket Memorial Airport", "Nantucket"],
  ACL: ["Aguaclara Airport", "Aguaclara"],
  ACM: ["Arica Airport", "Arica"],
  ACN: ["Ciudad Acu\u00f1a New International Airport", "Ciudad Acu\u00f1a"],
  ACO: ["Ascona Airport", "Ascona"],
  ACP: ["Sahand Airport", "Maragheh"],
  ACR: ["Araracuara Airport", "Araracuara"],
  ACS: ["Achinsk Airport", "Achinsk"],
  ACT: ["Waco Regional Airport", "Waco"],
  ACU: ["Achutupo Airport", "Achutupo"],
  ACV: ["California Redwood Coast-Humboldt County Airport", "Arcata/Eureka"],
  ACX: ["Xingyi Airport", "Xingyi"],
  ACY: ["Atlantic City International Airport", "Atlantic City"],
  ACZ: ["Zabol Airport"],
  ADA: ["Adana Airport", "Adana"],
  ADB: ["Adnan Menderes International Airport", "\u0130zmir"],
  ADC: ["Andakombe Airport", "Andekombe"],
  ADD: ["Addis Ababa Bole International Airport", "Addis Ababa"],
  ADE: ["Aden International Airport", "Aden"],
  ADF: ["Ad\u0131yaman Airport", "Ad\u0131yaman"],
  ADG: ["Lenawee County Airport", "Adrian"],
  ADH: ["Aldan Airport", "Aldan"],
  ADI: ["Arandis Airport", "Arandis"],
  ADJ: ["Amman-Marka International Airport", "Amman"],
  ADK: ["Adak Airport", "Adak Island"],
  ADL: ["Adelaide International Airport", "Adelaide"],
  ADM: ["Ardmore Municipal Airport", "Ardmore"],
  ADN: ["Andes Airport", "Andes"],
  ADO: ["Andamooka Airport"],
  ADP: ["Ampara Airport", "Ampara"],
  ADQ: ["Kodiak Airport", "Kodiak"],
  ADR: ["Robert F Swinnie Airport", "Andrews"],
  ADS: ["Addison Airport", "Dallas"],
  ADT: ["Ada Regional Airport", "Ada"],
  ADU: ["Ardabil Airport", "Ardabil"],
  ADV: ["El Daein Airport", "El Daein"],
  ADW: ["Joint Base Andrews", "Camp Springs"],
  ADX: ["RAF Leuchars", "St. Andrews"],
  ADY: ["Alldays Airport", "Alldays"],
  ADZ: ["Gustavo Rojas Pinilla International Airport", "San Andr\u00e9s"],
  AEA: ["Abemama Atoll Airport", "Abemama Atoll"],
  AEB: ["Baise Youjiang Airport", "Baise"],
  AEE: ["Adareil Airport"],
  AEG: ["Aek Godang Airport", "Padang Sidempuan-Sumatra Island"],
  AEH: ["Abeche Airport"],
  AEI: ["Algeciras Heliport", "Algeciras"],
  AEK: ["Aseki Airport", "Aseki"],
  AEL: ["Albert Lea Municipal Airport", "Albert Lea"],
  AEM: ["Amgu Airport", "Amgu"],
  AEO: ["Aioun el Atrouss Airport", "Aioun El Atrouss"],
  AEP: ["Jorge Newbery Airpark", "Buenos Aires"],
  AEQ: ["Ar Horqin Airport", "Chifeng"],
  AER: ["Sochi International Airport", "Sochi"],
  AES: ["\u00c5lesund Airport", "\u00c5lesund"],
  AET: ["Allakaket Airport", "Allakaket"],
  AEU: ["Abu Musa Island Airport", "Abu Musa"],
  AEX: ["Alexandria International Airport", "Alexandria"],
  AEY: ["Akureyri Airport", "Akureyri"],
  AFA: ["Suboficial Ay Santiago Germano Airport", "San Rafael"],
  AFD: ["Port Alfred Airport", "Port Alfred"],
  AFF: ["USAF Academy Airfield", "Colorado Springs"],
  AFI: ["Amalfi Airport", "Amalfi"],
  AFK: ["Kondavattavana Tank Seaplane Base", "Ampara"],
  AFL: ["Piloto Osvaldo Marques Dias Airport", "Alta Floresta"],
  AFN: ["Jaffrey Airport Silver Ranch Airport", "Jaffrey"],
  AFO: ["Afton Municipal Airport", "Afton"],
  AFR: ["Afore Airstrip"],
  AFS: ["Sugraly Airport", "Zarafshan"],
  AFT: ["Afutara Aerodrome", "Bila"],
  AFW: ["Fort Worth Alliance Airport", "Fort Worth"],
  AFY: ["Afyon Airport", "Afyonkarahisar"],
  AFZ: ["Sabzevar National Airport", "Sabzevar"],
  AGA: ["Al Massira Airport", "Agadir"],
  AGB: ["Augsburg Airport", "Augsburg"],
  AGC: ["Allegheny County Airport", "Pittsburgh"],
  AGD: ["Anggi Airport", "Anggi-Papua Island"],
  AGE: ["Wangerooge Airport", "Wangerooge"],
  AGF: ["Agen-La Garenne Airport", "Agen/La Garenne"],
  AGG: ["Angoram Airport", "Angoram"],
  AGH: ["\u00c4ngelholm-Helsingborg Airport", "\u00c4ngelholm"],
  AGI: ["Wageningen Airstrip", "Wageningen"],
  AGJ: ["Aguni Airport", "Aguni"],
  AGK: ["Kagua Airport", "Kagua"],
  AGL: ["Wanigela Airport"],
  AGN: ["Angoon Seaplane Base", "Angoon"],
  AGO: ["Ralph C Weiser Field", "Magnolia"],
  AGP: ["M\u00e1laga Airport", "M\u00e1laga"],
  AGQ: ["Agrinion Air Base", "Agrinion"],
  AGR: ["Agra Airport"],
  AGS: ["Augusta Regional At Bush Field", "Augusta"],
  AGT: ["Guarani International Airport", "Ciudad del Este"],
  AGU: ["Jes\u00fas Ter\u00e1n Paredo International Airport", "Aguascalientes"],
  AGV: ["Oswaldo Guevara Mujica Airport", "Acarigua"],
  AGW: ["Agnew Airport", "Agnew"],
  AGX: ["Agatti Airport"],
  AGY: ["Argyle Downs Airport", "Argyle Downs"],
  AGZ: ["Aggeneys Airport", "Aggeneys"],
  AHB: ["Abha Regional Airport", "Abha"],
  AHC: ["Amedee Army Air Field", "Herlong"],
  AHD: ["Ardmore Downtown Executive Airport", "Ardmore"],
  AHE: ["Ahe Airport", "Ahe Atoll"],
  AHF: ["Arapahoe Municipal Airport", "Arapahoe"],
  AHH: ["Amery Municipal Airport", "Amery"],
  AHI: ["Amahai Airport", "Amahai-Seram Island"],
  AHJ: ["Hongyuan Airport", "Aba"],
  AHL: ["Aishalton Airport", "Aishalton"],
  AHM: ["Ashland Municipal Sumner Parker Field", "Ashland"],
  AHN: ["Athens Ben Epps Airport", "Athens"],
  AHO: ["Alghero-Fertilia Airport", "Alghero"],
  AHS: ["Ahuas Airport", "Ahuas"],
  AHT: ["Amchitka Army Airfield", "Amchitka Island"],
  AHU: ["Cherif Al Idrissi Airport", "Al Hoceima"],
  AHY: ["Ambatolhy Airport", "Ambatolahy"],
  AHZ: ["Altiport L'Alpe d'Huez - Henri GIRAUD", "L'Alpe d'Huez"],
  AIA: ["Alliance Municipal Airport", "Alliance"],
  AIC: ["Ailinglaplap Airok Airport", "Bigatyelang Island"],
  AID: ["Anderson Municipal Darlington Field", "Anderson"],
  AIE: ["Aiome Airport", "Aiome"],
  AIF: ["Marcelo Pires Halzhausen Airport", "Assis"],
  AIG: ["Yalinga Airport", "Yalinga"],
  AIH: ["Aiambak Airport", "Aiambak"],
  AII: ["Ali-Sabieh Airport", "Ali-Sabieh"],
  AIK: ["Aiken Regional Airport", "Aiken"],
  AIL: ["Alligandi Airport", "Alligandi"],
  AIM: ["Ailuk Airport", "Ailuk Island"],
  AIN: ["Wainwright Airport", "Wainwright"],
  AIO: ["Atlantic Municipal Airport", "Atlantic"],
  AIP: ["Adampur Airport", "Adampur"],
  AIR: ["Aripuan\u00e3 Airport", "Aripuan\u00e3"],
  AIS: ["Arorae Island Airport", "Arorae Island"],
  AIT: ["Aitutaki Airport", "Aitutaki"],
  AIU: ["Enua Airport", "Atiu Island"],
  AIV: ["George Downer Airport", "Aliceville"],
  AIW: ["Ai-Ais Airport", "Ai-Ais"],
  AIY: ["Atlantic City Municipal Bader Field", "Atlantic City"],
  AIZ: ["Lee C Fine Memorial Airport", "Kaiser Lake Ozark"],
  AJA: [
    "Ajaccio-Napol\u00e9on Bonaparte Airport",
    "Ajaccio/Napol\u00e9on Bonaparte",
  ],
  AJF: ["Al-Jawf Domestic Airport", "Al-Jawf"],
  AJI: ["A\u011fr\u0131 Airport"],
  AJJ: ["Akjoujt Airport", "Akjoujt"],
  AJK: ["Arak Airport", "Araak"],
  AJL: ["Lengpui Airport", "Aizawl"],
  AJN: ["Ouani Airport", "Ouani"],
  AJR: ["Arvidsjaur Airport", "Arvidsjaur"],
  AJS: ["Abreojos Airport", "Abreojos"],
  AJU: ["Santa Maria Airport", "Aracaju"],
  AJY: ["Mano Dayak International Airport", "Agadez"],
  AKA: ["Ankang Wulipu Airport", "Ankang"],
  AKB: ["Atka Airport", "Atka"],
  AKC: ["Akron Fulton International Airport", "Akron"],
  AKD: ["Akola Airport", "Akola"],
  AKE: ["Akieni Airport", "Akieni"],
  AKF: ["Kufra Airport", "Kufra"],
  AKG: ["Anguganak Airport", "Anguganak"],
  AKH: ["Prince Sultan Air Base"],
  AKI: ["Akiak Airport", "Akiak"],
  AKJ: ["Asahikawa Airport", "Asahikawa / Hokkaid\u014d"],
  AKK: ["Akhiok Airport", "Akhiok"],
  AKL: ["Auckland International Airport", "Auckland"],
  AKM: ["Zakuoma Airport", "ZaKouma"],
  AKN: ["King Salmon Airport", "King Salmon"],
  AKO: ["Colorado Plains Regional Airport", "Akron"],
  AKP: ["Anaktuvuk Pass Airport", "Anaktuvuk Pass"],
  AKQ: ["Gunung Batin Airport", "Astraksetra-Sumatra Island"],
  AKR: ["Akure Airport", "Akure"],
  AKS: ["Gwaunaru'u Airport", "Auki"],
  AKT: ["RAF Akrotiri", "Akrotiri"],
  AKU: ["Aksu Airport", "Aksu"],
  AKV: ["Akulivik Airport", "Akulivik"],
  AKW: ["Aghajari Airport", "Aghajari"],
  AKX: ["Aktobe Airport", "Aktyubinsk"],
  AKY: ["Sittwe Airport", "Sittwe"],
  ALA: ["Almaty Airport", "Almaty"],
  ALB: ["Albany International Airport", "Albany"],
  ALC: ["Alicante International Airport", "Alicante"],
  ALD: ["Alerta Airport", "Fortaleza"],
  ALE: ["Alpine Casparis Municipal Airport", "Alpine"],
  ALF: ["Alta Airport", "Alta"],
  ALG: ["Houari Boumediene Airport", "Algiers"],
  ALH: ["Albany Airport", "Albany"],
  ALI: ["Alice International Airport", "Alice"],
  ALJ: ["Alexander Bay Airport", "Alexander Bay"],
  ALL: ["Villanova D'Albenga International Airport", "Albenga (SV)"],
  ALM: ["Alamogordo White Sands Regional Airport", "Alamogordo"],
  ALN: ["St Louis Regional Airport", "Alton/St Louis"],
  ALO: ["Waterloo Regional Airport", "Waterloo"],
  ALP: ["Aleppo International Airport", "Aleppo"],
  ALQ: ["Alegrete Novo Airport", "Alegrete"],
  ALR: ["Alexandra Airport", "Alexandra"],
  ALS: ["San Luis Valley Regional Bergman Field", "Alamosa"],
  ALT: ["Alenquer Airport", "Alenquer"],
  ALU: ["Alula Airport", "Alula"],
  ALV: ["Andorra la Vella Heliport", "Andorra La Vella"],
  ALW: ["Walla Walla Regional Airport", "Walla Walla"],
  ALX: ["Thomas C Russell Field", "Alexander City"],
  ALY: ["El Nouzha Airport", "Alexandria"],
  ALZ: ["Alitak Seaplane Base", "Lazy Bay"],
  AMA: ["Rick Husband Amarillo International Airport", "Amarillo"],
  AMB: ["Ambilobe Airport"],
  AMC: ["Am Timan Airport", "Am Timan"],
  AMD: ["Sardar Vallabhbhai Patel International Airport", "Ahmedabad"],
  AME: ["Alto Molocue Airport", "Alto Molocue"],
  AMF: ["Ama Airport"],
  AMH: ["Arba Minch Airport"],
  AMI: ["Selaparang Airport", "Mataram-Lombok Island"],
  AMJ: ["Cirilo Queir\u00f3z Airport", "Almenara"],
  AMK: ["Animas Air Park", "Durango"],
  AML: ["Puerto Armuelles Airport", "Puerto Armuelles"],
  AMM: ["Queen Alia International Airport", "Amman"],
  AMN: ["Gratiot Community Airport", "Alma"],
  AMO: ["Mao Airport", "Mao"],
  AMP: ["Ampanihy Airport", "Ampanihy"],
  AMQ: ["Pattimura Airport, Ambon", "Ambon"],
  AMS: ["Amsterdam Airport Schiphol", "Amsterdam"],
  AMT: ["Amata Airport"],
  AMU: ["Amanab Airport", "Amanab"],
  AMV: ["Amderma Airport", "Amderma"],
  AMW: ["Ames Municipal Airport", "Ames"],
  AMX: ["Ammaroo Airport"],
  AMY: ["Ambatomainty Airport"],
  AMZ: ["Ardmore Airport", "Manurewa"],
  ANA: ["Angama Airport", "Maasai Mara"],
  ANB: ["Anniston Regional Airport", "Anniston"],
  ANC: ["Ted Stevens Anchorage International Airport", "Anchorage"],
  AND: ["Anderson Regional Airport", "Anderson"],
  ANE: ["Angers-Loire Airport", "Angers/Marc\u00e9"],
  ANF: ["Andr\u00e9s Sabella G\u00e1lvez International Airport", "Antofagasta"],
  ANG: [
    "Angoul\u00eame-Brie-Champniers Airport",
    "Angoul\u00eame/Brie/Champniers",
  ],
  ANH: ["Anuha Island Resort Airport", "Anhua Island"],
  ANI: ["Aniak Airport", "Aniak"],
  ANJ: ["Zanaga Airport", "Zanaga"],
  ANK: ["Etimesgut Air Base", "Ankara"],
  ANL: ["Andulo Airport"],
  ANM: ["Antsirabato Airport"],
  ANN: ["Annette Island Airport", "Annette"],
  ANO: ["Angoche Airport", "Angoche"],
  ANP: ["Lee Airport", "Annapolis"],
  ANQ: ["Tri State Steuben County Airport", "Angola"],
  ANR: ["Antwerp International Airport (Deurne)", "Antwerp"],
  ANS: ["Andahuaylas Airport", "Andahuaylas"],
  ANU: ["V.C. Bird International Airport", "St. John's"],
  ANV: ["Anvik Airport", "Anvik"],
  ANW: ["Ainsworth Regional Airport", "Ainsworth"],
  ANX: ["And\u00f8ya Airport", "Andenes"],
  ANY: ["Anthony Municipal Airport", "Anthony"],
  ANZ: ["Angus Downs Airport", "Angus Downs Station"],
  AOA: ["Aroa Airport", "Agevairu"],
  AOB: ["Annanberg Airport"],
  AOC: ["Altenburg-Nobitz Airport", "Altenburg"],
  AOD: ["Abou-De\u00efa Airport", "Abou-De\u00efa"],
  AOE: ["Anadolu Airport", "Eski\u015fehir"],
  AOG: ["Anshan Air Base", "Anshan"],
  AOH: ["Lima Allen County Airport", "Lima"],
  AOI: ["Ancona Falconara Airport", "Ancona"],
  AOJ: ["Aomori Airport", "Aomori"],
  AOK: ["Karpathos Airport", "Karpathos Island"],
  AOL: ["Paso De Los Libres Airport", "Paso de los Libres"],
  AOM: ["Adam Airport", "Adam"],
  AOO: ["Altoona Blair County Airport", "Altoona"],
  AOP: ["Alferez FAP Alfredo Vladimir Sara Bauer Airport", "Andoas"],
  AOR: ["Sultan Abdul Halim Airport", "Alor Satar"],
  AOS: ["Amook Bay Seaplane Base", "Amook Bay"],
  AOT: ["Aosta Airport", "Aosta"],
  AOU: ["Attopeu Airport", "Attopeu"],
  APA: ["Centennial Airport", "Denver"],
  APB: ["Apolo Airport", "Apolo"],
  APC: ["Napa County Airport", "Napa"],
  APE: ["San Juan Aposento Airport", "San Juan Aposento"],
  APF: ["Naples Municipal Airport", "Naples"],
  APG: ["Phillips Army Air Field", "Aberdeen Proving Grounds(Aberdeen)"],
  APH: ["A P Hill AAF (Fort A P Hill) Airport", "Fort A. P. Hill"],
  API: ["Gomez Nino Apiay Air Base", "Apiay"],
  APK: ["Apataki Airport", "Apataki"],
  APL: ["Nampula Airport", "Nampula"],
  APN: ["Alpena County Regional Airport", "Alpena"],
  APO: ["Antonio Roldan Betancourt Airport", "Carepa"],
  APP: ["Asapa Airport"],
  APQ: ["Arapiraca Airport", "Arapiraca"],
  APR: ["April River Airport", "April River"],
  APS: ["An\u00e1polis Airport", "An\u00e1polis"],
  APT: ["Marion County Brown Field", "Jasper"],
  APU: ["Captain Jo\u00e3o Busse Airport", "Apucarana"],
  APV: ["Apple Valley Airport", "Apple Valley"],
  APW: ["Faleolo International Airport", "Apia"],
  APX: ["Arapongas Airport", "Arapongas"],
  APY: ["Alto Parna\u00edba Airport", "Alto Parna\u00edba"],
  APZ: ["Zapala Airport", "Zapala"],
  AQA: ["Araraquara Airport", "Araraquara"],
  AQB: ["Santa Cruz del Quiche Airport", "Santa Cruz del Quiche"],
  AQG: ["Anqing Tianzhushan Airport", "Anqing"],
  AQI: ["Al Qaisumah/Hafr Al Batin Airport", "Qaisumah"],
  AQJ: ["Aqaba King Hussein International Airport", "Aqaba"],
  AQM: ["Nova Vida Airport", "Ariquemes"],
  AQP: ["Rodr\u00edguez Ball\u00f3n International Airport", "Arequipa"],
  AQS: ["Biaugunu Airport", "Saqani, Cakaudrove"],
  AQY: ["Girdwood Airport", "Girdwood"],
  ARA: ["Acadiana Regional Airport", "New Iberia"],
  ARB: ["Ann Arbor Municipal Airport", "Ann Arbor"],
  ARC: ["Arctic Village Airport", "Arctic Village"],
  ARD: ["Mali Airport", "Alor Island"],
  ARE: ["Antonio Nery Juarbe Pol Airport", "Arecibo"],
  ARF: ["Acaricuara Airport", "Acaricuara"],
  ARG: ["Walnut Ridge Regional Airport", "Walnut Ridge"],
  ARH: ["Talagi Airport", "Archangelsk"],
  ARI: ["Chacalluta Airport", "Arica"],
  ARJ: ["Arso Airport", "Arso-Papua Island"],
  ARK: ["Arusha Airport", "Arusha"],
  ARL: ["Arly Airport", "Arly"],
  ARM: ["Armidale Airport", "Armidale"],
  ARN: ["Stockholm-Arlanda Airport", "Stockholm"],
  ARO: ["Arboletes Airport", "Arboletes"],
  ARP: ["Aragip Airport"],
  ARQ: ["El Troncal Airport", "Arauquita"],
  ARR: ["D. Casimiro Szlapelis Airport", "Alto Rio Senguerr"],
  ARS: ["Aragar\u00e7as Airport", "Aragar\u00e7as"],
  ART: ["Watertown International Airport", "Watertown"],
  ARU: ["Ara\u00e7atuba Airport", "Ara\u00e7atuba"],
  ARV: ["Lakeland-Noble F. Lee Memorial field", "Minocqua-Woodruff"],
  ARW: ["Arad International Airport", "Arad"],
  ARX: ["Asbury Park Neptune Air Terminal", "Asbury Park"],
  ARY: ["Ararat Airport"],
  ARZ: ["N'zeto Airport", "N'zeto"],
  ASA: ["Assab International Airport", "Asab"],
  ASB: ["Ashgabat International Airport", "Ashgabat"],
  ASC: ["Ascenci\u00f3n De Guarayos Airport", "Ascensi\u00f3n de Guarayos"],
  ASD: ["Andros Town Airport"],
  ASE: ["Aspen-Pitkin Co/Sardy Field", "Aspen"],
  ASF: ["Astrakhan Airport", "Astrakhan"],
  ASG: ["Ashburton Aerodrome"],
  ASH: ["Boire Field", "Nashua"],
  ASI: ["RAF Ascension Island", "Ascension Island"],
  ASJ: ["Amami Airport", "Amami"],
  ASK: ["Yamoussoukro Airport", "Yamoussoukro"],
  ASL: ["Harrison County Airport", "Marshall"],
  ASM: ["Asmara International Airport", "Asmara"],
  ASN: ["Talladega Municipal Airport", "Talladega"],
  ASO: ["Asosa Airport", "Asosa"],
  ASP: ["Alice Springs Airport", "Alice Springs"],
  ASQ: ["Austin Airport", "Austin"],
  ASR: ["Kayseri Erkilet Airport", "Kayseri"],
  ASS: ["Arathusa Safari Lodge Airport", "Arathusa"],
  AST: ["Astoria Regional Airport", "Astoria"],
  ASU: ["Silvio Pettirossi International Airport", "Asunci\u00f3n"],
  ASV: ["Amboseli Airport", "Amboseli National Park"],
  ASW: ["Aswan International Airport", "Aswan"],
  ASX: ["John F Kennedy Memorial Airport", "Ashland"],
  ASY: ["Ashley Municipal Airport", "Ashley"],
  ASZ: ["Asirim Airport"],
  ATA: ["Comandante FAP German Arias Graziani Airport", "Anta"],
  ATB: ["Atbara Airport", "Atbara"],
  ATC: ["Arthur's Town Airport", "Arthur's Town"],
  ATD: ["Uru Harbour Airport", "Atoifi"],
  ATE: ["Antlers Municipal Airport", "Antlers"],
  ATF: ["Chacho\u00e1n Airport", "Ambato"],
  ATG: ["Minhas Air Base", "Kamra"],
  ATH: ["Eleftherios Venizelos International Airport", "Athens"],
  ATI: ["Artigas International Airport", "Artigas"],
  ATJ: ["Antsirabe Airport", "Antsirabe"],
  ATK: ["Atqasuk Edward Burnell Sr Memorial Airport", "Atqasuk"],
  ATL: ["Hartsfield Jackson Atlanta International Airport", "Atlanta"],
  ATM: ["Altamira Airport", "Altamira"],
  ATN: ["Namatanai Airport", "Namatanai"],
  ATO: ["Ohio University Snyder Field", "Athens/Albany"],
  ATP: ["Aitape Airport", "Aitape"],
  ATQ: ["Sri Guru Ram Dass Jee International Airport", "Amritsar"],
  ATR: ["Atar International Airport", "Atar"],
  ATS: ["Artesia Municipal Airport", "Artesia"],
  ATT: ["Atmautluak Airport", "Atmautluak"],
  ATU: ["Casco Cove Coast Guard Station", "Attu"],
  ATV: ["Ati Airport", "Ati"],
  ATW: ["Appleton International Airport", "Appleton"],
  ATX: ["Atbasar Airport", "Atbasar"],
  ATY: ["Watertown Regional Airport", "Watertown"],
  ATZ: ["Assiut International Airport", "Assiut"],
  AUA: ["Queen Beatrix International Airport", "Oranjestad"],
  AUC: ["Santiago Perez Airport", "Arauca"],
  AUD: ["Augustus Downs Airport"],
  AUE: ["Abu Rudeis Airport", "Abu Rudeis"],
  AUF: ["Auxerre-Branches Airport", "Auxerre/Branches"],
  AUG: ["Augusta State Airport", "Augusta"],
  AUH: ["Abu Dhabi International Airport", "Abu Dhabi"],
  AUI: ["Aua Island Airport", "Aua Island"],
  AUJ: ["Ambunti Airport", "Ambunti"],
  AUK: ["Alakanuk Airport", "Alakanuk"],
  AUL: ["Aur Island Airport", "Aur Atoll"],
  AUM: ["Austin Municipal Airport", "Austin"],
  AUN: ["Auburn Municipal Airport", "Auburn"],
  AUO: ["Auburn University Regional Airport", "Auburn"],
  AUP: ["Agaun Airport"],
  AUQ: ["Hiva Oa-Atuona Airport", "Hiva Oa Island"],
  AUR: ["Aurillac Airport", "Aurillac"],
  AUS: ["Austin Bergstrom International Airport", "Austin"],
  AUT: ["Atauro Airport", "Atauro"],
  AUU: ["Aurukun Airport", "Aurukun"],
  AUV: ["Aumo Airport", "Aumo"],
  AUW: ["Wausau Downtown Airport", "Wausau"],
  AUX: ["Aragua\u00edna Airport", "Aragua\u00edna"],
  AUY: ["Aneityum Airport", "Anatom Island"],
  AUZ: ["Aurora Municipal Airport", "Chicago/Aurora"],
  AVA: ["Anshun Huangguoshu Airport", "Anshun"],
  AVB: ["Aviano Air Base", "Aviano (PN)"],
  AVG: ["Auvergne Airport"],
  AVI: ["Maximo Gomez Airport", "Ciego de Avila"],
  AVK: ["Arvaikheer Airport", "Arvaikheer"],
  AVL: ["Asheville Regional Airport", "Asheville"],
  AVN: ["Avignon-Caumont Airport", "Avignon/Caumont"],
  AVO: ["Avon Park Executive Airport", "Avon Park"],
  AVP: ["Wilkes Barre Scranton International Airport", "Wilkes-Barre/Scranton"],
  AVR: ["Alverca Air Base", "Vila Franca de Xira"],
  AVU: ["Avu Avu Airport"],
  AVV: ["Avalon Airport", "Melbourne"],
  AVW: ["Marana Regional Airport", "Tucson"],
  AVX: ["Catalina Airport", "Avalon"],
  AWA: ["Awassa Airport", "Awassa"],
  AWB: ["Awaba Airport", "Awaba"],
  AWD: ["Aniwa Airport", "Aniwa"],
  AWE: ["Alowe Airport", "Wonga Wongu\u00e9 Presidential Reserve"],
  AWK: ["Wake Island Airfield", "Wake Island"],
  AWM: ["West Memphis Municipal Airport", "West Memphis"],
  AWN: ["Alton Downs Airport"],
  AWP: ["Austral Downs Airport"],
  AWR: ["Awar Airport", "Awar"],
  AWZ: ["Ahwaz Airport", "Ahwaz"],
  AXA: ["Clayton J Lloyd International Airport", "The Valley"],
  AXB: ["Maxson Airfield", "Alexandria Bay"],
  AXC: ["Aramac Airport"],
  AXD: ["Dimokritos Airport", "Alexandroupolis"],
  AXE: ["Xanxer\u00ea Airport", "Xanxer\u00ea"],
  AXF: ["Alxa Left Banner Bayanhot Airport", "Bayanhot"],
  AXG: ["Algona Municipal Airport", "Algona"],
  AXJ: ["Amakusa Airport"],
  AXK: ["Ataq Airport"],
  AXL: ["Alexandria Homestead Airport"],
  AXM: ["El Eden Airport", "Armenia"],
  AXN: ["Chandler Field", "Alexandria"],
  AXP: ["Spring Point Airport", "Spring Point"],
  AXR: ["Arutua Airport"],
  AXS: ["Altus Quartz Mountain Regional Airport", "Altus"],
  AXT: ["Akita Airport", "Akita"],
  AXU: ["Axum Airport"],
  AXV: ["Neil Armstrong Airport", "Wapakoneta"],
  AXX: ["Angel Fire Airport", "Angel Fire"],
  AYA: ["Ayapel Airport", "Ayapel"],
  AYC: ["Ayacucho Airport", "Ayacucho"],
  AYD: ["Alroy Downs Airport", "Alroy Downs"],
  AYE: ["Ft Devens Moore Army Air Field"],
  AYG: ["Yaguara Airport", "San Vicente Del Cagu\u00e1n"],
  AYH: ["RAF Alconbury", "Huntingdon"],
  AYI: ["Yari Airport", "Yari"],
  AYK: ["Arkalyk North Airport", "Arkalyk"],
  AYL: ["Anthony Lagoon Airport"],
  AYM: ["Yas Island Seaplane Base", "Abu Dhabi"],
  AYN: ["Anyang Airport", "Anyang"],
  AYO: ["Juan De Ayolas Airport", "Ayolas"],
  AYP: ["Coronel FAP Alfredo Mendivil Duarte Airport", "Ayacucho"],
  AYQ: ["Ayers Rock Connellan Airport", "Ayers Rock"],
  AYR: ["Ayr Airport"],
  AYS: ["Waycross Ware County Airport", "Waycross"],
  AYT: ["Antalya International Airport", "Antalya"],
  AYU: ["Aiyura Airport", "Aiyura Valley"],
  AYW: ["Ayawasi Airport", "Ayawasi-Papua Island"],
  AYY: ["Arugam Bay SPB", "Pottuvil"],
  AYZ: ["Zahn's Airport", "Amityville"],
  AZA: ["Phoenix-Mesa-Gateway Airport", "Phoenix"],
  AZB: ["Amazon Bay Airport"],
  AZD: ["Shahid Sadooghi Airport", "Yazd"],
  AZG: ["Pablo L. Sidar Airport", "Apatzing\u00e1n"],
  AZI: ["Bateen Airport"],
  AZL: ["Fazenda Tucunar\u00e9 Airport", "Sapezal"],
  AZN: ["Andizhan Airport", "Andizhan"],
  AZO: ["Kalamazoo Battle Creek International Airport", "Kalamazoo"],
  AZP: ["Atizapan De Zaragoza Airport"],
  AZR: ["Touat Cheikh Sidi Mohamed Belkebir Airport"],
  AZS: ["Saman\u00e1 El Catey International Airport", "Samana"],
  AZT: ["Zapatoca Airport", "Zapatoca"],
  AZZ: ["Ambriz Airport", "Ambriz"],
  BAA: ["Bialla Airport", "Bialla, Matalilu, Ewase"],
  BAB: ["Beale Air Force Base", "Marysville"],
  BAC: ["Barranca De Upia Airport", "Barranca De Upia"],
  BAD: ["Barksdale Air Force Base", "Bossier City"],
  BAE: ["Barcelonnette - Saint-Pons Airport", "Le Castellet"],
  BAF: ["Westfield-Barnes Regional Airport", "Westfield/Springfield"],
  BAG: ["Loakan Airport", "Baguio City"],
  BAH: ["Bahrain International Airport", "Manama"],
  BAI: ["Buenos Aires Airport", "Punta Arenas"],
  BAJ: ["Bali Airport", "Unea Island"],
  BAL: ["Batman Airport", "Batman"],
  BAM: ["Battle Mountain Airport", "Battle Mountain"],
  BAN: ["Basongo Airport", "Basongo"],
  BAO: ["Udorn Air Base", "Ban Mak Khaen"],
  BAP: ["Baibara Airport", "Nabai"],
  BAQ: ["Ernesto Cortissoz International Airport", "Barranquilla"],
  BAR: ["Qionghai Bo'ao Airport", "Qionghai"],
  BAS: ["Ballalae Airport", "Ballalae"],
  BAT: ["Chafei Amsei Airport", "Barretos"],
  BAU: ["Bauru Airport", "Bauru"],
  BAV: ["Baotou Airport", "Baotou"],
  BAX: ["Barnaul Airport", "Barnaul"],
  BAY: ["Tautii Magheraus Airport", "Baia Mare"],
  BAZ: ["Barcelos Airport", "Barcelos"],
  BBA: ["Balmaceda Airport", "Balmaceda"],
  BBB: ["Benson Municipal Airport", "Benson"],
  BBC: ["Bay City Municipal Airport", "Bay City"],
  BBD: ["Curtis Field", "Brady"],
  BBE: ["Big Bell Airport", "Big Bell"],
  BBG: ["Butaritari Atoll Airport", "Butaritari Atoll"],
  BBH: ["Barth Airport"],
  BBI: ["Biju Patnaik Airport", "Bhubaneswar"],
  BBJ: ["Bitburg Airport", "Bitburg"],
  BBK: ["Kasane Airport", "Kasane"],
  BBL: ["Ballera Airport"],
  BBM: ["Battambang Airport", "Battambang"],
  BBN: ["Bario Airport", "Bario"],
  BBO: ["Berbera Airport", "Berbera"],
  BBP: ["Bembridge Airport", "Bembridge"],
  BBQ: ["Codrington Airport", "Codrington"],
  BBR: ["Baillif Airport", "Basse Terre"],
  BBS: ["Blackbushe Airport", "Yateley"],
  BBT: ["Berb\u00e9rati Airport", "Berb\u00e9rati"],
  BBU: ["B\u0103neasa International Airport", "Bucharest"],
  BBV: ["Nero-Mer Airport", "Grand-B\u00e9r\u00e9by"],
  BBW: ["Broken Bow Municipal Airport", "Broken Bow"],
  BBX: ["Wings Field", "Philadelphia"],
  BBY: ["Bambari Airport", "Bambari"],
  BBZ: ["Zambezi Airport", "Zambezi"],
  BCA: ["Gustavo Rizo Airport", "Baracoa"],
  BCB: ["Virginia Tech Montgomery Executive Airport", "Blacksburg"],
  BCC: ["Bear Creek 3 Airport", "Bear Creek"],
  BCD: ["Bacolod-Silay Airport", "Bacolod City"],
  BCE: ["Bryce Canyon Airport", "Bryce Canyon"],
  BCF: ["Bouca Airport", "Bouca"],
  BCG: ["Bemichi Airport", "Bemichi"],
  BCH: ["Cakung Airport", "Baucau"],
  BCI: ["Barcaldine Airport", "Barcaldine"],
  BCJ: ["Baca Grande Airfield", "Crestone"],
  BCK: ["Bolwarra Airport"],
  BCL: ["Barra del Colorado Airport", "Pococi"],
  BCM: ["Bac\u0103u Airport", "Bac\u0103u"],
  BCN: ["Barcelona International Airport", "Barcelona"],
  BCO: ["Baco Airport", "Baco"],
  BCP: ["Bambu Airport", "Bambu"],
  BCQ: ["Brak Airport", "Brak"],
  BCR: ["Novo Campo Airport", "Boca do Acre"],
  BCS: ["Southern Seaplane Airport", "Belle Chasse"],
  BCT: ["Boca Raton Airport", "Boca Raton"],
  BCU: ["Sir Abubakar Tafawa Balewa International Airport", "Bauchi"],
  BCV: ["Hector Silva Airstrip", "Belmopan"],
  BCW: ["Benguera Island Airport", "Benguera Island"],
  BCX: ["Beloretsk Airport", "Beloretsk"],
  BCZ: ["Milyakburra Airport", "Bickerton Island"],
  BDA: ["L.F. Wade International International Airport", "Hamilton"],
  BDB: ["Bundaberg Airport", "Bundaberg"],
  BDC: ["Barra do Corda Airport", "Barra Do Corda"],
  BDD: ["Badu Island Airport"],
  BDE: ["Baudette International Airport", "Baudette"],
  BDF: ["Rinkenberger Restricted Landing Area", "Bradford"],
  BDG: ["Blanding Municipal Airport", "Blanding"],
  BDH: ["Bandar Lengeh Airport", "Bandar Lengeh"],
  BDI: ["Bird Island Airport", "Bird Island"],
  BDJ: ["Syamsudin Noor Airport", "Banjarmasin-Borneo Island"],
  BDK: ["Soko Airport", "Bondoukou"],
  BDL: ["Bradley International Airport", "Hartford"],
  BDM: ["Band\u0131rma Airport"],
  BDN: ["Talhar Airport", "Badin"],
  BDO: ["Husein Sastranegara International Airport", "Bandung-Java Island"],
  BDP: ["Bhadrapur Airport", "Bhadrapur"],
  BDQ: ["Vadodara Airport", "Vadodara"],
  BDR: ["Igor I Sikorsky Memorial Airport", "Bridgeport"],
  BDS: ["Brindisi \u2013 Salento Airport", "Brindisi"],
  BDT: ["Gbadolite Airport"],
  BDU: ["Bardufoss Airport", "M\u00e5lselv"],
  BDV: ["Moba Airport", "Moba"],
  BDW: ["Bedford Downs Airport"],
  BDX: ["Broadus Airport", "Broadus"],
  BDY: ["Bandon State Airport", "Bandon"],
  BDZ: ["Baindoung Airport"],
  BEA: ["Bereina Airport", "Bereina Mission"],
  BEB: ["Benbecula Airport", "Balivanich"],
  BEC: ["Beech Factory Airport", "Wichita"],
  BED: ["Laurence G Hanscom Field", "Bedford"],
  BEE: ["Beagle Bay Airport", "Beagle Bay"],
  BEF: ["Bluefields Airport", "Bluefileds"],
  BEG: ["Belgrade Nikola Tesla Airport", "Belgrade"],
  BEH: ["Southwest Michigan Regional Airport", "Benton Harbor"],
  BEI: ["Beica Airport", "Beica"],
  BEJ: ["Kalimarau Airport", "Tanjung Redeb - Borneo Island"],
  BEK: ["Bareilly Air Force Station", "Bareilly"],
  BEL: [
    "Val de Cans/J\u00falio Cezar Ribeiro International Airport",
    "Bel\u00e9m",
  ],
  BEM: ["Beni Mellal Airport", "Beni Mellal"],
  BEN: ["Benina International Airport", "Benghazi"],
  BEO: ["Lake Macquarie Airport"],
  BEP: ["Bellary Airport", "Bellary"],
  BEQ: ["RAF Honington", "Thetford"],
  BER: ["Berlin Brandenburg Airport (U.C.)", "Berlin"],
  BES: ["Brest Bretagne Airport", "Brest/Guipavas"],
  BET: ["Bethel Airport", "Bethel"],
  BEU: ["Bedourie Airport"],
  BEV: ["Beersheba (Teyman) Airport", "Beersheba"],
  BEW: ["Beira Airport", "Beira"],
  BEX: ["RAF Benson", "Benson"],
  BEY: ["Beirut Rafic Hariri International Airport", "Beirut"],
  BEZ: ["Beru Airport", "Beru"],
  BFA: ["Bah\u00eda Negra Airport", "Bah\u00eda Negra"],
  BFC: ["Bloomfield River Airport"],
  BFD: ["Bradford Regional Airport", "Bradford"],
  BFE: ["Bielefeld Airport", "Bielefeld"],
  BFF: ["Western Neb. Rgnl/William B. Heilig Airport", "Scottsbluff"],
  BFG: ["Bullfrog Basin Airport", "Glen Canyon Natl Rec Area"],
  BFH: ["Bacacheri Airport", "Curitiba"],
  BFI: ["Boeing Field King County International Airport", "Seattle"],
  BFJ: ["Bijie Feixiong Airport", "Bijie"],
  BFK: ["Buckley Air Force Base", "Aurora"],
  BFL: ["Meadows Field", "Bakersfield"],
  BFM: ["Mobile Downtown Airport", "Mobile"],
  BFN: ["Bram Fischer International Airport", "Bloemfontain"],
  BFO: ["Buffalo Range Airport", "Chiredzi"],
  BFP: ["Beaver County Airport", "Beaver Falls"],
  BFQ: ["Bahia Pi\u00f1a Airport", "Bahia Pi\u00f1a"],
  BFR: ["Virgil I Grissom Municipal Airport", "Bedford"],
  BFS: ["Belfast International Airport", "Belfast"],
  BFT: ["Beaufort County Airport", "Beaufort"],
  BFU: ["Bengbu Airport", "Bengbu"],
  BFV: ["Buri Ram Airport"],
  BFW: ["Sidi Bel Abbes Airport", "Sidi Bel Abb\u00e8s"],
  BFX: ["Bafoussam Airport", "Bafoussam"],
  BGA: ["Palonegro Airport", "Bucaramanga"],
  BGB: ["Booue Airport", "Booue"],
  BGC: ["Bragan\u00e7a Airport", "Bragan\u00e7a"],
  BGD: ["Hutchinson County Airport", "Borger"],
  BGE: ["Decatur County Industrial Air Park", "Bainbridge"],
  BGF: ["Bangui M'Poko International Airport", "Bangui"],
  BGG: ["Bing\u00f6l \u00c7eltiksuyu Airport", "Bing\u00f6l"],
  BGH: ["Abbaye Airport", "Boghe"],
  BGI: ["Sir Grantley Adams International Airport", "Bridgetown"],
  BGJ: [
    "Borgarfj\u00f6r\u00f0ur eystri Airport",
    "Borgarfj\u00f6r\u00f0ur eystri",
  ],
  BGK: ["Big Creek Airport", "Big Creek"],
  BGL: ["Baglung Airport", "Baglung"],
  BGM: ["Greater Binghamton/Edwin A Link field", "Binghamton"],
  BGN: ["Belaya Gora Airport", "Belaya Gora"],
  BGO: ["Bergen Airport Flesland", "Bergen"],
  BGP: ["Bongo Airport", "Bongo"],
  BGQ: ["Big Lake Airport", "Big Lake"],
  BGR: ["Bangor International Airport", "Bangor"],
  BGT: ["Bagdad Airport", "Bagdad"],
  BGU: ["Bangassou Airport", "Bangassou"],
  BGV: ["Aeroclube de Bento Gon\u00e7alves Airport", "Bento Gon\u00e7alves"],
  BGW: ["Baghdad International Airport", "Baghdad"],
  BGX: ["Comandante Gustavo Kraemer Airport", "Bag\u00e9"],
  BGY: ["Il Caravaggio International Airport", "Bergamo"],
  BGZ: ["Braga Municipal Aerodrome", "Braga"],
  BHA: ["Los Perales Airport", "Bah\u00eda de Caraquez"],
  BHB: ["Hancock County-Bar Harbor Airport", "Bar Harbor"],
  BHC: ["Bhurban Heliport", "Bhurban"],
  BHD: ["George Best Belfast City Airport", "Belfast"],
  BHE: ["Woodbourne Airport", "Blenheim"],
  BHF: ["Bahia Cupica Airport", "Bah\u00eda Cupica"],
  BHG: ["Brus Laguna Airport", "Brus Laguna"],
  BHH: ["Bisha Airport"],
  BHI: ["Comandante Espora Airport", "Bahia Blanca"],
  BHJ: ["Bhuj Airport", "Bhuj"],
  BHK: ["Bukhara Airport", "Bukhara"],
  BHL: [
    "Bah\u00eda de los \u00c1ngeles Airport",
    "Bah\u00eda de los \u00c1ngeles",
  ],
  BHM: ["Birmingham-Shuttlesworth International Airport", "Birmingham"],
  BHN: ["Beihan Airport"],
  BHO: ["Raja Bhoj International Airport", "Bhopal"],
  BHP: ["Bhojpur Airport", "Bhojpur"],
  BHQ: ["Broken Hill Airport", "Broken Hill"],
  BHR: ["Bharatpur Airport", "Bharatpur"],
  BHS: ["Bathurst Airport", "Bathurst"],
  BHT: ["Brighton Downs Airport"],
  BHU: ["Bhavnagar Airport", "Bhavnagar"],
  BHV: ["Bahawalpur Airport", "Bahawalpur"],
  BHW: ["Bhagatanwala Airport", "Bhagatanwala"],
  BHX: ["Birmingham International Airport", "Birmingham"],
  BHY: ["Beihai Airport", "Beihai"],
  BIA: ["Bastia-Poretta Airport", "Bastia/Poretta"],
  BIB: ["Baidoa Airport", "Baidoa"],
  BID: ["Block Island State Airport", "Block Island"],
  BIE: ["Beatrice Municipal Airport", "Beatrice"],
  BIF: ["Biggs Army Air Field (Fort Bliss)", "Fort Bliss/El Paso"],
  BIG: ["Allen Army Airfield", "Delta Junction Ft Greely"],
  BIH: ["Eastern Sierra Regional Airport", "Bishop"],
  BII: ["Enyu Airfield", "Bikini Atoll"],
  BIJ: ["Biliau Airport", "Biliau"],
  BIK: ["Frans Kaisiepo Airport", "Biak-Supiori Island"],
  BIL: ["Billings Logan International Airport", "Billings"],
  BIM: ["South Bimini Airport", "South Bimini"],
  BIN: ["Bamiyan Airport", "Bamiyan"],
  BIO: ["Bilbao Airport", "Bilbao"],
  BIP: ["Bulimba Airport"],
  BIQ: ["Biarritz-Anglet-Bayonne Airport", "Biarritz/Anglet/Bayonne"],
  BIR: ["Biratnagar Airport", "Biratnagar"],
  BIS: ["Bismarck Municipal Airport", "Bismarck"],
  BIT: ["Baitadi Airport", "Baitadi"],
  BIU: ["Bildudalur Airport", "Bildudalur"],
  BIV: ["Bria Airport", "Bria"],
  BIW: ["Billiluna Airport"],
  BIX: ["Keesler Air Force Base", "Biloxi"],
  BIY: ["Bisho Airport", "Bisho"],
  BIZ: ["Bimin Airport"],
  BJA: ["Soummam Airport", "B\u00e9ja\u00efa"],
  BJB: ["Bojnord Airport", "Bojnord"],
  BJC: ["Rocky Mountain Metropolitan Airport", "Denver"],
  BJD: ["Bakkafj\u00f6r\u00f0ur Airport", "Bakkafj\u00f6r\u00f0ur"],
  BJE: ["Baleela Airport", "Baleela Base Camp"],
  BJF: ["B\u00e5tsfjord Airport", "B\u00e5tsfjord"],
  BJG: ["Boalang Airport", "Boalang-Celebes Island"],
  BJH: ["Bajhang Airport", "Bajhang"],
  BJI: ["Bemidji Regional Airport", "Bemidji"],
  BJJ: ["Wayne County Airport", "Wooster"],
  BJK: ["Nangasuri Airport", "Maikoor Island"],
  BJL: ["Banjul International Airport", "Banjul"],
  BJM: ["Bujumbura International Airport", "Bujumbura"],
  BJO: ["Bermejo Airport", "Bermejo"],
  BJP: ["Estadual Arthur Siqueira Airport", "Bragan\u00e7a Paulista"],
  BJQ: ["Bahja Airport", "Bahja"],
  BJR: ["Bahir Dar Airport", "Bahir Dar"],
  BJT: ["Bentota River Waterdrome", "Bentota"],
  BJU: ["Bajura Airport", "Bajura"],
  BJV: ["Milas Bodrum International Airport", "Bodrum"],
  BJW: ["Bajawa Soa Airport", "Bajawa"],
  BJX: ["Del Baj\u00edo International Airport", "Silao"],
  BJY: ["Batajnica Air Base", "Batajnica"],
  BJZ: ["Badajoz Airport", "Badajoz"],
  BKA: ["Bykovo Airport", "Moscow"],
  BKB: ["Nal Airport", "Bikaner"],
  BKC: ["Buckland Airport", "Buckland"],
  BKD: ["Stephens County Airport", "Breckenridge"],
  BKE: ["Baker City Municipal Airport", "Baker City"],
  BKF: ["Lake Brooks Seaplane Base", "Katmai National Park"],
  BKG: ["Branson Airport", "Branson"],
  BKH: ["Barking Sands Airport", "Kekaha"],
  BKI: ["Kota Kinabalu International Airport", "Kota Kinabalu"],
  BKJ: ["Bok\u00e9 Baralande Airport", "Bok\u00e9"],
  BKK: ["Suvarnabhumi Airport", "Bangkok"],
  BKL: ["Burke Lakefront Airport", "Cleveland"],
  BKM: ["Bakalalan Airport", "Bakalalan"],
  BKN: ["Balkanabat Airport", "Balkanabat"],
  BKO: ["Modibo Keita International Airport", "Bamako"],
  BKP: ["Barkly Downs Airport"],
  BKQ: ["Blackall Airport", "Blackall"],
  BKR: ["Bokoro Airport", "Bokoro"],
  BKS: ["Fatmawati Soekarno Airport", "Bengkulu-Sumatra Island"],
  BKT: ["Allen C Perkinson Blackstone Army Air Field", "Blackstone"],
  BKU: ["Betioky Airport", "Betioky"],
  BKW: ["Raleigh County Memorial Airport", "Beckley"],
  BKX: ["Brookings Regional Airport", "Brookings"],
  BKY: ["Bukavu Kavumu Airport"],
  BKZ: ["Bukoba Airport", "Bukoba"],
  BLA: [
    "General Jos\u00e9 Antonio Anzoategui International Airport",
    "Barcelona",
  ],
  BLB: ["Panama Pacific International Airport", "Panam\u00e1 City"],
  BLC: ["Bali Airport", "Bali"],
  BLD: ["Boulder City Municipal Airport", "Boulder City"],
  BLE: ["Borlange Airport"],
  BLF: ["Mercer County Airport", "Bluefield"],
  BLG: ["Belaga Airport", "Belaga"],
  BLH: ["Blythe Airport", "Blythe"],
  BLI: ["Bellingham International Airport", "Bellingham"],
  BLJ: ["Batna Airport", "Batna"],
  BLK: ["Blackpool International Airport", "Blackpool"],
  BLL: ["Billund Airport", "Billund"],
  BLM: ["Monmouth Executive Airport", "Belmar/Farmingdale"],
  BLN: ["Benalla Airport"],
  BLO: ["Hjaltabakki Airport", "Bl\u00f6ndu\u00f3s"],
  BLP: ["Huallaga Airport", "Bellavista"],
  BLQ: ["Bologna Guglielmo Marconi Airport", "Bologna"],
  BLR: ["Kempegowda International Airport", "Bangalore"],
  BLS: ["Bollon Airport"],
  BLT: ["Blackwater Airport"],
  BLU: ["Blue Canyon Nyack Airport", "Emigrant Gap"],
  BLV: ["Scott AFB/Midamerica Airport", "Belleville"],
  BLW: ["Beletwene Airport", "Beledweyne"],
  BLX: ["Belluno Airport", "Belluno (BL)"],
  BLY: ["Belmullet Aerodrome", "Belmullet"],
  BLZ: ["Chileka International Airport", "Blantyre"],
  BMA: ["Stockholm-Bromma Airport", "Stockholm"],
  BMB: ["Bumbar Airport", "Bumbar"],
  BMC: ["Brigham City Regional Airport", "Brigham City"],
  BMD: ["Belo sur Tsiribihina Airport", "Belo sur Tsiribihina"],
  BME: ["Broome International Airport", "Broome"],
  BMF: ["Bakouma Airport", "Bakouma"],
  BMG: ["Monroe County Airport", "Bloomington"],
  BMH: ["Bomai Airport", "Bomai"],
  BMI: [
    "Central Illinois Regional Airport at Bloomington-Normal",
    "Bloomington/Normal",
  ],
  BMJ: ["Baramita Airport", "Baramita"],
  BMK: ["Borkum Airport", "Borkum"],
  BML: ["Berlin Regional Airport", "Berlin"],
  BMM: ["Bitam Airport", "Bitam"],
  BMN: ["Bamarni Airport", "Bamarni"],
  BMO: ["Banmaw Airport", "Banmaw"],
  BMP: ["Brampton Island Airport"],
  BMQ: ["Bamburi Airport"],
  BMR: ["Baltrum Airport", "Baltrum"],
  BMS: ["S\u00f3crates Mariani Bittencourt Airport", "Brumado"],
  BMT: ["Beaumont Municipal Airport", "Beaumont"],
  BMU: ["Muhammad Salahuddin Airport", "Bima-Sumbawa Island"],
  BMV: ["Buon Ma Thuot Airport", "Buon Ma Thuot"],
  BMW: ["Bordj Badji Mokhtar Airport", "Bordj Badji Mokhtar"],
  BMX: ["Big Mountain Airport", "Big Mountain"],
  BMY: ["\u00cele Art - Waala Airport", "Waala"],
  BMZ: ["Bamu Airport", "Bamu"],
  BNA: ["Nashville International Airport", "Nashville"],
  BNB: ["Boende Airport", "Boende"],
  BNC: ["Beni Airport", "Beni"],
  BND: ["Bandar Abbas International Airport", "Bandar Abbas"],
  BNE: ["Brisbane International Airport", "Brisbane"],
  BNF: ["Baranof Warm Springs Float and Seaplane Base", "Baranof"],
  BNG: ["Banning Municipal Airport", "Banning"],
  BNH: ["Cape Air Seaplanes on Boston Harbor Seaplane Base", "Boston"],
  BNI: ["Benin Airport", "Benin"],
  BNJ: ["Bonn-Hangelar Airport", "Bonn"],
  BNK: ["Ballina Byron Gateway Airport", "Ballina"],
  BNL: ["Barnwell Regional Airport", "Barnwell"],
  BNM: ["Bodinumu Airport", "Bodinumu"],
  BNN: ["Br\u00f8nn\u00f8ysund Airport", "Br\u00f8nn\u00f8y"],
  BNO: ["Burns Municipal Airport", "Burns"],
  BNP: ["Bannu Airport", "Bannu"],
  BNQ: ["Baganga airport", "Baganga"],
  BNR: ["Banfora Airport", "Banfora"],
  BNS: ["Barinas Airport", "Barinas"],
  BNT: ["Bundi Airport", "Bundi"],
  BNU: ["Blumenau Airport", "Blumenau"],
  BNV: ["Boana Airport", "Boana"],
  BNW: ["Boone Municipal Airport", "Boone"],
  BNX: ["Banja Luka International Airport", "Banja Luka"],
  BNY: ["Bellona/Anua Airport", "Anua"],
  BNZ: ["Banz Airport", "Banz"],
  BOA: ["Boma Airport", "Boma"],
  BOB: ["Bora Bora Airport", "Motu Mute"],
  BOC: ["Bocas Del Toro International Airport", "Isla Col\u00f3n"],
  BOD: ["Bordeaux-M\u00e9rignac Airport", "Bordeaux/M\u00e9rignac"],
  BOE: ["Boundji Airport", "Boundji"],
  BOF: ["Bolling Air Force Base", "Washington"],
  BOG: ["El Dorado International Airport", "Bogota"],
  BOH: ["Bournemouth Airport", "Bournemouth"],
  BOI: ["Boise Air Terminal/Gowen Field", "Boise"],
  BOJ: ["Burgas Airport", "Burgas"],
  BOK: ["Brookings Airport", "Brookings"],
  BOM: ["Chhatrapati Shivaji International Airport", "Mumbai"],
  BON: ["Flamingo International Airport", "Kralendijk"],
  BOO: ["Bod\u00f8 Airport", "Bod\u00f8"],
  BOP: ["Bouar Airport", "Bouar"],
  BOQ: ["Boku Airport", "Boku"],
  BOR: ["Fontaine Airport", "Belfort"],
  BOS: ["General Edward Lawrence Logan International Airport", "Boston"],
  BOT: ["Bosset Airport", "Bosset"],
  BOU: ["Bourges Airport", "Bourges"],
  BOV: ["Boang Airport", "Boang Island"],
  BOW: ["Bartow Municipal Airport", "Bartow"],
  BOX: ["Borroloola Airport"],
  BOY: ["Bobo Dioulasso Airport", "Bobo Dioulasso"],
  BOZ: ["Bozoum Airport", "Bozoum"],
  BPA: ["Grumman Bethpage Airport", "Bethpage"],
  BPC: ["Bamenda Airport", "Bamenda"],
  BPD: ["Bapi Airstrip", "Bapi"],
  BPE: ["Qinhuangdao Beidaihe Airport", "Qinhuangdao"],
  BPF: ["Batuna Aerodrome", "Batuna Mission Station"],
  BPG: ["Barra do Gar\u00e7as Airport", "Barra Do Gar\u00e7as"],
  BPH: ["Bislig Airport"],
  BPI: ["Miley Memorial Field", "Big Piney"],
  BPK: ["Biangabip Airport", "Biangabip"],
  BPL: ["Alashankou Bole (Bortala) airport", "Bole"],
  BPM: ["Begumpet Airport", "Hyderabad"],
  BPN: ["Sultan Aji Muhamad Sulaiman Airport", "Kotamadya Balikpapan"],
  BPS: ["Porto Seguro Airport", "Porto Seguro"],
  BPT: ["Southeast Texas Regional Airport", "Beaumont/Port Arthur"],
  BPX: ["Qamdo Bangda Airport", "Bangda"],
  BPY: ["Besalampy Airport"],
  BQA: ["Dr.Juan C. Angara Airport", "Baler"],
  BQB: ["Busselton Regional Airport"],
  BQD: ["B\u00fa\u00f0ardalur Airport", "B\u00fa\u00f0ardalur"],
  BQE: ["Bubaque Airport", "Bubaque"],
  BQG: ["Bogorodskoye Airport", "Bogorodskoye"],
  BQH: ["London Biggin Hill Airport", "London"],
  BQI: ["Bagani Airport", "Bagani"],
  BQJ: ["Batagay Airport", "Batagay"],
  BQK: ["Brunswick Golden Isles Airport", "Brunswick"],
  BQL: ["Boulia Airport"],
  BQN: ["Rafael Hernandez Airport", "Aguadilla"],
  BQO: ["Bouna Airport", "Bouna"],
  BQQ: ["Barra Airport", "Barra"],
  BQS: ["Ignatyevo Airport", "Blagoveschensk"],
  BQT: ["Brest Airport", "Brest"],
  BQU: ["J F Mitchell Airport", "Bequia"],
  BQV: ["Bartlett Cove Seaplane Base", "Gustavus"],
  BQW: ["Balgo Hill Airport", "Balgo"],
  BRA: ["Barreiras Airport", "Barreiras"],
  BRB: ["Barreirinhas Airport"],
  BRC: ["San Carlos De Bariloche Airport", "San Carlos de Bariloche"],
  BRD: ["Brainerd Lakes Regional Airport", "Brainerd"],
  BRE: ["Bremen Airport", "Bremen"],
  BRG: ["Whitesburg Municipal Airport", "Whitesburg"],
  BRI: ["Bari Karol Wojty\u0142a Airport", "Bari"],
  BRK: ["Bourke Airport"],
  BRL: ["Southeast Iowa Regional Airport", "Burlington"],
  BRM: ["Barquisimeto International Airport", "Barquisimeto"],
  BRN: ["Bern Belp Airport", "Bern"],
  BRO: ["Brownsville South Padre Island International Airport", "Brownsville"],
  BRP: ["Biaru Airport", "Biaru"],
  BRQ: ["Brno-Tu\u0159any Airport", "Brno"],
  BRR: ["Barra Airport", "Eoligarry"],
  BRS: ["Bristol Airport", "Bristol"],
  BRT: ["Bathurst Island Airport"],
  BRU: ["Brussels Airport", "Brussels"],
  BRV: ["Bremerhaven Airport", "Bremerhaven"],
  BRW: ["Wiley Post Will Rogers Memorial Airport", "Utqia\u0121vik"],
  BRX: ["Maria Montez International Airport", "Barahona"],
  BRY: ["Samuels Field", "Bardstown"],
  BSA: ["Bosaso Airport", "Bosaso"],
  BSB: [
    "Presidente Juscelino Kubistschek International Airport",
    "Bras\u00edlia",
  ],
  BSC: ["Jos\u00e9 Celestino Mutis Airport", "Bah\u00eda Solano"],
  BSD: ["Baoshan Yunduan Airport"],
  BSE: ["Sematan Airport", "Sematan"],
  BSF: ["Bradshaw Army Airfield", "Camp Pohakuloa"],
  BSG: ["Bata Airport"],
  BSI: ["Balesin Island Airport", "Balesin"],
  BSJ: ["Bairnsdale Airport"],
  BSK: ["Biskra Airport", "Biskra"],
  BSL: ["EuroAirport Basel-Mulhouse-Freiburg Airport", "B\u00e2le/Mulhouse"],
  BSM: ["Bishe Kola Air Base", "Amol"],
  BSN: ["Bossangoa Airport", "Bossangoa"],
  BSO: ["Basco Airport", "Basco"],
  BSP: ["Bensbach Airport", "Bensbach"],
  BSQ: ["Bisbee Municipal Airport", "Bisbee"],
  BSR: ["Basrah International Airport", "Basrah"],
  BSS: ["Balsas Airport", "Balsas"],
  BST: ["Bost Airport", "Bost"],
  BSU: ["Basankusu Airport", "Basankusu"],
  BSV: ["Besakoa Airport", "Besakoa"],
  BSW: ["Boswell Bay Airport", "Boswell Bay"],
  BSX: ["Pathein Airport", "Pathein"],
  BSY: ["Bardera Airport"],
  BSZ: ["Bartletts Airport", "Egegik"],
  BTA: ["Bertoua Airport", "Bertoua"],
  BTB: ["Betou Airport", "Betou"],
  BTC: ["Batticaloa Airport", "Batticaloa"],
  BTD: ["Brunette Downs Airport"],
  BTE: ["Sherbro International Airport", "Bonthe"],
  BTF: ["Skypark Airport", "Bountiful"],
  BTG: ["Batangafo Airport", "Batangafo"],
  BTH: ["Hang Nadim International Airport", "Batam Island"],
  BTI: ["Barter Island LRRS Airport", "Barter Island Lrrs"],
  BTJ: ["Sultan Iskandar Muda International Airport", "Banda Aceh"],
  BTK: ["Bratsk Airport", "Bratsk"],
  BTL: ["W K Kellogg Airport", "Battle Creek"],
  BTM: ["Bert Mooney Airport", "Butte"],
  BTN: ["Marlboro County Jetport H.E. Avent Field", "Bennettsville"],
  BTO: ["Botopasi Airport", "Botopasi"],
  BTP: ["Pittsburgh/Butler Regional Airport", "Butler"],
  BTQ: ["Butare Airport", "Butare"],
  BTR: ["Baton Rouge Metropolitan Airport", "Baton Rouge"],
  BTS: ["M. R. \u0160tef\u00e1nik Airport", "Bratislava"],
  BTT: ["Bettles Airport", "Bettles"],
  BTU: ["Bintulu Airport", "Bintulu"],
  BTV: ["Burlington International Airport", "Burlington"],
  BTW: ["Batu Licin Airport", "Batu Licin-Borneo Island"],
  BTX: ["Betoota Airport"],
  BTY: ["Beatty Airport", "Beatty"],
  BTZ: ["Bursa Airport", "Bursa"],
  BUA: ["Buka Airport", "Buka Island"],
  BUB: ["Cram Field", "Burwell"],
  BUC: ["Burketown Airport"],
  BUD: ["Budapest Liszt Ferenc International Airport", "Budapest"],
  BUF: ["Buffalo Niagara International Airport", "Buffalo"],
  BUG: ["Benguela Airport", "Benguela"],
  BUI: ["Bokondini Airport", "Bokondini"],
  BUJ: ["Bou Saada Airport"],
  BUK: ["Al-Bough Airport", "Al-Bough"],
  BUL: ["Bulolo Airport", "Bulolo"],
  BUM: ["Butler Memorial Airport", "Butler"],
  BUN: ["Gerardo Tobar L\u00f3pez Airport", "Buenaventura"],
  BUO: ["Burao Airport", "Burao"],
  BUP: ["Bhatinda Air Force Station"],
  BUQ: ["Joshua Mqabuko Nkomo International Airport", "Bulawayo"],
  BUR: ["Bob Hope Airport", "Burbank"],
  BUS: ["Batumi International Airport", "Batumi"],
  BUT: ["Bathpalathang Airport", "Jakar"],
  BUU: ["Muara Bungo Airport", "Muara Bungo"],
  BUV: ["Bella Union Airport", "Bella Union"],
  BUW: ["Betoambari Airport", "Bau Bau-Butung Island"],
  BUX: ["Bunia Airport"],
  BUY: ["Bunbury Airport"],
  BUZ: ["Bushehr Airport", "Bushehr"],
  BVA: ["Paris Beauvais Till\u00e9 Airport", "Beauvais/Till\u00e9"],
  BVB: ["Atlas Brasil Cantanhede Airport", "Boa Vista"],
  BVC: ["Rabil Airport", "Rabil"],
  BVE: ["Brive Souillac Airport"],
  BVF: ["Bui-Dama Airport", "Bua"],
  BVG: ["Berlev\u00e5g Airport", "Berlev\u00e5g"],
  BVH: ["Brigadeiro Camar\u00e3o Airport", "Vilhena"],
  BVI: ["Birdsville Airport"],
  BVK: ["Huacaraje Airport", "Itenes"],
  BVL: ["Baures Airport", "Baures"],
  BVM: ["Belmonte Airport", "Belmonte"],
  BVO: ["Bartlesville Municipal Airport", "Bartlesville"],
  BVP: ["Bolovip Airstrip", "Bolovip"],
  BVR: ["Esperadinha Airport", "Brava Island"],
  BVS: ["Breves Airport", "Breves"],
  BVU: ["Beluga Airport", "Beluga"],
  BVV: ["Burevestnik Airport", "Iturup Island"],
  BVW: ["Batavia Downs Airport"],
  BVX: ["Batesville Regional Airport", "Batesville"],
  BVY: ["Beverly Municipal Airport", "Beverly"],
  BVZ: ["Beverley Springs Airport"],
  BWA: ["Gautam Buddha Airport", "Bhairawa"],
  BWB: ["Barrow Island Airport"],
  BWC: ["Brawley Municipal Airport", "Brawley"],
  BWD: ["Brownwood Regional Airport", "Brownwood"],
  BWE: ["Braunschweig-Wolfsburg Airport", "Braunschweig"],
  BWF: ["Barrow Walney Island Airport", "Barrow-in-Furness"],
  BWG: ["Bowling Green Warren County Regional Airport", "Bowling Green"],
  BWH: ["Butterworth Airport", "Butterworth"],
  BWI: [
    "Baltimore/Washington International Thurgood Marshall Airport",
    "Baltimore",
  ],
  BWJ: ["Bawan Airport"],
  BWK: ["Bol Airport", "Bra\u010d Island"],
  BWL: ["Earl Henry Airport", "Blackwell"],
  BWM: ["Bowman Regional Airport", "Bowman"],
  BWN: ["Brunei International Airport", "Bandar Seri Begawan"],
  BWO: ["Balakovo Airport", "Balakovo"],
  BWP: ["Bewani Airport", "Bewani"],
  BWQ: ["Brewarrina Airport"],
  BWT: ["Wynyard Airport", "Burnie"],
  BWU: ["Sydney Bankstown Airport", "Sydney"],
  BWW: ["Las Brujas Airport", "Cayo Santa Maria"],
  BWX: ["Blimbingsari Airport", "Banyuwangi"],
  BWY: ["RAF Bentwaters", "Woodbridge"],
  BXA: ["George R Carr Memorial Air Field", "Bogalusa"],
  BXB: ["Babo Airport", "Babo-Papua Island"],
  BXD: ["Bade Airport", "Bade-Papua Island"],
  BXE: ["Bakel Airport", "Bakel"],
  BXF: ["Bellburn Airstrip", "Pumululu National Park"],
  BXG: ["Bendigo Airport"],
  BXH: ["Balkhash Airport", "Balkhash"],
  BXI: ["Boundiali Airport", "Boundiali"],
  BXJ: ["Boralday Airport", "Aima Ata"],
  BXK: ["Buckeye Municipal Airport", "Buckeye"],
  BXL: ["Blue Lagoon Seaplane Base", "Nanuya Lailai Island"],
  BXM: ["Batom Airport", "Batom-Papua Island"],
  BXN: ["Ims\u0131k Airport"],
  BXO: ["Buochs Airport", "Buochs"],
  BXP: ["Bia\u0142a Podlaska Airfield", "Bia\u0142a Podlaska"],
  BXR: ["Bam Airport"],
  BXS: ["Borrego Valley Airport", "Borrego Springs"],
  BXT: ["Bontang Airport", "Bontang-Borneo Island"],
  BXU: ["Bancasi Airport", "Butuan City"],
  BXV: ["Brei\u00f0dalsv\u00edk Airport", "Brei\u00f0dalsv\u00edk"],
  BXW: ["Bawean Airport", "Bawean"],
  BXX: ["Borama Airport", "Borama"],
  BXY: ["Krainiy Airport", "Baikonur"],
  BXZ: ["Bunsil Airport", "Bunsil - Umboi Island"],
  BYA: ["Boundary Airport", "Boundary"],
  BYB: ["Dibba Airport", "Dibba Al-Baya"],
  BYC: ["Yacuiba Airport", "Yacu\u00edba"],
  BYD: ["Al-Bayda Airport", "Al-Bayda"],
  BYF: ["Albert-Bray Airport", "Albert/Bray"],
  BYG: ["Johnson County Airport", "Buffalo"],
  BYH: ["Arkansas International Airport", "Blytheville"],
  BYI: ["Burley Municipal Airport", "Burley"],
  BYJ: ["Beja Airport / Airbase", "Beja"],
  BYK: ["Bouak\u00e9 Airport"],
  BYL: ["Bella Yella Airport", "Beliyela"],
  BYM: ["Carlos Manuel de Cespedes Airport", "Bayamo"],
  BYN: ["Bayankhongor Airport", "Bayankhongor"],
  BYO: ["Bonito Airport", "Bonito"],
  BYP: ["Barimunya Airport"],
  BYQ: ["Bunyu Airport", "Bunju Island"],
  BYR: ["L\u00e6s\u00f8 Airport", "L\u00e6s\u00f8"],
  BYS: ["Bicycle Lake Army Air Field", "Fort Irwin/Barstow"],
  BYT: ["Bantry Aerodrome", "Bantry"],
  BYU: ["Bayreuth Airport", "Bayreuth"],
  BYV: ["Beira Lake Seaplane Base", "Colombo"],
  BYW: ["Blakely Island Airport", "Blakely Island"],
  BYX: ["Baniyala Airport", "Baniyala"],
  BZA: ["San Pedro Airport", "Bonanza"],
  BZB: ["Bazaruto Island Airport", "Bazaruto Island"],
  BZC: ["Umberto Modiano Airport", "Cabo Frio"],
  BZD: ["Balranald Airport"],
  BZE: ["Philip S. W. Goldson International Airport", "Belize City"],
  BZF: ["Benton Field", "Redding"],
  BZG: ["Bydgoszcz Ignacy Jan Paderewski Airport", "Bydgoszcz"],
  BZH: ["Bumi Airport", "Bumi"],
  BZI: ["Bal\u0131kesir Merkez Airport"],
  BZK: ["Bryansk Airport", "Bryansk"],
  BZL: ["Barisal Airport", "Barisal"],
  BZM: ["Bemolanga Airport", "Bemolanga"],
  BZN: ["Gallatin Field", "Bozeman"],
  BZO: ["Bolzano Airport", "Bolzano"],
  BZP: ["Bizant Airport", "Lakefield National Park"],
  BZR: ["B\u00e9ziers-Vias Airport", "B\u00e9ziers/Vias"],
  BZS: ["Buzzards Point Seaplane Base", "Washington"],
  BZT: ["Eagle Air Park", "Brazoria"],
  BZU: ["Buta Zega Airport"],
  BZV: ["Maya-Maya Airport", "Brazzaville"],
  BZX: ["Bazhong Enyang Airport", "Bazhong"],
  BZY: ["B\u0103l\u021bi International Airport", "B\u0103l\u021bi"],
  BZZ: ["RAF Brize Norton", "Brize Norton"],
  CAA: ["Catacamas Airport", "Catacamas"],
  CAB: ["Cabinda Airport", "Cabinda"],
  CAC: ["Cascavel Airport", "Cascavel"],
  CAD: ["Wexford County Airport", "Cadillac"],
  CAE: ["Columbia Metropolitan Airport", "Columbia"],
  CAF: ["Carauari Airport", "Carauari"],
  CAG: ["Cagliari Elmas Airport", "Cagliari"],
  CAH: ["C\u00e0 Mau Airport", "Ca Mau City"],
  CAI: ["Cairo International Airport", "Cairo"],
  CAJ: ["Canaima Airport", "Canaima"],
  CAK: ["Akron Canton Regional Airport", "Akron"],
  CAL: ["Campbeltown Airport", "Campbeltown"],
  CAM: ["Camiri Airport", "Camiri"],
  CAN: ["Guangzhou Baiyun International Airport", "Guangzhou"],
  CAO: ["Clayton Municipal Airpark", "Clayton"],
  CAP: ["Cap Haitien International Airport", "Cap Haitien"],
  CAQ: ["Juan H White Airport", "Caucasia"],
  CAR: ["Caribou Municipal Airport", "Caribou"],
  CAS: ["Anfa Airport", "Casablanca"],
  CAT: ["Cascais Airport", "Cascais"],
  CAU: ["Caruaru Airport", "Caruaru"],
  CAV: ["Cazombo Airport", "Cazombo"],
  CAW: ["Bartolomeu Lisandro Airport", "Campos Dos Goytacazes"],
  CAX: ["Carlisle Airport", "Carlisle"],
  CAY: ["Cayenne-Rochambeau Airport", "Cayenne / Rochambeau"],
  CAZ: ["Cobar Airport"],
  CBB: ["Jorge Wilsterman International Airport", "Cochabamba"],
  CBC: ["Cherrabun Airport"],
  CBD: ["Car Nicobar Air Force Station"],
  CBE: ["Greater Cumberland Regional Airport", "Cumberland"],
  CBF: ["Council Bluffs Municipal Airport", "Council Bluffs"],
  CBG: ["Cambridge Airport", "Cambridge"],
  CBH: ["B\u00e9char Boudghene Ben Ali Lotfi Airport", "B\u00e9char"],
  CBI: ["Cape Barren Island Airport"],
  CBJ: ["Cabo Rojo Airport", "Cabo Rojo"],
  CBK: ["Shalz Field", "Colby"],
  CBL: ['Aeropuerto "General Tomas de Heres". Ciudad Bolivar'],
  CBM: ["Columbus Air Force Base", "Columbus"],
  CBN: ["Penggung Airport", "Cirebon-Java Island"],
  CBO: ["Awang Airport", "Cotabato City"],
  CBQ: ["Margaret Ekpo International Airport", "Calabar"],
  CBR: ["Canberra International Airport", "Canberra"],
  CBS: ["Oro Negro Airport", "Cabimas"],
  CBT: ["Catumbela Airport", "Catumbela"],
  CBU: ["Cottbus-Drewitz Airport", "Cottbus"],
  CBV: ["Coban Airport", "Coban"],
  CBW: ["Campo Mour\u00e3o Airport", "Campo Mour\u00e3o"],
  CBX: ["Condobolin Airport"],
  CBY: ["Canobie Airport", "Canobie"],
  CCA: ["Chimore Airport", "Chimore"],
  CCB: ["Cable Airport", "Upland"],
  CCC: ["Jardines Del Rey Airport", "Cayo Coco"],
  CCD: ["Century City Heliport", "Los Angeles"],
  CCE: ["Capital International Airport"],
  CCF: ["Carcassonne Airport", "Carcassonne/Salvaza"],
  CCG: ["Crane County Airport", "Crane"],
  CCH: ["Chile Chico Airport", "Chile Chico"],
  CCI: ["Conc\u00f3rdia Airport", "Conc\u00f3rdia"],
  CCJ: ["Calicut International Airport", "Calicut"],
  CCK: ["Cocos (Keeling) Islands Airport", "Cocos (Keeling) Islands"],
  CCL: ["Chinchilla Airport"],
  CCM: ["Diom\u00edcio Freitas Airport", "Crici\u00fama"],
  CCN: ["Chakcharan Airport", "Chakcharan"],
  CCO: ["Carimagua Airport", "Puerto L\u00f3pez"],
  CCP: ["Carriel Sur Airport", "Concepcion"],
  CCR: ["Buchanan Field", "Concord"],
  CCS: ["Sim\u00f3n Bol\u00edvar International Airport", "Caracas"],
  CCT: ["Colonia Catriel Airport", "Colonia Catriel"],
  CCU: ["Netaji Subhash Chandra Bose International Airport", "Kolkata"],
  CCV: ["Craig Cove Airport", "Craig Cove"],
  CCW: ["Cowell Airport"],
  CCX: ["C\u00e1ceres Airport", "C\u00e1ceres"],
  CCY: ["Northeast Iowa Regional Airport", "Charles City"],
  CCZ: ["Chub Cay Airport"],
  CDA: ["Cooinda Airport"],
  CDB: ["Cold Bay Airport", "Cold Bay"],
  CDC: ["Cedar City Regional Airport", "Cedar City"],
  CDD: ["Cauquira Airport", "Cauquira"],
  CDE: ["Chengde Puning Airport", "Chengde"],
  CDF: ["Cortina Airport", "Cortina D'Ampezzo"],
  CDG: ["Charles de Gaulle International Airport", "Paris"],
  CDH: ["Harrell Field", "Camden"],
  CDI: ["Cachoeiro do Itapemirim Airport", "Cachoeiro Do Itapemirim"],
  CDJ: [
    "Concei\u00e7\u00e3o do Araguaia Airport",
    "Concei\u00e7\u00e3o Do Araguaia",
  ],
  CDL: ["Candle 2 Airport", "Candle"],
  CDN: ["Woodward Field", "Camden"],
  CDO: ["Cradock Airport", "Cradock"],
  CDP: ["Kadapa Airport", "Kadapa"],
  CDQ: ["Croydon Airport"],
  CDR: ["Chadron Municipal Airport", "Chadron"],
  CDS: ["Childress Municipal Airport", "Childress"],
  CDT: ["Castell\u00f3n-Costa Azahar Airport", "Castell\u00f3n de la Plana"],
  CDU: ["Camden Airport"],
  CDV: ["Merle K (Mudhole) Smith Airport", "Cordova"],
  CDW: ["Essex County Airport", "Caldwell"],
  CDY: ["Cagayan de Sulu Airport", "Mapun"],
  CEA: ["Cessna Aircraft Field", "Wichita"],
  CEB: ["Mactan Cebu International Airport", "Lapu-Lapu City"],
  CEC: ["Jack Mc Namara Field Airport", "Crescent City"],
  CED: ["Ceduna Airport"],
  CEE: ["Cherepovets Airport", "Cherepovets"],
  CEF: ["Westover ARB/Metropolitan Airport", "Springfield/Chicopee"],
  CEG: ["Hawarden Airport", "Hawarden"],
  CEH: ["Chelinda Malawi Airport"],
  CEI: ["Chiang Rai International Airport", "Chiang Rai"],
  CEJ: ["Chernihiv Shestovitsa Airport", "Chernigow"],
  CEK: ["Chelyabinsk Balandino Airport", "Chelyabinsk"],
  CEL: ["Cape Eleuthera Airport", "Cape Eleuthera"],
  CEM: ["Central Airport", "Central"],
  CEN: ["Ciudad Obreg\u00f3n International Airport", "Ciudad Obreg\u00f3n"],
  CEO: ["Waco Kungo Airport", "Waco Kungo"],
  CEP: ["Concepci\u00f3n Airport", "Concepci\u00f3n"],
  CEQ: ["Cannes-Mandelieu Airport", "Cannes/Mandelieu"],
  CER: ["Cherbourg-Maupertus Airport", "Cherbourg/Maupertus"],
  CES: ["Cessnock Airport"],
  CET: ["Cholet Le Pontreau Airport", "Cholet/Le Pontreau"],
  CEU: ["Oconee County Regional Airport", "Clemson"],
  CEV: ["Mettel Field", "Connersville"],
  CEW: ["Bob Sikes Airport", "Crestview"],
  CEX: ["Chena Hot Springs Airport", "Chena Hot Springs"],
  CEY: ["Kyle Oakley Field", "Murray"],
  CEZ: ["Cortez Municipal Airport", "Cortez"],
  CFB: ["Cabo Frio Airport", "Cabo Frio"],
  CFC: ["Ca\u00e7ador Airport", "Ca\u00e7ador"],
  CFD: ["Coulter Field", "Bryan"],
  CFE: ["Clermont-Ferrand Auvergne Airport", "Clermont-Ferrand/Auvergne"],
  CFF: ["Cafunfo Airport", "Cafunfo"],
  CFG: ["Jaime Gonzalez Airport", "Cienfuegos"],
  CFH: ["Clifton Hills Airport", "Clifton Hills"],
  CFI: ["Camfield Airport"],
  CFK: ["Ech Cheliff Airport"],
  CFM: ["Conklin (Leismer) Airport", "Conklin"],
  CFN: ["Donegal Airport", "Donegal"],
  CFO: ["Confresa Airport", "Confresa"],
  CFP: ["Carpentaria Downs Airport", "Carpentaria Downs"],
  CFQ: ["Creston Valley Regional Airport - Art Sutcliffe Field", "Creston"],
  CFR: ["Caen-Carpiquet Airport", "Caen/Carpiquet"],
  CFS: ["Coffs Harbour Airport", "Coffs Harbour"],
  CFT: ["Greenlee County Airport", "Clifton/Morenci"],
  CFU: ["Ioannis Kapodistrias International Airport", "Kerkyra Island"],
  CFV: ["Coffeyville Municipal Airport", "Coffeyville"],
  CGA: ["Craig Seaplane Base", "Craig"],
  CGB: ["Marechal Rondon Airport", "Cuiab\u00e1"],
  CGC: ["Cape Gloucester Airport", "Cape Gloucester"],
  CGD: ["Changde Airport", "Changde"],
  CGE: ["Cambridge Dorchester Airport", "Cambridge"],
  CGF: ["Cuyahoga County Airport", "Cleveland"],
  CGG: ["Casiguran Airport", "Casiguran"],
  CGH: ["Congonhas Airport", "S\u00e3o Paulo"],
  CGI: ["Cape Girardeau Regional Airport", "Cape Girardeau"],
  CGJ: ["Kasompe Airport", "Chingola"],
  CGK: ["Soekarno-Hatta International Airport", "Jakarta"],
  CGM: ["Camiguin Airport"],
  CGN: ["Cologne Bonn Airport", "Cologne"],
  CGO: ["Zhengzhou Xinzheng International Airport", "Zhengzhou"],
  CGP: ["Shah Amanat International Airport", "Chittagong"],
  CGQ: ["Longjia Airport", "Changchun"],
  CGR: ["Campo Grande Airport", "Campo Grande"],
  CGS: ["College Park Airport", "College Park"],
  CGT: ["Chinguetti Airport", "Chinguetti"],
  CGV: ["Caiguna Airport"],
  CGX: ["Chicago Meigs Airport", "Chicago"],
  CGY: ["Laguindingan Airport", "Cagayan de Oro City"],
  CGZ: ["Casa Grande Municipal Airport", "Casa Grande"],
  CHA: ["Lovell Field", "Chattanooga"],
  CHB: ["Chilas Airport", "Chilas"],
  CHC: ["Christchurch International Airport", "Christchurch"],
  CHF: ["Jinhae Airbase/Airport (G-813/K-10)", "Jinhae"],
  CHG: ["Chaoyang Airport", "Chaoyang"],
  CHH: ["Chachapoyas Airport", "Chachapoyas"],
  CHJ: ["Chipinge Airport", "Chipinge"],
  CHK: ["Chickasha Municipal Airport", "Chickasha"],
  CHL: ["Challis Airport", "Challis"],
  CHM: ["Teniente FAP Jaime A De Montreuil Morales Airport", "Chimbote"],
  CHN: ["Jeon Ju Airport (G-703)", "Jeon Ju"],
  CHO: ["Charlottesville Albemarle Airport", "Charlottesville"],
  CHP: ["Circle Hot Springs Airport", "Circle Hot Springs"],
  CHQ: ["Chania International Airport", "Souda"],
  CHR: [
    'Ch\u00e2teauroux-D\u00e9ols "Marcel Dassault" Airport',
    "Ch\u00e2teauroux/D\u00e9ols",
  ],
  CHS: ["Charleston Air Force Base-International Airport", "Charleston"],
  CHT: ["Chatham Islands-Tuuta Airport", "Waitangi"],
  CHU: ["Chuathbaluk Airport", "Chuathbaluk"],
  CHV: ["Aerodromo de Chaves", "Chaves"],
  CHX: ["Cap Manuel Ni\u00f1o International Airport", "Changuinola"],
  CHY: ["Choiseul Bay Airport"],
  CHZ: ["Chiloquin State Airport", "Chiloquin"],
  CIA: ["Ciampino\u2013G. B. Pastine International Airport", "Rome"],
  CIC: ["Chico Municipal Airport", "Chico"],
  CID: ["The Eastern Iowa Airport", "Cedar Rapids"],
  CIE: ["Collie Airport", "Collie"],
  CIF: ["Chifeng Airport", "Chifeng"],
  CIG: ["Craig Moffat Airport", "Craig"],
  CIH: ["Changzhi Airport", "Changzhi"],
  CII: ["\u00c7\u0131ld\u0131r Airport", "Ayd\u0131n"],
  CIJ: ["Capit\u00e1n An\u00edbal Arab Airport", "Cobija"],
  CIK: ["Chalkyitsik Airport", "Chalkyitsik"],
  CIL: ["Council Airport", "Council"],
  CIM: ["Cimitarra Airport", "Cimitarra"],
  CIN: ["Arthur N Neu Airport", "Carroll"],
  CIO: ["Teniente Col Carmelo Peralta Airport", "Concepci\u00f3n"],
  CIP: ["Chipata Airport", "Chipata"],
  CIQ: ["Chiquimula Airport", "Chiquimula"],
  CIR: ["Cairo Regional Airport", "Cairo"],
  CIS: ["Canton Island Airport", "Abariringa"],
  CIT: ["Shymkent Airport", "Shymkent"],
  CIU: ["Chippewa County International Airport", "Sault Ste Marie"],
  CIV: ["Chomley Seaplane Base", "Chomley"],
  CIW: ["Canouan Airport", "Canouan"],
  CIX: [
    "Capitan FAP Jose A Quinones Gonzales International Airport",
    "Chiclayo",
  ],
  CIY: ["Comiso Airport", "Comiso"],
  CIZ: ["Coari Airport", "Coari"],
  CJA: ["Mayor General FAP Armando Revoredo Iglesias Airport", "Cajamarca"],
  CJB: ["Coimbatore International Airport", "Coimbatore"],
  CJC: ["El Loa Airport", "Calama"],
  CJF: ["Coondewanna Airport", "Coondewanna"],
  CJH: ["Chilko Lake (Tsylos Park Lodge) Airport", "Chilko Lake"],
  CJJ: [
    "Cheongju International Airport/Cheongju Air Base (K-59/G-513)",
    "Cheongju",
  ],
  CJL: ["Chitral Airport", "Chitral"],
  CJM: ["Chumphon Airport"],
  CJN: ["Nusawiru Airport", "Cijulang"],
  CJS: ["Abraham Gonz\u00e1lez International Airport", "Ciudad Ju\u00e1rez"],
  CJT: ["San Antonio Copalar Airport", "Comit\u00e1n"],
  CJU: ["Jeju International Airport", "Jeju City"],
  CKA: ["Kegelman AF Aux Field", "Cherokee"],
  CKB: ["North Central West Virginia Airport", "Clarksburg"],
  CKC: ["Cherkasy International Airport", "Cherkasy"],
  CKD: ["Crooked Creek Airport", "Crooked Creek"],
  CKE: ["Lampson Field", "Lakeport"],
  CKG: ["Chongqing Jiangbei International Airport", "Chongqing"],
  CKH: ["Chokurdakh Airport", "Chokurdah"],
  CKI: ["Croker Island Airport"],
  CKK: ["Sharp County Regional Airport", "Ash Flat"],
  CKL: ["Chkalovskiy Air Base", "Moscow"],
  CKM: ["Fletcher Field", "Clarksdale"],
  CKN: ["Crookston Municipal Kirkwood Field", "Crookston"],
  CKO: ["Corn\u00e9lio Proc\u00f3pio Airport", "Corn\u00e9lio Proc\u00f3pio"],
  CKR: ["Crane Island Airstrip", "Crane Island"],
  CKS: ["Caraj\u00e1s Airport", "Parauapebas"],
  CKT: ["Sarakhs Airport", "Sarakhs"],
  CKU: ["Cordova Municipal Airport", "Cordova"],
  CKV: ["Clarksville\u2013Montgomery County Regional Airport", "Clarksville"],
  CKW: ["Graeme Rowley Aerodrome", "Christmas Creek Mine"],
  CKX: ["Chicken Airport", "Chicken"],
  CKY: ["Conakry International Airport", "Conakry"],
  CKZ: ["\u00c7anakkale Airport", "\u00c7anakkale"],
  CLA: ["Comilla Airport", "Comilla"],
  CLB: ["Castlebar Airport", "Castlebar"],
  CLC: ["Clear Lake Metroport", "Clear Lake"],
  CLD: ["Mc Clellan-Palomar Airport", "Carlsbad"],
  CLE: ["Cleveland Hopkins International Airport", "Cleveland"],
  CLG: ["New Coalinga Municipal Airport", "Coalinga"],
  CLH: ["Coolah Airport"],
  CLI: ["Clintonville Municipal Airport", "Clintonville"],
  CLJ: ["Cluj-Napoca International Airport", "Cluj-Napoca"],
  CLK: ["Clinton Regional Airport", "Clinton"],
  CLL: ["Easterwood Field", "College Station"],
  CLM: ["William R Fairchild International Airport", "Port Angeles"],
  CLN: ["Brig. Lysias Augusto Rodrigues Airport", "Carolina"],
  CLO: ["Alfonso Bonilla Aragon International Airport", "Cali"],
  CLP: ["Clarks Point Airport", "Clarks Point"],
  CLQ: ["Licenciado Miguel de la Madrid Airport", "Colima"],
  CLR: ["Cliff Hatfield Memorial Airport", "Calipatria"],
  CLS: ["Chehalis Centralia Airport", "Chehalis"],
  CLT: ["Charlotte Douglas International Airport", "Charlotte"],
  CLU: ["Columbus Municipal Airport", "Columbus"],
  CLV: ["Nelson Ribeiro Guimar\u00e3es Airport", "Caldas Novas"],
  CLW: ["Clearwater Air Park", "Clearwater"],
  CLX: ["Clorinda Airport", "Clorinda"],
  CLY: ["Calvi-Sainte-Catherine Airport", "Calvi/Sainte-Catherine"],
  CLZ: ["Calabozo Airport", "Guarico"],
  CMA: ["Cunnamulla Airport"],
  CMB: ["Bandaranaike International Colombo Airport", "Colombo"],
  CMC: ["Camocim Airport", "Camocim"],
  CMD: ["Cootamundra Airport"],
  CME: ["Ciudad del Carmen International Airport", "Ciudad del Carmen"],
  CMF: ["Chamb\u00e9ry-Savoie Airport", "Chamb\u00e9ry/Aix-les-Bains"],
  CMG: ["Corumb\u00e1 International Airport", "Corumb\u00e1"],
  CMH: ["John Glenn Columbus International Airport", "Columbus"],
  CMI: ["University of Illinois Willard Airport", "Champaign/Urbana"],
  CMJ: ["Chi Mei Airport", "Chi Mei"],
  CMK: ["Club Makokola Airport", "Club Makokola"],
  CML: ["Camooweal Airport"],
  CMM: ["Carmelita Airport", "Carmelita"],
  CMN: ["Mohammed V International Airport", "Casablanca"],
  CMO: ["Obbia Airport", "Obbia"],
  CMP: ["Santana do Araguaia Airport", "Santana Do Araguaia"],
  CMQ: ["Clermont Airport"],
  CMR: ["Colmar-Houssen Airport", "Colmar/Houssen"],
  CMS: ["Scusciuban Airport", "Scusciuban"],
  CMT: ["New Camet\u00e1 Airport", "Camet\u00e1"],
  CMU: ["Chimbu Airport", "Kundiawa"],
  CMV: ["Coromandel Aerodrome"],
  CMW: ["Ignacio Agramonte International Airport", "Camaguey"],
  CMX: ["Houghton County Memorial Airport", "Hancock"],
  CMY: ["Sparta Fort Mc Coy Airport", "Sparta"],
  CMZ: ["Caia Airport", "Caia"],
  CNA: ["Cananea National Airport", "Cananea"],
  CNB: ["Coonamble Airport"],
  CNC: ["Coconut Island Airport"],
  CND: ["Mihail Kog\u0103lniceanu International Airport", "Constan\u0163a"],
  CNE: ["Fremont County Airport", "Ca\u00f1on City"],
  CNF: ["Tancredo Neves International Airport", "Belo Horizonte"],
  CNG: [
    "Cognac-Ch\u00e2teaubernard (BA 709) Air Base",
    "Cognac/Ch\u00e2teaubernard",
  ],
  CNH: ["Claremont Municipal Airport", "Claremont"],
  CNI: ["Changhai Airport", "Changhai"],
  CNJ: ["Cloncurry Airport", "Cloncurry"],
  CNK: ["Blosser Municipal Airport", "Concordia"],
  CNL: ["Sindal Airport", "Sindal"],
  CNM: ["Cavern City Air Terminal", "Carlsbad"],
  CNN: ["Kannur International Airport", "Kannur"],
  CNO: ["Chino Airport", "Chino"],
  CNP: ["Neerlerit Inaat Airport", "Neerlerit Inaat"],
  CNQ: ["Corrientes Airport", "Corrientes"],
  CNR: ["Cha\u00f1aral Airport", "Cha\u00f1aral"],
  CNS: ["Cairns International Airport", "Cairns"],
  CNT: ["Charata Airport", "Charata"],
  CNU: ["Chanute Martin Johnson Airport", "Chanute"],
  CNV: ["S\u00f3crates Rezende Airport", "Canavieiras"],
  CNW: ["TSTC Waco Airport", "Waco"],
  CNX: ["Chiang Mai International Airport", "Chiang Mai"],
  CNY: ["Canyonlands Field", "Moab"],
  CNZ: ["Cangamba Airport", "Cangamba"],
  COA: ["Columbia Airport", "Columbia"],
  COB: ["Coolibah Airport", "Coolibah"],
  COC: ["Comodoro Pierrestegui Airport", "Concordia"],
  COD: ["Yellowstone Regional Airport", "Cody"],
  COE: ["Coeur D'Alene - Pappy Boyington Field", "Coeur d'Alene"],
  COF: ["Patrick Air Force Base", "Cocoa Beach"],
  COG: ["Mandinga Airport", "Condoto"],
  COH: ["Cooch Behar Airport"],
  COI: ["Merritt Island Airport", "Merritt Island"],
  COJ: ["Coonabarabran Airport"],
  COK: ["Cochin International Airport", "Kochi"],
  COL: ["Coll Airport", "Coll Island"],
  COM: ["Coleman Municipal Airport", "Coleman"],
  CON: ["Concord Municipal Airport", "Concord"],
  COO: ["Cadjehoun Airport", "Cotonou"],
  COP: ["Cooperstown-Westville Airport", "Cooperstown"],
  COQ: ["Choibalsan Airport"],
  COR: ["Ingeniero Ambrosio Taravella Airport", "Cordoba"],
  COS: ["City of Colorado Springs Municipal Airport", "Colorado Springs"],
  COT: ["Cotulla-La Salle County Airport", "Cotulla"],
  COU: ["Columbia Regional Airport", "Columbia"],
  COW: ["Tambillos Airport", "Coquimbo"],
  COX: ["Congo Town Airport", "Andros"],
  COY: ["Coolawanyah Airport", "Coolawanyah Station"],
  COZ: ["Constanza - Expedici\u00f3n 14 de Junio National Airport", "Costanza"],
  CPA: ["Cape Palmas Airport", "Harper"],
  CPB: ["Capurgan\u00e1 Airport", "Capurgan\u00e1"],
  CPC: ["Aviador C. Campos Airport", "Chapelco/San Martin de los Andes"],
  CPD: ["Coober Pedy Airport"],
  CPE: ["Ingeniero Alberto Acu\u00f1a Ongay International Airport", "Campeche"],
  CPF: ["Ngloram Airport", "Tjepu-Java Island"],
  CPG: ["Carmen De Patagones Airport", "Carmen de Patagones"],
  CPH: ["Copenhagen Kastrup Airport", "Copenhagen"],
  CPI: ["Cape Orford Airport"],
  CPL: ["Chaparral Airport", "Chaparral"],
  CPM: ["Compton Woodley Airport", "Compton"],
  CPN: ["Cape Rodney Airport", "Cape Rodney"],
  CPO: ["Desierto de Atacama Airport", "Copiapo"],
  CPP: ["Coposa Airport", "Pica"],
  CPQ: ["Amarais Airport", "Campinas"],
  CPR: ["Casper-Natrona County International Airport", "Casper"],
  CPS: ["St Louis Downtown Airport", "Cahokia/St Louis"],
  CPT: ["Cape Town International Airport", "Cape Town"],
  CPU: ["Cururupu Airport", "Cururupu"],
  CPV: ["Presidente Jo\u00e3o Suassuna Airport", "Campina Grande"],
  CPX: ["Benjamin Rivera Noriega Airport", "Culebra Island"],
  CQA: ["Canarana Airport", "Canarana"],
  CQD: ["Shahrekord Airport", "Shahrekord"],
  CQF: ["Calais-Dunkerque Airport", "Calais/Dunkerque"],
  CQM: ["Ciudad Real Central Airport", "Ciudad Real"],
  CQP: ["Cape Flattery Airport"],
  CQS: ["Costa Marques Airport", "Costa Marques"],
  CQT: ["Caquetania Airport", "Caquetania"],
  CRA: ["Craiova Airport", "Craiova"],
  CRB: ["Collarenebri Airport"],
  CRC: ["Santa Ana Airport", "Cartago"],
  CRD: ["General E. Mosconi Airport", "Comodoro Rivadavia"],
  CRE: ["Grand Strand Airport", "North Myrtle Beach"],
  CRF: ["Carnot Airport", "Carnot"],
  CRG: ["Jacksonville Executive at Craig Airport", "Jacksonville"],
  CRH: ["Cherrabah Airport", "Cherrabah Homestead Resort"],
  CRI: ["Colonel Hill Airport", "Colonel Hill"],
  CRJ: ["Coorabie Airport", "Coorabie"],
  CRK: ["Diosdado Macapagal International Airport", "Angeles/Mabalacat"],
  CRL: ["Brussels South Charleroi Airport", "Brussels"],
  CRM: ["Catarman National Airport", "Catarman"],
  CRO: ["Corcoran Airport", "Corcoran"],
  CRP: ["Corpus Christi International Airport", "Corpus Christi"],
  CRQ: ["Caravelas Airport", "Caravelas"],
  CRR: ["Ceres Airport", "Ceres"],
  CRS: ["C David Campbell Field Corsicana Municipal Airport", "Corsicana"],
  CRT: ["Z M Jack Stell Field", "Crossett"],
  CRU: ["Lauriston Airport", "Carriacou Island"],
  CRV: ["Crotone Airport", "Crotone"],
  CRW: ["Yeager Airport", "Charleston"],
  CRX: ["Roscoe Turner Airport", "Corinth"],
  CRY: ["Carlton Hill Airport", "Carlton Hill"],
  CRZ: ["Turkmenabat Airport", "T\u00fcrkmenabat"],
  CSA: ["Colonsay Airstrip", "Colonsay"],
  CSB: ["Caransebe\u015f Airport", "Caransebe\u015f"],
  CSC: ["Mojica Airport", "Ca\u00f1as"],
  CSD: ["Cresswell Downs Airport", "Cresswell Downs"],
  CSE: ["Crested Butte Airpark", "Crested Butte"],
  CSF: ["Creil Air Base", "Creil"],
  CSG: ["Columbus Metropolitan Airport", "Columbus"],
  CSH: ["Solovki Airport", "Solovetsky Islands"],
  CSI: ["Casino Airport"],
  CSK: ["Cap Skirring Airport", "Cap Skirring"],
  CSM: ["Clinton Sherman Airport", "Clinton"],
  CSN: ["Carson Airport", "Carson City"],
  CSO: ["Cochstedt Airport", "Magdeburg"],
  CSQ: ["Creston Municipal Airport", "Creston"],
  CSR: ["Casuarito Airport", "Casuarito"],
  CSS: ["Cassil\u00e2ndia Airport", "Cassil\u00e2ndia"],
  CST: ["Castaway Island Seaplane Base", "Castaway Island"],
  CSU: ["Santa Cruz do Sul Airport", "Santa Cruz Do Sul"],
  CSV: ["Crossville Memorial Whitson Field", "Crossville"],
  CSX: ["Changsha Huanghua International Airport", "Changsha"],
  CSY: ["Cheboksary Airport", "Cheboksary"],
  CSZ: ["Brigadier D.H.E. Ruiz Airport", "Coronel Suarez"],
  CTA: ["Catania-Fontanarossa Airport", "Catania"],
  CTB: ["Cut Bank International Airport", "Cut Bank"],
  CTC: ["Catamarca Airport", "Catamarca"],
  CTD: ["Alonso Valderrama Airport", "Chitr\u00e9"],
  CTE: ["Carti Airport", "Carti"],
  CTF: ["Coatepeque Airport", "Coatepeque"],
  CTG: ["Rafael Nu\u00f1ez International Airport", "Cartagena"],
  CTH: ["Chester County G O Carlson Airport", "Coatesville"],
  CTI: ["Cuito Cuanavale Airport", "Cuito Cuanavale"],
  CTK: ["Canton Municipal Airport", "Canton"],
  CTL: ["Charleville Airport", "Charleville"],
  CTM: ["Chetumal International Airport", "Chetumal"],
  CTN: ["Cooktown Airport"],
  CTO: ["Calverton Executive Airpark", "Calverton"],
  CTP: ["Carutapera Airport", "Carutapera"],
  CTQ: ["Santa Vit\u00f3ria do Palmar Airport", "Santa Vit\u00f3ria Do Palmar"],
  CTR: ["Cattle Creek Airport"],
  CTS: ["New Chitose Airport", "Chitose / Tomakomai"],
  CTT: ["Le Castellet Airport", "Le Castellet"],
  CTU: ["Chengdu Shuangliu International Airport", "Chengdu"],
  CTX: ["Cortland County Chase Field", "Cortland"],
  CTY: ["Cross City Airport", "Cross City"],
  CTZ: ["Sampson County Airport", "Clinton"],
  CUA: ["Ciudad Constituci\u00f3n Airport", "Ciudad Constituci\u00f3n"],
  CUB: ["Jim Hamilton L.B. Owens Airport", "Columbia"],
  CUC: ["Camilo Daza International Airport", "C\u00facuta"],
  CUD: ["Caloundra Airport"],
  CUE: ["Mariscal Lamar Airport", "Cuenca"],
  CUF: ["Cuneo International Airport", "Cuneo"],
  CUG: ["Cudal Airport"],
  CUH: ["Cushing Municipal Airport", "Cushing"],
  CUI: ["Currillo Airport", "Currillo"],
  CUJ: ["Culion Airport", "Culion Island"],
  CUK: ["Caye Caulker Airport", "Caye Caulker"],
  CUL: ["Bachigualato Federal International Airport", "Culiac\u00e1n"],
  CUM: ["Cuman\u00e1 (Antonio Jos\u00e9 de Sucre) Airport"],
  CUN: ["Canc\u00fan International Airport", "Canc\u00fan"],
  CUO: ["Carur\u00fa Airport", "Carur\u00fa"],
  CUP: ["General Francisco Berm\u00fadez Airport", "Car\u00fapano"],
  CUQ: ["Coen Airport", "Coen"],
  CUR: ["Hato International Airport", "Willemstad"],
  CUT: ["Cutral-Co Airport", "Cutral-Co"],
  CUU: ["General Roberto Fierro Villalobos International Airport", "Chihuahua"],
  CUV: ["Casigua El Cubo Airport", "Casigua El Cubo"],
  CUY: ["Cue Airport"],
  CUZ: ["Alejandro Velasco Astete International Airport", "Cusco"],
  CVB: ["Chungribu Airport", "Chungribu"],
  CVC: ["Cleve Airport"],
  CVE: ["Cove\u00f1as Airport", "Cove\u00f1as"],
  CVF: ["Courchevel Airport", "Courcheval"],
  CVG: ["Cincinnati Northern Kentucky International Airport", "Cincinnati"],
  CVH: ["Caviahue Airport", "Lafontaine"],
  CVI: ["Caleta Olivia Airport", "Caleta Olivia"],
  CVJ: ["General Mariano Matamoros Airport"],
  CVL: ["Cape Vogel Airport"],
  CVM: ["General Pedro Jose Mendez International Airport", "Ciudad Victoria"],
  CVN: ["Clovis Municipal Airport", "Clovis"],
  CVO: ["Corvallis Municipal Airport", "Corvallis"],
  CVQ: ["Carnarvon Airport", "Carnarvon"],
  CVR: ["Hughes Airport", "Culver City"],
  CVS: ["Cannon Air Force Base", "Clovis"],
  CVT: ["Coventry Airport", "Coventry"],
  CVU: ["Corvo Airport", "Corvo"],
  CWA: ["Central Wisconsin Airport", "Mosinee"],
  CWB: ["Afonso Pena Airport", "Curitiba"],
  CWC: ["Chernivtsi International Airport", "Chernivtsi"],
  CWE: ["Cairo West Airport", "El Cairo"],
  CWF: ["Chennault International Airport", "Lake Charles"],
  CWI: ["Clinton Municipal Airport", "Clinton"],
  CWJ: ["Cangyuan Washan Airport", "Cangyuan Va Autonomous County"],
  CWL: ["Cardiff International Airport", "Cardiff"],
  CWP: ["Campbellpore Airport", "Campbellpore"],
  CWR: ["Cowarie Airport"],
  CWS: ["Center Island Airport", "Center Island"],
  CWT: ["Cowra Airport"],
  CWW: ["Corowa Airport"],
  CWX: ["Cochise County Airport", "Willcox"],
  CXA: ["Caicara del Orinoco Airport"],
  CXB: ["Cox's Bazar Airport", "Cox's Bazar"],
  CXC: ["Chitina Airport", "Chitina"],
  CXF: ["Coldfoot Airport", "Coldfoot"],
  CXH: ["Vancouver Harbour Water Aerodrome", "Vancouver"],
  CXI: ["Cassidy International Airport", "Banana"],
  CXJ: ["Hugo Cantergiani Regional Airport", "Caxias Do Sul"],
  CXL: ["Calexico International Airport", "Calexico"],
  CXM: ["Camaxilo Airport", "Camaxilo"],
  CXN: ["Candala Airport", "Candala"],
  CXO: ["Conroe-North Houston Regional Airport", "Houston"],
  CXP: ["Tunggul Wulung Airport", "Cilacap-Java Island"],
  CXQ: ["Christmas Creek Station Airport", "Christmas Creek"],
  CXR: ["Cam Ranh Airport", "Nha Trang"],
  CXT: ["Charters Towers Airport"],
  CXY: ["Cat Cay Airport", "North Cat Cay"],
  CYA: ["Les Cayes Airport", "Les Cayes"],
  CYB: ["Gerrard Smith International Airport", "Cayman Brac"],
  CYC: ["Caye Chapel Airport", "Caye Chapel"],
  CYD: ["Town Airstrip", "San Ignacio"],
  CYF: ["Chefornak Airport", "Chefornak"],
  CYG: ["Corryong Airport"],
  CYI: ["Chiayi Airport", "Chiayi City"],
  CYL: ["Coyoles Airport", "Coyoles"],
  CYM: ["Chatham Seaplane Base", "Chatham"],
  CYO: ["Vilo Acu\u00f1a International Airport", "Cayo Largo del Sur"],
  CYP: ["Calbayog Airport", "Calbayog City"],
  CYR: ["Laguna de Los Patos International Airport", "Colonia del Sacramento"],
  CYS: ["Cheyenne Regional Jerry Olson Field", "Cheyenne"],
  CYT: ["Yakataga Airport", "Yakataga"],
  CYU: ["Cuyo Airport", "Cuyo"],
  CYW: ["Captain Rogelio Castillo National Airport", "Celaya"],
  CYX: ["Cherskiy Airport", "Cherskiy"],
  CYZ: ["Cauayan Airport", "Cauayan City"],
  CZA: ["Chichen Itza International Airport", "Chichen Itza"],
  CZB: ["Carlos Ruhl Airport", "Cruz Alta"],
  CZC: ["Copper Center 2 Airport", "Copper Center"],
  CZE: ["Jos\u00e9 Leonardo Chirinos Airport", "Coro"],
  CZF: ["Cape Romanzof LRRS Airport", "Cape Romanzof"],
  CZH: ["Corozal Municipal Airport", "Corozal"],
  CZJ: [
    "Coraz\u00f3n de Jes\u00fas Airport",
    "Coraz\u00f3n de Jes\u00fas and Nargan\u00e1 Islands",
  ],
  CZK: ["Cascade Locks State Airport", "Cascade Locks"],
  CZL: ["Mohamed Boudiaf International Airport", "Constantine"],
  CZM: ["Cozumel International Airport", "Cozumel"],
  CZN: ["Chisana Airport", "Chisana"],
  CZO: ["Chistochina Airport", "Chistochina"],
  CZP: ["Cape Pole Seaplane Base", "Cape Pole"],
  CZS: ["Cruzeiro do Sul Airport", "Cruzeiro Do Sul"],
  CZT: ["Dimmit County Airport", "Carrizo Springs"],
  CZU: ["Las Brujas Airport", "Corozal"],
  CZW: ["Cz\u0119stochowa-Rudniki", "Cz\u0119stochowa"],
  CZX: ["Changzhou Benniu Airport", "Changzhou"],
  CZY: ["Cluny Airport"],
  DAA: ["Davison Army Air Field", "Fort Belvoir"],
  DAB: ["Daytona Beach International Airport", "Daytona Beach"],
  DAC: ["Hazrat Shahjalal International Airport", "Dhaka"],
  DAD: ["Da Nang International Airport", "Da Nang"],
  DAF: ["Daup Airport"],
  DAG: ["Barstow Daggett Airport", "Daggett"],
  DAK: ["Dakhla Airport"],
  DAL: ["Dallas Love Field", "Dallas"],
  DAM: ["Damascus International Airport", "Damascus"],
  DAN: ["Danville Regional Airport", "Danville"],
  DAO: ["Dabo Airport"],
  DAP: ["Darchula Airport", "Darchula"],
  DAR: ["Julius Nyerere International Airport", "Dar es Salaam"],
  DAS: ["Great Bear Lake Airport", "Great Bear Lake"],
  DAT: ["Datong Airport", "Datong"],
  DAU: ["Daru Airport", "Daru"],
  DAV: ["Enrique Malek International Airport", "David"],
  DAX: ["Dachuan Airport", "Dazhou"],
  DAY: ["James M Cox Dayton International Airport", "Dayton"],
  DAZ: ["Darwaz Airport", "Darwaz"],
  DBA: ["Dalbandin Airport", "Dalbandin"],
  DBB: ["El Alamein International Airport", "El Alamein"],
  DBC: ["Baicheng Chang'an Airport", "Baicheng"],
  DBD: ["Dhanbad Airport"],
  DBK: ["Dutch Bay SPB", "Kalpitiya Island"],
  DBM: ["Debra Marcos Airport", "Debra Marcos"],
  DBN: ["W H 'Bud' Barron Airport", "Dublin"],
  DBO: ["Dubbo City Regional Airport", "Dubbo"],
  DBP: ["Debepare Airport", "Debepare"],
  DBQ: ["Dubuque Regional Airport", "Dubuque"],
  DBS: ["Dubois Municipal Airport", "Dubois"],
  DBT: ["Debre Tabor Airport", "Debre Tabor"],
  DBU: ["Dambulu Oya Tank Seaplane Base", "Dambulla"],
  DBV: ["Dubrovnik Airport", "Dubrovnik"],
  DBY: ["Dalby Airport"],
  DCA: ["Ronald Reagan Washington National Airport", "Washington"],
  DCF: ["Canefield Airport", "Canefield"],
  DCG: ["Dubai Creek SPB", "Dubai"],
  DCI: ["Decimomannu Air Base", "Decimomannu"],
  DCK: ["Dahl Creek Airport", "Dahl Creek"],
  DCM: ["Castres-Mazamet Airport", "Castres/Mazamet"],
  DCN: ["RAAF Base Curtin"],
  DCR: ["Decatur HI-Way Airfield", "Decatur"],
  DCT: ["Duncan Town Airport"],
  DCU: ["Pryor Field Regional Airport", "Decatur"],
  DCY: ["Daocheng Yading Airport", "Daocheng County"],
  DDC: ["Dodge City Regional Airport", "Dodge City"],
  DDD: ["Dhaalu Atoll Airport", "Kudahuvadhoo"],
  DDG: ["Dandong Airport", "Dandong"],
  DDM: ["Dodoima Airport"],
  DDN: ["Delta Downs Airport"],
  DDP: ["Dorado Beach Airport", "Dorado"],
  DDU: ["Dadu West Airport", "Dadu"],
  DEA: ["Dera Ghazi Khan Airport", "Dera Ghazi Khan"],
  DEB: ["Debrecen International Airport", "Debrecen"],
  DEC: ["Decatur Airport", "Decatur"],
  DED: ["Dehradun Airport", "Dehradun"],
  DEE: ["Mendeleyevo Airport", "Kunashir Island"],
  DEF: ["Dezful Airport"],
  DEH: ["Decorah Municipal Airport", "Decorah"],
  DEI: ["Denis Island Airport", "Denis Island"],
  DEL: ["Indira Gandhi International Airport", "New Delhi"],
  DEM: ["Dembidollo Airport", "Dembidollo"],
  DEN: ["Denver International Airport", "Denver"],
  DEP: ["Daporijo Airport", "Daporijo"],
  DER: ["Derim Airport", "Derim"],
  DES: ["Desroches Airport", "Desroches Island"],
  DET: ["Coleman A. Young Municipal Airport", "Detroit"],
  DEX: ["Nop Goliat Airport", "Dekai - Yahukimo"],
  DEZ: ["Deir ez-Zor Air Base", "Deir ez-Zor"],
  DFI: ["Defiance Memorial Airport", "Defiance"],
  DFP: ["Drumduff Airport", "Drumduff"],
  DFW: ["Dallas Fort Worth International Airport", "Dallas-Fort Worth"],
  DGA: ["Dangriga Airport", "Dangriga"],
  DGC: ["Degah Bur Airport", "Degah Bur"],
  DGD: ["Dalgaranga Gold Mine Airport"],
  DGE: ["Mudgee Airport", "Mudgee"],
  DGF: ["Douglas Lake Airport", "Douglas Lake"],
  DGG: ["Fishermans Airfield", "Daugo Island"],
  DGL: ["Douglas Municipal Airport", "Douglas"],
  DGM: ["Dandugama Seaplane Base", "Colombo"],
  DGN: ["Dahlgren Naval Surface Warfare Center Airport", "Dahlgren"],
  DGO: ["General Guadalupe Victoria International Airport", "Durango"],
  DGP: ["Daugavpils Intrenational Airport", "Daugavpils"],
  DGR: ["Dargaville Aerodrome"],
  DGT: ["Sibulan Airport", "Dumaguete City"],
  DGU: ["Dedougou Airport", "Dedougou"],
  DGW: ["Converse County Airport", "Douglas"],
  DHA: ["King Abdulaziz Air Base"],
  DHB: ["Deer Harbor SPB", "Deer Harbor"],
  DHD: ["Durham Downs Airport"],
  DHF: ["Al Dhafra Air Base"],
  DHG: ["Dalnegorsk Airport", "Dalnegorsk"],
  DHI: ["Dhangarhi Airport", "Dhangarhi"],
  DHM: ["Kangra Airport"],
  DHN: ["Dothan Regional Airport", "Dothan"],
  DHR: ["De Kooy Airport", "Den Helder"],
  DHT: ["Dalhart Municipal Airport", "Dalhart"],
  DIA: ["Doha International Airport", "Doha"],
  DIB: ["Dibrugarh Airport", "Dibrugarh"],
  DIE: ["Arrachart Airport"],
  DIG: ["Diqing Airport", "Shangri-La"],
  DIJ: ["Dijon-Bourgogne Airport", "Dijon/Longvic"],
  DIK: ["Dickinson Theodore Roosevelt Regional Airport", "Dickinson"],
  DIL: ["Presidente Nicolau Lobato International Airport", "Dili"],
  DIM: ["Dimbokro Airport", "Dimbokro"],
  DIN: ["Dien Bien Phu Airport", "Dien Bien Phu"],
  DIP: ["Diapaga Airport", "Diapaga"],
  DIQ: ["Brigadeiro Cabral Airport", "Divin\u00f3polis"],
  DIR: ["Aba Tenna Dejazmach Yilma International Airport", "Dire Dawa"],
  DIS: ["Ngot Nzoungou Airport", "Dolisie"],
  DIU: ["Diu Airport", "Diu"],
  DIW: ["Mawella Lagoon Seaplane Base", "Dickwella"],
  DIY: ["Diyarbakir Airport", "Diyarbakir"],
  DJA: ["Djougou Airport", "Djougou"],
  DJB: ["Sultan Thaha Airport", "Jambi-Sumatra Island"],
  DJE: ["Djerba Zarzis International Airport", "Djerba"],
  DJG: ["Djanet Inedbirene Airport", "Djanet"],
  DJH: ["Jebel Ali Seaplane Base", "Jebel Ali"],
  DJJ: ["Sentani International Airport", "Jayapura-Papua Island"],
  DJM: ["Djambala Airport", "Djambala"],
  DJN: ["Delta Junction Airport", "Delta Junction"],
  DJO: ["Daloa Airport"],
  DJR: ["Dajarra Airport"],
  DJU: ["Dj\u00fapivogur Airport", "Dj\u00fapivogur"],
  DKA: ["Katsina Airport", "Katsina"],
  DKI: ["Dunk Island Airport", "Dunk Island"],
  DKK: ["Chautauqua County-Dunkirk Airport", "Dunkirk"],
  DKR: ["L\u00e9opold S\u00e9dar Senghor International Airport", "Dakar"],
  DKS: ["Dikson Airport", "Dikson"],
  DKV: ["Docker River Airport"],
  DLA: ["Douala International Airport", "Douala"],
  DLC: ["Zhoushuizi Airport", "Dalian"],
  DLD: ["Geilo Airport Dagali", "Dagali"],
  DLE: ["Dole-Tavaux Airport", "Dole/Tavaux"],
  DLF: ["DLF Airport", "Del Rio"],
  DLG: ["Dillingham Airport", "Dillingham"],
  DLH: ["Duluth International Airport", "Duluth"],
  DLI: ["Lien Khuong Airport", "Da Lat"],
  DLK: ["Dulkaninna Airport", "Dulkaninna"],
  DLL: ["Dillon County Airport", "Dillon"],
  DLM: ["Dalaman International Airport", "Dalaman"],
  DLN: ["Dillon Airport", "Dillon"],
  DLR: ["Dalnerechensk Airport", "Dalnerechensk"],
  DLS: ["Columbia Gorge Regional the Dalles Municipal Airport", "The Dalles"],
  DLU: ["Dali Airport", "Xiaguan"],
  DLV: ["Delissaville Airport"],
  DLY: ["Dillon's Bay Airport", "Dillon's Bay"],
  DLZ: ["Dalanzadgad Airport", "Dalanzadgad"],
  DMA: ["Davis Monthan Air Force Base", "Tucson"],
  DMB: ["Taraz Airport", "Taraz"],
  DMD: ["Doomadgee Airport"],
  DME: ["Domodedovo International Airport", "Moscow"],
  DMK: ["Don Mueang International Airport", "Bangkok"],
  DMM: ["King Fahd International Airport", "Ad Dammam"],
  DMN: ["Deming Municipal Airport", "Deming"],
  DMO: ["Sedalia Memorial Airport", "Sedalia"],
  DMT: ["Diamantino Airport", "Diamantino"],
  DMU: ["Dimapur Airport", "Dimapur"],
  DNA: ["Kadena Air Base"],
  DNB: ["Dunbar Airport"],
  DND: ["Dundee Airport", "Dundee"],
  DNF: ["Martubah Airport", "Derna"],
  DNG: ["Doongan Airport"],
  DNH: ["Dunhuang Airport", "Dunhuang"],
  DNI: ["Wad Medani Airport", "Wad Medani"],
  DNK: ["Dnipropetrovsk International Airport", "Dnipropetrovsk"],
  DNL: ["Daniel Field", "Augusta"],
  DNN: ["Dalton Municipal Airport", "Dalton"],
  DNO: ["Dian\u00f3polis Airport", "Dian\u00f3polis"],
  DNP: ["Tulsipur Airport", "Dang"],
  DNQ: ["Deniliquin Airport", "Deniliquin"],
  DNR: ["Dinard-Pleurtuit-Saint-Malo Airport", "Dinard/Pleurtuit/Saint-Malo"],
  DNS: ["Denison Municipal Airport", "Denison"],
  DNU: ["Dinangat Airport", "Dinangat"],
  DNV: ["Vermilion Regional Airport", "Danville"],
  DNX: ["Galegu Airport", "Dinder"],
  DNZ: ["\u00c7ardak Airport", "Denizli"],
  DOA: ["Doany Airport", "Doany"],
  DOB: ["Rar Gwamar Airport", "Dobo-Warmar Island"],
  DOC: ["Dornoch Airfield", "Dornoch"],
  DOD: ["Dodoma Airport", "Dodoma"],
  DOE: ["Djumu-Djomoe Airport", "Djumu-Djomoe"],
  DOG: ["Dongola Airport", "Dongola"],
  DOH: ["Hamad International Airport", "Doha"],
  DOI: ["Doini Airport", "Castori Islets"],
  DOK: ["Donetsk International Airport", "Donetsk"],
  DOL: ["Deauville-Saint-Gatien Airport", "Deauville"],
  DOM: ["Douglas-Charles Airport", "Marigot"],
  DON: ["Dos Lagunas Airport", "Dos Lagunas"],
  DOO: ["Dorobisoro Airport", "Dorobisoro"],
  DOP: ["Dolpa Airport", "Dolpa"],
  DOR: ["Dori Airport", "Dori"],
  DOS: ["Dios Airport", "Dios"],
  DOU: ["Dourados Airport", "Dourados"],
  DOV: ["Dover Air Force Base", "Dover"],
  DOX: ["Dongara Airport"],
  DOY: ["Dongying Shengli Airport", "Dongying"],
  DPA: ["Dupage Airport", "Chicago/West Chicago"],
  DPB: ["Pampa Guanaco Airport", "Bahia Inutil"],
  DPE: ["St Aubin Airport", "Dieppe"],
  DPG: ["Michael AAF (Dugway Proving Ground) Airport", "Dugway Proving Ground"],
  DPK: ["Deer Park Airport", "Deer Park"],
  DPL: ["Dipolog Airport", "Dipolog City"],
  DPO: ["Devonport Airport", "Devonport"],
  DPS: ["Ngurah Rai (Bali) International Airport", "Denpasar-Bali Island"],
  DPT: ["Deputatskiy Airport", "Deputatskiy"],
  DPU: ["Dumpu Airport", "Dumpu"],
  DQA: ["Saertu Airport", "Daqing Shi"],
  DQH: ["Alpine Airstrip", "Deadhorse"],
  DQM: ["Duqm International Airport", "Duqm"],
  DRA: ["Desert Rock Airport", "Mercury"],
  DRB: ["Derby Airport"],
  DRC: ["Dirico Airport", "Dirico"],
  DRD: ["Dorunda Airport", "Dorunda Outstation"],
  DRE: ["Drummond Island Airport", "Drummond Island"],
  DRF: ["Drift River Airport", "Kenai"],
  DRG: ["Deering Airport", "Deering"],
  DRH: ["Dabra Airport", "Dabra-Papua Island"],
  DRI: ["Beauregard Regional Airport", "De Ridder"],
  DRJ: ["Drietabbetje Airport", "Drietabbetje"],
  DRK: ["Drake Bay Airport", "Puntarenas"],
  DRN: ["Dirranbandi Airport"],
  DRO: ["Durango La Plata County Airport", "Durango"],
  DRR: ["Durrie Airport"],
  DRS: ["Dresden Airport", "Dresden"],
  DRT: ["Del Rio International Airport", "Del Rio"],
  DRU: ["Drummond Airport", "Drummond"],
  DRV: ["Dharavandhoo Airport", "Baa Atoll"],
  DRW: ["Darwin International Airport", "Darwin"],
  DRY: ["Drysdale River Airport"],
  DSA: ["Robin Hood Doncaster Sheffield Airport", "Doncaster"],
  DSC: ["Dschang Airport", "Dschang"],
  DSD: ["La D\u00e9sirade Airport", "Grande Anse"],
  DSE: ["Combolcha Airport", "Dessie"],
  DSG: ["Dilasag Airport", "Dilasag"],
  DSI: ["Destin Executive Airport", "Destin"],
  DSK: ["Dera Ismael Khan Airport", "Dera Ismael Khan"],
  DSM: ["Des Moines International Airport", "Des Moines"],
  DSN: ["Ordos Ejin Horo Airport", "Ordos"],
  DSO: ["Sondok Airport", "S\u014fnd\u014fng-ni"],
  DSS: ["Blaise Diagne International Airport", "Dakar"],
  DSV: ["Dansville Municipal Airport", "Dansville"],
  DSX: ["Dongsha Island Airport", "Pratas Island"],
  DTA: ["Delta Municipal Airport", "Delta"],
  DTB: ["Silangit Airport", "Siborong-Borong"],
  DTD: ["Datadawai Airport", "Datadawai-Borneo Island"],
  DTE: ["Daet Airport", "Daet"],
  DTH: ["Furnace Creek Airport", "Death Valley National Park"],
  DTI: ["Diamantina Airport", "Diamantina"],
  DTL: ["Detroit Lakes Airport - Wething Field", "Detroit Lakes"],
  DTM: ["Dortmund Airport", "Dortmund"],
  DTN: ["Shreveport Downtown Airport", "Shreveport"],
  DTR: ["Decatur Shores Airport", "Decatur"],
  DTU: ["Wudalianchi Dedu Airport", "Wudalianchi"],
  DTW: ["Detroit Metropolitan Wayne County Airport", "Detroit"],
  DUA: ["Eaker Field", "Durant"],
  DUB: ["Dublin Airport", "Dublin"],
  DUC: ["Halliburton Field", "Duncan"],
  DUD: ["Dunedin Airport", "Dunedin"],
  DUE: ["Dundo Airport", "Chitato"],
  DUF: ["Pine Island Airport", "Corolla"],
  DUG: ["Bisbee Douglas International Airport", "Douglas Bisbee"],
  DUJ: ["DuBois Regional Airport", "Dubois"],
  DUK: ["Mubatuba Airport", "Mubatuba"],
  DUM: ["Pinang Kampai Airport", "Dumai-Sumatra Island"],
  DUN: ["Dundas Airport", "Pituffik"],
  DUQ: ["Duncan Airport", "Duncan"],
  DUR: ["King Shaka International Airport", "Durban"],
  DUS: ["D\u00fcsseldorf Airport", "D\u00fcsseldorf"],
  DUT: ["Unalaska Airport", "Unalaska"],
  DVD: ["Andavadoaka Airport", "Andavadoaka"],
  DVK: ["Diavik Airport", "Diavik"],
  DVL: ["Devils Lake Regional Airport", "Devils Lake"],
  DVN: ["Davenport Municipal Airport", "Davenport"],
  DVO: ["Francisco Bangoy International Airport", "Davao City"],
  DVP: ["Davenport Downs Airport"],
  DVR: ["Daly River Airport"],
  DVT: ["Phoenix Deer Valley Airport", "Phoenix"],
  DWA: ["Dwangwa Airport", "Dwangwa"],
  DWB: ["Soalala Airport", "Soalala"],
  DWC: ["Al Maktoum International Airport", "Jebel Ali"],
  DWD: ["King Salman Abdulaziz Airport", "Dawadmi"],
  DWF: ["Wright Field", "Dayton"],
  DWH: ["David Wayne Hooks Memorial Airport", "Houston"],
  DWN: ["Downtown Airpark", "Oklahoma City"],
  DWO: ["Diyawanna Oya Seaplane Base", "Kotte"],
  DWR: ["Dywer Airbase", "Camp Dwyer"],
  DWS: ["Lake Buena Vista STOLport", "Walt Disney World"],
  DXB: ["Dubai International Airport", "Dubai"],
  DXD: ["Dixie Airport", "New Dixie"],
  DXE: ["Bruce Campbell Field", "Madison"],
  DXR: ["Danbury Municipal Airport", "Danbury"],
  DYA: ["Dysart Airport"],
  DYG: ["Dayong Airport", "Dayong"],
  DYL: ["Doylestown Airport", "Doylestown"],
  DYM: ["Diamantina Lakes Airport", "Diamantina Lakes"],
  DYR: ["Ugolny Airport", "Anadyr"],
  DYS: ["Dyess Air Force Base", "Abilene"],
  DYU: ["Dushanbe Airport", "Dushanbe"],
  DYW: ["Daly Waters Airport", "Daly Waters"],
  DZA: ["Dzaoudzi Pamandzi International Airport", "Dzaoudzi"],
  DZI: ["Las Flores Airport", "Codazzi"],
  DZN: ["Zhezkazgan Airport", "Zhezkazgan"],
  DZO: ["Santa Bernardina International Airport", "Durazno"],
  DZU: ["Dazu Air Base", "Dazu"],
  EAA: ["Eagle Airport", "Eagle"],
  EAB: ["Abbse Airport", "Abbse"],
  EAE: ["Siwo Airport", "Emae Island"],
  EAL: ["Elenak Airport", "Mejato Island"],
  EAM: ["Nejran Airport"],
  EAN: ["Phifer Airfield", "Wheatland"],
  EAR: ["Kearney Regional Airport", "Kearney"],
  EAS: ["San Sebastian Airport", "Hondarribia"],
  EAT: ["Pangborn Memorial Airport", "Wenatchee"],
  EAU: ["Chippewa Valley Regional Airport", "Eau Claire"],
  EBA: ["Marina Di Campo Airport", "Marina  Di Campo"],
  EBB: ["Entebbe International Airport", "Kampala"],
  EBD: ["El Obeid Airport", "Al-Ubayyid"],
  EBG: ["El Bagre Airport", "El Bagre"],
  EBH: ["El Bayadh Airport", "El Bayadh"],
  EBJ: ["Esbjerg Airport", "Esbjerg"],
  EBL: ["Erbil International Airport", "Arbil"],
  EBM: ["El Borma Airport", "El Borma"],
  EBN: ["Ebadon Airport", "Ebadon Island"],
  EBO: ["Ebon Airport", "Ebon Atoll"],
  EBS: ["Webster City Municipal Airport", "Webster City"],
  EBU: [
    "Saint-\u00c9tienne-Bouth\u00e9on Airport",
    "Saint-\u00c9tienne/Bouth\u00e9on",
  ],
  EBW: ["Ebolowa Airport", "Ebolowa"],
  ECA: ["Iosco County Airport", "East Tawas"],
  ECG: [
    "Elizabeth City Regional Airport & Coast Guard Air Station",
    "Elizabeth City",
  ],
  ECH: ["Echuca Airport"],
  ECI: ["Costa Esmeralda Airport", "Tola"],
  ECN: ["Ercan International Airport", "Nicosia"],
  ECO: ["El Encanto Airport", "El Encanto"],
  ECP: ["Northwest Florida Beaches International Airport", "Panama City Beach"],
  ECR: ["El Charco Airport", "El Charco"],
  ECS: ["Mondell Field", "Newcastle"],
  ECV: ["Cuatro Vientos Airport", "Madrid"],
  EDA: ["Edna Bay Seaplane Base", "Edna Bay"],
  EDB: ["El Debba Airport", "El Debba"],
  EDD: ["Erldunda Airport"],
  EDE: ["Northeastern Regional Airport", "Edenton"],
  EDF: ["Elmendorf Air Force Base", "Anchorage"],
  EDI: ["Edinburgh Airport", "Edinburgh"],
  EDK: ["Captain Jack Thomas El Dorado Airport", "El Dorado"],
  EDL: ["Eldoret International Airport", "Eldoret"],
  EDM: ["La Roche-sur-Yon Airport", "La Roche-sur-Yon/Les Ajoncs"],
  EDN: ["Yedinka Airport", "Yedinka"],
  EDO: ["Bal\u0131kesir K\u00f6rfez Airport", "Edremit"],
  EDQ: ["Erandique Airport", "Erandique"],
  EDR: ["Pormpuraaw Airport", "Pormpuraaw"],
  EDW: ["Edwards Air Force Base", "Edwards"],
  EED: ["Needles Airport", "Needles"],
  EEK: ["Eek Airport", "Eek"],
  EEN: ["Dillant Hopkins Airport", "Keene"],
  EFD: ["Ellington Airport", "Houston"],
  EFG: ["Efogi Airport", "Efogi"],
  EFK: ["Northeast Kingdom International Airport", "Newport"],
  EFL: ["Kefallinia Airport", "Kefallinia Island"],
  EFW: ["Jefferson Municipal Airport", "Jefferson"],
  EGA: ["Engati Airstrip", "Engati"],
  EGC: ["Bergerac-Roumani\u00e8re Airport", "Bergerac/Roumani\u00e8re"],
  EGE: ["Eagle County Regional Airport", "Eagle"],
  EGH: ["El Gora Airport", "El Gora"],
  EGI: ["Duke Field", "Crestview"],
  EGL: ["Negele Airport", "Negele Borana"],
  EGM: ["Sege Airport", "Sege"],
  EGN: ["Geneina Airport", "Geneina"],
  EGO: ["Belgorod International Airport", "Belgorod"],
  EGP: ["Maverick County Memorial International Airport", "Eagle Pass"],
  EGS: ["Egilssta\u00f0ir Airport", "Egilssta\u00f0ir"],
  EGV: ["Eagle River Union Airport", "Eagle River"],
  EGX: ["Egegik Airport", "Egegik"],
  EHL: ["El Bolson Airport", "El Bolson"],
  EHM: ["Cape Newenham LRRS Airport", "Cape Newenham"],
  EHT: ["Rentschler Heliport", "East Hartford"],
  EIB: ["Eisenach-Kindel Airport", "Eisenach"],
  EIE: ["Yeniseysk Airport", "Yeniseysk"],
  EIH: ["Einasleigh Airport", "Einasleigh"],
  EIK: ["Yeysk Airport", "Yeysk"],
  EIL: ["Eielson Air Force Base", "Fairbanks"],
  EIN: ["Eindhoven Airport", "Eindhoven"],
  EIS: ["Terrance B. Lettsome International Airport", "Road Town"],
  EIY: ["Ein Yahav Airfield", "Sapir"],
  EJA: ["Yarigu\u00edes Airport", "Barrancabermeja"],
  EJH: ["Al Wajh Domestic Airport", "Al Wajh"],
  EJN: ["Ejin Banner-Taolai Airport", "Ejin Banner"],
  EJT: ["Enejit Airport", "Enejit Island"],
  EKA: ["Murray Field", "Eureka"],
  EKB: ["Ekibastuz Airport", "Ekibastuz"],
  EKD: ["Elkedra Airport"],
  EKI: ["Elkhart Municipal Airport", "Elkhart"],
  EKN: ["Elkins-Randolph Co-Jennings Randolph Field", "Elkins"],
  EKO: ["Elko Regional Airport", "Elko"],
  EKS: ["Shakhtyorsk Airport", "Shakhtersk"],
  EKT: ["Eskilstuna Airport", "Eskilstuna"],
  EKX: ["Addington Field", "Elizabethtown"],
  ELA: ["Eagle Lake Airport", "Eagle Lake"],
  ELB: ["Las Flores Airport", "El Banco"],
  ELC: ["Elcho Island Airport", "Elcho Island"],
  ELD: ["South Arkansas Regional At Goodwin Field", "El Dorado"],
  ELE: ["EL Real Airport", "El Real de Santa Mar\u00eda"],
  ELF: ["El Fasher Airport", "El Fasher"],
  ELG: ["El Golea Airport"],
  ELH: ["North Eleuthera Airport", "North Eleuthera"],
  ELI: ["Elim Airport", "Elim"],
  ELJ: ["El Recreo Airport", "Ruperto Polania"],
  ELK: ["Elk City Regional Business Airport", "Elk City"],
  ELL: ["Ellisras Matimba Airport", "Ellisras"],
  ELM: ["Elmira Corning Regional Airport", "Elmira/Corning"],
  ELN: ["Bowers Field", "Ellensburg"],
  ELO: ["El Dorado Airport", "El Dorado"],
  ELP: ["El Paso International Airport", "El Paso"],
  ELQ: ["Gassim Airport"],
  ELR: ["Elelim Airport", "Elelim-Papua Island"],
  ELS: ["Ben Schoeman Airport", "East London"],
  ELT: ["El Tor Airport"],
  ELU: ["Guemar Airport", "Guemar"],
  ELV: ["Elfin Cove Seaplane Base", "Elfin Cove"],
  ELW: ["Ellamar Seaplane Base", "Ellamar"],
  ELX: ["Isla de Tigre  Heliport", "Isla de Tigre"],
  ELY: ["Ely Airport Yelland Field", "Ely"],
  ELZ: ["Wellsville Municipal Arpt,Tarantine Field", "Wellsville"],
  EMA: ["East Midlands Airport", "Nottingham"],
  EMD: ["Emerald Airport", "Emerald"],
  EME: ["Emden Airport", "Emden"],
  EMG: ["Empangeni Airport", "Empangeni"],
  EMI: ["Emirau Airport", "Emirau Island"],
  EMK: ["Emmonak Airport", "Emmonak"],
  EML: ["Emmen Air Base"],
  EMM: ["Kemmerer Municipal Airport", "Kemmerer"],
  EMN: ["N\u00e9ma Airport", "N\u00e9ma"],
  EMO: ["Emo River Airstrip", "Emo Mission"],
  EMP: ["Emporia Municipal Airport", "Emporia"],
  EMS: ["Embessa Airport", "Embessa"],
  EMT: ["San Gabriel Valley Airport", "El Monte"],
  EMX: ["El Maiten Airport", "El Maiten"],
  EMY: ["El Minya Airport", "Minya"],
  ENA: ["Kenai Municipal Airport", "Kenai"],
  ENB: ["Eneabba Airport", "Eneabba"],
  ENC: ["Nancy-Essey Airport", "Nancy/Essey"],
  END: ["Vance Air Force Base", "Enid"],
  ENE: ["Ende (H Hasan Aroeboesman) Airport", "Ende-Flores Island"],
  ENF: ["Enontekio Airport", "Enontekio"],
  ENH: ["Enshi Airport", "Enshi"],
  ENI: ["El Nido Airport", "El Nido"],
  ENJ: ["El Naranjo Airport", "El Naranjo"],
  ENK: ["St Angelo Airport", "Enniskillen"],
  ENL: ["Centralia Municipal Airport", "Centralia"],
  ENN: ["Nenana Municipal Airport", "Nenana"],
  ENO: ["Encarnaci\u00f3n Airport", "Encarnaci\u00f3n"],
  ENS: ["Twente Airport", "Enschede"],
  ENT: ["Eniwetok Airport", "Eniwetok Atoll"],
  ENU: ["Akanu Ibiam International Airport", "Enegu"],
  ENV: ["Wendover Airport", "Wendover"],
  ENW: ["Kenosha Regional Airport", "Kenosha"],
  ENY: ["Yan'an Ershilipu Airport", "Yan'an"],
  EOH: ["Enrique Olaya Herrera Airport", "Medell\u00edn"],
  EOI: ["Eday Airport", "Eday"],
  EOK: ["Keokuk Municipal Airport", "Keokuk"],
  EOR: ["El Dorado Airport", "Bolivar"],
  EOZ: ["Elorza Airport"],
  EPA: ["El Palomar Airport", "El Palomar"],
  EPG: ["Browns Airport", "Weeping Water"],
  EPH: ["Ephrata Municipal Airport", "Ephrata"],
  EPL: ["\u00c9pinal-Mirecourt Airport", "\u00c9pinal/Mirecourt"],
  EPR: ["Esperance Airport", "Esperance"],
  EPS: ["Samana El Portillo Airport", "Samana"],
  EPT: ["Eliptamin Airport", "Eliptamin"],
  EPU: ["P\u00e4rnu Airport", "P\u00e4rnu"],
  EQS: ["Brigadier Antonio Parodi Airport", "Esquel"],
  ERA: ["Erigavo Airport", "Erigavo"],
  ERB: ["Ernabella Airport"],
  ERC: ["Erzincan Airport", "Erzincan"],
  ERD: ["Berdyansk Airport", "Berdyansk"],
  ERE: ["Erave Airport", "Erave"],
  ERF: ["Erfurt Airport", "Erfurt"],
  ERG: ["Yerbogachen Airport", "Erbogachen"],
  ERH: ["Moulay Ali Cherif Airport", "Errachidia"],
  ERI: ["Erie International Tom Ridge Field", "Erie"],
  ERL: ["Erenhot Saiwusu International Airport", "Erenhot"],
  ERM: ["Erechim Airport", "Erechim"],
  ERN: ["Eirunep\u00e9 Airport", "Eirunep\u00e9"],
  ERQ: ["Elrose Airport", "Elrose Mine"],
  ERR: ["Errol Airport", "Errol"],
  ERS: ["Eros Airport", "Windhoek"],
  ERT: ["Erdenet Airport", "Erdenet"],
  ERU: ["Erume Airport", "Erume"],
  ERV: ["Kerrville Municipal Louis Schreiner Field", "Kerrville"],
  ERZ: ["Erzurum International Airport", "Erzurum"],
  ESA: ["Esa'ala Airport", "Esa'ala"],
  ESB: ["Esenbo\u011fa International Airport", "Ankara"],
  ESC: ["Delta County Airport", "Escanaba"],
  ESD: ["Orcas Island Airport", "Eastsound"],
  ESE: ["Ensenada International Airport", "Ensenada"],
  ESF: ["Esler Regional Airport", "Alexandria"],
  ESG: [
    "Dr. Luis Maria Arga\u00f1a International Airport",
    "Mariscal Estigarribia",
  ],
  ESH: ["Shoreham Airport", "Brighton"],
  ESI: ["Espinosa Airport", "Espinosa"],
  ESK: ["Eski\u015fehir Air Base"],
  ESL: ["Elista Airport", "Elista"],
  ESM: ["General Rivadeneira Airport", "Tachina"],
  ESN: ["Easton Newnam Field", "Easton"],
  ESO: ["Ohkay Owingeh Airport", "Espanola"],
  ESP: ["Stroudsburg Pocono Airport", "East Stroudsburg"],
  ESR: ["Ricardo Garc\u00eda Posada Airport", "El Salvador"],
  ESS: ["Essen Mulheim Airport"],
  EST: ["Estherville Municipal Airport", "Estherville"],
  ESU: ["Mogador Airport", "Essaouira"],
  ESW: ["Easton State Airport", "Easton"],
  ETB: ["West Bend Municipal Airport", "West Bend"],
  ETD: ["Etadunna Airport", "Etadunna"],
  ETE: ["Metema Airport", "Metema"],
  ETH: ["Eilat Airport", "Eilat"],
  ETL: ["Svetlaya Airport", "Svetlaya"],
  ETM: ["Ramon Airport", "Eilat"],
  ETN: ["Eastland Municipal Airport", "Eastland"],
  ETR: ["Santa Rosa International Airport", "Santa Rosa"],
  ETS: ["Enterprise Municipal Airport", "Enterprise"],
  ETZ: ["Metz-Nancy-Lorraine Airport", "Metz / Nancy"],
  EUA: ["Kaufana Airport", "Eua Island"],
  EUC: ["Eucla Airport"],
  EUE: ["Eureka Airport", "Eureka"],
  EUF: ["Weedon Field", "Eufaula"],
  EUG: ["Mahlon Sweet Field", "Eugene"],
  EUM: ["Neum\u00fcnster Airport", "Neum\u00fcnster"],
  EUN: ["Hassan I Airport", "El Aai\u00fan"],
  EUO: ["Paratebueno Airport", "Paratebueno"],
  EUQ: ["Evelio Javier Airport", "San Jose"],
  EUX: ["F. D. Roosevelt Airport", "Sint Eustatius"],
  EVA: ["Ben Bruce Memorial Airpark", "Evadale"],
  EVD: ["Eva Downs Airport", "Eva Downs"],
  EVE: ["Harstad/Narvik Airport, Evenes", "Evenes"],
  EVG: ["Sveg Airport"],
  EVH: ["Evans Head Aerodrome"],
  EVM: ["Eveleth Virginia Municipal Airport", "Eveleth"],
  EVN: ["Zvartnots International Airport", "Yerevan"],
  EVV: ["Evansville Regional Airport", "Evansville"],
  EVW: ["Evanston-Uinta County Airport-Burns Field", "Evanston"],
  EVX: ["\u00c9vreux-Fauville (BA 105) Air Base", "\u00c9vreux/Fauville"],
  EWB: ["New Bedford Regional Airport", "New Bedford"],
  EWE: ["Ewer Airport", "Asmat"],
  EWI: ["Enarotali Airport", "Enarotali-Papua Island"],
  EWK: ["Newton City-County Airport", "Newton"],
  EWN: ["Coastal Carolina Regional Airport", "New Bern"],
  EWO: ["Ewo Airport", "Ewo"],
  EWR: ["Newark Liberty International Airport", "Newark"],
  EWY: ["RAF Greenham Common", "Berkshire"],
  EXI: ["Excursion Inlet Seaplane Base", "Excursion Inlet"],
  EXM: ["Exmouth Airport"],
  EXT: ["Exeter International Airport", "Exeter"],
  EYK: ["Beloyarskiy Airport"],
  EYL: ["Y\u00e9liman\u00e9 Airport", "Y\u00e9liman\u00e9"],
  EYP: ["El Yopal Airport", "El Yopal"],
  EYR: ["Yerington Municipal Airport", "Yerington"],
  EYS: ["Eliye Springs Airport", "Eliye Springs"],
  EYW: ["Key West International Airport", "Key West"],
  EZE: ["Ministro Pistarini International Airport", "Buenos Aires"],
  EZS: ["Elaz\u0131\u011f Airport", "Elaz\u0131\u011f"],
  EZV: ["Berezovo Airport"],
  FAA: ["Faranah Airport", "Faranah"],
  FAB: ["Farnborough Airport", "Farnborough"],
  FAC: ["Faaite Airport"],
  FAE: ["Vagar Airport", "Vagar"],
  FAF: ["Felker Army Air Field", "Fort Eustis"],
  FAG: ["Fagurh\u00f3lsm\u00fdri Airport", "Fagurh\u00f3lsm\u00fdri"],
  FAH: ["Farah Airport", "Farah"],
  FAI: ["Fairbanks International Airport", "Fairbanks"],
  FAJ: ["Diego Jimenez Torres Airport", "Fajardo"],
  FAK: ["False Island Seaplane Base", "False Island"],
  FAL: ["Falcon State Airport", "Roma"],
  FAM: ["Farmington Regional Airport", "Farmington"],
  FAN: ["Lista Airport", "Farsund"],
  FAO: ["Faro Airport", "Faro"],
  FAQ: ["Frieda River Airport", "Frieda River"],
  FAR: ["Hector International Airport", "Fargo"],
  FAS: [
    "F\u00e1skr\u00fa\u00f0sfj\u00f6r\u00f0ur Airport",
    "F\u00e1skr\u00fa\u00f0sfj\u00f6r\u00f0ur",
  ],
  FAT: ["Fresno Yosemite International Airport", "Fresno"],
  FAU: ["Fahud Airport", "Fahud"],
  FAV: ["Fakarava Airport"],
  FAY: ["Fayetteville Regional Grannis Field", "Fayetteville"],
  FAZ: ["Fasa Airport", "Fasa"],
  FBA: ["Fonte Boa Airport", "Fonte Boa"],
  FBD: ["Fayzabad Airport", "Fayzabad"],
  FBE: ["Francisco Beltr\u00e3o Airport", "Francisco Beltr\u00e3o"],
  FBG: ["Simmons Army Air Field", "Fort Bragg"],
  FBK: ["Ladd AAF Airfield", "Fairbanks/Ft Wainwright"],
  FBL: ["Faribault Municipal Airport-Liz Wall Strohfus Field", "Faribault"],
  FBM: ["Lubumbashi International Airport", "Lubumbashi"],
  FBR: ["Fort Bridger Airport", "Fort Bridger"],
  FBS: ["Friday Harbor Seaplane Base", "Friday Harbor"],
  FBU: ["Oslo, Fornebu Airport", "Oslo"],
  FBY: ["Fairbury Municipal Airport", "Fairbury"],
  FCA: ["Glacier Park International Airport", "Kalispell"],
  FCB: ["Ficksburg Sentraoes Airport", "Ficksburg"],
  FCH: ["Fresno Chandler Executive Airport", "Fresno"],
  FCM: ["Flying Cloud Airport", "Minneapolis"],
  FCN: ["Nordholz Naval Airbase", "Cuxhaven"],
  FCO: ["Leonardo da Vinci\u2013Fiumicino Airport", "Rome"],
  FCS: ["Butts AAF (Fort Carson) Air Field", "Fort Carson"],
  FCY: ["Forrest City Municipal Airport", "Forrest City"],
  FDA: ["Fundaci\u00f3n Airport", "Fundaci\u00f3n"],
  FDE: ["F\u00f8rde Airport", "F\u00f8rde"],
  FDF: [
    "Martinique Aim\u00e9 C\u00e9saire International Airport",
    "Fort-de-France",
  ],
  FDH: ["Friedrichshafen Airport", "Friedrichshafen"],
  FDK: ["Frederick Municipal Airport", "Frederick"],
  FDO: ["San Fernando Airport", "San Fernando"],
  FDR: ["Frederick Regional Airport", "Frederick"],
  FDU: ["Bandundu Airport"],
  FDY: ["Findlay Airport", "Findlay"],
  FEA: ["Fetlar Airport", "Fetlar Island"],
  FEB: ["Sanfebagar Airport", "Sanfebagar"],
  FEC: ["Jo\u00e3o Durval Carneiro Airport", "Feira De Santana"],
  FEG: ["Fergana International Airport", "Fergana"],
  FEJ: ["Feij\u00f3 Airport", "Feij\u00f3"],
  FEK: ["Ferkessedougou Airport", "Ferkessedougou"],
  FEL: ["F\u00fcrstenfeldbruck Air Base", "F\u00fcrstenfeldbruck"],
  FEN: ["Fernando de Noronha Airport", "Fernando De Noronha"],
  FEP: ["Albertus Airport", "Freeport"],
  FET: ["Fremont Municipal Airport", "Fremont"],
  FEW: ["Francis E Warren Air Force Base", "Cheyenne"],
  FEZ: ["Sa\u00efss Airport", "Fes"],
  FFA: ["First Flight Airport", "Kill Devil Hills"],
  FFD: ["RAF Fairford", "Fairford"],
  FFL: ["Fairfield Municipal Airport", "Fairfield"],
  FFM: [
    "Fergus Falls Municipal Airport - Einar Mickelson Field",
    "Fergus Falls",
  ],
  FFO: ["Wright-Patterson Air Force Base", "Dayton"],
  FFT: ["Capital City Airport", "Frankfort"],
  FFU: ["Futaleuf\u00fa Airport", "Futaleufu"],
  FGD: ["Fderik Airport", "Fderik"],
  FGI: ["Fagali'i Airport", "Apia"],
  FGL: ["Fox Heliport", "Fox"],
  FGU: ["Fangatau Airport"],
  FHU: [
    "Sierra Vista Municipal Libby Army Air Field",
    "Fort Huachuca Sierra Vista",
  ],
  FHZ: ["Fakahina Airport", "Fakahina"],
  FID: ["Elizabeth Field", "Fishers Island"],
  FIE: ["Fair Isle Airport", "Fair Isle"],
  FIG: ["Fria Airport", "Fria"],
  FIH: ["Ndjili International Airport", "Kinshasa"],
  FIK: ["Finke Airport", "Finke"],
  FIL: ["Fillmore Municipal Airport", "Fillmore"],
  FIN: ["Finschhafen Airport", "Buki"],
  FIV: ["Five Finger CG Heliport", "Five Finger"],
  FIZ: ["Fitzroy Crossing Airport"],
  FJR: ["Fujairah International Airport"],
  FKB: ["Karlsruhe Baden-Baden Airport", "Baden-Baden"],
  FKI: ["Bangoka International Airport", "Kisangani"],
  FKJ: ["Fukui Airport"],
  FKL: ["Venango Regional Airport", "Franklin"],
  FKN: ["Franklin Municipal-John Beverly Rose Airport", "Franklin"],
  FKQ: ["Fakfak Airport", "Fakfak-Papua Island"],
  FKS: ["Fukushima Airport", "Sukagawa"],
  FLA: ["Gustavo Artunduaga Paredes Airport", "Florencia"],
  FLB: ["Cangapara Airport", "Floriano"],
  FLD: ["Fond du Lac County Airport", "Fond du Lac"],
  FLE: ["Fort Lee Army Airfield", "Fort Lee"],
  FLF: ["Flensburg-Sch\u00e4ferhaus Airport", "Flensburg"],
  FLG: ["Flagstaff Pulliam Airport", "Flagstaff"],
  FLH: ["Flotta Isle Airport", "Flotta Isle"],
  FLI: ["Holt Airport", "Flateyri"],
  FLL: ["Fort Lauderdale Hollywood International Airport", "Fort Lauderdale"],
  FLM: ["Filadelfia Airport", "Filadelfia"],
  FLN: ["Herc\u00edlio Luz International Airport", "Florian\u00f3polis"],
  FLO: ["Florence Regional Airport", "Florence"],
  FLP: ["Marion County Regional Airport", "Flippin"],
  FLR: ["Peretola Airport", "Firenze"],
  FLS: ["Flinders Island Airport", "Flinders Island"],
  FLT: ["Flat Airport", "Flat"],
  FLU: ["Flushing Airport", "Queens"],
  FLV: ["Sherman Army Air Field", "Fort Leavenworth"],
  FLW: ["Flores Airport", "Santa Cruz das Flores"],
  FLX: ["Fallon Municipal Airport", "Fallon"],
  FLY: ["Finley Airport"],
  FLZ: ["Dr Ferdinand Lumban Tobing Airport", "Sibolga-Sumatra Island"],
  FMA: ["Formosa Airport", "Formosa"],
  FMC: ["Five Mile Airport", "Five Mile"],
  FME: ["Tipton Airport", "Fort Meade(Odenton)"],
  FMG: ["Flamingo Airport", "Brasilito"],
  FMH: ["Cape Cod Coast Guard Air Station", "Falmouth"],
  FMI: ["Kalemie Airport"],
  FMM: ["Memmingen Allgau Airport", "Memmingen"],
  FMN: ["Four Corners Regional Airport", "Farmington"],
  FMO: ["M\u00fcnster Osnabr\u00fcck Airport", "M\u00fcnster"],
  FMS: ["Fort Madison Municipal Airport", "Fort Madison"],
  FMU: ["Florence Municipal Airport", "Florence"],
  FMY: ["Page Field", "Fort Myers"],
  FNA: ["Lungi International Airport", "Freetown"],
  FNB: ["Neubrandenburg Airport", "Neubrandenburg"],
  FNC: ["Madeira Airport", "Funchal"],
  FNE: ["Fane Airport", "Fane Mission"],
  FNG: ["Fada N'gourma Airport", "Fada N'gourma"],
  FNH: ["Fincha Airport", "Fincha"],
  FNI: ["N\u00eemes-Arles-Camargue Airport", "N\u00eemes/Garons"],
  FNJ: ["Pyongyang Sunan International Airport", "Pyongyang"],
  FNL: ["Northern Colorado Regional Airport", "Fort Collins/Loveland"],
  FNR: ["Funter Bay Seaplane Base", "Funter Bay"],
  FNT: ["Bishop International Airport", "Flint"],
  FNU: ["Oristano-Fenosu Airport", "Oristano"],
  FOA: ["Foula Airfield", "Foula"],
  FOB: ["Fort Bragg Airport", "Fort Bragg"],
  FOC: ["Fuzhou Changle International Airport", "Fuzhou"],
  FOD: ["Fort Dodge Regional Airport", "Fort Dodge"],
  FOE: ["Topeka Regional Airport - Forbes Field", "Topeka"],
  FOG: ['Foggia "Gino Lisa" Airport', "Foggia"],
  FOK: ["Francis S Gabreski Airport", "Westhampton Beach"],
  FOM: ["Foumban Nkounja Airport", "Foumban"],
  FON: ["Arenal Airport", "La Fortuna/San Carlos"],
  FOO: ["Kornasoren Airfield", "Kornasoren-Numfoor Island"],
  FOR: ["Pinto Martins International Airport", "Fortaleza"],
  FOS: ["Forrest Airport"],
  FOT: ["Forster (Wallis Is) Airport"],
  FOU: ["Fougamou Airport", "Fougamou"],
  FOY: ["Foya Airport", "Foya"],
  FPO: ["Grand Bahama International Airport", "Freeport"],
  FPR: ["St Lucie County International Airport", "Fort Pierce"],
  FPY: ["Perry-Foley Airport", "Perry"],
  FRA: ["Frankfurt am Main Airport", "Frankfurt am Main"],
  FRB: ["Forbes Airport", "Forbes"],
  FRC: ["Tenente Lund Pressoto Airport", "Franca"],
  FRD: ["Friday Harbor Airport", "Friday Harbor"],
  FRE: ["Fera/Maringe Airport", "Fera Island"],
  FRG: ["Republic Airport", "Farmingdale"],
  FRH: ["French Lick Municipal Airport", "French Lick"],
  FRI: ["Marshall Army Air Field", "Fort Riley(Junction City)"],
  FRJ: ["Fr\u00e9jus Airport", "Fr\u00e9jus"],
  FRK: ["Fr\u00e9gate Island Airport", "Fr\u00e9gate Island"],
  FRL: ["Forl\u00ec Airport", "Forl\u00ec (FC)"],
  FRM: ["Fairmont Municipal Airport", "Fairmont"],
  FRN: ["Bryant Army Heliport", "Fort Richardson(Anchorage)"],
  FRO: ["Flor\u00f8 Airport", "Flor\u00f8"],
  FRQ: ["Feramin Airport", "Feramin"],
  FRR: ["Front Royal Warren County Airport", "Front Royal"],
  FRS: ["Mundo Maya International Airport", "San Benito"],
  FRT: ["El Avellano Airport", "Frutillar"],
  FRU: ["Manas International Airport", "Bishkek"],
  FRW: ["Francistown Airport", "Francistown"],
  FRY: ["Eastern Slopes Regional Airport", "Fryeburg"],
  FRZ: ["Fritzlar Airport", "Fritzlar"],
  FSC: ["Figari Sud-Corse Airport", "Figari Sud-Corse"],
  FSD: ["Joe Foss Field Airport", "Sioux Falls"],
  FSI: ["Henry Post Army Air Field (Fort Sill)", "Fort Sill"],
  FSK: ["Fort Scott Municipal Airport", "Fort Scott"],
  FSL: ["Fossil Downs Airport", "Fossil Downs Station"],
  FSM: ["Fort Smith Regional Airport", "Fort Smith"],
  FSN: ["Haley Army Airfield", "Fort Sheridan"],
  FSP: ["St Pierre Airport", "Saint-Pierre"],
  FSS: ["RAF Kinloss", "Kinloss"],
  FST: ["Fort Stockton Pecos County Airport", "Fort Stockton"],
  FSU: ["Fort Sumner Municipal Airport", "Fort Sumner"],
  FSZ: ["Mt. Fuji Shizuoka Airport", "Makinohara / Shimada"],
  FTA: ["Futuna Airport", "Futuna Island"],
  FTE: ["El Calafate Airport", "El Calafate"],
  FTI: ["Fitiuta Airport", "Fitiuta Village"],
  FTK: ["Godman Army Air Field", "Fort Knox"],
  FTU: ["T\u00f4lanaro Airport", "T\u00f4lanaro"],
  FTW: ["Fort Worth Meacham International Airport", "Fort Worth"],
  FTX: ["Owando Airport", "Owando"],
  FTY: ["Fulton County Airport Brown Field", "Atlanta"],
  FUB: ["Fulleborn Airport", "Fulleborn"],
  FUE: ["Fuerteventura Airport", "Fuerteventura Island"],
  FUG: ["Fuyang Xiguan Airport", "Fuyang"],
  FUJ: ["Fukue Airport", "Goto"],
  FUK: ["Fukuoka Airport", "Fukuoka"],
  FUL: ["Fullerton Municipal Airport", "Fullerton"],
  FUM: ["Fuma Airport", "Fuma"],
  FUN: ["Funafuti International Airport", "Funafuti"],
  FUO: ["Foshan Shadi Airport", "Foshan"],
  FUT: ["Pointe Vele Airport", "Futuna Island"],
  FVL: ["Flora Valley Airport"],
  FVM: ["Fuvahmulah Airport", "Fuvahmulah Island"],
  FVR: ["Oombulgurri Airport", "Forrest River Mission"],
  FWA: ["Fort Wayne International Airport", "Fort Wayne"],
  FWH: ["NAS Fort Worth JRB/Carswell Field", "Fort Worth"],
  FWL: ["Farewell Airport", "Farewell"],
  FXE: ["Fort Lauderdale Executive Airport", "Fort Lauderdale"],
  FXO: ["Cuamba Airport", "Cuamba"],
  FXY: ["Forest City Municipal Airport", "Forest City"],
  FYJ: ["Dongji Aiport", "Fuyuan"],
  FYM: ["Fayetteville Municipal Airport", "Fayetteville"],
  FYN: ["Fuyun Koktokay Airport", "Fuyun"],
  FYT: ["Faya Largeau Airport"],
  FYU: ["Fort Yukon Airport", "Fort Yukon"],
  FYV: ["Drake Field", "Fayetteville"],
  FZO: ["Bristol Filton Airport", "Bristol"],
  GAB: ["Gabbs Airport", "Gabbs"],
  GAC: ["Celaque Airport", "Gracias"],
  GAD: ["Northeast Alabama Regional Airport", "Gadsden"],
  GAE: ["Gab\u00e8s Matmata International Airport", "Gab\u00e8s"],
  GAF: ["Gafsa Ksar International Airport", "Gafsa"],
  GAG: ["Gage Airport", "Gage"],
  GAH: ["Gayndah Airport"],
  GAI: ["Montgomery County Airpark", "Gaithersburg"],
  GAJ: ["Yamagata Airport", "Yamagata"],
  GAL: ["Edward G. Pitka Sr Airport", "Galena"],
  GAM: ["Gambell Airport", "Gambell"],
  GAN: ["Gan International Airport", "Gan"],
  GAO: ["Mariana Grajales Airport", "Guant\u00e1namo"],
  GAP: ["Gusap Airport", "Gusap"],
  GAQ: ["Gao Airport"],
  GAR: ["Garaina Airport", "Garaina"],
  GAS: ["Garissa Airport", "Garissa"],
  GAT: ["Gap - Tallard Airfield"],
  GAU: ["Lokpriya Gopinath Bordoloi International Airport", "Guwahati"],
  GAV: ["Gag Island Airport", "Gag Island"],
  GAW: ["Gangaw Airport", "Gangaw"],
  GAX: ["Gamba Airport", "Gamba"],
  GAY: ["Gaya Airport"],
  GAZ: ["Guasopa Airport", "Woodlark (Muyua) Island"],
  GBA: ["Cotswold Airport", "Kemble"],
  GBB: ["Gabala International Airport", "Gabala"],
  GBC: ["Gasuke Airport", "Gasuke"],
  GBD: ["Great Bend Municipal Airport", "Great Bend"],
  GBE: ["Sir Seretse Khama International Airport", "Gaborone"],
  GBF: ["Negarbo(Negabo) Airport", "Negarbo"],
  GBG: ["Galesburg Municipal Airport", "Galesburg"],
  GBH: ["Galbraith Lake Airport", "Galbraith Lake"],
  GBI: ["Auxiliary Airfield", "Grand Bahama"],
  GBJ: ["Les Bases Airport", "Grand Bourg"],
  GBK: ["Gbangbatok Airport", "Gbangbatok"],
  GBL: ["South Goulburn Is Airport"],
  GBM: ["Garbaharey Airport", "Garbaharey"],
  GBP: ["Gamboola Airport"],
  GBR: ["Walter J. Koladza Airport", "Great Barrington"],
  GBT: ["Gorgan Airport", "Gorgan"],
  GBU: ["Khashm El Girba Airport", "Khashm El Girba"],
  GBV: ["Gibb River Airport"],
  GBW: ["Ginbata Airport", "Ginbata"],
  GBZ: ["Great Barrier Aerodrome", "Claris"],
  GCA: ["Guacamayas Airport", "Guacamayas"],
  GCC: ["Gillette Campbell County Airport", "Gillette"],
  GCD: ["Grand Coulee Dam Airport", "Electric City"],
  GCH: ["Gachsaran Airport", "Gachsaran"],
  GCI: ["Guernsey Airport", "Saint Peter Port"],
  GCJ: ["Grand Central Airport", "Midrand"],
  GCK: ["Garden City Regional Airport", "Garden City"],
  GCM: ["Owen Roberts International Airport", "Georgetown"],
  GCN: ["Grand Canyon National Park Airport", "Grand Canyon"],
  GCT: ["Grand Canyon Bar Ten Airstrip", "Whitmore"],
  GCV: ["Gravatai Airport", "Gravatai"],
  GCW: ["Grand Canyon West Airport", "Peach Springs"],
  GCY: ["Greeneville-Greene County Municipal Airport", "Greeneville"],
  GDA: ["Gounda Airport", "Gounda"],
  GDC: ["Donaldson Field Airport", "Greenville"],
  GDD: ["Gordon Downs Airport", "Gordon Downs"],
  GDE: ["Gode Airport", "Gode"],
  GDG: ["Magdagachi Airport", "Magdagachi"],
  GDH: ["Golden Horn Lodge Seaplane Base", "Golden Horn Lodge"],
  GDI: ["Gordil Airport", "Melle"],
  GDJ: ["Gandajika Airport", "Gandajika"],
  GDL: ["Don Miguel Hidalgo Y Costilla International Airport", "Guadalajara"],
  GDM: ["Gardner Municipal Airport", "Gardner"],
  GDN: ["Gda\u0144sk Lech Wa\u0142\u0119sa Airport", "Gda\u0144sk"],
  GDO: ["Guasdalito Airport"],
  GDP: ["Guadalupe Airport", "Guadalupe"],
  GDQ: ["Gonder Airport", "Gondar"],
  GDT: ["JAGS McCartney International Airport", "Cockburn Town"],
  GDV: ["Dawson Community Airport", "Glendive"],
  GDW: ["Gladwin Zettel Memorial Airport", "Gladwin"],
  GDX: ["Sokol Airport", "Magadan"],
  GDZ: ["Gelendzhik Airport", "Gelendzhik"],
  GEA: ["Noum\u00e9a Magenta Airport", "Noum\u00e9a"],
  GEB: ["Gebe Airport", "Gebe Island"],
  GEC: ["Lefkoniko Airport", "Ge\u00e7itkale"],
  GED: ["Sussex County Airport", "Georgetown"],
  GEE: ["Georgetown (Tas) Airport"],
  GEF: ["Geva Airport", "Liangia"],
  GEG: ["Spokane International Airport", "Spokane"],
  GEL: ["Santo \u00c2ngelo Airport", "Santo \u00c2ngelo"],
  GEM: ["President Obiang Nguema International Airport", "Mengomey\u00e9n"],
  GEO: ["Cheddi Jagan International Airport", "Georgetown"],
  GER: ["Rafael Cabrera Airport", "Nueva Gerona"],
  GES: ["General Santos International Airport", "General Santos"],
  GET: ["Geraldton Airport", "Geraldton"],
  GEV: ["G\u00e4llivare Airport", "G\u00e4llivare"],
  GEW: ["Gewoia Airport", "Gewoia"],
  GEX: ["Geelong Airport"],
  GEY: ["South Big Horn County Airport", "Greybull"],
  GFA: ["Malmstrom Air Force Base", "Great Falls"],
  GFD: ["Pope Field", "Greenfield"],
  GFE: ["Grenfell Airport", "Grenfell"],
  GFF: ["Griffith Airport", "Griffith"],
  GFK: ["Grand Forks International Airport", "Grand Forks"],
  GFL: ["Floyd Bennett Memorial Airport", "Glens Falls"],
  GFN: ["Grafton Airport"],
  GFO: ["Bartica A Airport", "Bartica"],
  GFR: ["Granville Airport", "Granville"],
  GFY: ["Grootfontein Airport", "Grootfontein"],
  GGC: ["Lumbala Airport", "Lumbala N'guimbo"],
  GGD: ["Gregory Downs Airport"],
  GGE: ["Georgetown County Airport", "Georgetown"],
  GGG: ["East Texas Regional Airport", "Longview"],
  GGL: ["Gilgal Airport", "Villa Claret"],
  GGM: ["Kakamega Airport", "Kakamega"],
  GGN: ["Gagnoa Airport", "Gagnoa"],
  GGO: ["Guiglo Airport", "Guiglo"],
  GGR: ["Garowe Airport", "Garowe"],
  GGS: ["Gobernador Gregores Airport", "Gobernador Gregores"],
  GGT: ["Exuma International Airport", "George Town"],
  GGW: ["Wokal Field Glasgow International Airport", "Glasgow"],
  GHA: ["Noum\u00e9rat - Moufdi Zakaria Airport", "Gharda\u00efa"],
  GHB: ["Governor's Harbour Airport", "Governor's Harbour"],
  GHC: ["Great Harbour Cay Airport", "Bullocks Harbour"],
  GHE: ["Garachin\u00e9 Airport", "Garachin\u00e9"],
  GHF: ["Giebelstadt Airport", "Giebelstadt"],
  GHK: ["Gush Katif Airport", "Khan Yunis"],
  GHM: ["Centerville Municipal Airport", "Centerville"],
  GHN: ["Guanghan Airport", "Civil Aviation Flight University of China"],
  GHT: ["Ghat Airport", "Ghat"],
  GHU: ["Gualeguaychu Airport", "Gualeguaychu"],
  GIB: ["Gibraltar Airport", "Gibraltar"],
  GIC: ["Boigu Airport"],
  GID: ["Gitega Airport", "Gitega"],
  GIF: ["Winter Haven Regional Airport - Gilbert Field", "Winter Haven"],
  GIG: [
    "Rio Gale\u00e3o \u2013 Tom Jobim International Airport",
    "Rio De Janeiro",
  ],
  GII: ["Siguiri Airport", "Siguiri"],
  GIL: ["Gilgit Airport", "Gilgit"],
  GIR: ["Santiago Vila Airport", "Girardot"],
  GIS: ["Gisborne Airport", "Gisborne"],
  GIT: ["Mchauru Airport", "Geita"],
  GIU: ["Sigiriya Air Force Base", "Sigiriya"],
  GIY: ["Giyani Airport", "Giyani"],
  GIZ: ["Jizan Regional Airport", "Jizan"],
  GJA: ["La Laguna Airport", "Guanaja"],
  GJL: ["Jijel Ferhat Abbas Airport", "Jijel"],
  GJM: ["Guajar\u00e1-Mirim Airport", "Guajar\u00e1-Mirim"],
  GJR: ["Gj\u00f6gur Airport", "Gj\u00f6gur"],
  GJT: ["Grand Junction Regional Airport", "Grand Junction"],
  GKA: ["Goroka Airport", "Goronka"],
  GKD: ["Imroz Airport", "G\u00f6k\u00e7eada"],
  GKE: ["Geilenkirchen Air Base"],
  GKH: ["Palungtar Airport", "Gorkha"],
  GKK: ["Kooddoo Airport", "Huvadhu Atoll"],
  GKL: ["Great Keppel Is Airport", "Great Keppel Island"],
  GKN: ["Gulkana Airport", "Gulkana"],
  GKT: ["Gatlinburg-Pigeon Forge Airport", "Sevierville"],
  GLA: ["Glasgow International Airport", "Glasgow"],
  GLB: ["San Carlos Apache Airport", "Globe"],
  GLC: ["Geladi Airport", "Geladi"],
  GLD: ["Renner Field-Goodland Municipal Airport", "Goodland"],
  GLE: ["Gainesville Municipal Airport", "Gainesville"],
  GLF: ["Golfito Airport", "Golfito"],
  GLG: ["Glengyle Airport"],
  GLH: ["Mid Delta Regional Airport", "Greenville"],
  GLI: ["Glen Innes Airport"],
  GLK: ["Galcaio Airport", "Galcaio"],
  GLL: ["Gol Airport", "Klanten flyplass"],
  GLM: ["Glenormiston Airport"],
  GLN: ["Goulimime Airport", "Goulimime"],
  GLO: ["Gloucestershire Airport", "Staverton"],
  GLP: ["Gulgubip Airport", "Gulgubip"],
  GLR: ["Gaylord Regional Airport", "Gaylord"],
  GLS: ["Scholes International At Galveston Airport", "Galveston"],
  GLT: ["Gladstone Airport", "Gladstone"],
  GLU: ["Gelephu Airport", "Gelephu"],
  GLV: ["Golovin Airport", "Golovin"],
  GLW: ["Glasgow Municipal Airport", "Glasgow"],
  GLX: ["Gamarmalamo Airport", "Galela-Celebes Island"],
  GLZ: ["Gilze Rijen Air Base", "Breda"],
  GMA: ["Gemena Airport", "Gemena"],
  GMB: ["Gambella Airport", "Gambela"],
  GMD: ["Ben Slimane Airport", "Ben Slimane"],
  GME: ["Gomel Airport", "Gomel"],
  GMI: ["Gasmata Island Airport", "Gasmata Island"],
  GML: ["Gostomel Airport", "Kiev"],
  GMM: ["Gamboma Airport", "Gamboma"],
  GMN: ["Greymouth Airport"],
  GMO: ["Gombe Lawanti International Airport", "Gombe"],
  GMP: ["Gimpo International Airport", "Seoul"],
  GMQ: ["Golog Maqin Airport", "Golog"],
  GMR: ["Totegegie Airport"],
  GMS: ["Ant\u00f4nio Guerreiro Airport", "Guimar\u00e3es"],
  GMT: ["Granite Mountain Air Station", "Granite Mountain"],
  GMU: ["Greenville Downtown Airport", "Greenville"],
  GMV: ["Monument Valley Airport", "Goulding's Lodge"],
  GMZ: ["La Gomera Airport", "Alajero, La Gomera Island"],
  GNA: ["Hrodna Airport", "Hrodna"],
  GNB: ["Grenoble-Is\u00e8re Airport", "Grenoble/Saint-Geoirs"],
  GND: ["Point Salines International Airport", "Saint George's"],
  GNF: ["Gansner Field", "Quincy"],
  GNG: ["Gooding Municipal Airport", "Gooding"],
  GNI: ["Lyudao Airport", "Lyudao"],
  GNM: ["Guanambi Airport", "Guanambi"],
  GNN: ["Ghinnir Airport", "Ghinnir"],
  GNR: ["Dr. Arturo H. Illia Airport", "General Roca"],
  GNS: ["Binaka Airport", "Gunung Sitoli-Nias Island"],
  GNT: ["Grants-Milan Municipal Airport", "Grants"],
  GNU: ["Goodnews Airport", "Goodnews"],
  GNV: ["Gainesville Regional Airport", "Gainesville"],
  GNY: ["\u015eanl\u0131urfa GAP Airport", "\u015eanl\u0131urfa"],
  GNZ: ["Ghanzi Airport", "Ghanzi"],
  GOA: ["Genoa Cristoforo Colombo Airport", "Genova"],
  GOB: ["Robe Airport", "Goba"],
  GOC: ["Gora Airstrip", "Gora"],
  GOE: ["Gonaili Airport", "Gonaili"],
  GOG: ["Gobabis Airport", "Gobabis"],
  GOH: ["Godthaab / Nuuk Airport", "Nuuk"],
  GOI: ["Dabolim Airport", "Vasco da Gama"],
  GOJ: ["Nizhny Novgorod Strigino International Airport", "Nizhny Novgorod"],
  GOK: ["Guthrie-Edmond Regional Airport", "Guthrie"],
  GOL: ["Gold Beach Municipal Airport", "Gold Beach"],
  GOM: ["Goma International Airport", "Goma"],
  GON: ["Groton New London Airport", "Groton (New London)"],
  GOO: ["Goondiwindi Airport"],
  GOP: ["Gorakhpur Airport", "Gorakhpur"],
  GOQ: ["Golmud Airport", "Golmud"],
  GOR: ["Gore Airport", "Gore"],
  GOT: ["Gothenburg-Landvetter Airport", "Gothenburg"],
  GOU: ["Garoua International Airport", "Garoua"],
  GOV: ["Gove Airport", "Nhulunbuy"],
  GOZ: ["Gorna Oryahovitsa Airport", "Gorna Oryahovitsa"],
  GPA: ["Araxos Airport", "Patras"],
  GPB: ["Tancredo Thomas de Faria Airport", "Guarapuava"],
  GPD: ["Mount Gordon Airport", "Mount Gordon Mine"],
  GPI: ["Juan Casiano Airport", "Guapi"],
  GPL: ["Guapiles Airport", "Pococi"],
  GPN: ["Garden Point Airport"],
  GPO: ["General Pico Airport", "General Pico"],
  GPS: ["Seymour Airport", "Baltra"],
  GPT: ["Gulfport Biloxi International Airport", "Gulfport"],
  GPZ: ["Grand Rapids Itasca Co-Gordon Newstrom field", "Grand Rapids"],
  GQQ: ["Galion Municipal Airport", "Galion"],
  GRB: ["Austin Straubel International Airport", "Green Bay"],
  GRC: ["Grand Cess Airport", "Grand Cess"],
  GRD: ["Greenwood County Airport", "Greenwood"],
  GRE: ["Greenville Airport", "Greenville"],
  GRF: ["Gray Army Air Field", "Fort Lewis/Tacoma"],
  GRG: ["Gardez Airport", "Gardez"],
  GRH: ["Garuahi Airport", "Garuahi"],
  GRI: ["Central Nebraska Regional Airport", "Grand Island"],
  GRJ: ["George Airport", "George"],
  GRK: ["Robert Gray  Army Air Field Airport", "Fort Hood/Killeen"],
  GRL: ["Garasa Airport", "Au"],
  GRM: ["Grand Marais Cook County Airport", "Grand Marais"],
  GRN: ["Gordon Municipal Airport", "Gordon"],
  GRO: ["Girona Airport", "Girona"],
  GRP: ["Gurupi Airport", "Gurupi"],
  GRQ: ["Eelde Airport", "Groningen"],
  GRR: ["Gerald R. Ford International Airport", "Grand Rapids"],
  GRS: ["Grosseto Air Base", "Grosetto"],
  GRT: ["Gujrat Airport", "Gujrat"],
  GRU: [
    "Guarulhos - Governador Andr\u00e9 Franco Montoro International Airport",
    "S\u00e3o Paulo",
  ],
  GRV: ["Grozny North Airport", "Grozny"],
  GRW: ["Graciosa Airport", "Santa Cruz da Graciosa"],
  GRX: ["Federico Garcia Lorca Airport", "Granada"],
  GRY: ["Gr\u00edmsey Airport", "Gr\u00edmsey"],
  GRZ: ["Graz Airport", "Graz"],
  GSA: ["Long Pasia Airport", "Long Miau"],
  GSB: ["Seymour Johnson Air Force Base", "Goldsboro"],
  GSC: ["Gascoyne Junction Airport", "Gascoyne Junction"],
  GSE: ["Gothenburg City Airport", "Gothenburg"],
  GSH: ["Goshen Municipal Airport", "Goshen"],
  GSI: ["Grand-Santi Airport", "Grand-Santi"],
  GSJ: ["San Jos\u00e9 Airport", "Puerto San Jos\u00e9"],
  GSL: ["Taltheilei Narrows Airport", "Taltheilei Narrows"],
  GSM: ["Qeshm International Airport", "Qeshm"],
  GSN: ["Mount Gunson Airport", "Mount Gunson"],
  GSO: ["Piedmont Triad International Airport", "Greensboro"],
  GSP: ["Greenville Spartanburg International Airport", "Greenville"],
  GSQ: ["Shark El Oweinat International Airport"],
  GSR: ["Gardo Airport", "Gardo"],
  GSS: ["Sabi Sabi Airport", "Belfast"],
  GST: ["Gustavus Airport", "Gustavus"],
  GSU: ["Azaza Airport", "Gedaref"],
  GSW: [
    "Greater Southwest International Airport-Amon Carter Field",
    "Fort Worth",
  ],
  GSY: ["Binbrook Airfield", "Grimsby"],
  GTA: ["Gatokae Airport", "Gatokae"],
  GTB: ["Genting Airport", "Genting"],
  GTE: ["Groote Eylandt Airport", "Groote Eylandt"],
  GTF: ["Great Falls International Airport", "Great Falls"],
  GTG: ["Grantsburg Municipal Airport", "Grantsburg"],
  GTI: ["R\u00fcgen Airport", "R\u00fcgen"],
  GTK: ["Sungei Tekai Airport", "Sungei Tekai"],
  GTN: ["Glentanner Airport", "Glentanner Station"],
  GTO: ["Jalaluddin Airport", "Gorontalo-Celebes Island"],
  GTP: ["Grants Pass Airport", "Grants Pass"],
  GTR: ["Golden Triangle Regional Airport", "Columbus/W Point/Starkville"],
  GTS: ["Granite Downs Airport"],
  GTT: ["Georgetown Airport"],
  GTW: ["Hole\u0161ov Airport", "Hole\u0161ov"],
  GTY: ["Gettysburg Regional Airport", "Gettysburg"],
  GTZ: ["Kirawira B Aerodrome", "Grumeti Game Reserve"],
  GUA: ["La Aurora Airport", "Guatemala City"],
  GUB: ["Guerrero Negro Airport", "Guerrero Negro"],
  GUC: ["Gunnison Crested Butte Regional Airport", "Gunnison"],
  GUD: ["Goundam Airport", "Goundam"],
  GUE: ["Guriaso (Keraso) Airport", "Guriaso"],
  GUF: ["Jack Edwards Airport", "Gulf Shores"],
  GUG: ["Guari Airport", "Guari"],
  GUH: ["Gunnedah Airport"],
  GUI: ["Guiria Airport"],
  GUJ: ["Guaratinguet\u00e1 Airport", "Guaratinguet\u00e1"],
  GUL: ["Goulburn Airport"],
  GUM: [
    "Antonio B. Won Pat International Airport",
    "Hag\u00e5t\u00f1a, Guam International Airport",
  ],
  GUO: ["Jicalapa Airport", "Jicalapa"],
  GUP: ["Gallup Municipal Airport", "Gallup"],
  GUQ: ["Guanare Airport", "Guanare"],
  GUR: ["Gurney Airport", "Gurney"],
  GUS: ["Grissom Air Reserve Base", "Peru"],
  GUT: ["G\u00fctersloh Air Base", "G\u00fctersloh"],
  GUU: ["Grundarfj\u00f6r\u00f0ur Airport", "Grundarfj\u00f6r\u00f0ur"],
  GUV: ["Mougulu Airport", "Mougulu"],
  GUW: ["Atyrau Airport", "Atyrau"],
  GUX: ["Guna Airport"],
  GUY: ["Guymon Municipal Airport", "Guymon"],
  GUZ: ["Guarapari Airport", "Guarapari"],
  GVA: ["Geneva Cointrin International Airport", "Geneva"],
  GVE: ["Gordonsville Municipal Airport", "Gordonsville"],
  GVI: ["Green River Airport", "Green River"],
  GVL: ["Lee Gilmer Memorial Airport", "Gainesville"],
  GVN: ["Maygatka Airport.", "Sovetskaya Gavan"],
  GVP: ["Greenvale Airport"],
  GVR: ["Coronel Altino Machado de Oliveira Airport", "Governador Valadares"],
  GVT: ["Majors Airport", "Greenville"],
  GVW: ["Richards-Gebaur Air Force Base", "Belton"],
  GVX: ["G\u00e4vle Sandviken Airport", "G\u00e4vle / Sandviken"],
  GWA: ["Gwa Airport", "Gwa"],
  GWD: ["Gwadar International Airport", "Gwadar"],
  GWE: ["Thornhill Air Base", "Gweru"],
  GWL: ["Gwalior Airport", "Gwalior"],
  GWN: ["Gnarowein Airport", "Gnarowein"],
  GWO: ["Greenwood\u2013Leflore Airport", "Greenwood"],
  GWS: ["Glenwood Springs Municipal Airport", "Glenwood Springs"],
  GWT: ["Westerland Sylt Airport", "Westerland"],
  GWV: ["Glendale Fokker Field", "Glendale"],
  GWW: ["Gatow Airport", "Spandau"],
  GWY: ["Galway Airport", "Galway"],
  GXF: ["Sayun International Airport", "Sayun"],
  GXG: ["Negage Airport", "Negage"],
  GXH: ["Gannan Xiahe Airport", "Xiahe"],
  GXQ: ["Teniente Vidal Airport", "Coyhaique"],
  GXX: ["Yagoua Airport", "Yagoua"],
  GXY: ["Greeley\u2013Weld County Airport", "Greeley"],
  GYA: ["Capit\u00e1n de Av. Emilio Beltr\u00e1n Airport", "Guayaramer\u00edn"],
  GYD: ["Heydar Aliyev International Airport", "Baku"],
  GYE: ["Jos\u00e9 Joaqu\u00edn de Olmedo International Airport", "Guayaquil"],
  GYG: ["Magan Airport", "Magan"],
  GYI: ["Gisenyi Airport", "Gisenyi"],
  GYL: ["Argyle Airport"],
  GYM: [
    "General Jos\u00e9 Mar\u00eda Y\u00e1\u00f1ez International Airport",
    "Guaymas",
  ],
  GYN: ["Santa Genoveva Airport", "Goi\u00e2nia"],
  GYP: ["Gympie Airport"],
  GYR: ["Phoenix Goodyear Airport", "Goodyear"],
  GYS: ["Guangyuan Airport", "Guangyuan"],
  GYU: ["Guyuan Liupanshan Airport", "Guyuan"],
  GYY: ["Gary Chicago International Airport", "Gary"],
  GZA: ["Yasser Arafat International Airport", "Rafah"],
  GZI: ["Ghazni Airport", "Ghazni"],
  GZM: ["Xewkija Heliport", "Gozo"],
  GZO: ["Nusatupe Airport", "Gizo"],
  GZP: ["Gazipa\u015fa Airport", "Gazipa\u015fa"],
  GZT: ["Gaziantep International Airport", "Gaziantep"],
  GZW: ["Qazvin Airport", "Qazvin"],
  HAA: ["Hasvik Airport", "Hasvik"],
  HAB: ["Marion County Rankin Fite Airport", "Hamilton"],
  HAC: ["Hachijojima Airport", "Hachijojima"],
  HAD: ["Halmstad Airport", "Halmstad"],
  HAE: ["Lava Falls Heliport", "Supai Village"],
  HAF: ["Half Moon Bay Airport", "Half Moon Bay"],
  HAH: ["Prince Said Ibrahim International Airport", "Moroni"],
  HAI: ["Three Rivers Municipal Dr Haines Airport", "Three Rivers"],
  HAJ: ["Hannover Airport", "Hannover"],
  HAK: ["Haikou Meilan International Airport", "Haikou"],
  HAL: ["Halali Airport", "Halali"],
  HAM: ["Hamburg Airport", "Hamburg"],
  HAN: ["Noi Bai International Airport", "Hanoi"],
  HAO: ["Butler Co Regional Airport - Hogan Field", "Hamilton"],
  HAQ: ["Hanimaadhoo Airport", "Haa Dhaalu Atoll"],
  HAR: ["Capital City Airport", "Harrisburg"],
  HAS: ["Ha'il Airport", "Ha'il"],
  HAT: ["Heathlands Airport"],
  HAU: ["Haugesund Airport", "Karm\u00f8y"],
  HAV: ["Jos\u00e9 Mart\u00ed International Airport", "Havana"],
  HAW: ["Haverfordwest Airport", "Haverfordwest"],
  HAX: ["Hatbox Field", "Muskogee"],
  HAY: ["Haycock Airport", "Haycock"],
  HAZ: ["Hatzfeldhaven Airport", "Hatzfeldhaven"],
  HBA: ["Hobart International Airport", "Hobart"],
  HBB: ["Industrial Airpark", "Hobbs"],
  HBD: ["Habi Airport", "Habi"],
  HBE: ["Borg El Arab International Airport", "Alexandria"],
  HBG: ["Hattiesburg Bobby L Chain Municipal Airport", "Hattiesburg"],
  HBH: ["Entrance Island Seaplane Base", "Entrance Island"],
  HBK: ["Holbrook Municipal Airport", "Holbrook"],
  HBR: ["Hobart Regional Airport", "Hobart"],
  HBT: ["Hambantota Seaplane Base", "Hambantota"],
  HBU: ["Bulgan Sum Airport", "Bulgan"],
  HBX: ["Hubli Airport", "Hubli"],
  HCA: ["Big Spring Mc Mahon-Wrinkle Airport", "Big Spring"],
  HCC: ["Columbia County Airport", "Hudson"],
  HCJ: ["Hechi Jinchengjiang Airport", "Hechi"],
  HCM: ["Eil Airport", "Eyl"],
  HCN: ["Hengchun Airport", "Hengchung"],
  HCQ: ["Halls Creek Airport"],
  HCR: ["Holy Cross Airport", "Holy Cross"],
  HCW: ["Cheraw Municipal Airport/Lynch Bellinger Field", "Cheraw"],
  HDB: ["Heidelberg Army Helicopter Base", "Heidelberg"],
  HDD: ["Hyderabad Airport", "Hyderabad"],
  HDE: ["Brewster Field", "Holdrege"],
  HDF: ["Heringsdorf Airport", "Heringsdorf"],
  HDG: ["Handan Airport", "Handan"],
  HDH: ["Dillingham Airfield", "Mokuleia"],
  HDM: ["Hamadan Airport", "Hamadan"],
  HDN: ["Yampa Valley Airport", "Hayden"],
  HDR: ["Havadarya Airport", "Havadarya"],
  HDS: ["Hoedspruit Air Force Base Airport", "Hoedspruit"],
  HDY: ["Hat Yai International Airport", "Hat Yai"],
  HEA: ["Herat Airport"],
  HEB: ["Hinthada Airport", "Hinthada"],
  HED: ["Herendeen Bay Airport", "Herendeen Bay"],
  HEE: ["Thompson-Robbins Airport", "Helena/West Helena"],
  HEG: ["Heglig Airport", "Heglig Oilfield"],
  HEH: ["Heho Airport", "Heho"],
  HEI: ["Heide-B\u00fcsum Airport", "B\u00fcsum"],
  HEK: ["Heihe Airport", "Heihe"],
  HEL: ["Helsinki Vantaa Airport", "Helsinki"],
  HEM: ["Helsinki Malmi Airport", "Helsinki"],
  HEO: ["Haelogo Airport", "Haelogo"],
  HER: ["Heraklion International Nikos Kazantzakis Airport", "Heraklion"],
  HES: ["Hermiston Municipal Airport", "Hermiston"],
  HET: ["Baita International Airport", "Hohhot"],
  HEW: ["Athen Helenikon Airport", "Athens"],
  HEX: ["Herrera Airport", "Santo Domingo"],
  HEY: ["Hanchey Army (Fort Rucker) Heliport", "Fort Rucker Ozark"],
  HEZ: ["Hardy-Anders Field / Natchez-Adams County Airport", "Natchez"],
  HFA: ["Haifa International Airport", "Haifa"],
  HFD: ["Hartford Brainard Airport", "Hartford"],
  HFE: ["Hefei Luogang International Airport", "Hefei"],
  HFF: ["Mackall Army Air Field", "Camp Mackall"],
  HFN: ["Hornafj\u00f6r\u00f0ur Airport", "H\u00f6fn"],
  HFS: ["Hagfors Airport"],
  HFT: ["Hammerfest Airport", "Hammerfest"],
  HGA: ["Egal International Airport", "Hargeisa"],
  HGD: ["Hughenden Airport"],
  HGE: ["Higuerote Airport", "Higuerote"],
  HGH: ["Hangzhou Xiaoshan International Airport", "Hangzhou"],
  HGI: ["Paloich Airport, Heliport", "Higleig"],
  HGL: ["Helgoland-D\u00fcne Airport", "Helgoland"],
  HGN: ["Mae Hong Son Airport"],
  HGO: ["Korhogo Airport", "Korhogo"],
  HGR: ["Hagerstown Regional Richard A Henson Field", "Hagerstown"],
  HGS: ["Hastings Airport", "Freetown"],
  HGT: ["Tusi AHP (Hunter Liggett) Heliport", "Fort Hunter Ligget Jolon"],
  HGU: ["Mount Hagen Kagamuga Airport", "Mount Hagen"],
  HGZ: ["Hog River Airport", "Hogatza"],
  HHE: ["Hachinohe Airport"],
  HHH: ["Hilton Head Airport", "Hilton Head Island"],
  HHI: ["Wheeler Army Airfield", "Wahiawa"],
  HHN: ["Frankfurt-Hahn Airport", "Frankfurt am Main"],
  HHP: ["Shun Tak Heliport", "Hong Kong"],
  HHQ: ["Hua Hin Airport", "Hua Hin"],
  HHR: ["Jack Northrop Field Hawthorne Municipal Airport", "Hawthorne"],
  HHZ: ["Hikueru Atoll Airport", "Hikueru Atoll"],
  HIA: ["Lianshui Airport", "Huai'an"],
  HIB: ["Range Regional Airport", "Hibbing"],
  HID: ["Horn Island Airport", "Horn Island"],
  HIE: ["Mount Washington Regional Airport", "Whitefield"],
  HIF: ["Hill Air Force Base", "Ogden"],
  HIG: ["Highbury Airport"],
  HII: ["Lake Havasu City Airport", "Lake Havasu City"],
  HIJ: ["Hiroshima Airport", "Hiroshima"],
  HIL: ["Shilavo Airport", "Shilavo"],
  HIM: ["Hingurakgoda Air Force Base", "Polonnaruwa Town"],
  HIN: ["Sacheon Air Base/Airport", "Sacheon"],
  HIO: ["Portland Hillsboro Airport", "Portland"],
  HIP: ["Headingly Airport"],
  HIR: ["Honiara International Airport", "Honiara"],
  HIS: ["Hayman Island Resort Seaplane Base", "Hayman Island"],
  HIT: ["Haivaro Airport", "Haivaro"],
  HIW: ["Hiroshimanishi Airport"],
  HJJ: ["Zhijiang Airport", "Huaihua"],
  HJR: ["Khajuraho Airport", "Khajuraho"],
  HJT: ["Khujirt Airport", "Khujirt"],
  HKA: ["Blytheville Municipal Airport", "Blytheville"],
  HKB: ["Healy Lake Airport", "Healy Lake"],
  HKD: ["Hakodate Airport", "Hakodate"],
  HKG: ["Hong Kong International Airport", "Hong Kong"],
  HKK: ["Hokitika Airfield"],
  HKN: ["Kimbe Airport", "Hoskins"],
  HKP: ["Kaanapali Airport", "Kaanapali"],
  HKS: ["Hawkins Field", "Jackson"],
  HKT: ["Phuket International Airport", "Phuket"],
  HKV: ["Malevo Airport", "Haskovo"],
  HKY: ["Hickory Regional Airport", "Hickory"],
  HLA: ["Lanseria Airport", "Johannesburg"],
  HLB: ["Hillenbrand Industries Airport", "Batesville"],
  HLC: ["Hill City Municipal Airport", "Hill City"],
  HLD: ["Dongshan Airport", "Hailar"],
  HLE: ["St. Helena Airport", "Jamestown"],
  HLF: ["Hultsfred Airport"],
  HLG: ["Wheeling Ohio County Airport", "Wheeling"],
  HLH: ["Ulanhot Airport", "Ulanhot"],
  HLI: ["Hollister Municipal Airport", "Hollister"],
  HLJ: ["Barysiai Airport", "Barysiai"],
  HLL: ["Hillside Airport", "Hillside"],
  HLM: ["Park Township Airport", "Holland"],
  HLN: ["Helena Regional Airport", "Helena"],
  HLO: ["Ingjaldssan\u00f0ur Airport", "Onundarfj\u00f6r\u00f0ur"],
  HLP: ["Halim Perdanakusuma International Airport", "Jakarta"],
  HLR: ["Hood Army Air Field", "Fort Hood(Killeen)"],
  HLS: ["St Helens Airport"],
  HLT: ["Hamilton Airport"],
  HLU: ["Nesson Airport", "Houailou"],
  HLV: ["Helenvale Airport", "Helenvale"],
  HLW: ["Hluhluwe Airport", "Hluhluwe"],
  HLZ: ["Hamilton International Airport", "Hamilton"],
  HMA: ["Khanty Mansiysk Airport", "Khanty-Mansiysk"],
  HMB: ["Sohag International Airport", "Sohag"],
  HME: ["Oued Irara Airport", "Hassi Messaoud"],
  HMG: ["Hermannsburg Airport"],
  HMI: ["Hami Airport", "Hami"],
  HMJ: ["Khmelnytskyi Airport", "Khmelnytskyi"],
  HMN: ["Holloman Air Force Base", "Alamogordo"],
  HMO: ["General Ignacio P. Garcia International Airport", "Hermosillo"],
  HMR: ["Stafsberg Airport", "Hamar"],
  HMT: ["Hemet Ryan Airport", "Hemet"],
  HMV: ["Hemavan Airport"],
  HMY: ["Seosan Air Base", "Seosan"],
  HNA: ["Hanamaki Airport"],
  HNB: ["Huntingburg Airport", "Huntingburg"],
  HNC: ["Billy Mitchell Airport", "Hatteras"],
  HND: ["Tokyo Haneda International Airport", "Tokyo"],
  HNE: ["Tahneta Pass Airport", "Tahneta Pass Lodge"],
  HNH: ["Hoonah Airport", "Hoonah"],
  HNI: ["Heiweni Airport", "Heiweni"],
  HNL: ["Daniel K Inouye International Airport", "Honolulu"],
  HNM: ["Hana Airport", "Hana"],
  HNN: ["Honinabi Airport", "Honinabi"],
  HNS: ["Haines Airport", "Haines"],
  HNY: ["Hengyang Nanyue Airport", "Hengyang"],
  HOA: ["Hola Airport", "Hola"],
  HOB: ["Lea County Regional Airport", "Hobbs"],
  HOC: ["Komako Airport", "Komako"],
  HOD: ["Hodeidah International Airport", "Hodeida"],
  HOE: ["Ban Huoeisay Airport", "Huay Xai"],
  HOF: ["Al Ahsa Airport"],
  HOG: ["Frank Pais International Airport", "Holguin"],
  HOH: ["Hohenems-Dornbirn Airport", "Hohenems / Dornbirn"],
  HOI: ["Hao Airport"],
  HOK: ["Hooker Creek Airport", "Lajamanu"],
  HOM: ["Homer Airport", "Homer"],
  HON: ["Huron Regional Airport", "Huron"],
  HOO: ["Nhon Co Airfield", "Quang Duc"],
  HOP: ["Campbell AAF (Fort Campbell) Air Field", "Fort Campbell/Hopkinsville"],
  HOQ: ["Hof-Plauen Airport", "Hof"],
  HOR: ["Horta Airport", "Horta"],
  HOS: ["Chos Malal Airport", "Chos Malal"],
  HOT: ["Memorial Field", "Hot Springs"],
  HOU: ["William P Hobby Airport", "Houston"],
  HOV: ["\u00d8rsta-Volda Airport, Hovden", "\u00d8rsta"],
  HOX: ["Hommalinn Airport", "Hommalinn"],
  HOY: ["Hoy/Longhope Airfield", "Orkney Isle"],
  HPA: ["Lifuka Island Airport", "Lifuka"],
  HPB: ["Hooper Bay Airport", "Hooper Bay"],
  HPE: ["Hope Vale Airport", "Hope Vale"],
  HPG: ["Shennongjia Hongping Airport", "Shennongjia"],
  HPH: ["Cat Bi International Airport", "Haiphong"],
  HPN: ["Westchester County Airport", "White Plains"],
  HPR: ["Pretoria Central Heliport", "Pretoria"],
  HPT: ["Hampton Municipal Airport", "Hampton"],
  HPV: ["Princeville Airport", "Hanalei"],
  HPY: ["Baytown Airport", "Baytown"],
  HQM: ["Bowerman Airport", "Hoquiam"],
  HRA: ["Mansehra Airport", "Mansehra"],
  HRB: ["Taiping Airport", "Harbin"],
  HRC: ["Sary Su Airport", "Zhayrem"],
  HRE: ["Robert Gabriel Mugabe International Airport", "Harare"],
  HRG: ["Hurghada International Airport", "Hurghada"],
  HRI: ["Mattala Rajapaksa International Airport"],
  HRK: ["Kharkiv International Airport", "Kharkiv"],
  HRL: ["Valley International Airport", "Harlingen"],
  HRM: ["Hassi R'Mel Airport"],
  HRO: ["Boone County Airport", "Harrison"],
  HRR: ["Herrera Airport", "Campi\u00f1a"],
  HRS: ["Harrismith Airport", "Harrismith"],
  HRT: ["RAF Linton-On-Ouse", "Linton-On-Ouse"],
  HRY: ["Henbury Airport"],
  HRZ: ["Walter B\u00fcndchen Airport", "Horizontina"],
  HSB: ["Harrisburg-Raleigh Airport", "Harrisburg"],
  HSC: ["Shaoguan Guitou Airport", "Shaoguan"],
  HSG: ["Saga Airport", "Saga"],
  HSH: ["Henderson Executive Airport", "Las Vegas"],
  HSI: ["Hastings Municipal Airport", "Hastings"],
  HSJ: ["Shangjie Airport", "Zhengzhou"],
  HSK: ["Huesca/Pirineos Airport", "Monflorite/Alcal\u00e1 del Obispo"],
  HSL: ["Huslia Airport", "Huslia"],
  HSM: ["Horsham Airport"],
  HSN: ["Zhoushan Airport", "Zhoushan"],
  HSP: ["Ingalls Field", "Hot Springs"],
  HSS: ["Hissar Airport"],
  HST: ["Homestead ARB Airport", "Homestead"],
  HSV: ["Huntsville International Carl T Jones Field", "Huntsville"],
  HSZ: ["Hsinchu Air Base", "Hsinchu City"],
  HTA: ["Chita-Kadala Airport", "Chita"],
  HTG: ["Khatanga Airport", "Khatanga"],
  HTH: ["Hawthorne Industrial Airport", "Hawthorne"],
  HTI: ["Hamilton Island Airport", "Hamilton Island"],
  HTL: ["Roscommon County - Blodgett Memorial Airport", "Houghton Lake"],
  HTM: ["Khatgal Airport", "Hatgal"],
  HTN: ["Hotan Airport", "Hotan"],
  HTO: ["East Hampton Airport", "East Hampton"],
  HTR: ["Hateruma Airport", "Hateruma"],
  HTS: ["Tri-State/Milton J. Ferguson Field", "Huntington"],
  HTT: ["Huatugou Airport", "Mengnai"],
  HTU: ["Hopetoun Airport"],
  HTV: ["Huntsville Regional Airport", "Huntsville"],
  HTW: ["Lawrence County Airpark", "Chesapeake/Huntington Wva"],
  HTY: ["Hatay Airport", "Hatay"],
  HTZ: ["Hato Corozal Airport", "Hato Corozal"],
  HUA: ["Redstone Army Air Field", "Redstone Arsnl Huntsville"],
  HUB: ["Humbert River Airport"],
  HUC: ["Humacao Airport", "Humacao"],
  HUD: ["Humboldt Municipal Airport", "Humboldt"],
  HUE: ["Humera Airport", "Humera"],
  HUF: ["Terre Haute Regional Airport, Hulman Field", "Terre Haute"],
  HUG: ["Huehuetenango Airport", "Huehuetenango"],
  HUH: ["Huahine-Fare Airport", "Fare"],
  HUI: ["Phu Bai Airport", "Hue"],
  HUJ: ["Stan Stamper Municipal Airport", "Hugo"],
  HUK: ["Hukuntsi Airport", "Hukuntsi"],
  HUL: ["Houlton International Airport", "Houlton"],
  HUM: ["Houma Terrebonne Airport", "Houma"],
  HUN: ["Hualien Airport", "Hualien City"],
  HUO: ["Holingol Huolinhe Airport", "Holingol"],
  HUQ: ["Hon Airport"],
  HUS: ["Hughes Airport", "Hughes"],
  HUT: ["Hutchinson Municipal Airport", "Hutchinson"],
  HUU: ["Alferez Fap David Figueroa Fernandini Airport", "Hu\u00e1nuco"],
  HUV: ["Hudiksvall Airport", "Hudiksvall"],
  HUW: ["Humait\u00e1 Airport", "Humait\u00e1"],
  HUX: ["Bah\u00edas de Huatulco International Airport", "Huatulco"],
  HUY: ["Humberside Airport", "Grimsby"],
  HUZ: ["Huizhou Airport", "Huizhou"],
  HVA: ["Analalava Airport"],
  HVB: ["Hervey Bay Airport", "Hervey Bay"],
  HVD: ["Khovd Airport", "Khovd"],
  HVE: ["Hanksville Airport", "Hanksville"],
  HVG: ["Valan Airport", "Honningsv\u00e5g"],
  HVK: ["H\u00f3lmav\u00edk Airport", "H\u00f3lmav\u00edk"],
  HVM: ["Kr\u00f3ksta\u00f0armelar Airport", "Hvammstangi"],
  HVN: ["Tweed New Haven Airport", "New Haven"],
  HVR: ["Havre City County Airport", "Havre"],
  HVS: ["Hartsville Regional Airport", "Hartsville"],
  HWA: ["Hawabango Airport", "Hawabango"],
  HWD: ["Hayward Executive Airport", "Hayward"],
  HWK: ["Wilpena Pound Airport", "Hawker"],
  HWN: ["Hwange National Park Airport", "Hwange"],
  HWO: ["North Perry Airport", "Hollywood"],
  HXD: ["Delingha Airport", "Delingha"],
  HXX: ["Hay Airport"],
  HYA: ["Barnstable Municipal Boardman Polando Field", "Hyannis"],
  HYC: ["Wycombe Air Park", "High Wycombe"],
  HYD: ["Rajiv Gandhi International Airport", "Hyderabad"],
  HYF: ["Hayfields Airport", "Bainyik"],
  HYG: ["Hydaburg Seaplane Base", "Hydaburg"],
  HYL: ["Hollis Clark Bay Seaplane Base", "Hollis"],
  HYN: ["Huangyan Luqiao Airport", "Huangyan"],
  HYR: ["Sawyer County Airport", "Hayward"],
  HYS: ["Hays Regional Airport", "Hays"],
  HYV: ["Hyvink\u00e4\u00e4 Airfield", "Hyvink\u00e4\u00e4"],
  HZB: ["Merville-Calonne Airport", "Merville/Calonne"],
  HZG: ["Hanzhong Chenggu Airport", "Hanzhong"],
  HZH: ["Liping Airport", "Liping"],
  HZK: ["H\u00fasav\u00edk Airport", "H\u00fasav\u00edk"],
  HZL: ["Hazleton Municipal Airport", "Hazleton"],
  HZP: ["Fort Mackay / Horizon Airport", "Fort Mackay"],
  HZV: ["Hazyview Airport", "Hazyview"],
  IAA: ["Igarka Airport", "Igarka"],
  IAB: ["Mc Connell Air Force Base", "Wichita"],
  IAD: ["Washington Dulles International Airport", "Washington"],
  IAG: ["Niagara Falls International Airport", "Niagara Falls"],
  IAH: ["George Bush Intercontinental Houston Airport", "Houston"],
  IAL: ["Ialibu Airport", "Ialibu"],
  IAM: ["In Am\u00e9nas Airport", "Am\u00e9nas"],
  IAN: ["Bob Baker Memorial Airport", "Kiana"],
  IAO: ["Siargao Airport", "Del Carmen"],
  IAR: ["Tunoshna Airport"],
  IAS: ["Ia\u015fi Airport", "Ia\u015fi"],
  IBA: ["Ibadan Airport", "Ibadan"],
  IBB: ["General Villamil Airport", "Isabela"],
  IBE: ["Perales Airport", "Ibagu\u00e9"],
  IBI: ["Iboki Airport", "Iboki"],
  IBL: ["Indigo Bay Lodge Airport", "Bazaruto Island"],
  IBO: ["Ibo Airport", "Ibo"],
  IBP: ["Iberia Airport", "Iberia"],
  IBR: ["Hyakuri Airport", "Omitama"],
  IBZ: ["Ibiza Airport", "Ibiza"],
  ICA: ["Icabar\u00fa Airport"],
  ICC: ["Andr\u00e9s Miguel Salazar Marcano Airport", "Isla de Coche"],
  ICI: ["Cicia Airport", "Cicia"],
  ICK: ["Nieuw Nickerie Airport", "Nieuw Nickerie"],
  ICL: ["Schenck Field", "Clarinda"],
  ICN: ["Incheon International Airport", "Seoul"],
  ICO: ["Sicogon Airstrip", "Sicogon Island"],
  ICR: ["Nicaro Airport", "Nicaro"],
  ICS: ["Cascade Airport", "Cascade"],
  ICT: ["Wichita Eisenhower National Airport", "Wichita"],
  ICY: ["Icy Bay Airport", "Icy Bay"],
  IDA: ["Idaho Falls Regional Airport", "Idaho Falls"],
  IDB: ["Idre Airport", "Idre"],
  IDF: ["Idiofa Airport", "Idiofa"],
  IDH: ["Idaho County Airport", "Grangeville"],
  IDI: ["Indiana County/Jimmy Stewart Fld/ Airport", "Indiana"],
  IDK: ["Indulkana Airport"],
  IDN: ["Indagen Airport", "Indagen"],
  IDO: ["Santa Izabel do Morro Airport", "Cristal\u00e2ndia"],
  IDP: ["Independence Municipal Airport", "Independence"],
  IDR: ["Devi Ahilyabai Holkar Airport", "Indore"],
  IDY: ["\u00cele d'Yeu Airport", "\u00cele d'Yeu"],
  IEG: ["Zielona G\u00f3ra-Babimost Airport", "Babimost"],
  IEJ: ["Ie Jima Airport", "Ie"],
  IES: ["Riesa-G\u00f6hlis Airport", "Riesa"],
  IEV: ["Kiev Zhuliany International Airport", "Kiev"],
  IFA: ["Iowa Falls Municipal Airport", "Iowa Falls"],
  IFF: ["Iffley Airport"],
  IFH: ["Hesa Airport", "Hesa"],
  IFJ: ["\u00cdsafj\u00f6r\u00f0ur Airport", "\u00cdsafj\u00f6r\u00f0ur"],
  IFL: ["Innisfail Airport"],
  IFN: ["Esfahan Shahid Beheshti International Airport", "Isfahan"],
  IFO: ["Ivano-Frankivsk International Airport", "Ivano-Frankivsk"],
  IFP: ["Laughlin Bullhead International Airport", "Bullhead City"],
  IFU: ["Ifuru Airport", "Ifuru Island"],
  IGA: ["Inagua Airport", "Matthew Town"],
  IGB: ["Cabo F.A.A. H. R. Bord\u00f3n Airport", "Ingeniero Jacobacci"],
  IGD: ["I\u011fd\u0131r Airport", "I\u011fd\u0131r"],
  IGE: ["Tchongorove Airport", "Iguela"],
  IGG: ["Igiugig Airport", "Igiugig"],
  IGH: ["Ingham Airport"],
  IGL: ["\u00c7i\u011fli Airport"],
  IGM: ["Kingman Airport", "Kingman"],
  IGN: ["Iligan Airport"],
  IGO: ["Chigorod\u00f3 Airport", "Chigorod\u00f3"],
  IGR: ["Cataratas Del Iguaz\u00fa International Airport", "Puerto Iguazu"],
  IGS: ["Ingolstadt Manching Airport", "Manching"],
  IGT: ["Magas Airport", "Magas"],
  IGU: ["Cataratas International Airport", "Foz Do Igua\u00e7u"],
  IHC: ["Inhaca Airport", "Inhaca"],
  IHN: ["Qishn Airport", "Qishn"],
  IHO: ["Ihosy Airport", "Ihosy"],
  IHR: ["Iran Shahr Airport", "Iranshahr"],
  IHU: ["Ihu Airport", "Ihu"],
  IIA: ["Inishmaan Aerodrome", "Inis Me\u00e1in"],
  IIL: ["Ilam Airport", "Ilam"],
  IIS: ["Nissan Island Airport", "Nissan Island"],
  IJK: ["Izhevsk Airport", "Izhevsk"],
  IJU: ["Iju\u00ed Airport", "Iju\u00ed"],
  IKA: ["Imam Khomeini International Airport", "Tehran"],
  IKB: ["Wilkes County Airport", "North Wilkesboro"],
  IKI: ["Iki Airport", "Iki"],
  IKK: ["Greater Kankakee Airport", "Kankakee"],
  IKL: ["Ikela Airport", "Ikela"],
  IKO: ["Nikolski Air Station", "Nikolski"],
  IKP: ["Inkerman Airport"],
  IKS: ["Tiksi Airport", "Tiksi"],
  IKT: ["Irkutsk Airport", "Irkutsk"],
  IKU: ["Issyk-Kul International Airport", "Tamchy"],
  ILA: ["Illaga Airport", "Illaga-Papua Island"],
  ILD: ["Lleida-Alguaire Airport", "Lleida"],
  ILE: ["Skylark Field", "Killeen"],
  ILF: ["Ilford Airport", "Ilford"],
  ILG: ["New Castle Airport", "Wilmington"],
  ILH: ["Illesheim Air Base", "Storck Barracks"],
  ILI: ["Iliamna Airport", "Iliamna"],
  ILK: ["Atsinanana Airport", "Ilaka"],
  ILL: ["Willmar Municipal -John L Rice Field", "Willmar"],
  ILM: ["Wilmington International Airport", "Wilmington"],
  ILN: ["Wilmington Airpark", "Wilmington"],
  ILO: ["Iloilo International Airport", "Iloilo City"],
  ILP: ["\u00cele des Pins Airport", "\u00cele des Pins"],
  ILQ: ["Ilo Airport", "Ilo"],
  ILR: ["Ilorin International Airport", "Ilorin"],
  ILU: ["Kilaguni Airport", "Kilaguni"],
  ILX: ["Ileg Airport", "Ileg"],
  ILY: ["Islay Airport", "Port Ellen"],
  ILZ: ["\u017dilina Airport", "\u017dilina"],
  IMA: ["Iamalele Airport", "Iamalele, Fergusson Island"],
  IMB: ["Imbaimadai Airport", "Imbaimadai"],
  IMD: ["Imonda Airport", "Imonda"],
  IMF: ["Imphal Airport", "Imphal"],
  IMG: ["Inhaminga Airport", "Inhaminga"],
  IMI: ["Ine Airport", "Arno Atoll"],
  IMK: ["Simikot Airport", "Simikot"],
  IML: ["Imperial Municipal Airport", "Imperial"],
  IMM: ["Immokalee Regional Airport", "Immokalee"],
  IMN: ["Imane Airport", "Imane"],
  IMO: ["Zemio Airport", "Zemio"],
  IMP: ["Prefeito Renato Moreira Airport", "Imperatriz"],
  IMQ: ["Maku National Airport", "Showt"],
  IMT: ["Ford Airport", "Iron Mountain / Kingsford"],
  INA: ["Inta Airport", "Inta"],
  INB: ["Independence Airport", "Independence"],
  INC: ["Yinchuan Airport", "Yinchuan"],
  IND: ["Indianapolis International Airport", "Indianapolis"],
  INE: ["Chinde Airport", "Chinde"],
  INF: ["In Guezzam Airport", "In Guezzam"],
  ING: ["Lago Argentino Airport", "El Calafate"],
  INH: ["Inhambane Airport", "Inhambabe"],
  INI: ["Nis Airport", "Nis"],
  INJ: ["Injune Airport", "Injune"],
  INK: ["Winkler County Airport", "Wink"],
  INL: ["Falls International Airport", "International Falls"],
  INM: ["Innamincka Airport"],
  INN: ["Innsbruck Airport", "Innsbruck"],
  INO: ["Inongo Airport", "Inongo"],
  INQ: ["Inisheer Aerodrome", "Inis O\u00edrr"],
  INS: ["Creech Air Force Base", "Indian Springs"],
  INT: ["Smith Reynolds Airport", "Winston Salem"],
  INU: ["Nauru International Airport", "Yaren District"],
  INV: ["Inverness Airport", "Inverness"],
  INW: ["Winslow Lindbergh Regional Airport", "Winslow"],
  INX: ["Inanwatan Airport", "Inanwatan Airport-Papua Island"],
  INY: ["Inyati Airport", "Inyati"],
  INZ: ["In Salah Airport", "In Salah"],
  IOA: ["Ioannina Airport", "Ioannina"],
  IOK: ["Iokea Airport", "Iokea"],
  IOM: ["Isle of Man Airport", "Castletown"],
  ION: ["Impfondo Airport", "Impfondo"],
  IOP: ["Ioma Airport", "Ioma"],
  IOR: ["Inishmore Aerodrome", "Inis M\u00f3r"],
  IOS: ["Bahia - Jorge Amado Airport", "Ilh\u00e9us"],
  IOU: ["Edmond Can\u00e9 Airport", "\u00cele Ouen"],
  IOW: ["Iowa City Municipal Airport", "Iowa City"],
  IPA: ["Ipota Airport", "Ipota"],
  IPC: ["Mataveri Airport", "Isla De Pascua"],
  IPE: ["Ipil Airport", "Ipil"],
  IPG: ["Ipiranga Airport", "Santo Ant\u00f4nio Do I\u00e7\u00e1"],
  IPH: ["Sultan Azlan Shah Airport", "Ipoh"],
  IPI: ["San Luis Airport", "Ipiales"],
  IPL: ["Imperial County Airport", "Imperial"],
  IPN: ["Usiminas Airport", "Ipatinga"],
  IPT: ["Williamsport Regional Airport", "Williamsport"],
  IPU: ["Ipia\u00fa Airport", "Ipia\u00fa"],
  IQA: ["Al Asad Air Base", "H\u012bt"],
  IQM: ["Qiemo Yudu Airport", "Qiemo"],
  IQN: ["Qingyang Airport", "Qingyang"],
  IQQ: ["Diego Aracena Airport", "Iquique"],
  IQT: [
    "Coronel FAP Francisco Secada Vignetta International Airport",
    "Iquitos",
  ],
  IRA: ["Ngorangora Airport", "Kirakira"],
  IRB: ["Iraan Municipal Airport", "Iraan"],
  IRC: ["Circle City /New/ Airport", "Circle"],
  IRD: ["Ishurdi Airport", "Ishurdi"],
  IRE: ["Irec\u00ea Airport", "Irec\u00ea"],
  IRG: ["Lockhart River Airport", "Lockhart River"],
  IRI: ["Iringa Airport", "Nduli"],
  IRJ: ["Capitan V A Almonacid Airport", "La Rioja"],
  IRK: ["Kirksville Regional Airport", "Kirksville"],
  IRM: ["Igrim Airport"],
  IRN: ["Iriona Airport", "Iriona"],
  IRO: ["Birao Airport", "Birao"],
  IRP: ["Matari Airport"],
  IRS: ["Kirsch Municipal Airport", "Sturgis"],
  IRU: ["Iranamadu Seaplane Base", "Iranamadu"],
  IRZ: ["Tapuruquara Airport", "Santa Isabel Do Rio Negro"],
  ISA: ["Mount Isa Airport", "Mount Isa"],
  ISB: ["New Islamabad International Airport", "Islamabad"],
  ISC: ["St. Mary's Airport", "St. Mary's"],
  ISD: ["Santa B\u00e1rbara Airport", "Iscuand\u00e9"],
  ISE: ["S\u00fcleyman Demirel International Airport", "Isparta"],
  ISG: ["New Ishigaki Airport", "Ishigaki"],
  ISI: ["Isisford Airport"],
  ISJ: ["Isla Mujeres Airport"],
  ISK: ["Nashik Airport", "Nasik"],
  ISM: ["Kissimmee Gateway Airport", "Orlando"],
  ISN: ["Sloulin Field International Airport", "Williston"],
  ISO: ["Kinston Regional Jetport At Stallings Field", "Kinston"],
  ISP: ["Long Island Mac Arthur Airport", "Islip"],
  ISQ: ["Schoolcraft County Airport", "Manistique"],
  ISS: ["Wiscasset Airport", "Wiscasset"],
  IST: ["Istanbul New Airport", "Istanbul"],
  ISU: ["Sulaymaniyah International Airport", "Sulaymaniyah"],
  ISW: ["Alexander Field South Wood County Airport", "Wisconsin Rapids"],
  ITA: ["Itacoatiara Airport", "Itacoatiara"],
  ITB: ["Itaituba Airport", "Itaituba"],
  ITE: ["Ituber\u00e1 Airport", "Ituber\u00e1"],
  ITH: ["Ithaca Tompkins Regional Airport", "Ithaca"],
  ITI: ["Agropecu\u00e1ria Castanhais Airport", "Cumaru Do Norte"],
  ITK: ["Itokama Airport", "Itokama"],
  ITM: ["Osaka International Airport", "Osaka"],
  ITN: ["Itabuna Airport", "Itabuna"],
  ITO: ["Hilo International Airport", "Hilo"],
  ITP: ["Itaperuna Airport", "Itaperuna"],
  ITQ: ["Itaqui Airport", "Itaqui"],
  ITR: ["Francisco Vilela do Amaral Airport", "Itumbiara"],
  ITU: ["Iturup Airport", "Kurilsk"],
  IUA: ["Canandaigua Airport", "Ontario County IDA"],
  IUE: ["Niue International Airport", "Alofi"],
  IUL: ["Ilu Airport", "Ilu-Papua Island"],
  IUS: ["Inus Airport", "Inus"],
  IVA: ["Ampampamena Airport"],
  IVC: ["Invercargill Airport", "Invercargill"],
  IVG: ["Berane Airport", "Berane"],
  IVH: ["Ivishak Airport", "Ivishak River"],
  IVI: ["Viveros Island Airport", "Isla Viveros"],
  IVL: ["Ivalo Airport", "Ivalo"],
  IVR: ["Inverell Airport", "Inverell"],
  IVW: ["Inverway Airport", "Inverway"],
  IWA: ["Ivanovo South Airport", "Ivanovo"],
  IWD: ["Gogebic Iron County Airport", "Ironwood"],
  IWJ: ["Iwami Airport", "Masuda"],
  IWK: ["Iwakuni Marine Corps Air Station", "Iwakuni"],
  IWO: ["Iwo Jima Airport"],
  IWS: ["West Houston Airport", "Houston"],
  IXA: ["Agartala Airport", "Agartala"],
  IXB: ["Bagdogra Airport", "Siliguri"],
  IXC: ["Chandigarh Airport", "Chandigarh"],
  IXD: ["Allahabad Airport", "Allahabad"],
  IXE: ["Mangalore International Airport", "Mangalore"],
  IXG: ["Belgaum Airport", "Belgaum"],
  IXH: ["Kailashahar Airport"],
  IXI: ["North Lakhimpur Airport", "Lilabari"],
  IXJ: ["Jammu Airport", "Jammu"],
  IXK: ["Keshod Airport"],
  IXL: ["Leh Kushok Bakula Rimpochee Airport", "Leh"],
  IXM: ["Madurai Airport", "Madurai"],
  IXN: ["Khowai Airport", "Khowai"],
  IXP: ["Pathankot Airport", "Pathankot"],
  IXQ: ["Kamalpur Airport"],
  IXR: ["Birsa Munda Airport", "Ranchi"],
  IXS: ["Silchar Airport", "Silchar"],
  IXT: ["Pasighat Airport", "Pasighat"],
  IXU: ["Aurangabad Airport", "Aurangabad"],
  IXV: ["Along Airport"],
  IXW: ["Sonari Airport", "Jamshedpur"],
  IXY: ["Kandla Airport", "Kandla"],
  IXZ: ["Vir Savarkar International Airport", "Port Blair"],
  IYK: ["Inyokern Airport", "Inyokern"],
  IZA: ["Zona da Mata Regional Airport", "Juiz De Fora"],
  IZO: ["Izumo Airport", "Izumo"],
  IZT: ["Ixtepec Airport", "Ixtepec"],
  JAA: ["Jalalabad Airport"],
  JAB: ["Jabiru Airport"],
  JAC: ["Jackson Hole Airport", "Jackson"],
  JAD: ["Perth Jandakot Airport", "Perth"],
  JAE: ["Shumba Airport", "Ja\u00e9n"],
  JAF: ["Kankesanturai Airport", "Jaffna"],
  JAG: ["Shahbaz Air Base", "Jacobabad"],
  JAI: ["Jaipur International Airport", "Jaipur"],
  JAK: ["Jacmel Airport", "Jacmel"],
  JAL: ["El Lencero Airport", "Xalapa"],
  JAM: ["Bezmer Air Base", "Yambol"],
  JAN: ["Jackson-Medgar Wiley Evers International Airport", "Jackson"],
  JAP: ["Chacarita Airport", "Puntarenas"],
  JAQ: ["Jacquinot Bay Airport", "Jacquinot Bay"],
  JAR: ["Jahrom Airport", "Jahrom"],
  JAS: ["Jasper County Airport-Bell Field", "Jasper"],
  JAT: ["Jabot Airport", "Ailinglapalap Atoll"],
  JAU: ["Francisco Carle Airport", "Jauja"],
  JAV: ["Ilulissat Airport", "Ilulissat"],
  JAX: ["Jacksonville International Airport", "Jacksonville"],
  JBB: ["Noto Hadinegoro, Jember Airport", "Jember"],
  JBK: ["Berkley Municipal Heliport", "Oakland"],
  JBQ: ["La Isabela International Airport", "La Isabela"],
  JBR: ["Jonesboro Municipal Airport", "Jonesboro"],
  JBT: ["Bethel Seaplane Base", "Bethel"],
  JCA: ["Croisette Heliport", "Cannes"],
  JCB: ["Santa Terezinha Airport", "Joa\u00e7aba"],
  JCH: ["Qasigiannguit Heliport", "Qasigiannguit"],
  JCI: ["New Century Aircenter Airport", "Olathe"],
  JCJ: ["Chuja Port Heliport", "Chuja, Hach\u2019uja-do (Chujado Islands)"],
  JCK: ["Julia Creek Airport"],
  JCM: ["Jacobina Airport", "Jacobina"],
  JCR: ["Jacareacanga Airport", "Jacareacanga"],
  JCT: ["Kimble County Airport", "Junction"],
  JCU: ["Ceuta Heliport", "Ceuta"],
  JCY: ["LBJ Ranch Airport", "Johnson City"],
  JDA: ["Grant Co Regional/Ogilvie Field", "John Day"],
  JDF: ["Francisco de Assis Airport", "Juiz De Fora"],
  JDG: ["Jeongseok Airport", "Jeju Island"],
  JDH: ["Jodhpur Airport", "Jodhpur"],
  JDN: ["Jordan Airport", "Jordan"],
  JDO: ["Orlando Bezerra de Menezes Airport", "Juazeiro Do Norte"],
  JDP: ["Paris Issy-les-Moulineaux Heliport", "Issy-les-Moulineaux"],
  JDR: [
    "Prefeito Oct\u00e1vio de Almeida Neves Airport",
    "S\u00e3o Jo\u00e3o Del Rei",
  ],
  JDZ: ["Jingdezhen Airport", "Jingdezhen"],
  JED: ["King Abdulaziz International Airport", "Jeddah"],
  JEE: ["J\u00e9r\u00e9mie Airport", "Jeremie"],
  JEF: ["Jefferson City Memorial Airport", "Jefferson City"],
  JEG: ["Aasiaat Airport", "Aasiaat"],
  JEJ: ["Jeh Airport", "Ailinglapalap Atoll"],
  JEK: ["Jeki Airport", "Lower Zambezi Natational Park"],
  JEQ: ["Jequi\u00e9 Airport", "Jequi\u00e9"],
  JER: ["Jersey Airport", "Saint Helier"],
  JFK: ["John F Kennedy International Airport", "New York"],
  JFN: ["Northeast Ohio Regional Airport", "Ashtabula"],
  JFR: ["Paamiut Airport", "Paamiut"],
  JGA: ["Jamnagar Airport", "Jamnagar"],
  JGB: ["Jagdalpur Airport", "Jagdalpur"],
  JGD: ["Jiagedaqi Airport", "Jiagedaqi"],
  JGN: ["Jiayuguan Airport", "Jiayuguan"],
  JGO: ["Qeqertarsuaq Heliport", "Qeqertarsuaq Airport"],
  JGR: ["Groennedal Heliport", "Groennedal"],
  JGS: ["Jinggangshan Airport", "Ji'an"],
  JHB: ["Senai International Airport", "Johor Bahru"],
  JHG: ["Xishuangbanna Gasa Airport", "Jinghong"],
  JHL: ["Fort MacKay/Albian Aerodrome", "Albian Village"],
  JHM: ["Kapalua Airport", "Lahaina"],
  JHQ: ["Shute Harbour Airport"],
  JHS: ["Sisimiut Airport", "Sisimiut"],
  JHW: ["Chautauqua County-Jamestown Airport", "Jamestown"],
  JIA: ["Ju\u00edna Airport", "Ju\u00edna"],
  JIB: ["Djibouti-Ambouli Airport", "Djibouti City"],
  JIC: ["Jinchuan Airport", "Jinchang"],
  JIJ: ["Wilwal International Airport", "Jijiga"],
  JIK: ["Ikaria Airport", "Ikaria Island"],
  JIL: ["Jilin Airport", "Jilin"],
  JIM: ["Jimma Airport", "Jimma"],
  JIN: ["Jinja Airport", "Jinja"],
  JIO: ["Jos Orno Imsula Airport", "Tiakur"],
  JIP: ["Jipijapa Airport", "Jipijapa"],
  JIQ: ["Qianjiang Wulingshan Airport", "Qianjiang"],
  JIR: ["Jiri Airport", "Jiri"],
  JIU: ["Jiujiang Lushan Airport", "Jiujiang"],
  JIW: ["Jiwani Airport", "Jiwani"],
  JJD: ["Jericoacoara Airport", "Jijoca de Jericoacoara"],
  JJG: ["Humberto Ghizzo Bortoluzzi Regional Airport", "Jaguaruna"],
  JJI: ["Juanjui Airport", "Juanju\u00ed"],
  JJM: ["Mulika Lodge Airport", "Meru-Kinna"],
  JJN: ["Quanzhou Jinjiang International Airport", "Quanzhou"],
  JJU: ["Qaqortoq Heliport", "Qaqortoq"],
  JKG: ["J\u00f6nk\u00f6ping Airport", "J\u00f6nk\u00f6ping"],
  JKH: ["Chios Island National Airport", "Chios Island"],
  JKL: ["Kalymnos Airport", "Kalymnos Island"],
  JKR: ["Janakpur Airport", "Janakpur"],
  JKV: ["Cherokee County Airport", "Jacksonville"],
  JLA: ["Quartz Creek Airport", "Cooper Landing"],
  JLN: ["Joplin Regional Airport", "Joplin"],
  JLR: ["Jabalpur Airport"],
  JLS: ["Jales Airport", "Jales"],
  JMB: ["Jamba Airport", "Jamba"],
  JMJ: ["Lancang Jingmai Airport", "Lancang Lahu Autonomous County"],
  JMK: ["Mikonos Airport", "Mykonos Island"],
  JMO: ["Jomsom Airport", "Jomsom"],
  JMS: ["Jamestown Regional Airport", "Jamestown"],
  JMU: ["Jiamusi Airport", "Jiamusi"],
  JNA: ["Janu\u00e1ria Airport", "Janu\u00e1ria"],
  JNB: ["OR Tambo International Airport", "Johannesburg"],
  JNG: ["Jining Qufu Airport", "Jining"],
  JNI: ["Junin Airport", "Junin"],
  JNJ: ["Ja'Aluni Airport", "Duqm"],
  JNN: ["Nanortalik Heliport", "Nanortalik"],
  JNS: ["Narsaq Heliport", "Narsaq"],
  JNU: ["Juneau International Airport", "Juneau"],
  JNX: ["Naxos Airport", "Naxos Island"],
  JNZ: ["Jinzhou Airport", "Jinzhou"],
  JOE: ["Joensuu Airport", "Joensuu / Liperi"],
  JOG: ["Adi Sutjipto International Airport", "Yogyakarta-Java Island"],
  JOH: ["Port St Johns Airport", "Port St Johns"],
  JOI: ["Lauro Carneiro de Loyola Airport", "Joinville"],
  JOJ: ["Doris Lake", "Hope Bay"],
  JOK: ["Yoshkar-Ola Airport", "Yoshkar-Ola"],
  JOL: ["Jolo Airport"],
  JOM: ["Njombe Airport", "Njombe"],
  JON: ["Johnston Atoll Airport", "Johnston Atoll"],
  JOP: ["Josephstaal Airport", "Josephstaal"],
  JOS: ["Yakubu Gowon Airport", "Jos"],
  JOT: ["Joliet Regional Airport", "Joliet"],
  JPA: ["Presidente Castro Pinto International Airport", "Jo\u00e3o Pessoa"],
  JPB: ["Pan Am Building Heliport", "New York"],
  JPN: ["Pentagon Army Heliport", "Washington"],
  JPR: ["Ji-Paran\u00e1 Airport", "Ji-Paran\u00e1"],
  JQA: ["Qaarsut Airport", "Uummannaq"],
  JQE: ["Jaqu\u00e9 Airport", "Jaqu\u00e9"],
  JRA: ["West 30th St. Heliport", "New York"],
  JRB: ["Downtown-Manhattan/Wall St Heliport", "New York"],
  JRF: ["Kalaeloa Airport", "Kapolei"],
  JRH: ["Jorhat Airport", "Jorhat"],
  JRN: ["Juruena Airport", "Juruena"],
  JRO: ["Kilimanjaro International Airport", "Arusha"],
  JRS: ["Jerusalem Airport", "Jerusalem"],
  JSA: ["Jaisalmer Airport"],
  JSH: ["Sitia Airport", "Crete Island"],
  JSI: ["Skiathos Island National Airport", "Skiathos"],
  JSJ: ["Jiansanjiang Airport", "Jiansanjiang"],
  JSK: ["Jask Airport", "Jask"],
  JSM: ["Jose De San Martin Airport", "Chubut"],
  JSR: ["Jessore Airport", "Jashahor"],
  JST: ["John Murtha Johnstown Cambria County Airport", "Johnstown"],
  JSU: ["Maniitsoq Airport", "Maniitsoq"],
  JSY: ["Syros Airport", "Syros Island"],
  JTC: ["Bauru - Arealva Airport", "Bauru"],
  JTI: ["Jata\u00ed Airport", "Jata\u00ed"],
  JTR: ["Santorini Airport", "Santorini Island"],
  JTY: ["Astypalaia Airport", "Astypalaia Island"],
  JUA: ["In\u00e1cio Lu\u00eds do Nascimento Airport", "Juara"],
  JUB: ["Juba International Airport", "Juba"],
  JUH: ["Jiuhuashan Airport", "Chizhou"],
  JUI: ["Juist Airport", "Juist"],
  JUJ: [
    "Gobernador Horacio Guzman International Airport",
    "San Salvador de Jujuy",
  ],
  JUL: ["Inca Manco Capac International Airport", "Juliaca"],
  JUM: ["Jumla Airport", "Jumla"],
  JUN: ["Jundah Airport"],
  JUO: ["Jurado Airport", "Jurado"],
  JUR: ["Jurien Bay Airport"],
  JUT: ["Jutigalpa airport", "Jutigalpa"],
  JUV: ["Upernavik Airport", "Upernavik"],
  JUZ: ["Quzhou Airport", "Quzhou"],
  JVA: ["Ankavandra Airport", "Ankavandra"],
  JVI: ["Central Jersey Regional Airport", "Manville"],
  JVL: ["Southern Wisconsin Regional Airport", "Janesville"],
  JWA: ["Jwaneng Airport"],
  JWN: ["Zanjan Airport", "Zanjan"],
  JXA: ["Jixi Xingkaihu Airport", "Jixi"],
  JXN: ["Jackson County Reynolds Field", "Jackson"],
  JYR: ["Jiroft Airport", "Jiroft"],
  JYV: ["Jyvaskyla Airport", "Jyv\u00e4skyl\u00e4n Maalaiskunta"],
  JZH: ["Jiuzhai Huanglong Airport", "Jiuzhaigou"],
  KAA: ["Kasama Airport", "Kasama"],
  KAB: ["Kariba International Airport", "Kariba"],
  KAC: ["Kamishly Airport", "Kamishly"],
  KAD: ["Kaduna Airport", "Kaduna"],
  KAE: ["Kake Seaplane Base", "Kake"],
  KAF: ["Karato Airport", "Karato"],
  KAG: ["Gangneung Airport (K-18)", "Gangneung"],
  KAI: ["Kaieteur International Airport", "Kaieteur Falls"],
  KAJ: ["Kajaani Airport", "Kajaani"],
  KAL: ["Kaltag Airport", "Kaltag"],
  KAM: ["Kamaran Airport", "Kamaran"],
  KAN: ["Mallam Aminu International Airport", "Kano"],
  KAO: ["Kuusamo Airport", "Kuusamo"],
  KAP: ["Kapanga Airport", "Kapanga"],
  KAQ: ["Kamulai Airport", "Kamulai Mission"],
  KAR: ["Kamarang Airport", "Kamarang"],
  KAS: ["Karasburg Airport", "Karasburg"],
  KAT: ["Kaitaia Airport", "Kaitaia"],
  KAU: ["Kauhava Airport", "Kauhava"],
  KAV: ["Kavanayen Airport"],
  KAW: ["Kawthoung Airport", "Kawthoung"],
  KAX: ["Kalbarri Airport", "Kalbarri"],
  KAY: ["Wakaya Island Airport", "Wakaya Island"],
  KAZ: ["Kao Airport", "Kao-Celebes Island"],
  KBA: ["Kabala Airport", "Kabala"],
  KBB: ["Kirkimbie Station Airport", "Kirkimbie"],
  KBC: ["Birch Creek Airport", "Birch Creek"],
  KBD: ["Kimberley Downs Airport", "Kimberley Downs"],
  KBE: ["Bell Island Hot Springs Seaplane Base", "Bell Island"],
  KBF: ["Karubaga Airport", "Karubaga-Papua Island"],
  KBH: ["Kalat Airport", "Kalat"],
  KBI: ["Kribi Airport", "Kribi"],
  KBJ: ["Kings Canyon Airport", "Kings Canyon Resort"],
  KBL: ["Hamid Karzai International Airport", "Kabul"],
  KBM: ["Kabwum"],
  KBN: ["Tunta Airport", "Kabinda"],
  KBO: ["Kabalo Airport", "Kabalo"],
  KBP: ["Boryspil International Airport", "Kiev"],
  KBQ: ["Kasungu Airport", "Kasungu"],
  KBR: ["Sultan Ismail Petra Airport", "Kota Baharu"],
  KBS: ["Bo Airport", "Bo"],
  KBT: ["Kaben Airport", "Kaben"],
  KBU: ["Gusti Syamsir Alam Airport", "Stagen - Laut Island"],
  KBV: ["Krabi Airport", "Krabi"],
  KBW: ["Chignik Bay Seaplane Base", "Chignik"],
  KBX: ["Kambuaya Airport", "Kambuaya-Papua Island"],
  KBY: ["Streaky Bay Airport"],
  KBZ: ["Kaikoura Airport"],
  KCA: ["Kuqa Airport", "Kuqa"],
  KCB: ["Tepoe Airstrip", "Kasikasima"],
  KCC: ["Coffman Cove Seaplane Base", "Coffman Cove"],
  KCD: ["Kamur Airport", "Kamur-Papua Island"],
  KCE: ["Collinsville Airport"],
  KCF: ["Kadanwari Airport", "Kadanwari"],
  KCG: ["Chignik Fisheries Airport", "Chignik"],
  KCH: ["Kuching International Airport", "Kuching"],
  KCI: ["Kon Airport", "Kon"],
  KCJ: ["Komaio Airport", "Komaio"],
  KCK: ["Kirensk Airport", "Kirensk"],
  KCL: ["Chignik Lagoon Airport", "Chignik Flats"],
  KCM: ["Kahramanmara\u015f Airport", "Kahramanmara\u015f"],
  KCN: ["Chernofski Harbor Seaplane Base", "Chernofski Harbor"],
  KCO: ["Cengiz Topel Airport"],
  KCQ: ["Chignik Lake Airport", "Chignik Lake"],
  KCR: ["Colorado Creek Airport", "Colorado Creek"],
  KCS: ["Kings Creek Airport"],
  KCT: ["Koggala Airport", "Galle"],
  KCU: ["Masindi Airport", "Masindi"],
  KCZ: ["K\u014dchi Ry\u014dma Airport", "Nankoku"],
  KDA: ["Kolda North Airport", "Kolda"],
  KDB: ["Kambalda Airport", "Kambalda"],
  KDC: ["Kandi Airport", "Kandi"],
  KDD: ["Khuzdar Airport", "Khuzdar"],
  KDE: ["Koroba Airport", "Koroba"],
  KDH: ["Kandahar Airport"],
  KDI: ["Wolter Monginsidi Airport", "Kendari-Celebes Island"],
  KDJ: ["Ville Airport", "N'Djol\u00e9"],
  KDK: ["Kodiak Municipal Airport", "Kodiak"],
  KDL: ["K\u00e4rdla Airport", "K\u00e4rdla"],
  KDM: ["Kaadedhdhoo Airport", "Huvadhu Atoll"],
  KDN: ["Ndende Airport", "Ndende"],
  KDO: ["Kadhdhoo Airport", "Kadhdhoo"],
  KDP: ["Kandep Airport", "Kandep"],
  KDQ: ["Kamberatoro Airport", "Kamberatoro Mission"],
  KDR: ["Kandrian Airport", "Kandrian"],
  KDS: ["Kamaran Downs Airport", "Kamaran Downs"],
  KDT: ["Kamphaeng Saen Airport", "Nakhon Pathom"],
  KDU: ["Skardu Airport", "Skardu"],
  KDV: ["Vunisea Airport", "Vunisea"],
  KDW: ["Victoria Reservoir Seaplane Base", "Kandy"],
  KDX: ["Kadugli Airport", "Kadugli"],
  KDY: ["Typliy Klyuch Airport", "Khandyga"],
  KDZ: ["Polgolla Reservoir Seaplane Base", "Kandy"],
  KEA: ["Keisah Airport", "Keisah-Papua Island"],
  KEB: ["Nanwalek Airport", "Nanwalek"],
  KEC: ["Kasenga Airport", "Kasenga"],
  KED: ["Ka\u00e9di Airport", "Ka\u00e9di"],
  KEE: ["Kelle Airport", "Kelle"],
  KEF: ["Keflavik International Airport", "Reykjav\u00edk"],
  KEG: ["Keglsugl Airport", "Denglagu Mission"],
  KEH: ["Kenmore Air Harbor Inc Seaplane Base", "Kenmore"],
  KEI: ["Kepi Airport", "Kepi-Papua Island"],
  KEJ: ["Kemerovo Airport", "Kemerovo"],
  KEK: ["Ekwok Airport", "Ekwok"],
  KEL: ["Kiel-Holtenau Airport", "Kiel"],
  KEM: ["Kemi-Tornio Airport", "Kemi / Tornio"],
  KEN: ["Kenema Airport", "Kenema"],
  KEO: ["Odienne Airport", "Odienne"],
  KEP: ["Nepalgunj Airport", "Nepalgunj"],
  KEQ: ["Kebar Airport", "Kebar-Papua Island"],
  KER: ["Kerman Airport", "Kerman"],
  KES: ["Kelsey Airport", "Kelsey"],
  KET: ["Kengtung Airport", "Kengtung"],
  KEV: ["Halli Airport", "Halli / Kuorevesi"],
  KEW: ["Keewaywin Airport", "Keewaywin"],
  KEX: ["Kanabea Airport", "Kanabea"],
  KEY: ["Kericho Airport", "Kericho"],
  KEZ: ["Kelani-Peliyagoda Seaplane Base", "Colombo"],
  KFA: ["Kiffa Airport", "Kiffa"],
  KFE: ["Fortescue - Dave Forrest Aerodrome", "Cloudbreak Village"],
  KFG: ["Kalkgurung Airport"],
  KFP: ["False Pass Airport", "False Pass"],
  KFS: ["Kastamonu Airport", "Kastamonu"],
  KGA: ["Kananga Airport", "Kananga"],
  KGB: ["Konge Airport", "Konge"],
  KGC: ["Kingscote Airport"],
  KGD: ["Khrabrovo Airport", "Kaliningrad"],
  KGE: ["Kaghau Airport", "Kagau Island"],
  KGF: ["Sary-Arka Airport", "Karaganda"],
  KGG: ["K\u00e9dougou Airport", "K\u00e9dougou"],
  KGH: ["Yongai Airport", "Yongai"],
  KGI: ["Kalgoorlie Boulder Airport", "Kalgoorlie"],
  KGJ: ["Karonga Airport", "Karonga"],
  KGK: ["Koliganek Airport", "Koliganek"],
  KGL: ["Kigali International Airport", "Kigali"],
  KGM: ["Kungim Airport", "Kungim"],
  KGO: ["Kirovograd Airport", "Kirovograd"],
  KGP: ["Kogalym International Airport", "Kogalym"],
  KGR: ["Kulgera Airport", "Kulgera"],
  KGS: ["Kos Airport", "Kos Island"],
  KGT: ["Kangding Airport", "Kangding"],
  KGU: ["Keningau Airport", "Keningau"],
  KGW: ["Kagi Airport", "Kagi"],
  KGX: ["Grayling Airport", "Grayling"],
  KGY: ["Kingaroy Airport"],
  KGZ: ["Glacier Creek Airport", "Glacier Creek"],
  KHA: ["Khaneh Airport", "Khaneh"],
  KHC: ["Kerch Airport", "Kerch"],
  KHD: ["Khoram Abad Airport"],
  KHE: ["Kherson International Airport", "Kherson"],
  KHG: ["Kashgar Airport", "Kashgar"],
  KHH: ["Kaohsiung International Airport", "Kaohsiung City"],
  KHI: ["Jinnah International Airport", "Karachi"],
  KHJ: ["Kauhajoki Airport"],
  KHK: ["Khark Island Airport"],
  KHL: ["Khan Jahan Ali Airport", "Khulna"],
  KHM: ["Kanti Airport", "Kanti"],
  KHN: ["Nanchang Changbei International Airport", "Nanchang"],
  KHO: ["Khoka Moya Airport", "Khoka Moya"],
  KHR: ["Kharkhorin Airport"],
  KHS: ["Khasab Air Base", "Khasab"],
  KHT: ["Khost Airport", "Khost"],
  KHU: ["Kakhnovka Airfield", "Kremenchuk"],
  KHV: ["Khabarovsk-Novy Airport", "Khabarovsk"],
  KHW: ["Khwai River Lodge Airport", "Khwai River Lodge"],
  KHY: ["Khoy Airport", "Khoy"],
  KHZ: ["Kauehi Airport", "Kauehi"],
  KIA: ["Kaiapit Airport", "Kaiapit"],
  KIB: ["Ivanof Bay Seaplane Base", "Ivanof Bay"],
  KIC: ["Mesa Del Rey Airport", "King City"],
  KID: ["Kristianstad Airport", "Kristianstad"],
  KIE: ["Aropa Airport", "Kieta"],
  KIF: ["Kingfisher Lake Airport", "Kingfisher Lake"],
  KIG: ["Koingnaas Airport", "Koingnaas"],
  KIH: ["Kish International Airport", "Kish Island"],
  KII: ["Kibuli Airstrip", "Kibuli"],
  KIJ: ["Niigata Airport", "Niigata"],
  KIK: ["Kirkuk Air Base", "Kirkuk"],
  KIL: ["Kilwa Airport", "Kilwa"],
  KIM: ["Kimberley Airport", "Kimberley"],
  KIN: ["Norman Manley International Airport", "Kingston"],
  KIO: ["Kili Airport", "Kili Island"],
  KIP: ["Kickapoo Downtown Airport", "Wichita Falls"],
  KIQ: ["Kira Airport", "Kira"],
  KIR: ["Kerry Airport", "Killarney"],
  KIS: ["Kisumu Airport", "Kisumu"],
  KIT: ["Kithira Airport", "Kithira Island"],
  KIU: ["Kiunga Airport", "Kiunga"],
  KIV: ["Chi\u015fin\u0103u International Airport", "Chi\u015fin\u0103u"],
  KIW: ["Southdowns Airport", "Kitwe"],
  KIX: ["Kansai International Airport", "Osaka"],
  KIY: ["Kilwa Masoko Airport", "Kilwa Masoko"],
  KIZ: ["Kikinonda Airport", "Kikinonda"],
  KJA: ["Yemelyanovo Airport", "Krasnoyarsk"],
  KJH: ["Kaili Airport", "Huangping"],
  KJI: ["Kanas Airport", "Burqin"],
  KJK: ["Wevelgem Airport", "Wevelgem"],
  KJP: ["Kerama Airport", "Kerama"],
  KJU: ["Kamiraba Airport", "Kamiraba"],
  KKA: ["Koyuk Alfred Adams Airport", "Koyuk"],
  KKB: ["Kitoi Bay Seaplane Base", "Kitoi Bay"],
  KKC: ["Khon Kaen Airport", "Khon Kaen"],
  KKD: ["Kokoda Airport", "Kokoda"],
  KKE: ["Kerikeri Airport", "Kerikeri"],
  KKG: ["Konawaruk Airport", "Konawaruk"],
  KKH: ["Kongiganak Airport", "Kongiganak"],
  KKI: ["Akiachak Airport", "Akiachak"],
  KKJ: ["Kitaky\u016bsh\u016b Airport", "Kitaky\u016bsh\u016b"],
  KKK: ["Kalakaket Creek AS Airport", "Kalakaket Creek"],
  KKL: ["Karluk Lake Seaplane Base", "Karluk Lake"],
  KKM: ["Sa Pran Nak Airport"],
  KKN: ["Kirkenes Airport (H\u00f8ybuktmoen)", "Kirkenes"],
  KKO: ["Kaikohe Airport"],
  KKP: ["Koolburra Airport", "Koolburra"],
  KKQ: ["Krasnoselkup Airport", "Krasnoselkup"],
  KKR: ["Kaukura Airport"],
  KKS: ["Kashan Airport", "Kashan"],
  KKT: ["Kentland Municipal Airport", "Kentland"],
  KKU: ["Ekuk Airport", "Ekuk"],
  KKW: ["Kikwit Airport"],
  KKX: ["Kikai Airport", "Kikai"],
  KKY: ["Kilkenny Airport", "Kilkenny"],
  KKZ: ["Kaoh Kong Airport", "Kaoh Kong"],
  KLA: ["Kololo Airstrip", "Kampala"],
  KLB: ["Kalabo Airport", "Kalabo"],
  KLC: ["Kaolack Airport", "Kaolack"],
  KLD: ["Migalovo Air Base", "Tver"],
  KLE: ["Ka\u00e9l\u00e9 Airport", "Ka\u00e9l\u00e9"],
  KLF: ["Grabtsevo Airport", "Kaluga"],
  KLG: ["Kalskag Airport", "Kalskag"],
  KLH: ["Kolhapur Airport"],
  KLI: ["Kotakoli Airport"],
  KLJ: ["Klaip\u0117da Airport", "Klaip\u0117da"],
  KLK: ["Kalokol Airport", "Kalokol"],
  KLL: ["Levelock Airport", "Levelock"],
  KLM: ["Kalaleh Airport", "Kalaleh"],
  KLN: ["Larsen Bay Airport", "Larsen Bay"],
  KLO: ["Kalibo International Airport", "Kalibo"],
  KLQ: ["Keluang Airport", "Keluang-Sumatra Island"],
  KLR: ["Kalmar Airport"],
  KLS: ["Southwest Washington Regional Airport", "Kelso"],
  KLU: ["Klagenfurt Airport", "Klagenfurt am W\u00f6rthersee"],
  KLV: ["Karlovy Vary International Airport", "Karlovy Vary"],
  KLW: ["Klawock Airport", "Klawock"],
  KLX: ["Kalamata Airport", "Kalamata"],
  KLY: ["Kinkungwa Airport", "Kalima"],
  KLZ: ["Kleinsee Airport", "Kleinsee"],
  KMA: ["Kerema Airport", "Kerema"],
  KMB: ["Koinambe Airport", "Konambe"],
  KMC: ["King Khaled Military City Airport", "King Khaled Military City"],
  KME: ["Kamembe Airport", "Kamembe"],
  KMF: ["Kamina Airport", "Hoieti"],
  KMG: ["Kunming Changshui International Airport", "Kunming"],
  KMH: ["Johan Pienaar Airport", "Kuruman"],
  KMI: ["Miyazaki Airport", "Miyazaki"],
  KMJ: ["Kumamoto Airport", "Kumamoto"],
  KMK: ["Makabana Airport", "Makabana"],
  KML: ["Kamileroi Airport"],
  KMM: ["Kiman Airport", "Kiman-Papua Island"],
  KMN: ["Kamina Base Airport"],
  KMO: ["Manokotak Airport", "Manokotak"],
  KMP: ["Keetmanshoop Airport", "Keetmanshoop"],
  KMQ: ["Komatsu Airport", "Kanazawa"],
  KMR: ["Karimui Airport", "Karimui"],
  KMS: ["Kumasi Airport", "Kumasi"],
  KMT: ["Kampot Airport", "Kampot"],
  KMU: ["Kisimayu Airport"],
  KMV: ["Kalay Airport", "Kalemyo"],
  KMW: ["Kostroma Sokerkino Airport", "Kostroma"],
  KMX: ["King Khaled Air Base"],
  KMZ: ["Kaoma Airport", "Kaoma"],
  KNA: ["Vi\u00f1a del mar Airport", "Vi\u00f1a Del Mar"],
  KND: ["Kindu Airport", "Kindu"],
  KNE: ["Kanainj Airport", "Kanainj"],
  KNF: ["RAF Marham", "Marham"],
  KNG: ["Kaimana Airport", "Kaimana-Papua Island"],
  KNH: ["Kinmen Airport", "Shang-I"],
  KNI: ["Katanning Airport"],
  KNJ: ["Kindamba Airport", "Kindamba"],
  KNK: ["Kokhanok Airport", "Kokhanok"],
  KNL: ["Kelanoa Airport", "Kelanoa"],
  KNM: ["Kaniama Airport", "Kaniama"],
  KNN: ["Kankan Airport", "Kankan"],
  KNO: ["Kualanamu International Airport"],
  KNP: ["Capanda Airport", "Capanda"],
  KNQ: ["Kon\u00e9 Airport", "Kon\u00e9"],
  KNR: ["Jam Airport", "Kangan"],
  KNS: ["King Island Airport"],
  KNT: ["Kennett Memorial Airport", "Kennett"],
  KNU: ["Kanpur Airport", "Kanpur"],
  KNW: ["New Stuyahok Airport", "New Stuyahok"],
  KNX: ["Kununurra Airport", "Kununurra"],
  KNZ: ["Kenieba Airport", "Kenieba"],
  KOA: ["Ellison Onizuka Kona International At Keahole Airport", "Kailua/Kona"],
  KOC: ["Koumac Airport", "Koumac"],
  KOD: ["Kotabangun Airport", "Kotabangun-Borneo Island"],
  KOE: ["El Tari Airport", "Kupang-Timor Island"],
  KOF: ["Komatipoort Airport", "Komatipoort"],
  KOH: ["Koolatah Airport"],
  KOI: ["Kirkwall Airport", "Orkney Islands"],
  KOJ: ["Kagoshima Airport", "Kagoshima"],
  KOK: ["Kokkola-Pietarsaari Airport", "Kokkola / Kruunupyy"],
  KOL: ["Koumala Airport", "Koumala"],
  KOM: ["Komo-Manda Airport", "Komo"],
  KON: ["Kontum Airport", "Kontum"],
  KOO: ["Kongolo Airport", "Kongolo"],
  KOP: ["Nakhon Phanom Airport"],
  KOQ: ["K\u00f6then Airport", "K\u00f6then"],
  KOR: ["Kakoro(Koroko) Airstrip", "Kakoro"],
  KOS: ["Sihanoukville International Airport", "Sihanukville"],
  KOT: ["Kotlik Airport", "Kotlik"],
  KOU: ["Koulamoutou Mabimbi Airport", "Koulamoutou"],
  KOV: ["Kokshetau Airport", "Kokshetau"],
  KOW: ["Ganzhou Airport", "Ganzhou"],
  KOX: ["Kokonau Airport", "Kokonau-Papua Island"],
  KOY: ["Olga Bay Seaplane Base", "Olga Bay"],
  KOZ: ["Ouzinkie Airport", "Ouzinkie"],
  KPA: ["Kopiago Airport", "Kopiago"],
  KPB: ["Point Baker Seaplane Base", "Point Baker"],
  KPC: ["Port Clarence Coast Guard Station", "Port Clarence"],
  KPE: ["Yapsiei Airport"],
  KPF: ["Kondobol Airport", "Kondobol"],
  KPI: ["Kapit Airport", "Kapit"],
  KPL: ["Kapal Airport", "Kapal"],
  KPM: ["Kompiam Airport"],
  KPN: ["Kipnuk Airport", "Kipnuk"],
  KPO: ["Pohang Airport (G-815/K-3)", "Pohang"],
  KPP: ["Kalpowar Airport", "Kalpower"],
  KPR: ["Port Williams Seaplane Base", "Port Williams"],
  KPS: ["Kempsey Airport"],
  KPT: ["Jackpot Airport/Hayden Field", "Jackpot"],
  KPV: ["Perryville Airport", "Perryville"],
  KPY: ["Port Bailey Seaplane Base", "Port Bailey"],
  KQA: ["Akutan Seaplane Base", "Akutan"],
  KQH: ["Kishangarh Airport", "Ajmer"],
  KQL: ["Kol Airport", "Kol"],
  KQT: ["Qurghonteppa International Airport", "Kurgan-Tyube"],
  KRA: ["Kerang Airport"],
  KRB: ["Karumba Airport"],
  KRC: ["Departi Parbo Airport", "Sungai Penuh"],
  KRD: ["Kurundi Airport", "Kurundi Station"],
  KRE: ["Kirundo Airport", "Kirundo"],
  KRF: ["Kramfors Sollefte\u00e5 Airport", "Kramfors / Sollefte\u00e5"],
  KRG: ["Karasabai Airport", "Karasabai"],
  KRH: ["Redhill Aerodrome", "Redhill"],
  KRI: ["Kikori Airport", "Kikori"],
  KRJ: ["Karawari Airstrip", "Amboin"],
  KRK: ["Krak\u00f3w John Paul II International Airport", "Krak\u00f3w"],
  KRL: ["Korla Airport", "Korla"],
  KRM: ["Karanambo Airport", "Karanambo"],
  KRN: ["Kiruna Airport", "Kiruna"],
  KRO: ["Kurgan Airport", "Kurgan"],
  KRP: ["Karup Airport", "Karup"],
  KRQ: ["Kramatorsk Airport", "Kramatorsk"],
  KRR: ["Krasnodar Pashkovsky International Airport", "Krasnodar"],
  KRS: ["Kristiansand Airport", "Kjevik"],
  KRT: ["Khartoum International Airport", "Khartoum"],
  KRU: ["Kerau Airport", "Gunim"],
  KRV: ["Kerio Valley Airport", "Kimwarer"],
  KRW: ["Turkmenbashi Airport", "Krasnovodsk"],
  KRX: ["Kar Kar Airport", "Kar Kar Island"],
  KRY: ["Karamay Airport", "Karamay"],
  KRZ: ["Basango Mboliasa Airport", "Kiri"],
  KSA: ["Kosrae International Airport", "Okat"],
  KSB: ["Kasonombe Airport", "Kasonombe"],
  KSC: ["Ko\u0161ice Airport", "Ko\u0161ice"],
  KSD: ["Karlstad Airport", "Karlstad"],
  KSE: ["Kasese Airport", "Kasese"],
  KSF: ["Kassel-Calden Airport", "Kassel"],
  KSG: ["Kisengam Airport", "Kisenden"],
  KSH: ["Shahid Ashrafi Esfahani Airport", "Kermanshah"],
  KSI: ["Kissidougou Airport", "Kissidougou"],
  KSJ: ["Kasos Airport", "Kasos Island"],
  KSK: ["Karlskoga Airport"],
  KSL: ["Kassala Airport", "Kassala"],
  KSM: ["St Mary's Airport", "St Mary's"],
  KSN: ["Kostanay West Airport", "Kostanay"],
  KSO: ["Kastoria National Airport", "Kastoria"],
  KSP: ["Kosipe Airport", "Kosipe Mission"],
  KSQ: ["Karshi Khanabad Airport", "Khanabad"],
  KSS: ["Sikasso Airport", "Sikasso"],
  KST: ["Kosti Airport", "Kosti"],
  KSU: ["Kristiansund Airport (Kvernberget)", "Kvernberget"],
  KSV: ["Springvale Airport"],
  KSW: ["Kiryat Shmona Airport", "Kiryat Shmona"],
  KSX: ["Yasuru Airport", "Yasuru"],
  KSY: ["Kars Airport", "Kars"],
  KSZ: ["Kotlas Airport", "Kotlas"],
  KTA: ["Karratha Airport", "Karratha"],
  KTB: ["Thorne Bay Seaplane Base", "Thorne Bay"],
  KTC: ["Katiola Airport", "Katiola"],
  KTD: ["Kitadaito Airport", "Kitadait\u014djima"],
  KTE: ["Kerteh Airport", "Kerteh"],
  KTF: ["Takaka Airport"],
  KTG: ["Ketapang(Rahadi Usman) Airport", "Ketapang-Borneo Island"],
  KTH: ["Tikchik Lodge Seaplane Base", "Tikchik"],
  KTI: ["Kratie Airport", "Kratie"],
  KTK: ["Kunua Airport", "Kunua"],
  KTL: ["Kitale Airport", "Kitale"],
  KTM: ["Tribhuvan International Airport", "Kathmandu"],
  KTN: ["Ketchikan International Airport", "Ketchikan"],
  KTO: ["Kato Airport", "Kato"],
  KTP: ["Tinson Pen Airport", "Tinson Pen"],
  KTQ: ["Kitee Airport"],
  KTR: ["Tindal Airport"],
  KTS: ["Brevig Mission Airport", "Brevig Mission"],
  KTT: ["Kittil\u00e4 Airport", "Kittil\u00e4"],
  KTU: ["Kota Airport", "Kota"],
  KTV: ["Kamarata Airport", "Kamarata"],
  KTW: ["Katowice International Airport", "Katowice"],
  KTX: ["Koutiala Airport", "Koutiala"],
  KTY: ["Katukurunda Air Force Base", "Kalutara"],
  KUA: ["Kuantan Airport", "Kuantan"],
  KUC: ["Kuria Airport", "Kuria"],
  KUD: ["Kudat Airport", "Kudat"],
  KUE: ["Kukudu Airport", "Kolombangara Island"],
  KUF: ["Kurumoch International Airport", "Samara"],
  KUG: ["Kubin Airport", "Moa Island"],
  KUH: ["Kushiro Airport", "Kushiro"],
  KUK: ["Kasigluk Airport", "Kasigluk"],
  KUL: ["Kuala Lumpur International Airport", "Kuala Lumpur"],
  KUM: ["Yakushima Airport", "Yakushima"],
  KUN: ["Kaunas International Airport", "Kaunas"],
  KUO: ["Kuopio Airport", "Kuopio / Siilinj\u00e4rvi"],
  KUP: ["Kupiano Airport", "Kupiano"],
  KUQ: ["Kuri Airport", "Kuri"],
  KUS: ["Kulusuk Airport", "Kulusuk"],
  KUT: ["Kopitnari Airport", "Kutaisi"],
  KUU: ["Kullu Manali Airport"],
  KUV: ["Kunsan Air Base", "Kunsan"],
  KUX: ["Kuyol Airport"],
  KUY: ["Kamusi Airport", "Kamusi"],
  KVA: ["Alexander the Great International Airport", "Kavala"],
  KVB: ["Sk\u00f6vde Airport", "Sk\u00f6vde"],
  KVC: ["King Cove Airport", "King Cove"],
  KVD: ["Ganja Airport", "Ganja"],
  KVE: ["Kitava Airport", "Kitava Island"],
  KVG: ["Kavieng Airport", "Kavieng"],
  KVK: ["Kirovsk-Apatity Airport", "Apatity"],
  KVL: ["Kivalina Airport", "Kivalina"],
  KVM: ["Markovo Airport", "Markovo"],
  KVR: ["Kavalerovo Airport", "Kavalerovo"],
  KVX: ["Pobedilovo Airport", "Kirov"],
  KWA: ["Bucholz Army Air Field", "Kwajalein"],
  KWB: ["Dewadaru Airport", "Karimunjawa-Karimunjawa Island"],
  KWE: ["Longdongbao Airport", "Guiyang"],
  KWF: ["Waterfall Seaplane Base", "Waterfall"],
  KWG: ["Kryvyi Rih International Airport", "Kryvyi Rih"],
  KWH: ["Khwahan Airport", "Khwahan"],
  KWI: ["Kuwait International Airport", "Kuwait City"],
  KWJ: ["Gwangju Airport", "Gwangju"],
  KWK: ["Kwigillingok Airport", "Kwigillingok"],
  KWL: ["Guilin Liangjiang International Airport", "Guilin City"],
  KWM: ["Kowanyama Airport", "Kowanyama"],
  KWN: ["Quinhagak Airport", "Quinhagak"],
  KWO: ["Kawito Airport", "Kawito"],
  KWP: ["West Point Village Seaplane Base", "West Point"],
  KWS: ["Kwailabesi Airport", "Kwailabesi"],
  KWT: ["Kwethluk Airport", "Kwethluk"],
  KWV: ["Kurwina Airport", "Kurwina"],
  KWX: ["Kiwai Airport", "Kiwai Island"],
  KWY: ["Kiwayu Airport", "Kiwayu"],
  KWZ: ["Kolwezi Airport"],
  KXA: ["Kasaan Seaplane Base", "Kasaan"],
  KXE: ["P C Pelser Airport", "Klerksdorp"],
  KXF: ["Koro Island Airport", "Koro Island"],
  KXK: ["Komsomolsk-on-Amur Airport", "Komsomolsk-on-Amur"],
  KXR: ["Karoola Airport", "Buka Island"],
  KXU: ["Katiu Airport", "Katiu"],
  KYA: ["Konya Airport", "Konya"],
  KYD: ["Lanyu Airport", "Orchid Island"],
  KYE: ["Rene Mouawad Air Base", "Tripoli"],
  KYF: ["Yeelirrie Airport"],
  KYI: ["Yalata Mission Airport", "Yalata Mission"],
  KYK: ["Karluk Airport", "Karluk"],
  KYL: ["Port Largo Airport", "Key Largo"],
  KYO: ["Tampa North Aero Park Airport", "Tampa"],
  KYP: ["Kyaukpyu Airport", "Kyaukpyu"],
  KYS: ["Kayes Dag Dag Airport"],
  KYT: ["Kyauktu Airport", "Kyauktu"],
  KYU: ["Koyukuk Airport", "Koyukuk"],
  KYX: ["Yalumet Airport", "Yalumet"],
  KYZ: ["Kyzyl Airport", "Kyzyl"],
  KZB: ["Zachar Bay Seaplane Base", "Zachar Bay"],
  KZC: ["Kampong Chhnang Airport", "Kampong Chhnang"],
  KZD: ["Krakor Airport", "Krakor"],
  KZF: ["Kaintiba Airport", "Kaintiba"],
  KZG: ["Flugplatz Kitzingen", "Kitzingen"],
  KZI: ["Filippos Airport", "Kozani"],
  KZN: ["Kazan International Airport", "Kazan"],
  KZO: ["Kzyl-Orda Southwest Airport", "Kzyl-Orda"],
  KZR: ["Zafer Airport", "Alt\u0131nta\u015f"],
  KZS: ["Kastelorizo Airport", "Kastelorizo Island"],
  LAA: ["Lamar Municipal Airport", "Lamar"],
  LAB: ["Lab Lab Airport", "Lab Lab Mission"],
  LAC: ["Layang-Layang Airport", "Spratley Islands"],
  LAD: ["Quatro de Fevereiro Airport", "Luanda"],
  LAE: ["Nadzab Airport", "Lae"],
  LAF: ["Purdue University Airport", "Lafayette"],
  LAG: ["La Guaira Airport", "La Guaira"],
  LAH: ["Oesman Sadik Airport, Labuha", "Labuha-Halmahera Island"],
  LAI: ["Lannion-C\u00f4te de Granit Airport", "Lannion"],
  LAJ: ["Lages Airport", "Lages"],
  LAK: ["Aklavik/Freddie Carmichael Airport", "Aklavik"],
  LAL: ["Lakeland Linder International Airport", "Lakeland"],
  LAM: ["Los Alamos Airport", "Los Alamos"],
  LAN: ["Capital City Airport", "Lansing"],
  LAO: ["Laoag International Airport", "Laoag City"],
  LAP: ["Manuel M\u00e1rquez de Le\u00f3n International Airport", "La Paz"],
  LAQ: ["La Abraq Airport", "Al Bayda'"],
  LAR: ["Laramie Regional Airport", "Laramie"],
  LAS: ["McCarran International Airport", "Las Vegas"],
  LAU: ["Manda Airstrip", "Lamu"],
  LAW: ["Lawton Fort Sill Regional Airport", "Lawton"],
  LAX: ["Los Angeles International Airport", "Los Angeles"],
  LAY: ["Ladysmith Airport", "Ladysmith"],
  LAZ: ["Bom Jesus da Lapa Airport", "Bom Jesus Da Lapa"],
  LBA: ["Leeds Bradford Airport", "Leeds"],
  LBB: ["Lubbock Preston Smith International Airport", "Lubbock"],
  LBC: ["L\u00fcbeck Blankensee Airport", "L\u00fcbeck"],
  LBD: ["Khudzhand Airport", "Khudzhand"],
  LBE: ["Arnold Palmer Regional Airport", "Latrobe"],
  LBF: ["North Platte Regional Airport Lee Bird Field", "North Platte"],
  LBG: ["Paris-Le Bourget Airport", "Paris"],
  LBH: ["Palm Beach Seaplane Base", "Sydney"],
  LBI: ["Albi-Le S\u00e9questre Airport", "Albi/Le S\u00e9questre"],
  LBJ: ["Komodo Airport", "Labuan Bajo - Flores Island"],
  LBK: ["Liboi Airport", "Liboi"],
  LBL: ["Liberal Mid-America Regional Airport", "Liberal"],
  LBM: ["Luabo Airport", "Luabo"],
  LBN: ["Lake Baringo Airport", "Lake Baringo"],
  LBO: ["Lusambo Airport", "Lusambo"],
  LBP: ["Long Banga Airport", "Long Banga"],
  LBQ: ["Lambarene Airport", "Lambarene"],
  LBR: ["L\u00e1brea Airport", "L\u00e1brea"],
  LBS: ["Labasa Airport"],
  LBT: ["Lumberton Regional Airport", "Lumberton"],
  LBU: ["Labuan Airport", "Labuan"],
  LBV: ["Libreville Leon M'ba International Airport", "Libreville"],
  LBW: ["Long Bawan Airport", "Long Bawan-Borneo Island"],
  LBX: ["Lubang Airport"],
  LBY: ["La Baule-Escoublac Airport", "La Baule-Escoublac"],
  LBZ: ["Lucapa Airport", "Lucapa"],
  LCA: ["Larnaca International Airport", "Larnarca"],
  LCB: ["Pontes e Lacerda Airport", "Pontes e Lacerda"],
  LCC: ["Lecce Galatina Air Base"],
  LCD: ["Louis Trichardt Airport", "Louis Trichardt"],
  LCE: ["Goloson International Airport", "La Ceiba"],
  LCF: ["Las Vegas Airport", "Rio Dulce"],
  LCG: ["A Coru\u00f1a Airport", "Culleredo"],
  LCH: ["Lake Charles Regional Airport", "Lake Charles"],
  LCI: ["Laconia Municipal Airport", "Laconia"],
  LCJ: [
    "\u0141\u00f3d\u017a W\u0142adys\u0142aw Reymont Airport",
    "\u0141\u00f3d\u017a",
  ],
  LCK: ["Rickenbacker International Airport", "Columbus"],
  LCL: ["La Coloma Airport", "Pinar del Rio"],
  LCM: ["La Cumbre Airport", "La Cumbre"],
  LCN: ["Balcanoona Airport"],
  LCO: ["Lague Airport", "Lague"],
  LCP: ["Loncopue Airport", "Loncopue"],
  LCQ: ["Lake City Gateway Airport", "Lake City"],
  LCR: ["La Chorrera Airport", "La Chorrera"],
  LCV: ["Lucca-Tassignano Airport", "Lucca"],
  LCX: ["Longyan Guanzhishan Airport", "Longyan"],
  LCY: ["London City Airport", "London"],
  LDA: ["Malda Airport", "Malda"],
  LDB: ["Governador Jos\u00e9 Richa Airport", "Londrina"],
  LDC: ["Lindeman Island Airport", "Lindeman Island"],
  LDE: [
    "Tarbes-Lourdes-Pyr\u00e9n\u00e9es Airport",
    "Tarbes/Lourdes/Pyr\u00e9n\u00e9es",
  ],
  LDG: ["Leshukonskoye Airport", "Leshukonskoye"],
  LDH: ["Lord Howe Island Airport", "Lord Howe Island"],
  LDI: ["Lindi Airport", "Lindi"],
  LDJ: ["Linden Airport", "Linden"],
  LDK: ["Lidk\u00f6ping-Hovby Airport", "Lidk\u00f6ping"],
  LDM: ["Mason County Airport", "Ludington"],
  LDN: ["Lamidanda Airport", "Lamidanda"],
  LDO: ["Ladouanie Airport", "Aurora"],
  LDS: ["Lindu Airport", "Yichun"],
  LDU: ["Lahad Datu Airport", "Lahad Datu"],
  LDV: ["Landivisiau Air Base", "Landivisiau"],
  LDW: ["Lansdowne Airport", "Lansdowne Station"],
  LDX: ["Saint-Laurent-du-Maroni Airport", "Saint-Laurent-du-Maroni"],
  LDY: ["City of Derry Airport", "Derry"],
  LDZ: ["Londolozi Airport", "Londolozi"],
  LEA: ["Learmonth Airport", "Exmouth"],
  LEB: ["Lebanon Municipal Airport", "Lebanon"],
  LEC: ["Coronel Hor\u00e1cio de Mattos Airport", "Len\u00e7\u00f3is"],
  LED: ["Pulkovo Airport", "St. Petersburg"],
  LEE: ["Leesburg International Airport", "Leesburg"],
  LEF: ["Lebakeng Airport", "Lebakeng"],
  LEH: ["Le Havre Octeville Airport", "Le Havre/Octeville"],
  LEI: ["Almer\u00eda International Airport", "Almer\u00eda"],
  LEJ: ["Leipzig/Halle Airport", "Leipzig"],
  LEK: ["Tata Airport", "Lab\u00e9"],
  LEL: ["Lake Evella Airport"],
  LEM: ["Lemmon Municipal Airport", "Lemmon"],
  LEN: ["Leon Airport", "Le\u00f3n"],
  LEO: ["Lekoni Airport", "Lekoni"],
  LEP: ["Leopoldina Airport", "Leopoldina"],
  LEQ: ["Land's End Airport", "Land's End"],
  LER: ["Leinster Airport"],
  LES: ["Lesobeng Airport", "Lesobeng"],
  LET: ["Alfredo V\u00e1squez Cobo International Airport", "Leticia"],
  LEU: [
    "Pirineus - la Seu d'Urgel Airport",
    "La Seu d'Urgell Pyrenees and Andorra",
  ],
  LEV: ["Levuka Airfield", "Bureta"],
  LEW: ["Auburn Lewiston Municipal Airport", "Auburn/Lewiston"],
  LEX: ["Blue Grass Airport", "Lexington"],
  LEY: ["Lelystad Airport", "Lelystad"],
  LEZ: ["La Esperanza Airport", "La Esperanza"],
  LFB: ["Lumbo Airport", "Lumbo"],
  LFI: ["Langley Air Force Base", "Hampton"],
  LFK: ["Angelina County Airport", "Lufkin"],
  LFM: ["Lamerd Airport", "Lamerd"],
  LFN: ["Triangle North Executive Airport", "Louisburg"],
  LFO: ["Kelafo East Airport", "Kelafo"],
  LFP: ["Lakefield Airport", "Lakefield"],
  LFQ: ["Linfen Qiaoli Airport", "Linfen"],
  LFR: ["La Fria Airport"],
  LFT: ["Lafayette Regional Airport", "Lafayette"],
  LFW: ["Lom\u00e9-Tokoin Airport", "Lom\u00e9"],
  LGA: ["La Guardia Airport", "New York"],
  LGB: ["Long Beach /Daugherty Field/ Airport", "Long Beach"],
  LGC: ["LaGrange Callaway Airport", "Lagrange"],
  LGD: ["La Grande/Union County Airport", "La Grande"],
  LGE: ["Mulan Airport", "Lake Gregory"],
  LGF: ["Laguna Army Airfield", "Yuma Proving Ground(Yuma)"],
  LGG: ["Li\u00e8ge Airport", "Li\u00e8ge"],
  LGH: ["Leigh Creek Airport"],
  LGI: ["Deadman's Cay Airport", "Deadman's Cay"],
  LGK: ["Langkawi International Airport", "Langkawi"],
  LGL: ["Long Lellang Airport", "Long Datih"],
  LGM: ["Laiagam Airport", "Laiagam"],
  LGN: ["Linga Linga Airport", "Linga Linga"],
  LGO: ["Langeoog Airport", "Langeoog"],
  LGP: ["Legazpi City International Airport", "Legazpi City"],
  LGQ: ["Nueva Loja Airport", "Lago Agrio"],
  LGR: ["Cochrane Airport", "Cochrane"],
  LGS: ["Comodoro D.R. Salom\u00f3n Airport", "Malargue"],
  LGT: ["La Gaviota Airport"],
  LGU: ["Logan-Cache Airport", "Logan"],
  LGW: ["London Gatwick Airport", "London"],
  LGX: ["Lugh Ganane Airport", "Luuq"],
  LGY: ["Lagunillas Airport", "Lagunillas"],
  LHA: ["Lahr Airport"],
  LHC: ["Arlington Municipal Airport", "Arlington"],
  LHE: ["Alama Iqbal International Airport", "Lahore"],
  LHG: ["Lightning Ridge Airport"],
  LHI: ["Lereh Airport", "Lereh-Papua Island"],
  LHK: ["Guangzhou MR Air Base", "Guanghua"],
  LHP: ["Lehu Airport", "Lehu"],
  LHR: ["London Heathrow Airport", "London"],
  LHS: ["Las Heras Airport", "Las Heras"],
  LHU: ["Lianshulu Airport", "Lianshulu Lodge"],
  LHV: ["William T. Piper Memorial Airport", "Lock Haven"],
  LHW: ["Lanzhou Zhongchuan Airport", "Lanzhou"],
  LIA: ["Liangping Airport", "Liangping"],
  LIB: ["Limbunya Airport", "Limbunya"],
  LIC: ["Limon Municipal Airport", "Limon"],
  LID: ["Valkenburg Naval Air Base", "Leiden"],
  LIE: ["Libenge Airport", "Libenge"],
  LIF: ["Lifou Airport", "Lifou"],
  LIG: ["Limoges Airport", "Limoges/Bellegarde"],
  LIH: ["Lihue Airport", "Lihue"],
  LII: ["Mulia Airport", "Mulia-Papua Island"],
  LIK: ["Likiep Airport", "Likiep Island"],
  LIL: ["Lille-Lesquin Airport", "Lille/Lesquin"],
  LIM: ["Jorge Ch\u00e1vez International Airport", "Lima"],
  LIN: ["Milano Linate Airport", "Milan"],
  LIO: ["Limon International Airport", "Puerto Limon"],
  LIP: ["Lins Airport", "Lins"],
  LIQ: ["Lisala Airport"],
  LIR: ["Daniel Oduber Quiros International Airport", "Liberia"],
  LIS: ["Humberto Delgado Airport (Lisbon Portela Airport)", "Lisbon"],
  LIT: ["Bill & Hillary Clinton National Airport/Adams Field", "Little Rock"],
  LIV: ["Livengood Camp Airport", "Livengood"],
  LIW: ["Loikaw Airport", "Loikaw"],
  LIX: ["Likoma Island Airport", "Likoma Island"],
  LIY: [
    "Wright AAF (Fort Stewart)/Midcoast Regional Airport",
    "Fort Stewart(Hinesville)",
  ],
  LIZ: ["Loring International Airport", "Limestone"],
  LJA: ["Lodja Airport", "Lodja"],
  LJG: ["Lijiang Airport", "Lijiang"],
  LJN: ["Texas Gulf Coast Regional Airport", "Angleton/Lake Jackson"],
  LJU: ["Ljubljana Jo\u017ee Pu\u010dnik Airport", "Ljubljana"],
  LKA: ["Gewayentana Airport", "Larantuka-Flores Island"],
  LKB: ["Lakeba Island Airport", "Lakeba Island"],
  LKC: ["Lekana Airport", "Lekana"],
  LKD: ["Lakeland Airport", "Lakeland Downs"],
  LKE: ["Kenmore Air Harbor Seaplane Base", "Seattle"],
  LKG: ["Lokichoggio Airport", "Lokichoggio"],
  LKH: ["Long Akah Airport", "Long Akah"],
  LKK: ["Kulik Lake Airport", "Kulik Lake"],
  LKL: ["Banak Airport", "Lakselv"],
  LKN: ["Leknes Airport", "Leknes"],
  LKO: ["Chaudhary Charan Singh International Airport", "Lucknow"],
  LKP: ["Lake Placid Airport", "Lake Placid"],
  LKU: ["Lake Rudolf Airport", "Lake Rudolf"],
  LKV: ["Lake County Airport", "Lakeview"],
  LKW: ["Lekhwair Airport"],
  LKY: ["Lake Manyara Airport", "Lake Manyara National Park"],
  LKZ: ["RAF Lakenheath", "Lakenheath"],
  LLA: ["Lule\u00e5 Airport", "Lule\u00e5"],
  LLB: ["Libo Airport", "Libo County"],
  LLE: ["Riverside Airport", "Malelane"],
  LLF: ["Lingling Airport", "Yongzhou"],
  LLG: ["Chillagoe Airport"],
  LLH: ["Reginaldo Hammer Airport", "La Lima"],
  LLI: ["Lalibella Airport", "Lalibela"],
  LLJ: ["Lalmonirhat Airport", "Lalmonirhat"],
  LLK: ["Lankaran International Airport", "Lankaran"],
  LLL: ["Lissadell Airport", "Lissadell Station"],
  LLM: ["Long Lama Airport", "Long Lama"],
  LLN: ["Kelila Airport", "Kelila-Papua Island"],
  LLS: ["Alferez Armando Rodriguez Airport", "Las Lomitas"],
  LLU: ["Alluitsup Paa Heliport", "Alluitsup Paa"],
  LLV: ["L\u00fcliang Airport", "L\u00fcliang"],
  LLW: ["Lilongwe International Airport", "Lilongwe"],
  LLX: ["Caledonia County Airport", "Lyndonville"],
  LLY: ["South Jersey Regional Airport", "Mount Holly"],
  LMA: ["Minchumina Airport", "Minchumina"],
  LMB: ["Salima Airport", "Salima"],
  LMC: ["El Refugio/La Macarena Airport", "La Macarena"],
  LMD: ["Los Menucos Airport", "Los Menucos"],
  LME: ["Le Mans-Arnage Airport", "Le Mans/Arnage"],
  LMI: ["Lumi Airport", "Lumi"],
  LML: ["Lae Island Airport", "Lae Island"],
  LMM: ["Valle del Fuerte International Airport", "Los Mochis"],
  LMN: ["Limbang Airport", "Limbang"],
  LMO: ["RAF Lossiemouth", "Lossiemouth"],
  LMP: ["Lampedusa Airport", "Lampedusa"],
  LMQ: ["Marsa Brega Airport"],
  LMR: ["Lime Acres Finsch Mine Airport", "Lime Acres"],
  LMS: ["Louisville Winston County Airport", "Louisville"],
  LMT: ["Crater Lake-Klamath Regional Airport", "Klamath Falls"],
  LMY: ["Lake Murray Airport", "Lake Murray"],
  LMZ: ["Palma Airport", "Palma"],
  LNA: ["Palm Beach County Park Airport", "West Palm Beach"],
  LNB: ["Lamen Bay Airport", "Lamen Bay"],
  LNC: ["Lengbati Airport"],
  LND: ["Hunt Field", "Lander"],
  LNE: ["Lonorore Airport", "Lonorore"],
  LNF: ["Munbil Airport"],
  LNG: ["Lese Airport", "Lese"],
  LNH: ["Lake Nash Airport", "Alpurrurulam"],
  LNI: ["Lonely Air Station", "Lonely"],
  LNJ: ["Lintsang Airfield", "Lincang"],
  LNK: ["Lincoln Airport", "Lincoln"],
  LNL: ["Longnan Chengzhou Airport", "Longnan"],
  LNM: ["Langimar Airport"],
  LNN: ["Willoughby Lost Nation Municipal Airport", "Willoughby"],
  LNO: ["Leonora Airport", "Leonora"],
  LNP: ["Lonesome Pine Airport", "Wise"],
  LNQ: ["Loani Airport", "Loani"],
  LNR: ["Tri-County Regional Airport", "Lone Rock"],
  LNS: ["Lancaster Airport", "Lancaster"],
  LNV: ["Londolovit Airport", "Londolovit"],
  LNX: ["Smolensk North Airport", "Smolensk"],
  LNY: ["Lanai Airport", "Lanai City"],
  LNZ: ["Linz H\u00f6rsching Airport", "Linz"],
  LOA: ["Lorraine Airport"],
  LOB: ["San Rafael Airport", "Los Andes"],
  LOC: ["Lock Airport", "Lock"],
  LOD: ["Longana Airport", "Longana"],
  LOE: ["Loei Airport"],
  LOF: ["Loen Airport", "Loen Island"],
  LOH: ["Camilo Ponce Enriquez Airport", "La Toma (Catamayo)"],
  LOI: ["Helmuth Baungarten Airport", "Lontras"],
  LOK: ["Lodwar Airport", "Lodwar"],
  LOL: ["Derby Field", "Lovelock"],
  LOM: ["Francisco Primo de Verdad y Ramos Airport", "Lagos de Moreno"],
  LOO: ["Laghouat Airport", "Laghouat"],
  LOP: ["Lombok International Airport", "Mataram"],
  LOQ: ["Lobatse Airport", "Lobatse"],
  LOR: ["Lowe AHP (Fort Rucker) Heliport", "Fort Rucker Ozark"],
  LOS: ["Murtala Muhammed International Airport", "Lagos"],
  LOT: ["Lewis University Airport", "Chicago/Romeoville"],
  LOU: ["Bowman Field", "Louisville"],
  LOV: ["Monclova International Airport"],
  LOW: ["Louisa County Airport/Freeman Field", "Louisa"],
  LOY: ["Loyengalani Airport", "Loyengalani"],
  LOZ: ["London-Corbin Airport/Magee Field", "London"],
  LPA: ["Gran Canaria Airport", "Gran Canaria Island"],
  LPB: ["El Alto International Airport", "La Paz / El Alto"],
  LPC: ["Lompoc Airport", "Lompoc"],
  LPD: ["La Pedrera Airport", "La Pedrera"],
  LPE: ["La Primavera Airport", "La Primavera"],
  LPF: ["Liupanshui Yuezhao Airport", "Liupanshui"],
  LPG: ["La Plata Airport", "La Plata"],
  LPI: ["Link\u00f6ping City Airport", "Link\u00f6ping"],
  LPJ: ["Armando Schwarck Airport", "Guayabal"],
  LPK: ["Lipetsk Airport", "Lipetsk"],
  LPL: ["Liverpool John Lennon Airport", "Liverpool"],
  LPM: ["Lamap Airport", "Lamap"],
  LPN: ["Leron Plains Airport", "Leron Plains"],
  LPO: ["La Porte Municipal Airport", "La Porte"],
  LPP: ["Lappeenranta Airport", "Lappeenranta"],
  LPQ: ["Luang Phabang International Airport", "Luang Phabang"],
  LPS: ["Lopez Island Airport", "Lopez"],
  LPT: ["Lampang Airport"],
  LPU: ["Long Apung Airport", "Long Apung-Borneo Island"],
  LPX: ["Liep\u0101ja International Airport", "Liep\u0101ja"],
  LPY: ["Le Puy-Loudes Airport", "Le Puy/Loudes"],
  LQK: ["Pickens County Airport", "Pickens"],
  LQM: ["Caucaya Airport", "Puerto Legu\u00edzamo"],
  LQN: ["Qala-I-Naw Airport", "Qala-I-Naw"],
  LRA: ["Larisa Airport", "Larisa"],
  LRB: ["Leribe Airport", "Leribe"],
  LRD: ["Laredo International Airport", "Laredo"],
  LRE: ["Longreach Airport", "Longreach"],
  LRF: ["Little Rock Air Force Base", "Jacksonville"],
  LRG: ["Loralai Airport", "Loralai"],
  LRH: [
    "La Rochelle-\u00cele de R\u00e9 Airport",
    "La Rochelle/\u00cele de R\u00e9",
  ],
  LRJ: ["Le Mars Municipal Airport", "Le Mars"],
  LRL: ["Niamtougou International Airport", "Niamtougou"],
  LRM: ["Casa De Campo International Airport", "La Romana"],
  LRO: ["Sharpe AAF", "Lathrop"],
  LRQ: ["Laurie River Airport", "Laurie River"],
  LRR: ["Lar Airport", "Lar"],
  LRS: ["Leros Airport", "Leros Island"],
  LRT: [
    "Lorient South Brittany (Bretagne Sud) Airport",
    "Lorient/Lann/Bihou\u00e9",
  ],
  LRU: ["Las Cruces International Airport", "Las Cruces"],
  LRV: ["Los Roques Airport", "Gran Roque Island"],
  LSA: ["Losuia Airport", "Losuia"],
  LSB: ["Lordsburg Municipal Airport", "Lordsburg"],
  LSC: ["La Florida Airport", "La Serena-Coquimbo"],
  LSE: ["La Crosse Municipal Airport", "La Crosse"],
  LSF: ["Lawson Army Air Field (Fort Benning)", "Fort Benning(Columbus)"],
  LSH: ["Lashio Airport", "Lashio"],
  LSI: ["Sumburgh Airport", "Lerwick"],
  LSJ: ["Long Island Airport", "Long Island"],
  LSK: ["Lusk Municipal Airport", "Lusk"],
  LSL: ["Los Chiles Airport", "Los Chiles"],
  LSM: ["Long Semado Airport", "Long Semado"],
  LSN: ["Los Banos Municipal Airport", "Los Banos"],
  LSO: ["Les Sables-d'Olonne Talmont Airport", "Les Sables-d'Olonne"],
  LSP: ["Josefa Camejo International Airport", "Paraguan\u00e1"],
  LSQ: ["Mar\u00eda Dolores Airport", "Los Angeles"],
  LSS: ["Terre-de-Haut Airport", "Les Saintes"],
  LST: ["Launceston Airport", "Launceston"],
  LSU: ["Long Sukang Airport", "Long Sukang"],
  LSV: ["Nellis Air Force Base", "Las Vegas"],
  LSW: ["Malikus Saleh Airport", "Lhok Seumawe-Sumatra Island"],
  LSX: ["Lhok Sukon Airport", "Lhok Sukon-Sumatra Island"],
  LSY: ["Lismore Airport", "Lismore"],
  LSZ: ["Lo\u0161inj Island Airport", "Lo\u0161inj"],
  LTA: ["Tzaneen Airport", "Tzaneen"],
  LTB: ["Latrobe Airport", "Latrobe"],
  LTC: ["Lai Airport", "Lai"],
  LTD: ["Ghadames East Airport", "Ghadames"],
  LTF: ["Leitre Airport", "Leitre"],
  LTG: ["Langtang Airport", "Langtang"],
  LTH: ["Lathrop Wells Airport / Jackass Aeropark", "Amargosa Valley"],
  LTI: ["Altai Airport", "Altai"],
  LTK: ["Bassel Al-Assad International Airport", "Latakia"],
  LTL: ["Lastourville Airport", "Lastourville"],
  LTM: ["Lethem Airport", "Lethem"],
  LTN: ["London Luton Airport", "London"],
  LTO: ["Loreto International Airport", "Loreto"],
  LTP: ["Lyndhurst Airport", "Lyndhurst"],
  LTQ: ["Le Touquet-C\u00f4te d'Opale Airport", "Le Touquet-Paris-Plage"],
  LTR: ["Letterkenny Airport", "Letterkenny"],
  LTS: ["Altus Air Force Base", "Altus"],
  LTT: ["La M\u00f4le Airport", "La M\u00f4le"],
  LTU: ["Murod Kond Airport", "Latur"],
  LTV: ["Lotus Vale Airport", "Lotus Vale"],
  LTW: ["St. Mary's County Regional Airport", "Leonardtown"],
  LTX: ["Cotopaxi International Airport", "Latacunga"],
  LUA: ["Lukla Airport", "Lukla"],
  LUB: ["Lumid Pau Airport", "Lumid Pau"],
  LUC: ["Laucala Island Airport", "Laucala Island"],
  LUD: ["Luderitz Airport", "Luderitz"],
  LUE: ["Lu\u010denec Airport", "Lu\u010denec"],
  LUF: ["Luke Air Force Base", "Glendale"],
  LUG: ["Lugano Airport", "Lugano"],
  LUH: ["Ludhiana Airport"],
  LUI: ["Carta Airport", "La Uni\u00f3n"],
  LUJ: ["Lusikisiki Airport", "Lusikisiki"],
  LUK: ["Cincinnati Municipal Airport Lunken Field", "Cincinnati"],
  LUL: ["Hesler Noble Field", "Laurel"],
  LUM: ["Mangshi Airport", "Luxi"],
  LUN: ["Kenneth Kaunda International Airport Lusaka", "Lusaka"],
  LUO: ["Luena Airport", "Luena"],
  LUP: ["Kalaupapa Airport", "Kalaupapa"],
  LUQ: ["Brigadier Mayor D Cesar Raul Ojeda Airport", "San Luis"],
  LUR: ["Cape Lisburne LRRS Airport", "Cape Lisburne"],
  LUS: ["Lusanga Airport", "Lusanga"],
  LUT: ["New Laura Airport"],
  LUU: ["Laura Airport"],
  LUV: ["Dumatumbun Airport", "Langgur-Seram Island"],
  LUW: ["Syukuran Aminuddin Amir Airport", "Luwok - Celebes Island"],
  LUX: ["Luxembourg-Findel International Airport", "Luxembourg"],
  LUY: ["Lushoto Airport", "Lushoto"],
  LUZ: ["Lublin Airport", "Lublin"],
  LVA: ["Laval-Entrammes Airport", "Laval/Entrammes"],
  LVB: ["Santana do Livramento Airport", "Santana do Livramento"],
  LVD: ["Lime Village Airport", "Lime Village"],
  LVI: ["Livingstone Airport", "Livingstone"],
  LVK: ["Livermore Municipal Airport", "Livermore"],
  LVL: ["Lawrenceville Brunswick Municipal Airport", "Lawrenceville"],
  LVM: ["Mission Field", "Livingston"],
  LVO: ["Laverton Airport"],
  LVP: ["Lavan Island Airport"],
  LVR: ["Municipal Bom Futuro Airport", "Lucas do Rio Verde"],
  LVS: ["Las Vegas Municipal Airport", "Las Vegas"],
  LWA: ["Lebak Rural Airport", "Lebak"],
  LWB: ["Greenbrier Valley Airport", "Lewisburg"],
  LWC: ["Lawrence Municipal Airport", "Lawrence"],
  LWE: ["Lewoleba Airport", "Lewoleba-Lembata Island"],
  LWH: ["Lawn Hill Airport"],
  LWI: ["Lowai Airport", "Lowai"],
  LWK: ["Lerwick / Tingwall Airport", "Lerwick"],
  LWL: ["Wells Municipal Airport/Harriet Field", "Wells"],
  LWM: ["Lawrence Municipal Airport", "Lawrence"],
  LWN: ["Gyumri Shirak Airport", "Gyumri"],
  LWO: ["Lviv International Airport", "Lviv"],
  LWR: ["Leeuwarden Air Base", "Leeuwarden"],
  LWS: ["Lewiston Nez Perce County Airport", "Lewiston"],
  LWT: ["Lewistown Municipal Airport", "Lewistown"],
  LWV: ["Lawrenceville Vincennes International Airport", "Lawrenceville"],
  LWY: ["Lawas Airport", "Lawas"],
  LXA: ["Lhasa Gonggar Airport", "Lhasa"],
  LXG: ["Luang Namtha Airport", "Luang Namtha"],
  LXN: ["Jim Kelly Field", "Lexington"],
  LXR: ["Luxor International Airport", "Luxor"],
  LXS: ["Limnos Airport", "Limnos Island"],
  LXU: ["Lukulu Airport", "Lukulu"],
  LXV: ["Lake County Airport", "Leadville"],
  LYA: ["Luoyang Airport", "Luoyang"],
  LYB: ["Edward Bodden Airfield", "Little Cayman"],
  LYC: ["Lycksele Airport"],
  LYE: ["RAF Lyneham", "Lyneham"],
  LYG: ["Lianyungang Airport", "Lianyungang"],
  LYH: ["Lynchburg Regional Preston Glenn Field", "Lynchburg"],
  LYI: ["Shubuling Airport", "Linyi"],
  LYK: ["Lunyuk Airport", "Lunyuk-Simbawa Island"],
  LYM: ["Lympne Airport", "Ashford"],
  LYN: ["Lyon-Bron Airport", "Lyon/Bron"],
  LYO: ["Lyons-Rice County Municipal Airport", "Lyons"],
  LYP: ["Faisalabad International Airport", "Faisalabad"],
  LYR: ["Svalbard Airport, Longyear", "Longyearbyen"],
  LYS: ["Lyon Saint-Exup\u00e9ry Airport", "Lyon"],
  LYT: ["Lady Elliot Island Airstrip", "Lady Elliot Island"],
  LYU: ["Ely Municipal Airport", "Ely"],
  LYX: ["Lydd Airport", "Lydd, Ashford"],
  LZA: ["Luiza Airport", "Luiza"],
  LZC: ["L\u00e1zaro C\u00e1rdenas Airport", "L\u00e1zaro C\u00e1rdenas"],
  LZH: ["Liuzhou Bailian Airport", "Liuzhou"],
  LZI: ["Luozi Airport", "Luozi"],
  LZM: ["Luzamba Airport", "Luzamba"],
  LZN: ["Matsu Nangan Airport", "Nangang Island"],
  LZO: ["Luzhou Airport", "Luzhou"],
  LZR: ["Lizard Island Airport"],
  LZU: ["Gwinnett County Briscoe Field", "Lawrenceville"],
  LZY: ["Nyingchi Airport", "Nyingchi"],
  MAA: ["Chennai International Airport", "Chennai"],
  MAB: ["Jo\u00e3o Correa da Rocha Airport", "Marab\u00e1"],
  MAC: ["Macon Downtown Airport", "Macon"],
  MAD: ["Adolfo Su\u00e1rez Madrid\u2013Barajas Airport", "Madrid"],
  MAE: ["Madera Municipal Airport", "Madera"],
  MAF: ["Midland International Airport", "Midland"],
  MAG: ["Madang Airport", "Madang"],
  MAH: ["Menorca Airport", "Menorca Island"],
  MAI: ["Mangochi Airport", "Mangochi"],
  MAJ: ["Marshall Islands International Airport", "Majuro Atoll"],
  MAK: ["Malakal Airport", "Malakal"],
  MAL: ["Mangole Airport, Falabisahaya", "Mangole Island"],
  MAM: ["General Servando Canales International Airport", "Matamoros"],
  MAN: ["Manchester Airport", "Manchester"],
  MAO: ["Eduardo Gomes International Airport", "Manaus"],
  MAP: ["Mamai Airport", "Mamai"],
  MAQ: ["Mae Sot Airport"],
  MAR: ["La Chinita International Airport", "Maracaibo"],
  MAS: ["Momote Airport", "Manus Island"],
  MAT: ["Tshimpi Airport", "Matadi"],
  MAU: ["Maupiti Airport"],
  MAV: ["Maloelap Island Airport", "Maloelap Island"],
  MAW: ["Malden Regional Airport", "Malden"],
  MAX: ["Ouro Sogui Airport", "Matam"],
  MAY: ["Clarence A. Bain Airport", "Mangrove Cay"],
  MAZ: ["Eugenio Maria De Hostos Airport", "Mayaguez"],
  MBA: ["Mombasa Moi International Airport", "Mombasa"],
  MBB: ["Marble Bar Airport"],
  MBC: ["M'Bigou Airport", "M'Bigou"],
  MBD: ["Mmabatho International Airport", "Mafeking"],
  MBE: ["Monbetsu Airport", "Monbetsu"],
  MBG: ["Mobridge Municipal Airport", "Mobridge"],
  MBH: ["Maryborough Airport", "Maryborough"],
  MBI: ["Songwe Airport", "Mbeya"],
  MBJ: ["Sangster International Airport", "Montego Bay"],
  MBK: ["Regional Orlando Villas Boas Airport", "Matup\u00e1"],
  MBL: ["Manistee Co Blacker Airport", "Manistee"],
  MBM: ["Mkambati Airport", "Mkambati"],
  MBO: ["Mamburao Airport"],
  MBP: ["Moyobamba Airport", "Moyobamba"],
  MBQ: ["Mbarara Airport", "Mbarara"],
  MBS: ["MBS International Airport", "Saginaw"],
  MBT: ["Moises R. Espinosa Airport", "Masbate"],
  MBU: ["Babanakira Airport", "Mbambanakira"],
  MBV: ["Masa Airport", "Masa"],
  MBW: ["Melbourne Moorabbin Airport", "Melbourne"],
  MBX: ["Maribor Airport"],
  MBY: ["Omar N Bradley Airport", "Moberly"],
  MBZ: ["Mau\u00e9s Airport", "Mau\u00e9s"],
  MCA: ["Macenta Airport", "Macenta"],
  MCB: ["Mc Comb/Pike County Airport/John E Lewis Field", "Mc Comb"],
  MCC: ["Mc Clellan Airfield", "Sacramento"],
  MCD: ["Mackinac Island Airport", "Mackinac Island"],
  MCE: ["Merced Regional Macready Field", "Merced"],
  MCF: ["Mac Dill Air Force Base", "Tampa"],
  MCG: ["McGrath Airport", "McGrath"],
  MCH: ["General Manuel Serrano Airport", "Machala"],
  MCI: ["Kansas City International Airport", "Kansas City"],
  MCJ: ["Jorge Isaac Airport", "La Mina-Maicao"],
  MCK: ["Mc Cook Ben Nelson Regional Airport", "Mc Cook"],
  MCL: ["McKinley National Park Airport", "McKinley Park"],
  MCM: ["Monaco Heliport", "Fontvieille"],
  MCN: ["Middle Georgia Regional Airport", "Macon"],
  MCO: ["Orlando International Airport", "Orlando"],
  MCP: ["Alberto Alcolumbre Airport", "Macap\u00e1"],
  MCQ: ["Miskolc Airport", "Miskolc"],
  MCR: ["Melchor de Mencos Airport", "Melchor de Mencos"],
  MCS: ["Monte Caseros Airport", "Monte Caseros"],
  MCT: ["Muscat International Airport", "Muscat"],
  MCU: ["Montlu\u00e7on-Gu\u00e9ret Airport", "Montlu\u00e7on/Gu\u00e9ret"],
  MCV: ["McArthur River Mine Airport", "McArthur River Mine"],
  MCW: ["Mason City Municipal Airport", "Mason City"],
  MCX: ["Uytash Airport", "Makhachkala"],
  MCY: ["Sunshine Coast Airport", "Maroochydore"],
  MCZ: ["Zumbi dos Palmares Airport", "Macei\u00f3"],
  MDA: ["Martindale Army Heliport", "San Antonio"],
  MDB: ["Melinda Airport", "Melinda"],
  MDC: ["Sam Ratulangi Airport", "Manado-Celebes Island"],
  MDD: ["Midland Airpark", "Midland"],
  MDE: ["Jose Maria C\u00f3rdova International Airport", "Medell\u00edn"],
  MDF: ["Taylor County Airport", "Medford"],
  MDG: ["Mudanjiang Hailang International Airport", "Mudanjiang"],
  MDH: ["Southern Illinois Airport", "Carbondale/Murphysboro"],
  MDI: ["Makurdi Airport", "Makurdi"],
  MDJ: ["Madras Municipal Airport", "Madras"],
  MDK: ["Mbandaka Airport", "Mbandaka"],
  MDL: ["Mandalay International Airport", "Mandalay"],
  MDM: ["Munduku Airport", "Munduku"],
  MDN: ["Madison Municipal Airport", "Madison"],
  MDO: ["Middleton Island Airport", "Middleton Island"],
  MDP: ["Mindiptana Airport", "Mindiptana-Papua Island"],
  MDQ: ["\u00c1stor Piazzola International Airport", "Mar del Plata"],
  MDR: ["Medfra Airport", "Medfra"],
  MDS: ["Middle Caicos Airport", "Middle Caicos"],
  MDT: ["Harrisburg International Airport", "Harrisburg"],
  MDU: ["Mendi Airport", "Mendi"],
  MDV: ["M\u00e9douneu Airport", "M\u00e9douneu, Gabon"],
  MDW: ["Chicago Midway International Airport", "Chicago"],
  MDX: ["Mercedes Airport", "Mercedes"],
  MDY: ["Henderson Field", "Sand Island"],
  MDZ: ["El Plumerillo Airport", "Mendoza"],
  MEA: ["Maca\u00e9 Airport", "Maca\u00e9"],
  MEB: ["Melbourne Essendon Airport"],
  MEC: ["Eloy Alfaro International Airport", "Manta"],
  MED: ["Prince Mohammad Bin Abdulaziz Airport", "Medina"],
  MEE: ["Mar\u00e9 Airport", "Mar\u00e9"],
  MEF: ["Melfi Airport", "Melfi"],
  MEG: ["Malanje Airport", "Malanje"],
  MEH: ["Mehamn Airport", "Mehamn"],
  MEI: ["Key Field", "Meridian"],
  MEJ: ["Port Meadville Airport", "Meadville"],
  MEK: ["Bassatine Airport", "Meknes"],
  MEL: ["Melbourne International Airport", "Melbourne"],
  MEM: ["Memphis International Airport", "Memphis"],
  MEN: ["Mende-Brenoux Airfield", "Mende/Br\u00e9noux"],
  MEO: ["Dare County Regional Airport", "Manteo"],
  MEP: ["Mersing Airport", "Mersing"],
  MEQ: ["Seunagan Airport", "Peureumeue-Sumatra Island"],
  MER: ["Castle Airport", "Merced"],
  MES: ["Soewondo Air Force Base", "Medan-Sumatra Island"],
  MET: ["Moreton Airport", "Moreton"],
  MEU: ["Monte Dourado Airport", "Almeirim"],
  MEV: ["Minden-Tahoe Airport", "Minden"],
  MEW: ["Mweka Airport", "Mweka"],
  MEX: ["Licenciado Benito Juarez International Airport", "Mexico City"],
  MEY: ["Meghauli Airport", "Meghauli"],
  MEZ: ["Musina(Messina) Airport", "Musina"],
  MFA: ["Mafia Island Airport", "Mafia Island"],
  MFB: ["Monfort Airport", "Monfort"],
  MFC: ["Mafeteng Airport", "Mafeteng"],
  MFD: ["Mansfield Lahm Regional Airport", "Mansfield"],
  MFE: ["Mc Allen Miller International Airport", "Mc Allen"],
  MFF: ["Moanda Airport", "Moanda"],
  MFG: ["Muzaffarabad Airport", "Muzaffarabad"],
  MFH: ["Mesquite Airport", "Mesquite"],
  MFI: ["Marshfield Municipal Airport", "Marshfield"],
  MFJ: ["Moala Airport", "Moala"],
  MFK: ["Matsu Beigan Airport", "Beigan Island"],
  MFL: ["Mount Full Stop Airport", "Wando Vale"],
  MFM: ["Macau International Airport", "Macau"],
  MFN: ["Milford Sound Airport"],
  MFO: ["Manguna Airport", "Manguna"],
  MFP: ["Manners Creek Airport"],
  MFQ: ["Maradi Airport", "Maradi"],
  MFR: ["Rogue Valley International Medford Airport", "Medford"],
  MFS: ["Miraflores Airport", "Miraflores"],
  MFT: ["Machu Pichu Airport", "Machu Pichu"],
  MFU: ["Mfuwe Airport", "Mfuwe"],
  MFV: ["Accomack County Airport", "Melfa"],
  MFW: ["Magaruque Airport", "Magaruque Island"],
  MFX: ["M\u00e9ribel Altiport"],
  MFZ: ["Meselia Airport", "Demgulu"],
  MGA: ["Augusto C. Sandino (Managua) International Airport", "Managua"],
  MGB: ["Mount Gambier Airport"],
  MGC: ["Michigan City Municipal Airport", "Michigan City"],
  MGD: ["Magdalena Airport", "Magdalena"],
  MGE: ["Dobbins Air Reserve Base", "Marietta"],
  MGF: [
    "Regional de Maring\u00e1 - S\u00edlvio Nane Junior Airport",
    "Maring\u00e1",
  ],
  MGG: ["Margarima Airport", "Margarima"],
  MGH: ["Margate Airport", "Margate"],
  MGI: ["Matagorda Island Air Force Base", "Matagorda Island"],
  MGJ: ["Orange County Airport", "Montgomery"],
  MGK: ["Mong Tong Airport", "Mong Tong"],
  MGL: ["M\u00f6nchengladbach Airport", "M\u00f6nchengladbach"],
  MGM: ["Montgomery Regional (Dannelly Field) Airport", "Montgomery"],
  MGN: ["Baracoa Airport", "Magangu\u00e9"],
  MGP: ["Manga Airport", "Manga Mission"],
  MGQ: ["Aden Adde International Airport", "Mogadishu"],
  MGR: ["Moultrie Municipal Airport", "Moultrie"],
  MGS: ["Mangaia Island Airport", "Mangaia Island"],
  MGT: ["Milingimbi Airport", "Milingimbi Island"],
  MGU: ["Manaung Airport", "Manaung"],
  MGV: ["Margaret River (Station) Airport"],
  MGW: ["Morgantown Municipal Walter L. Bill Hart Field", "Morgantown"],
  MGX: ["Moabi Airport", "Moabi"],
  MGY: ["Dayton-Wright Brothers Airport", "Dayton"],
  MGZ: ["Myeik Airport", "Mkeik"],
  MHA: ["Mahdia Airport", "Mahdia"],
  MHB: ["Mechanics Bay Heliport", "Auckland City"],
  MHC: ["Mocopulli Airport", "Dalcahue"],
  MHD: ["Mashhad International Airport", "Mashhad"],
  MHE: ["Mitchell Municipal Airport", "Mitchell"],
  MHF: ["Morichal Airport", "Morichal"],
  MHG: ["Mannheim-City Airport", "Mannheim"],
  MHH: ["Leonard M Thompson International Airport", "Marsh Harbour"],
  MHI: ["Moucha Airport", "Moucha Island"],
  MHK: ["Manhattan Regional Airport", "Manhattan"],
  MHL: ["Marshall Memorial Municipal Airport", "Marshall"],
  MHN: ["Hooker County Airport", "Mullen"],
  MHO: ["Mount House Airport"],
  MHP: ["Minsk 1 Airport", "Minsk"],
  MHQ: ["Mariehamn Airport"],
  MHR: ["Sacramento Mather Airport", "Sacramento"],
  MHS: ["Dunsmuir Muni-Mott Airport", "Dunsmuir"],
  MHT: ["Manchester-Boston Regional Airport", "Manchester"],
  MHU: ["Mount Hotham Airport", "Mount Hotham"],
  MHV: ["Mojave Airport", "Mojave"],
  MHW: ["Monteagudo Airport", "El Ba\u00f1ado"],
  MHX: ["Manihiki Island Airport", "Manihiki Island"],
  MHY: ["Morehead Airport", "Morehead"],
  MHZ: ["RAF Mildenhall", "Mildenhall"],
  MIA: ["Miami International Airport", "Miami"],
  MIB: ["Minot Air Force Base", "Minot"],
  MID: ["Licenciado Manuel Crescencio Rejon Int Airport", "M\u00e9rida"],
  MIE: ["Delaware County Johnson Field", "Muncie"],
  MIF: ["Roy Hurd Memorial Airport", "Monahans"],
  MIG: ["Mianyang Airport", "Mianyang"],
  MIH: ["Mitchell Plateau Airport", "Mitchell Plateau"],
  MII: [
    "Frank Miloye Milenkowichi\u2013Mar\u00edlia State Airport",
    "Mar\u00edlia",
  ],
  MIJ: ["Mili Island Airport", "Mili Island"],
  MIK: ["Mikkeli Airport", "Mikkeli"],
  MIM: ["Merimbula Airport", "Merimbula"],
  MIN: ["Minnipa Airport"],
  MIP: ["Ramon Air Base", "Beersheba"],
  MIR: ["Monastir Habib Bourguiba International Airport", "Monastir"],
  MIS: ["Misima Island Airport", "Misima Island"],
  MIT: ["Shafter Airport - Minter Field", "Shafter"],
  MIU: ["Maiduguri International Airport", "Maiduguri"],
  MIV: ["Millville Municipal Airport", "Millville"],
  MIY: ["Mittebah Airport"],
  MIZ: ["Mainoru Airstrip", "Mainoru"],
  MJA: ["Manja Airport", "Manja"],
  MJB: ["Mejit Atoll Airport", "Mejit Atoll"],
  MJC: ["Man Airport"],
  MJD: ["Moenjodaro Airport", "Moenjodaro"],
  MJE: ["Majkin Airport", "Majkin"],
  MJF: ["Mosj\u00f8en Airport (Kj\u00e6rstad)"],
  MJG: ["Mayajigua Airport", "Mayajigua"],
  MJI: ["Mitiga Airport", "Tripoli"],
  MJJ: ["Moki Airport", "Moki"],
  MJK: ["Shark Bay Airport", "Denham"],
  MJL: ["Mouilla Ville Airport", "Mouila"],
  MJM: ["Mbuji Mayi Airport", "Mbuji Mayi"],
  MJN: ["Amborovy Airport"],
  MJO: ["Mount Etjo Airport", "Mount Etjo Safari Lodge"],
  MJP: ["Manjimup Airport"],
  MJR: ["Miramar Airport", "Miramar"],
  MJS: ["Maganja da Costa Airport", "Maganja"],
  MJT: ["Mytilene International Airport", "Mytilene"],
  MJU: ["Tampa Padang Airport", "Mamuju-Celebes Island"],
  MJV: ["San Javier Airport", "San Javier"],
  MJW: ["Mahenye Airport", "Gonarezhou National Park"],
  MJX: ["Ocean County Airport", "Toms River"],
  MJY: ["Mangunjaya Airport", "Mangungaya-Sumatra Island"],
  MJZ: ["Mirny Airport", "Mirny"],
  MKA: [
    "Mari\u00e1nsk\u00e9 L\u00e1zn\u011b Airport",
    "Mari\u00e1nsk\u00e9 L\u00e1zn\u011b",
  ],
  MKB: ["Mekambo Airport", "Mekambo"],
  MKC: ["Charles B. Wheeler Downtown Airport", "Kansas City"],
  MKE: ["General Mitchell International Airport", "Milwaukee"],
  MKG: ["Muskegon County Airport", "Muskegon"],
  MKH: ["Mokhotlong Airport", "Mokhotlong"],
  MKI: ["M'Boki Airport", "Mboki"],
  MKJ: ["Makoua Airport", "Makoua"],
  MKK: ["Molokai Airport", "Kaunakakai"],
  MKL: ["McKellar-Sipes Regional Airport", "Jackson"],
  MKM: ["Mukah Airport", "Mukah"],
  MKN: ["Malekolon Airport", "Babase Island"],
  MKP: ["Makemo Airport"],
  MKQ: ["Mopah Airport", "Merauke-Papua Island"],
  MKR: ["Meekatharra Airport"],
  MKS: ["Mekane Selam Airport", "Mekane Selam"],
  MKU: ["Makokou Airport", "Makokou"],
  MKV: ["Mount Cavenagh Airport"],
  MKW: ["Rendani Airport", "Manokwari-Papua Island"],
  MKY: ["Mackay Airport", "Mackay"],
  MKZ: ["Malacca Airport", "Malacca"],
  MLA: ["Malta International Airport", "Valletta"],
  MLB: ["Melbourne International Airport", "Melbourne"],
  MLC: ["Mc Alester Regional Airport", "Mc Alester"],
  MLE: ["Mal\u00e9 International Airport", "Mal\u00e9"],
  MLG: ["Abdul Rachman Saleh Airport", "Malang-Java Island"],
  MLI: ["Quad City International Airport", "Moline"],
  MLK: ["Malta Airport", "Malta"],
  MLL: ["Marshall Don Hunter Sr Airport", "Marshall"],
  MLM: ["General Francisco J. Mujica International Airport", "Morelia"],
  MLN: ["Melilla Airport", "Melilla"],
  MLO: ["Milos Airport", "Milos Island"],
  MLP: ["Malabang Airport", "Malabang"],
  MLQ: ["Malalaua Airport", "Malalaua"],
  MLR: ["Millicent Airport"],
  MLS: ["Frank Wiley Field", "Miles City"],
  MLU: ["Monroe Regional Airport", "Monroe"],
  MLV: ["Merluna Airport", "Merluna"],
  MLW: ["Spriggs Payne Airport", "Monrovia"],
  MLX: ["Malatya Erha\u00e7 Airport", "Malatya"],
  MLY: ["Manley Hot Springs Airport", "Manley Hot Springs"],
  MLZ: ["Cerro Largo International Airport", "Melo"],
  MMB: ["Memanbetsu Airport", "\u014czora"],
  MMC: ["Ciudad Mante National Airport", "Ciudad Mante"],
  MMD: ["Minami-Daito Airport", "Minamidait\u014d"],
  MME: ["Durham Tees Valley Airport", "Durham"],
  MMF: ["Mamfe Airport", "Mamfe"],
  MMG: ["Mount Magnet Airport"],
  MMH: ["Mammoth Yosemite Airport", "Mammoth Lakes"],
  MMI: ["McMinn County Airport", "Athens"],
  MMJ: ["Matsumoto Airport", "Matsumoto"],
  MMK: ["Murmansk Airport", "Murmansk"],
  MML: [
    "Southwest Minnesota Regional Airport - Marshall/Ryan Field",
    "Marshall",
  ],
  MMM: ["Middlemount Airport"],
  MMO: ["Maio Airport", "Vila do Maio"],
  MMQ: ["Mbala Airport", "Mbala"],
  MMS: ["Selfs Airport", "Marks"],
  MMT: ["Mc Entire Joint National Guard Base", "Eastover"],
  MMU: ["Morristown Municipal Airport", "Morristown"],
  MMV: ["Mal Airport", "Mal Island"],
  MMX: ["Malm\u00f6 Sturup Airport", "Malm\u00f6"],
  MMY: ["Miyako Airport", "Miyako City"],
  MMZ: ["Maimana Airport"],
  MNA: ["Melangguane Airport", "Karakelong Island"],
  MNB: ["Muanda Airport"],
  MNC: ["Nacala Airport", "Nacala"],
  MND: ["Medina Airport", "Medina"],
  MNE: ["Mungeranie Airport", "Mungeranie"],
  MNF: ["Mana Island Airport", "Mana Island"],
  MNG: ["Maningrida Airport", "Maningrida"],
  MNH: ["Rustaq Airport", "Al Masna'ah"],
  MNI: ["John A. Osborne Airport", "Gerald's Park"],
  MNJ: ["Mananjary Airport"],
  MNK: ["Maiana Airport", "Maiana"],
  MNL: ["Ninoy Aquino International Airport", "Manila"],
  MNM: ["Menominee Regional Airport", "Menominee"],
  MNN: ["Marion Municipal Airport", "Marion"],
  MNO: ["Manono Airport", "Manono"],
  MNP: ["Maron Island Airport", "Hermit Islands"],
  MNQ: ["Monto Airport"],
  MNR: ["Mongu Airport", "Mongu"],
  MNS: ["Mansa Airport", "Mansa"],
  MNU: ["Mawlamyine Airport", "Mawlamyine"],
  MNW: ["Macdonald Downs Airport"],
  MNX: ["Manicor\u00e9 Airport", "Manicor\u00e9"],
  MNY: ["Mono Airport", "Stirling Island"],
  MNZ: ["Manassas Regional Airport/Harry P. Davis Field", "Manassas"],
  MOA: ["Orestes Acosta Airport", "Moa"],
  MOB: ["Mobile Regional Airport", "Mobile"],
  MOC: ["M\u00e1rio Ribeiro Airport", "Montes Claros"],
  MOD: ["Modesto City Co-Harry Sham Field", "Modesto"],
  MOE: ["Momeik Airport"],
  MOF: ["Maumere(Wai Oti) Airport", "Maumere-Flores Island"],
  MOG: ["Mong Hsat Airport", "Mong Hsat"],
  MOH: ["Maleo Airport", "Morowali"],
  MOI: ["Mitiaro Island Airport", "Mitiaro Island"],
  MOJ: ["Moengo Airstrip", "Moengo"],
  MOL: ["Molde Airport", "\u00c5r\u00f8"],
  MOM: ["Letfotar Airport", "Moudjeria"],
  MON: ["Mount Cook Airport"],
  MOO: ["Moomba Airport"],
  MOQ: ["Morondava Airport"],
  MOT: ["Minot International Airport", "Minot"],
  MOU: ["Mountain Village Airport", "Mountain Village"],
  MOV: ["Moranbah Airport", "Moranbah"],
  MOZ: ["Moorea Airport"],
  MPA: ["Katima Mulilo Airport", "Mpacha"],
  MPB: ["Miami Seaplane Base", "Miami"],
  MPC: ["Muko Muko Airport", "Muko Muko-Sumatra Island"],
  MPD: ["Sindhri Tharparkar Airport", "Sindhri"],
  MPE: ["Griswold Airport", "Madison"],
  MPF: ["Mapoda Airport", "Mapoda"],
  MPG: ["Makini Airport", "Makini"],
  MPH: ["Godofredo P. Ramos Airport", "Malay"],
  MPI: ["Mamitupo Airport", "Mamitupo"],
  MPK: ["Mokpo Airport", "Gonghang-ro (Mokpo)"],
  MPL: [
    "Montpellier-M\u00e9diterran\u00e9e Airport",
    "Montpellier/M\u00e9diterran\u00e9e",
  ],
  MPM: ["Maputo Airport", "Maputo"],
  MPN: ["Mount Pleasant Airport", "Mount Pleasant"],
  MPP: ["Mulatupo Airport", "Mulatupo"],
  MPT: ["Maliana Airport", "Maliana"],
  MPU: ["Mapua(Mabua) Airport", "Tatau Island"],
  MPV: ["Edward F Knapp State Airport", "Barre/Montpelier"],
  MPW: ["Mariupol International Airport", "Mariupol"],
  MPX: ["Miyanmin Airport", "Miyanmin"],
  MPY: ["Maripasoula Airport", "Maripasoula"],
  MPZ: ["Mount Pleasant Municipal Airport", "Mount Pleasant"],
  MQA: ["Mandora Airport", "Mandora"],
  MQB: ["Macomb Municipal Airport", "Macomb"],
  MQC: ["Miquelon Airport", "Miquelon"],
  MQD: ["Maquinchao Airport", "Maquinchao"],
  MQE: ["Marqua Airport", "Marqua"],
  MQF: ["Magnitogorsk International Airport", "Magnitogorsk"],
  MQG: ["Midgard Airport", "Midgard"],
  MQH: ["Mina\u00e7u Airport", "Mina\u00e7u"],
  MQJ: ["Moma Airport", "Honuu"],
  MQK: ["San Mat\u00edas Airport", "San Mat\u00edas"],
  MQL: ["Mildura Airport", "Mildura"],
  MQM: ["Mardin Airport", "Mardin"],
  MQN: ["Mo i Rana Airport, R\u00f8ssvoll", "Mo i Rana"],
  MQO: ["Malam Airport", "Malam"],
  MQP: ["Kruger Mpumalanga International Airport", "Mpumalanga"],
  MQQ: ["Moundou Airport"],
  MQS: ["Mustique Airport", "Mustique Island"],
  MQT: ["Sawyer International Airport", "Marquette"],
  MQU: ["Mariquita Airport", "Mariquita"],
  MQV: ["Mostaganem Airport"],
  MQX: ["Mekele Airport"],
  MQY: ["Smyrna Airport", "Smyrna"],
  MQZ: ["Margaret River Airport"],
  MRA: ["Misratah Airport", "Misratah"],
  MRB: ["Eastern WV Regional Airport/Shepherd Field", "Martinsburg"],
  MRC: ["Maury County Airport", "Columbia/Mount Pleasant"],
  MRD: ["Alberto Carnevalli Airport", "M\u00e9rida"],
  MRE: ["Mara Serena Lodge Airstrip", "Masai Mara"],
  MRF: ["Marfa Municipal Airport", "Marfa"],
  MRG: ["Mareeba Airport"],
  MRH: ["May River Airstrip", "May River"],
  MRI: ["Merrill Field", "Anchorage"],
  MRJ: ["Colon Airport", "Marcala"],
  MRK: ["Marco Island Executive Airport", "Marco Island"],
  MRM: ["Manari Airport", "Manari"],
  MRO: ["Hood Airport", "Masterton"],
  MRP: ["Marla Airport"],
  MRQ: ["Marinduque Airport", "Gasan"],
  MRR: ["Jose Maria Velasco Ibarra Airport", "Macar\u00e1"],
  MRS: ["Marseille Provence Airport", "Marseille"],
  MRU: ["Sir Seewoosagur Ramgoolam International Airport", "Port Louis"],
  MRV: ["Mineralnyye Vody Airport", "Mineralnyye Vody"],
  MRW: ["Lolland Falster Maribo Airport", "Lolland Falster / Maribo"],
  MRX: ["Mahshahr Airport"],
  MRY: ["Monterey Peninsula Airport", "Monterey"],
  MRZ: ["Moree Airport", "Moree"],
  MSA: ["Muskrat Dam Airport", "Muskrat Dam"],
  MSB: ["Marigot Seaplane Base", "Saint Martin"],
  MSC: ["Falcon Field", "Mesa"],
  MSE: ["Kent International Airport", "Manston"],
  MSH: ["Masirah Air Base", "Masirah"],
  MSI: ["Masalembo Airport", "Masalembo Island"],
  MSJ: ["Misawa Air Base", "Misawa"],
  MSL: ["Northwest Alabama Regional Airport", "Muscle Shoals"],
  MSM: ["Masi Manimba Airport", "Masi Manimba"],
  MSN: ["Dane County Regional Truax Field", "Madison"],
  MSO: ["Missoula International Airport", "Missoula"],
  MSP: [
    "Minneapolis-St Paul International/Wold-Chamberlain Airport",
    "Minneapolis",
  ],
  MSQ: ["Minsk National Airport", "Minsk"],
  MSR: ["Mu\u015f Airport", "Mu\u015f"],
  MSS: ["Massena International Richards Field", "Massena"],
  MST: ["Maastricht Aachen Airport", "Maastricht"],
  MSU: ["Moshoeshoe I International Airport", "Maseru"],
  MSW: ["Massawa International Airport", "Massawa"],
  MSX: ["Mossendjo Airport", "Mossendjo"],
  MSY: ["Louis Armstrong New Orleans International Airport", "New Orleans"],
  MSZ: ["Namibe Airport", "Namibe"],
  MTA: ["Matamata Glider Airport"],
  MTB: ["Montelibano Airport", "Montel\u00edbano"],
  MTC: ["Selfridge Air National Guard Base Airport", "Mount Clemens"],
  MTD: ["Mount Sanford Station Airport"],
  MTE: ["Monte Alegre Airport", "Monte Alegre"],
  MTF: ["Mizan Teferi Airport", "Mizan Teferi"],
  MTG: [
    "Vila Bela da Sant\u00edssima Trindade Airport",
    "Vila Bela Da Sant\u00edssima Trindade",
  ],
  MTH: ["The Florida Keys Marathon Airport", "Marathon"],
  MTI: ["Mosteiros Airport", "Vila do Mosteiros"],
  MTJ: ["Montrose Regional Airport", "Montrose"],
  MTK: ["Makin Island Airport", "Makin Island"],
  MTL: ["Maitland Airport"],
  MTN: ["Martin State Airport", "Baltimore"],
  MTO: ["Coles County Memorial Airport", "Mattoon/Charleston"],
  MTQ: ["Mitchell Airport"],
  MTR: ["Los Garzones Airport", "Monter\u00eda"],
  MTS: ["Matsapha Airport", "Manzini"],
  MTT: ["Minatitl\u00e1n/Coatzacoalcos National Airport", "Minatitl\u00e1n"],
  MTU: ["Montepuez Airport", "Montepuez"],
  MTV: ["Mota Lava Airport", "Ablow"],
  MTX: ["Metro Field", "Fairbanks"],
  MTY: ["General Mariano Escobedo International Airport", "Monterrey"],
  MTZ: ["Bar Yehuda Airfield", "Masada"],
  MUA: ["Munda Airport"],
  MUB: ["Maun Airport", "Maun"],
  MUC: ["Munich Airport", "Munich"],
  MUD: ["Mueda Airport", "Mueda"],
  MUE: ["Waimea Kohala Airport", "Kamuela"],
  MUF: ["Muting Airport", "Muting"],
  MUG: ["Mulege Airport", "Mulege"],
  MUH: ["Mersa Matruh Airport", "Mersa Matruh"],
  MUJ: ["Mui River Airport", "Omo National Park"],
  MUK: ["Mauke Airport", "Mauke Island"],
  MUM: ["Mumias Airport", "Mumias"],
  MUN: ["Matur\u00edn Airport"],
  MUO: ["Mountain Home Air Force Base", "Mountain Home"],
  MUP: ["Mulga Park Airport"],
  MUQ: ["Muccan Station Airport", "Muccan Station"],
  MUR: ["Marudi Airport", "Marudi"],
  MUS: ["Minami Torishima Airport"],
  MUW: ["Ghriss Airport"],
  MUX: ["Multan International Airport", "Multan"],
  MUY: ["Mouyondzi Airport", "Mouyondzi"],
  MUZ: ["Musoma Airport", "Musoma"],
  MVA: ["Reykjahl\u00ed\u00f0 Airport", "Myvatn"],
  MVB: [
    "M'Vengue El Hadj Omar Bongo Ondimba International Airport",
    "Franceville",
  ],
  MVC: ["Monroe County Aeroplex Airport", "Monroeville"],
  MVD: ["Carrasco International /General C L Berisso Airport", "Montevideo"],
  MVF: ["Dix-Sept Rosado Airport", "Mossor\u00f3"],
  MVI: ["Manetai Airport", "Manetai"],
  MVJ: ["Marlboro Airport", "Mandeville"],
  MVK: ["Mulka Airport", "Mulka"],
  MVL: ["Morrisville Stowe State Airport", "Morrisville"],
  MVO: ["Mongo Airport", "Mongo"],
  MVP: ["Fabio Alberto Leon Bentley Airport", "Mit\u00fa"],
  MVQ: ["Mogilev Airport", "Mogilev"],
  MVR: ["Salak Airport", "Maroua"],
  MVS: ["Mucuri Airport", "Mucuri"],
  MVT: ["Mataiva Airport"],
  MVU: ["Musgrave Airport", "Musgrave"],
  MVV: ["Meg\u00e8ve Airport", "Verdun"],
  MVW: ["Skagit Regional Airport", "Burlington/Mount Vernon"],
  MVX: ["Minvoul Airport", "Minvoul"],
  MVY: ["Martha's Vineyard Airport", "Martha's Vineyard"],
  MVZ: ["Masvingo International Airport", "Masvingo"],
  MWA: ["Williamson County Regional Airport", "Marion"],
  MWB: ["Morawa Airport"],
  MWC: ["Lawrence J Timmerman Airport", "Milwaukee"],
  MWD: ["Mianwali Air Base", "Mianwali"],
  MWE: ["Merowe New Airport", "Merowe"],
  MWF: ["Maewo-Naone Airport", "Maewo Island"],
  MWG: ["Marawaka Airport", "Marawaka"],
  MWH: ["Grant County International Airport", "Moses Lake"],
  MWI: ["Maramuni Airport", "Maramuni"],
  MWJ: ["Matthews Ridge Airport", "Matthews Ridge"],
  MWK: ["Tarempa Airport", "Matak Island"],
  MWL: ["Mineral Wells Airport", "Mineral Wells"],
  MWN: ["Mwadui Airport", "Mwadui"],
  MWO: ["Middletown Regional Airport", "Middletown"],
  MWP: ["Mountain Airport", "Mountain"],
  MWQ: ["Magway Airport", "Magway"],
  MWR: ["Motswari Airport", "Motswari Private Game Reserve"],
  MWT: ["Moolawatana Airport", "Moolawatana Station"],
  MWX: ["Muan International Airport", "Piseo-ri (Muan)"],
  MWY: ["Miranda Downs Airport", "Miranda Downs"],
  MWZ: ["Mwanza Airport", "Mwanza"],
  MXB: ["Andi Jemma Airport", "Masamba-Celebes Island"],
  MXC: ["Monticello Airport", "Monticello"],
  MXD: ["Marion Downs Airport"],
  MXF: ["Maxwell Air Force Base", "Montgomery"],
  MXH: ["Moro Airport", "Moro"],
  MXI: ["Mati National Airport", "Mati"],
  MXJ: ["Minna Airport", "Minna"],
  MXK: ["Mindik Airport", "Mindik"],
  MXL: [
    "General Rodolfo S\u00e1nchez Taboada International Airport",
    "Mexicali",
  ],
  MXM: ["Morombe Airport"],
  MXN: ["Morlaix-Ploujean Airport", "Morlaix/Ploujean"],
  MXP: ["Malpensa International Airport", "Milan"],
  MXR: ["Myrhorod Air Base", "Myrhorod"],
  MXS: ["Maota Airport", "Maota"],
  MXT: ["Maintirano Airport", "Maintirano"],
  MXU: ["Mullewa Airport"],
  MXV: ["M\u00f6r\u00f6n Airport", "M\u00f6r\u00f6n"],
  MXW: ["Mandalgobi Airport", "Mandalgobi"],
  MXX: ["Mora Airport", "Mora"],
  MXY: ["Mc Carthy Airport", "Mccarthy"],
  MXZ: ["Meixian Airport", "Meixian"],
  MYA: ["Moruya Airport", "Moruya"],
  MYB: ["Mayumba Airport", "Mayumba"],
  MYC: ["Escuela Mariscal Sucre Airport", "Maracay"],
  MYD: ["Malindi Airport", "Malindi"],
  MYE: ["Miyakejima Airport", "Miyakejima"],
  MYF: ["Montgomery-Gibbs Executive Airport", "San Diego"],
  MYG: ["Mayaguana Airport", "Abrahams Bay"],
  MYI: ["Murray Island Airport", "Murray Island"],
  MYJ: ["Matsuyama Airport", "Matsuyama"],
  MYK: ["May Creek Airport", "May Creek"],
  MYL: ["McCall Municipal Airport", "McCall"],
  MYM: ["Monkey Mountain Airport", "Monkey Mountain"],
  MYN: ["Mareb Airport", "Mareb"],
  MYO: ["Camballin Airport", "Myroodah"],
  MYP: ["Mary Airport", "Mary"],
  MYQ: ["Mysore Airport", "Mysore"],
  MYR: ["Myrtle Beach International Airport", "Myrtle Beach"],
  MYS: ["Moyale Airport", "Moyale"],
  MYT: ["Myitkyina Airport", "Myitkyina"],
  MYU: ["Mekoryuk Airport", "Mekoryuk"],
  MYV: ["Yuba County Airport", "Marysville"],
  MYW: ["Mtwara Airport", "Mtwara"],
  MYX: ["Menyamya Airport", "Menyamya"],
  MYY: ["Miri Airport", "Miri"],
  MYZ: ["Monkey Bay Airport", "Monkey Bay"],
  MZA: ["Mayor PNP Nancy Flores Paucar Airport", "Mazamari"],
  MZB: ["Moc\u00edmboa da Praia Airport", "Moc\u00edmboa da Praia"],
  MZC: ["Mitzic Airport", "Mitzic"],
  MZD: ["M\u00e9ndez Airport", "Santiago de M\u00e9ndez"],
  MZE: ["Manatee Airport"],
  MZG: ["Makung Airport", "Makung City"],
  MZH: ["Amasya Merzifon Airport", "Amasya"],
  MZI: ["Mopti Airport"],
  MZJ: ["Pinal Airpark", "Marana"],
  MZK: ["Marakei Airport", "Marakei"],
  MZL: ["La Nubia Airport", "Manizales"],
  MZM: ["Metz-Frescaty (BA 128) Air Base", "Metz/Frescaty"],
  MZO: ["Sierra Maestra Airport", "Manzanillo"],
  MZP: ["Motueka Airport"],
  MZQ: ["Mkuze Airport", "Mkuze"],
  MZR: ["Mazar I Sharif Airport"],
  MZS: ["Mostyn Airport", "Mostyn"],
  MZT: ["General Rafael Buelna International Airport", "Mazatl\u00e1n"],
  MZU: ["Muzaffarpur Airport"],
  MZV: ["Mulu Airport", "Mulu"],
  MZW: ["Mecheria Airport", "Mecheria"],
  MZY: ["Mossel Bay Airport", "Mossel Bay"],
  MZZ: ["Marion Municipal Airport", "Marion"],
  NAA: ["Narrabri Airport", "Narrabri"],
  NAC: ["Naracoorte Airport"],
  NAD: ["Macanal Airport", "Macanal"],
  NAE: ["Natitingou Airport", "Natitingou"],
  NAF: ["Banaina Airport", "Banaina-Borneo Island"],
  NAG: ["Dr. Babasaheb Ambedkar International Airport", "Naqpur"],
  NAH: ["Naha Airport", "Tahuna-Sangihe Island"],
  NAI: ["Annai Airport", "Annai"],
  NAJ: ["Nakhchivan Airport", "Nakhchivan"],
  NAK: ["Nakhon Ratchasima Airport"],
  NAL: ["Nalchik Airport", "Nalchik"],
  NAM: ["Namlea Airport", "Namlea-Buru Island"],
  NAN: ["Nadi International Airport", "Nadi"],
  NAO: ["Nanchong Airport", "Nanchong"],
  NAP: ["Naples International Airport", "N\u00e1poli"],
  NAQ: ["Qaanaaq Airport", "Qaanaaq"],
  NAR: ["Puerto Nare Airport", "Armenia"],
  NAS: ["Lynden Pindling International Airport", "Nassau"],
  NAT: ["Governador Alu\u00edzio Alves International Airport", "Natal"],
  NAU: ["Napuka Island Airport", "Napuka Island"],
  NAV: ["Nev\u015fehir Kapadokya Airport", "Nev\u015fehir"],
  NAW: ["Narathiwat Airport"],
  NAY: ["Beijing Nanyuan Airport", "Beijing"],
  NAZ: ["Nana Airport", "Star Harbor"],
  NBA: ["Nambaiyufa Airport", "Nambaiyufa"],
  NBB: ["Barranco Minas Airport", "Barranco Minas"],
  NBC: ["Begishevo Airport", "Nizhnekamsk"],
  NBE: ["Enfidha - Hammamet International Airport", "Enfidha"],
  NBG: ["New Orleans NAS JRB/Alvin Callender Field", "New Orleans"],
  NBH: ["Nambucca Heads Airport", "Nambucca Heads"],
  NBL: ["San Blas Airport", "Wannukandi"],
  NBN: ["Annob\u00f3n Airport", "San Antonio de Pal\u00e9"],
  NBO: ["Jomo Kenyatta International Airport", "Nairobi"],
  NBS: ["Changbaishan Airport", "Baishan"],
  NBW: ["Leeward Point Field", "Guantanamo Bay Naval Station"],
  NBX: ["Nabire Airport", "Nabire-Papua Island"],
  NCA: ["North Caicos Airport"],
  NCE: ["Nice-C\u00f4te d'Azur Airport", "Nice"],
  NCG: ["Nuevo Casas Grandes Airport"],
  NCH: ["Nachingwea Airport", "Nachingwea"],
  NCI: ["Necocli Airport", "Necocli"],
  NCJ: ["Sunchales Aeroclub Airport", "Sunchales"],
  NCL: ["Newcastle Airport", "Newcastle"],
  NCN: ["Chenega Bay Airport", "Chenega"],
  NCO: ["Quonset State Airport", "North Kingstown"],
  NCR: ["San Carlos", "San Carlos"],
  NCS: ["Newcastle Airport", "Newcastle"],
  NCT: ["Guanacaste Airport", "Nicoya/Guanacate"],
  NCU: ["Nukus Airport", "Nukus"],
  NCY: ["Annecy-Haute-Savoie-Mont Blanc Airport", "Annecy/Meythet"],
  NDA: ["Bandanaira Airport", "Banda-Naira Island"],
  NDB: ["Nouadhibou International Airport", "Nouadhibou"],
  NDC: ["Nanded Airport", "Nanded"],
  NDD: ["Sumbe Airport", "Sumbe"],
  NDE: ["Mandera Airport", "Mandera"],
  NDF: ["Ndalatandos Airport", "Ndalatandos"],
  NDG: ["Qiqihar Sanjiazi Airport", "Qiqihar"],
  NDI: ["Namudi Airport", "Namudi"],
  NDJ: ["N'Djamena International Airport", "N'Djamena"],
  NDK: ["Namorik Atoll Airport", "Namorik Atoll"],
  NDL: ["N'D\u00e9l\u00e9 Airport", "N'D\u00e9l\u00e9"],
  NDM: ["Mendi Airport", "Mendi"],
  NDN: ["Nadunumu Airport", "Nadunumu"],
  NDR: ["Nador International Airport", "Nador"],
  NDS: ["Sandstone Airport", "Sandstone"],
  NDU: ["Rundu Airport", "Rundu"],
  NDY: ["Sanday Airport", "Sanday"],
  NEC: ["Necochea Airport", "Necochea"],
  NEF: ["Neftekamsk Airport", "Neftekamsk"],
  NEG: ["Negril Airport", "Negril"],
  NEJ: ["Nejjo Airport", "Nejjo"],
  NEK: ["Nekemte Airport", "Nekemte"],
  NEL: ["Lakehurst Maxfield Field Airport", "Lakehurst"],
  NEN: ["Whitehouse Naval Outlying Field", "Jacksonville"],
  NER: ["Chulman Airport", "Neryungri"],
  NEU: ["Sam Neua Airport"],
  NEV: ["Vance W. Amory International Airport", "Charlestown"],
  NEW: ["Lakefront Airport", "New Orleans"],
  NFG: ["Nefteyugansk Airport", "Nefteyugansk"],
  NFL: ["Fallon Naval Air Station", "Fallon"],
  NFO: ["Mata'aho Airport", "Angaha, Niuafo'ou Island"],
  NGA: ["Young Airport"],
  NGB: ["Ningbo Lishe International Airport", "Ningbo"],
  NGD: ["Captain Auguste George Airport", "Anegada"],
  NGE: ["N'Gaound\u00e9r\u00e9 Airport", "N'Gaound\u00e9r\u00e9"],
  NGF: ["Kaneohe Bay MCAS (Marion E. Carl Field) Airport", "Kaneohe"],
  NGI: ["Ngau Airport", "Ngau"],
  NGK: ["Nogliki Airport", "Nogliki-Sakhalin Island"],
  NGL: ["Ngala Airport", "Ngala"],
  NGO: ["Chubu Centrair International Airport", "Tokoname"],
  NGQ: ["Ngari Gunsa Airport", "Shiquanhe"],
  NGS: ["Nagasaki Airport", "Nagasaki"],
  NGU: ["Norfolk Naval Station (Chambers Field)", "Norfolk"],
  NGX: ["Manang Airport", "Ngawal"],
  NGZ: ["Alameda Naval Air Station", "Alameda"],
  NHA: ["Nha Trang Air Base", "Nha Trang"],
  NHD: ["Al Minhad Air Base", "Dubai"],
  NHF: ["New Halfa Airport", "New Halfa"],
  NHK: ["Patuxent River Naval Air Station (Trapnell Field)", "Patuxent River"],
  NHS: ["Nushki Airport", "Nushki"],
  NHT: ["RAF Northolt", "London"],
  NHV: ["Nuku Hiva Airport"],
  NHX: ["Naval Outlying Field Barin", "Foley"],
  NHZ: ["Brunswick Executive Airport", "Brunswick"],
  NIA: ["Nimba Airport", "Nimba"],
  NIB: ["Nikolai Airport", "Nikolai"],
  NIC: ["Nicosia International Airport", "Nicosia"],
  NIF: ["Camp Nifty Airport"],
  NIG: ["Nikunau Airport", "Nikunau"],
  NIM: ["Diori Hamani International Airport", "Niamey"],
  NIO: ["Nioki Airport", "Nioki"],
  NIP: ["Jacksonville Naval Air Station (Towers Field)", "Jacksonville"],
  NIS: ["Simberi Airport", "Simberi Island"],
  NIT: ["Niort-Souch\u00e9 Airport", "Niort/Souch\u00e9"],
  NIU: ["Naiu Airport", "Naiu Atoll"],
  NIX: ["Nioro du Sahel Airport", "Nioro du Sahel"],
  NJA: ["Atsugi Naval Air Facility"],
  NJC: ["Nizhnevartovsk Airport", "Nizhnevartovsk"],
  NJF: ["Al Najaf International Airport", "Najaf"],
  NJK: ["El Centro NAF Airport (Vraciu Field)", "El Centro"],
  NKB: ["Noonkanbah Airport"],
  NKC: ["Nouakchott\u2013Oumtounsy International Airport", "Nouakchott"],
  NKD: ["Sinak Airport", "Sinak"],
  NKG: ["Nanjing Lukou Airport", "Nanjing"],
  NKI: ["Naukati Bay Seaplane Base", "Tuxekan Island"],
  NKL: ["N'Kolo-Fuma Airport", "N'Kolo-Fuma"],
  NKM: ["Nagoya Airport", "Nagoya"],
  NKN: ["Nankina Airport", "Gwarawon"],
  NKP: ["Nukutepipi Airport", "Nukutepipi"],
  NKS: ["Nkongsamba Airport", "Nkongsamba"],
  NKT: [
    "\u015e\u0131rnak \u015eerafettin El\u00e7i Airport",
    "\u015e\u0131rnak",
  ],
  NKU: ["Nkaus Airport", "Nkaus"],
  NKW: ["Diego Garcia Naval Support Facility", "Diego Garcia"],
  NKX: ["Miramar Marine Corps Air Station - Mitscher Field", "San Diego"],
  NKY: ["Yokangassi Airport", "Nkayi"],
  NLA: ["Simon Mwansa Kapwepwe International Airport", "Ndola"],
  NLC: ["Lemoore Naval Air Station (Reeves Field) Airport", "Lemoore"],
  NLD: ["Quetzalc\u00f3atl International Airport", "Nuevo Laredo"],
  NLE: ["Jerry Tyler Memorial Airport", "Niles"],
  NLF: ["Darnley Island Airport", "Darnley Island"],
  NLG: ["Nelson Lagoon Airport", "Nelson Lagoon"],
  NLH: ["Ninglang Luguhu Airport", "Ninglang"],
  NLI: ["Nikolayevsk-na-Amure Airport", "Nikolayevsk-na-Amure Airport"],
  NLK: ["Norfolk Island International Airport", "Burnt Pine"],
  NLL: ["Nullagine Airport"],
  NLN: ["Kneeland Airport", "Eureka"],
  NLO: ["Ndolo Airport", "N'dolo"],
  NLP: ["Nelspruit Airport", "Nelspruit"],
  NLS: ["Nicholson Airport"],
  NLT: ["Xinyuan Nalati Airport", "Xinyuan County"],
  NLU: ["Santa Lucia Air Force Base", "Reyes Acozac"],
  NLV: ["Mykolaiv International Airport", "Nikolayev"],
  NMA: ["Namangan Airport", "Namangan"],
  NMB: ["Daman Airport"],
  NMC: ["Normans Cay Airport"],
  NME: ["Nightmute Airport", "Nightmute"],
  NMG: ["San Miguel Airport", "Isla del Rey"],
  NML: ["Fort McMurray / Mildred Lake Airport", "Fort McMurray"],
  NMN: ["Nomane Airport", "Namane"],
  NMP: ["New Moon Airport", "Basalt"],
  NMR: ["Nappa Merrie Airport"],
  NMS: ["Namsang Airport", "Namsang"],
  NMT: ["Namtu Airport", "Namtu"],
  NNA: ["Kenitra Airport"],
  NNB: ["Santa Ana Airport", "Santa Ana Island"],
  NND: ["Nangade Airport", "Nangade"],
  NNG: ["Nanning Wuxu Airport", "Nanning"],
  NNI: ["Namutoni Airport", "Namutoni"],
  NNK: ["Naknek Airport", "Naknek"],
  NNL: ["Nondalton Airport", "Nondalton"],
  NNM: ["Naryan Mar Airport", "Naryan Mar"],
  NNR: ["Connemara Regional Airport", "Inverin"],
  NNT: ["Nan Airport"],
  NNU: ["Nanuque Airport", "Nanuque"],
  NNX: ["Nunukan Airport", "Nunukan-Nunukan Island"],
  NNY: ["Nanyang Jiangying Airport", "Nanyang"],
  NOA: ["Nowra Airport"],
  NOB: ["Nosara Airport", "Nicoya"],
  NOC: ["Ireland West Knock Airport", "Charleston"],
  NOD: ["Norden-Norddeich Airport", "Norddeich"],
  NOG: ["Nogales International Airport"],
  NOJ: ["Noyabrsk Airport", "Noyabrsk"],
  NOK: ["Xavantina Airport", "Nova Xavantina"],
  NOM: ["Nomad River Airport", "Nomad River"],
  NON: ["Nonouti Airport", "Nonouti"],
  NOO: ["Naoro Airport", "Naoro Vilage"],
  NOP: ["Sinop Airport", "Sinop"],
  NOR: ["Nor\u00f0fj\u00f6r\u00f0ur Airport", "Nor\u00f0fj\u00f6r\u00f0ur"],
  NOS: ["Fascene Airport", "Nosy Be"],
  NOT: ["Marin County Airport - Gnoss Field", "Novato"],
  NOU: ["La Tontouta International Airport", "Noum\u00e9a"],
  NOV: ["Nova Lisboa Airport", "Huambo"],
  NOZ: ["Spichenkovo Airport", "Novokuznetsk"],
  NPA: ["Pensacola Naval Air Station/Forrest Sherman Field", "Pensacola"],
  NPE: ["Hawke's Bay Airport", "Napier"],
  NPG: ["Nipa Airport", "Nipa"],
  NPH: ["Nephi Municipal Airport", "Nephi"],
  NPL: ["New Plymouth Airport", "New Plymouth"],
  NPO: ["Nanga Pinoh Airport", "Nanga Pinoh-Borneo Island"],
  NPP: ["Napperby Airport", "Napperby"],
  NPR: ["Novo Progresso Airport", "Novo Progresso"],
  NPT: ["Newport State Airport", "Newport"],
  NPU: ["San Pedro Airport", "San Pedro de Urab\u00e1"],
  NQA: ["Millington-Memphis Airport", "Millington"],
  NQI: ["Kingsville Naval Air Station", "Kingsville"],
  NQL: ["Niquel\u00e2ndia Airport", "Niquel\u00e2ndia"],
  NQN: ["Presidente Peron Airport", "Neuquen"],
  NQT: ["Nottingham Airport", "Nottingham"],
  NQU: ["Reyes Murillo Airport", "Nuqu\u00ed"],
  NQX: ["Naval Air Station Key West/Boca Chica Field", "Key West"],
  NQY: ["Newquay Cornwall Airport", "Newquay"],
  NRA: ["Narrandera Airport", "Narrandera"],
  NRB: ["Naval Station Mayport (Admiral David L. Mcdonald Field)", "Mayport"],
  NRC: ["NASA Crows Landing Airport", "Crows Landing"],
  NRD: ["Norderney Airport", "Norderney"],
  NRE: ["Namrole Airport", "Namrole-Buru Island"],
  NRG: ["Narrogin Airport", "Narrogin"],
  NRI: ["Grand Lake Regional Airport", "Afton"],
  NRK: ["Norrk\u00f6ping Airport", "Norrk\u00f6ping"],
  NRL: ["North Ronaldsay Airport", "North Ronaldsay"],
  NRM: ["Nara Airport", "Nara"],
  NRN: ["Weeze Airport", "Weeze"],
  NRR: ["Jos\u00e9 Aponte de la Torre Airport", "Ceiba"],
  NRS: ["Naval Outlying Field Imperial Beach (Ream Field)", "Imperial Beach"],
  NRT: ["Narita International Airport", "Tokyo"],
  NRY: ["Newry Airport", "Newry"],
  NSB: ["Bimini North Seaplane Base", "Bimini"],
  NSE: ["Whiting Field Naval Air Station - North", "Milton"],
  NSH: ["Noshahr Airport"],
  NSI: ["Yaound\u00e9 Nsimalen International Airport", "Yaound\u00e9"],
  NSK: ["Norilsk-Alykel Airport", "Norilsk"],
  NSL: ["Slayton Municipal Airport", "Slayton"],
  NSM: ["Norseman Airport"],
  NSN: ["Nelson Airport", "Nelson"],
  NSO: ["Scone Airport"],
  NST: ["Nakhon Si Thammarat Airport", "Nakhon Si Thammarat"],
  NSV: ["Noosa Airport"],
  NSY: ["Sigonella Navy Air Base", "Sigonella (CT)"],
  NTA: ["Natadola Airport", "Mbatiri"],
  NTB: ["Notodden Airport", "Notodden"],
  NTC: ["Paradise Island Airport", "Santa Carolina"],
  NTD: ["Point Mugu Naval Air Station (Naval Base Ventura Co)", "Point Mugu"],
  NTE: ["Nantes Atlantique Airport", "Nantes"],
  NTG: ["Nantong Airport", "Nantong"],
  NTI: ["Stenkol Airport", "Bintuni"],
  NTJ: ["Manti-Ephraim Airport", "Manti"],
  NTL: ["Newcastle Airport", "Williamtown"],
  NTN: ["Normanton Airport", "Normanton"],
  NTO: ["Agostinho Neto Airport", "Ponta do Sol"],
  NTQ: ["Noto Airport", "Wajima"],
  NTR: ["Del Norte International Airport"],
  NTT: ["Kuini Lavenia Airport", "Niuatoputapu"],
  NTU: ["Oceana Naval Air Station", "Virginia Beach"],
  NTX: ["Ranai Airport", "Ranai-Natuna Besar Island"],
  NTY: ["Pilanesberg International Airport", "Pilanesberg"],
  NUB: ["Numbulwar Airport"],
  NUD: ["En Nahud Airport", "En Nahud"],
  NUE: ["Nuremberg Airport", "Nuremberg"],
  NUF: ["Castlereagh Lake Seaplane Base", "Hatton"],
  NUG: ["Nuguria Airstrip", "Nuguria Island"],
  NUI: ["Nuiqsut Airport", "Nuiqsut"],
  NUK: ["Nukutavake Airport", "Nukutavake"],
  NUL: ["Nulato Airport", "Nulato"],
  NUP: ["Nunapitchuk Airport", "Nunapitchuk"],
  NUQ: ["Moffett Federal Airfield", "Mountain View"],
  NUR: ["Nullabor Motel Airport"],
  NUS: ["Norsup Airport", "Norsup"],
  NUU: ["Nakuru Airport", "Nakuru"],
  NUW: ["Whidbey Island Naval Air Station (Ault Field)", "Oak Harbor"],
  NUX: ["Novy Urengoy Airport", "Novy Urengoy"],
  NVA: ["Benito Salas Airport", "Neiva"],
  NVG: ["Nueva Guinea Airport", "Nueva Guinea"],
  NVI: ["Navoi Airport", "Navoi"],
  NVK: ["Narvik Framnes Airport", "Narvik"],
  NVN: ["Nervino Airport", "Beckwourth"],
  NVP: ["Novo Aripuan\u00e3 Airport", "Novo Aripuan\u00e3"],
  NVR: ["Novgorod Airport", "Velikiy Novgorod"],
  NVS: ["Nevers-Fourchambault Airport", "Nevers/Fourchambault"],
  NVT: ["Ministro Victor Konder International Airport", "Navegantes"],
  NVY: ["Neyveli Airport", "Neyveli"],
  NWA: ["Moh\u00e9li Bandar Es Eslam Airport"],
  NWI: ["Norwich International Airport", "Norwich"],
  NWT: ["Nowata Airport", "Nowata"],
  NYA: ["Nyagan Airport", "Nyagan"],
  NYE: ["Nyeri Airport", "Nyeri"],
  NYI: ["Sunyani Airport", "Sunyani"],
  NYK: ["Nanyuki Airport", "Nanyuki"],
  NYM: ["Nadym Airport", "Nadym"],
  NYN: ["Nyngan Airport"],
  NYO: ["Stockholm Skavsta Airport", "Stockholm / Nyk\u00f6ping"],
  NYR: ["Nyurba Airport", "Nyurba"],
  NYT: ["Naypyidaw Airport", "Pyinmana"],
  NYU: ["Bagan Airport", "Nyaung U"],
  NYW: ["Monywar Airport", "Monywar"],
  NZA: ["Nzagi Airport", "Nzagi"],
  NZC: ["Maria Reiche Neuman Airport", "Nazca"],
  NZE: ["Nz\u00e9r\u00e9kor\u00e9 Airport", "Nz\u00e9r\u00e9kor\u00e9"],
  NZH: ["Manzhouli Xijiao Airport"],
  NZL: ["Chengjisihan Airport", "Zhalantun"],
  NZY: ["North Island Naval Air Station-Halsey Field", "San Diego"],
  O62: ["Carmel Valley"],
  OAA: ["Shank Air Base"],
  OAG: ["Orange Airport", "Orange"],
  OAH: ["Shindand Airport"],
  OAI: ["Bagram Air Base", "Bagram"],
  OAJ: ["Albert J Ellis Airport", "Jacksonville"],
  OAK: ["Metropolitan Oakland International Airport", "Oakland"],
  OAL: ["Cacoal Airport", "Cacoal"],
  OAM: ["Oamaru Airport"],
  OAN: ["El Array\u00e1n Airport", "Olanchito"],
  OAR: ["Marina Municipal Airport", "Marina"],
  OAS: ["Sharana Airstrip", "Sharana"],
  OAX: ["Xoxocotl\u00e1n International Airport", "Oaxaca"],
  OAZ: ["Camp Bastion Airport"],
  OBC: ["Obock Airport", "Obock"],
  OBD: ["Obano Airport", "Obano-Papua Island"],
  OBE: ["Okeechobee County Airport", "Okeechobee"],
  OBF: ["Oberpfaffenhofen Airport"],
  OBI: ["\u00d3bidos Airport", "\u00d3bidos"],
  OBK: ["Sky Harbor Airport", "Northbrook"],
  OBL: ["Zoersel (Oostmalle) Airfield", "Zoersel"],
  OBM: ["Morobe Airport", "Morobe"],
  OBN: ["Oban Airport", "North Connel"],
  OBO: ["Tokachi-Obihiro Airport", "Obihiro"],
  OBS: [
    "Aubenas-Ard\u00e8che M\u00e9ridional Airport",
    "Aubenas/Ard\u00e8che M\u00e9ridional",
  ],
  OBU: ["Kobuk Airport", "Kobuk"],
  OBX: ["Obo Airport", "Obo"],
  OBY: ["Ittoqqortoormiit Heliport", "Ittoqqortoormiit"],
  OCA: ["Ocean Reef Club Airport", "Key Largo"],
  OCC: ["Francisco De Orellana Airport", "Coca"],
  OCE: ["Ocean City Municipal Airport", "Ocean City"],
  OCF: ["Ocala International Airport - Jim Taylor Field", "Ocala"],
  OCH: ["A L Mangham Jr. Regional Airport", "Nacogdoches"],
  OCJ: ["Boscobel Aerodrome", "Ocho Rios"],
  OCM: ["Boolgeeda"],
  OCN: ["Oceanside Municipal Airport", "Oceanside"],
  OCS: ["Corisco International Airport", "Corisco Island"],
  OCV: ["Aguas Claras Airport", "Oca\u00f1a"],
  OCW: ["Warren Field", "Washington"],
  ODA: ["Ouadda Airport", "Ouadda"],
  ODB: ["C\u00f3rdoba Airport", "C\u00f3rdoba"],
  ODC: ["Oakdale Airport", "Oakdale"],
  ODD: ["Oodnadatta Airport"],
  ODE: ["Odense Airport", "Odense"],
  ODH: ["RAF Odiham", "Odiham"],
  ODJ: ["Ouanda Djall\u00e9 Airport", "Ouanda Djall\u00e9"],
  ODL: ["Cordillo Downs Airport", "Cordillo Downs"],
  ODN: ["Long Seridan Airport", "Long Seridan"],
  ODO: ["Bodaybo Airport", "Bodaybo"],
  ODR: ["Ord River Airport", "Ord River"],
  ODS: ["Odessa International Airport", "Odessa"],
  ODW: ["AJ Eisenberg Airport", "Oak Harbor"],
  ODY: ["Oudomsay Airport", "Oudomsay"],
  OEA: ["O'Neal Airport", "Vincennes"],
  OEC: ["Oecussi Airport", "Oecussi-Ambeno"],
  OEL: ["Oryol Yuzhny Airport", "Orel"],
  OEM: ["Vincent Fayks Airport", "Paloemeu"],
  OEO: ["L O Simenstad Municipal Airport", "Osceola"],
  OER: ["\u00d6rnsk\u00f6ldsvik Airport", "\u00d6rnsk\u00f6ldsvik"],
  OES: ["Antoine de Saint Exup\u00e9ry Airport", "San Antonio Oeste"],
  OFF: ["Offutt Air Force Base", "Omaha"],
  OFI: ["Ouango Fitini Airport", "Ouango Fitini"],
  OFJ: ["\u00d3lafsfj\u00f6r\u00f0ur Airport", "\u00d3lafsfj\u00f6r\u00f0ur"],
  OFK: ["Karl Stefan Memorial Airport", "Norfolk"],
  OFU: ["Ofu Village Airport", "Ofu Village"],
  OGA: ["Searle Field", "Ogallala"],
  OGB: ["Orangeburg Municipal Airport", "Orangeburg"],
  OGD: ["Ogden Hinckley Airport", "Ogden"],
  OGE: ["Ogeranang Airport"],
  OGG: ["Kahului Airport", "Kahului"],
  OGL: ["Eugene F. Correira International Airport", "Ogle"],
  OGM: ["Ogubsucum Airport", "Ustupo"],
  OGN: ["Yonaguni Airport"],
  OGO: ["Abengourou Airport", "Abengourou"],
  OGR: ["Bongor Airport", "Bongor"],
  OGS: ["Ogdensburg International Airport", "Ogdensburg"],
  OGU: ["Ordu Giresun Airport", "Ordu"],
  OGX: ["Ain el Beida Airport", "Ouargla"],
  OGZ: ["Beslan Airport", "Beslan"],
  OHA: ["RNZAF Base Ohakea"],
  OHB: ["Ambohibary Airport", "Moramanga"],
  OHD: ["Ohrid St. Paul the Apostle Airport", "Ohrid"],
  OHE: ["Gu-Lian Airport", "Mohe"],
  OHH: ["Okha Airport", "Okha"],
  OHI: ["Oshakati Airport", "Oshakati"],
  OHO: ["Okhotsk Airport", "Okhotsk"],
  OHR: ["Wyk auf F\u00f6hr Airport", "Wyk auf F\u00f6hr"],
  OHS: ["Sohar Airport", "Sohar"],
  OHT: ["Kohat Airport", "Kohat"],
  OIA: ["Ouril\u00e2ndia do Norte Airport", "Ouril\u00e2ndia do Norte"],
  OIC: ["Lt Warren Eaton Airport", "Norwich"],
  OIL: ["Splane Memorial Airport", "Oil City"],
  OIM: ["Oshima Airport", "Izu Oshima"],
  OIR: ["Okushiri Airport", "Okushiri Island"],
  OIT: ["Oita Airport", "Oita"],
  OJC: ["Johnson County Executive Airport", "Olathe"],
  OKA: ["Naha Airport", "Naha"],
  OKB: ["Orchid Beach Airport", "Orchid Beach"],
  OKC: ["Will Rogers World Airport", "Oklahoma City"],
  OKD: ["Okadama Airport", "Sapporo"],
  OKE: ["Okierabu Airport", "Okinoerabujima"],
  OKF: ["Okaukuejo Airport", "Okaukuejo"],
  OKG: ["Okoyo Airport", "Okoyo"],
  OKH: ["RAF Cottesmore", "Cottesmore"],
  OKI: ["Oki Airport", "Okinoshima"],
  OKJ: ["Okayama Airport", "Okayama City"],
  OKK: ["Kokomo Municipal Airport", "Kokomo"],
  OKL: ["Oksibil Airport", "Oksibil-Papua Island"],
  OKM: ["Okmulgee Regional Airport", "Okmulgee"],
  OKN: ["Okondja Airport", "Okondja"],
  OKO: ["Yokota Air Base", "Fussa"],
  OKP: ["Oksapmin Airport", "Oksapmin"],
  OKQ: ["Okaba Airport", "Okaba-Papua Island"],
  OKR: ["Yorke Island Airport", "Yorke Island"],
  OKS: ["Garden County Airport", "Oshkosh"],
  OKT: ["Oktyabrskiy Airport", "Kzyl-Yar"],
  OKU: ["Mokuti Lodge Airport", "Mokuti Lodge"],
  OKV: ["Okao Airport", "Okao"],
  OKY: ["Oakey Airport"],
  OLA: ["\u00d8rland Airport", "\u00d8rland"],
  OLB: ["Olbia Costa Smeralda Airport", "Olbia (SS)"],
  OLC: ["Senadora Eunice Micheles Airport", "S\u00e3o Paulo De Oliven\u00e7a"],
  OLD: ["Dewitt Field,Old Town Municipal Airport", "Old Town"],
  OLF: ["L M Clayton Airport", "Wolf Point"],
  OLI: ["Rif Airport", "Rif"],
  OLJ: ["North West Santo Airport", "Olpoi"],
  OLK: ["Fuerte Olimpo Airport", "Fuerte Olimpo"],
  OLL: ["Oyo Ollombo Airport", "Oyo"],
  OLM: ["Olympia Regional Airport", "Olympia"],
  OLN: ["Colonia Sarmiento Airport", "Sarmiento"],
  OLO: ["Olomouc-Ne\u0159ed\u00edn Airport", "Olomouc"],
  OLP: ["Olympic Dam Airport", "Olympic Dam"],
  OLQ: ["Olsobip Airport", "Olsobip"],
  OLR: ["Salerno Landing Zone Airport"],
  OLS: ["Nogales International Airport", "Nogales"],
  OLV: ["Olive Branch Airport", "Olive Branch"],
  OLZ: ["Olyokminsk Airport", "Olyokminsk"],
  OMA: ["Eppley Airfield", "Omaha"],
  OMB: ["Omboue Hopital Airport", "Omboue"],
  OMC: ["Ormoc Airport", "Ormoc City"],
  OMD: ["Oranjemund Airport", "Oranjemund"],
  OME: ["Nome Airport", "Nome"],
  OMF: ["King Hussein Air College", "Mafraq"],
  OMG: ["Omega Airport", "Omega"],
  OMH: ["Urmia Airport", "Urmia"],
  OMI: ["Omidiyeh Airport", "Omidiyeh"],
  OMK: ["Omak Airport", "Omak"],
  OML: ["Omkalai Airport", "Omkalai"],
  OMM: ["Marmul Airport", "Marmul"],
  OMN: ["Osmanabad Airport", "Osmanabad"],
  OMO: ["Mostar International Airport", "Mostar"],
  OMR: ["Oradea International Airport", "Oradea"],
  OMS: ["Omsk Central Airport", "Omsk"],
  OMY: ["Preah Vinhear Airport", "Tbeng Meanchey"],
  ONB: ["Ononge Airport", "Onange Mission"],
  OND: ["Ondangwa Airport", "Ondangwa"],
  ONG: ["Mornington Island Airport"],
  ONI: ["Moanamani Airport", "Moanamani-Papua Island"],
  ONJ: ["Odate Noshiro Airport", "Odate"],
  ONK: ["Olenyok Airport", "Olenyok"],
  ONL: ["The O'Neill Municipal John L Baker Field", "O'Neill"],
  ONO: ["Ontario Municipal Airport", "Ontario"],
  ONP: ["Newport Municipal Airport", "Newport"],
  ONQ: ["Zonguldak Airport", "Zonguldak"],
  ONR: ["Monkira Airport"],
  ONS: ["Onslow Airport"],
  ONT: ["Ontario International Airport", "Ontario"],
  ONU: ["Ono-i-Lau Airport", "Ono-i-Lau"],
  ONX: ["Enrique Adolfo Jimenez Airport", "Col\u00f3n"],
  OOK: ["Toksook Bay Airport", "Toksook Bay"],
  OOL: ["Gold Coast Airport", "Gold Coast"],
  OOM: ["Cooma Snowy Mountains Airport", "Cooma"],
  OOR: ["Mooraberree Airport"],
  OOT: ["Onotoa Airport", "Onotoa"],
  OPA: ["K\u00f3pasker Airport", "K\u00f3pasker"],
  OPF: ["Opa-locka Executive Airport", "Miami"],
  OPI: ["Oenpelli Airport"],
  OPO: ["Francisco de S\u00e1 Carneiro Airport", "Porto"],
  OPS: ["Presidente Jo\u00e3o Batista Figueiredo Airport", "Sinop"],
  OPU: ["Balimo Airport", "Balimo"],
  OPW: ["Opuwa Airport", "Opuwa"],
  ORA: ["Or\u00e1n Airport", "Or\u00e1n"],
  ORB: ["\u00d6rebro Airport", "\u00d6rebro"],
  ORC: ["Orocue Airport", "Orocue"],
  ORD: ["Chicago O'Hare International Airport", "Chicago"],
  ORE: ["Orl\u00e9ans-Bricy (BA 123) Air Base", "Orl\u00e9ans/Bricy"],
  ORF: ["Norfolk International Airport", "Norfolk"],
  ORG: ["Zorg en Hoop Airport", "Paramaribo"],
  ORH: ["Worcester Regional Airport", "Worcester"],
  ORJ: ["Orinduik Airport", "Orinduik"],
  ORK: ["Cork Airport", "Cork"],
  ORL: ["Orlando Executive Airport", "Orlando"],
  ORM: ["Sywell Aerodrome", "Northampton"],
  ORN: ["Es Senia Airport", "Oran"],
  ORO: ["Yoro Airport", "Yoro"],
  ORP: ["Orapa Airport", "Orapa"],
  ORR: ["Yorketown Airport"],
  ORT: ["Northway Airport", "Northway"],
  ORU: ["Juan Mendoza Airport", "Oruro"],
  ORV: ["Robert (Bob) Curtis Memorial Airport", "Noorvik"],
  ORW: ["Ormara Airport", "Ormara Raik"],
  ORX: ["Oriximin\u00e1 Airport", "Oriximin\u00e1"],
  ORY: ["Paris-Orly Airport", "Paris"],
  ORZ: ["Orange Walk Airport", "Orange Walk"],
  OSC: ["Oscoda Wurtsmith Airport", "Oscoda"],
  OSD: ["\u00c5re \u00d6stersund Airport", "\u00d6stersund"],
  OSE: ["Omora Airport", "Omora"],
  OSF: ["Ostafyevo International Airport", "Moscow"],
  OSG: ["Ossima Airport", "Ossima"],
  OSH: ["Wittman Regional Airport", "Oshkosh"],
  OSI: ["Osijek Airport", "Osijek"],
  OSK: ["Oskarshamn Airport"],
  OSL: ["Oslo Gardermoen Airport", "Oslo"],
  OSM: ["Mosul International Airport", "Mosul"],
  OSN: ["Osan Air Base"],
  OSO: ["Osborne Mine Airport"],
  OSP: ["Redzikowo Air Base"],
  OSR: ["Ostrava Leos Jan\u00e1\u010dek Airport", "Ostrava"],
  OSS: ["Osh Airport", "Osh"],
  OST: ["Ostend-Bruges International Airport", "Ostend"],
  OSU: ["The Ohio State University Airport - Don Scott Field", "Columbus"],
  OSW: ["Orsk Airport", "Orsk"],
  OSY: ["Namsos H\u00f8knes\u00f8ra Airport", "Namsos"],
  OSZ: ["Koszalin Zegrze Pomorskie Air Base", "Zegrze Pomorskie"],
  OTC: ["Bol Airport", "Bol"],
  OTD: ["Contadora Airport", "Contadora Island"],
  OTH: ["Southwest Oregon Regional Airport", "North Bend"],
  OTI: ["Pitu Airport", "Gotalalamo-Morotai Island"],
  OTJ: ["Otjiwarongo Airport", "Otjiwarongo"],
  OTK: ["Tillamook Airport", "Tillamook"],
  OTL: ["Boutilimit Airport", "Boutilimit"],
  OTM: ["Ottumwa Regional Airport", "Ottumwa"],
  OTN: ["Ed-Air Airport", "Oaktown"],
  OTP: ["Henri Coand\u0103 International Airport", "Bucharest"],
  OTR: ["Coto 47 Airport", "Corredores"],
  OTS: ["Anacortes Airport", "Anacortes"],
  OTT: ["Andre Maggi Airport", "Cotrigua\u00e7u"],
  OTU: ["Otu Airport", "Remedios"],
  OTV: ["Ontong Java Atoll Airstrip", "Ontong Java Atoll"],
  OTY: ["Oria Airport", "Oria"],
  OTZ: ["Ralph Wien Memorial Airport", "Kotzebue"],
  OUA: ["Ouagadougou Airport", "Ouagadougou"],
  OUD: ["Angads Airport", "Oujda"],
  OUE: ["Ouesso Airport"],
  OUG: ["Ouahigouya Airport", "Ouahigouya"],
  OUH: ["Oudtshoorn Airport", "Oudtshoorn"],
  OUK: ["Outer Skerries Airport", "Grunay Island"],
  OUL: ["Oulu Airport", "Oulu / Oulunsalo"],
  OUM: ["Oum Hadjer Airport", "Oum Hadjer"],
  OUN: ["University of Oklahoma Westheimer Airport", "Norman"],
  OUR: ["Batouri Airport", "Batouri"],
  OUS: ["Ourinhos Airport", "Ourinhos"],
  OUT: ["Bousso Airport", "Bousso"],
  OUZ: ["Tazadit Airport", "Zou\u00e9rate"],
  OVA: ["Bekily Airport", "Bekily"],
  OVB: ["Tolmachevo Airport", "Novosibirsk"],
  OVD: ["Asturias Airport", "Ran\u00f3n"],
  OVE: ["Oroville Municipal Airport", "Oroville"],
  OVG: ["Overberg Airport", "Overberg"],
  OVL: ["El Tuqui Airport", "Ovalle"],
  OVR: ["Olavarria Airport", "Olavarria"],
  OVS: ["Sovetskiy Airport", "Sovetskiy"],
  OWA: ["Owatonna Degner Regional Airport", "Owatonna"],
  OWB: ["Owensboro Daviess County Airport", "Owensboro"],
  OWD: ["Norwood Memorial Airport", "Norwood"],
  OWK: ["Central Maine Airport of Norridgewock", "Norridgewock"],
  OXB: ["Osvaldo Vieira International Airport", "Bissau"],
  OXC: ["Waterbury Oxford Airport", "Oxford"],
  OXD: ["Miami University Airport", "Oxford"],
  OXF: ["Oxford (Kidlington) Airport", "Kidlington"],
  OXO: ["Orientos Airport", "Orientos"],
  OXR: ["Oxnard Airport", "Oxnard"],
  OXY: ["Morney Airport"],
  OYA: ["Goya Airport", "Goya"],
  OYE: ["Oyem Airport", "Oyem"],
  OYG: ["Moyo Airport", "Moyo"],
  OYK: ["Oiapoque Airport", "Oiapoque"],
  OYL: ["Moyale Airport", "Moyale (Lower)"],
  OYN: ["Ouyen Airport"],
  OYO: ["Tres Arroyos Airport", "Tres Arroyos"],
  OYP: [
    "Saint-Georges-de-l'Oyapock Airport",
    "Saint-Georges-de-l'Oyapock Airport",
  ],
  OZA: ["Ozona Municipal Airport", "Ozona"],
  OZC: ["Labo Airport", "Ozamiz City"],
  OZG: ["Zagora Airport", "Zagora"],
  OZH: ["Zaporizhzhia International Airport", "Zaporizhia"],
  OZP: ["Moron Air Base", "Mor\u00f3n"],
  OZR: ["Cairns AAF (Fort Rucker) Air Field", "Fort Rucker/Ozark"],
  OZZ: ["Ouarzazate Airport", "Ouarzazate"],
  PAA: ["Hpa-N Airport", "Hpa-N"],
  PAB: ["Bilaspur Airport", "Bilaspur"],
  PAC: ["Marcos A. Gelabert International Airport", "Albrook"],
  PAD: ["Paderborn Lippstadt Airport", "Paderborn"],
  PAE: ["Snohomish County (Paine Field) Airport", "Everett"],
  PAF: ["Pakuba Airfield", "Kabalega Falls"],
  PAG: ["Pagadian Airport", "Pagadian City"],
  PAH: ["Barkley Regional Airport", "Paducah"],
  PAJ: ["Parachinar Airport", "Parachinar"],
  PAK: ["Port Allen Airport", "Hanapepe"],
  PAL: ["German Olano Air Base", "La Dorada"],
  PAM: ["Tyndall Air Force Base", "Panama City"],
  PAN: ["Pattani Airport"],
  PAO: ["Palo Alto Airport of Santa Clara County", "Palo Alto"],
  PAP: ["Toussaint Louverture International Airport", "Port-au-Prince"],
  PAQ: ['Warren "Bud" Woods Palmer Municipal Airport', "Palmer"],
  PAS: ["Paros National Airport", "Paros Island"],
  PAT: ["Lok Nayak Jayaprakash Airport", "Patna"],
  PAU: ["Pauk Airport", "Pauk"],
  PAV: ["Paulo Afonso Airport", "Paulo Afonso"],
  PAX: ["Port-de-Paix Airport", "Port-de-Paix"],
  PAY: ["Pamol Airport", "Pamol"],
  PAZ: ["El Taj\u00edn National Airport", "Poza Rica"],
  PBB: ["Parana\u00edba Airport", "Parana\u00edba"],
  PBC: ["Hermanos Serd\u00e1n International Airport", "Puebla"],
  PBD: ["Porbandar Airport", "Porbandar"],
  PBE: ["Puerto Berrio Airport", "Puerto Berrio"],
  PBF: ["Pine Bluff Regional Airport, Grider Field", "Pine Bluff"],
  PBG: ["Plattsburgh International Airport", "Plattsburgh"],
  PBH: ["Paro Airport", "Paro"],
  PBI: ["Palm Beach International Airport", "West Palm Beach"],
  PBJ: ["Tavie Airport", "Paama Island"],
  PBL: ["General Bartolome Salom International Airport"],
  PBM: ["Johan Adolf Pengel International Airport", "Zandery"],
  PBN: ["Porto Amboim Airport", "Port Amboim"],
  PBO: ["Paraburdoo Airport", "Paraburdoo"],
  PBP: ["Islita Airport", "Nandayure"],
  PBQ: ["Pimenta Bueno Airport", "Pimenta Bueno"],
  PBR: ["Puerto Barrios Airport", "Puerto Barrios"],
  PBT: ["Puerto Leda Airport", "Puerto Leda"],
  PBU: ["Putao Airport", "Putao"],
  PBV: ["Porto dos Ga\u00fachos Airport", "Porto Dos Ga\u00fachos"],
  PBX: ["Fazenda Piraguassu Airport", "Porto Alegre Do Norte"],
  PBZ: ["Plettenberg Bay Airport", "Plettenberg Bay"],
  PCA: ["Portage Creek Airport", "Portage Creek"],
  PCB: ["Pondok Cabe Air Base", "Jakarta"],
  PCC: ["Puerto Rico Airport", "Puerto Rico"],
  PCD: ["Prairie Du Chien Municipal Airport", "Prairie Du Chien"],
  PCF: ["Potchefstroom Airport", "Potchefstroom"],
  PCG: ["Paso Caballos Airport", "Paso Caballos"],
  PCH: ["Palacios Airport", "Palacios"],
  PCJ: ["Puerto La Victoria Airport", "Puerto La Victoria"],
  PCL: ["Cap FAP David Abenzur Rengifo International Airport", "Pucallpa"],
  PCM: ["Playa del Carmen Airport", "Solidaridad"],
  PCN: ["Picton Aerodrome", "Koromiko"],
  PCO: ["Punta Colorada Airport", "La Ribera"],
  PCP: ["Principe Airport"],
  PCQ: ["Boun Neau Airport", "Phongsaly"],
  PCR: ["German Olano Airport", "Puerto Carre\u00f1o"],
  PCS: ["Picos Airport", "Picos"],
  PCT: ["Princeton Airport", "Princeton/Rocky Hill"],
  PCU: ["Poplarville Pearl River County Airport", "Poplarville"],
  PCV: ["Punta Chivato Airport", "Punta Chivato"],
  PDA: ["Obando Airport", "Puerto In\u00edrida"],
  PDB: ["Pedro Bay Airport", "Pedro Bay"],
  PDC: ["Mueo Airport", "Mueo"],
  PDD: ["Ponta do Ouro Airport", "Ponta do Ouro"],
  PDE: ["Pandie Pandie Airport"],
  PDF: ["Prado Airport", "Prado"],
  PDG: [
    "Minangkabau International Airport",
    "Ketaping/Padang - Sumatra Island",
  ],
  PDI: ["Pindiu Airport", "Pindiu"],
  PDK: ["DeKalb Peachtree Airport", "Atlanta"],
  PDL: ["Jo\u00e3o Paulo II Airport", "Ponta Delgada"],
  PDM: ["Capt Justiniano Montenegro Airport", "Pedasi"],
  PDN: ["Parndana Airport", "Kangaroo Island"],
  PDO: ["Pendopo Airport", "Talang Gudang-Sumatra Island"],
  PDP: ["Capitan Corbeta CA Curbelo International Airport", "Punta del Este"],
  PDR: ["Presidente Jos\u00e9 Sarney Airport", "Presidente Dutra"],
  PDS: ["Piedras Negras International Airport"],
  PDT: ["Eastern Oregon Regional At Pendleton Airport", "Pendleton"],
  PDU: ["Tydeo Larre Borges Airport", "Paysandu"],
  PDV: ["Plovdiv International Airport", "Plovdiv"],
  PDX: ["Portland International Airport", "Portland"],
  PDZ: ["Pedernales Airport"],
  PEA: ["Penneshaw Airport", "Ironstone"],
  PEC: ["Pelican Seaplane Base", "Pelican"],
  PED: ["Pardubice Airport", "Pardubice"],
  PEE: ["Bolshoye Savino Airport", "Perm"],
  PEF: ["Peenem\u00fcnde Airport", "Peenem\u00fcnde"],
  PEG: [
    "Perugia San Francesco d'Assisi \u2013 Umbria International Airport",
    "Perugia",
  ],
  PEH: ["Comodoro Pedro Zanni Airport", "Pehuaj\u00f3"],
  PEI: ["Mateca\u00f1a International Airport", "Pereira"],
  PEK: ["Beijing Capital International Airport", "Beijing"],
  PEL: ["Pelaneng Airport", "Pelaneng"],
  PEM: ["Padre Aldamiz International Airport", "Puerto Maldonado"],
  PEN: ["Penang International Airport", "Penang"],
  PEP: ["Peppimenarti Airport", "Peppimenarti"],
  PEQ: ["Pecos Municipal Airport", "Pecos"],
  PER: ["Perth International Airport", "Perth"],
  PES: ["Petrozavodsk Airport", "Petrozavodsk"],
  PET: ["Jo\u00e3o Sim\u00f5es Lopes Neto International Airport", "Pelotas"],
  PEU: ["Puerto Lempira Airport", "Puerto Lempira"],
  PEV: ["P\u00e9cs-Pog\u00e1ny Airport", "P\u00e9cs-Pog\u00e1ny"],
  PEW: ["Peshawar International Airport", "Peshawar"],
  PEX: ["Pechora Airport", "Pechora"],
  PEY: ["Penong Airport", "Penong"],
  PEZ: ["Penza Airport", "Penza"],
  PFB: ["Lauro Kurtz Airport", "Passo Fundo"],
  PFC: ["Pacific City State Airport", "Pacific City"],
  PFJ: ["Patreksfj\u00f6r\u00f0ur Airport", "Patreksfj\u00f6r\u00f0ur"],
  PFO: ["Paphos International Airport", "Paphos"],
  PFQ: ["Parsabade Moghan Airport", "Parsabad"],
  PFR: ["Ilebo Airport", "Ilebo"],
  PGA: ["Page Municipal Airport", "Page"],
  PGB: ["Pangoa Airport", "Pangoa"],
  PGC: ["Grant County Airport", "Petersburg"],
  PGD: ["Charlotte County Airport", "Punta Gorda"],
  PGE: ["Yegepa Airport"],
  PGF: [
    "Perpignan-Rivesaltes (Llaban\u00e8re) Airport",
    "Perpignan/Rivesaltes",
  ],
  PGH: ["Pantnagar Airport", "Pantnagar"],
  PGI: ["Chitato Airport", "Chitato"],
  PGK: [
    "Pangkal Pinang (Depati Amir) Airport",
    "Pangkal Pinang-Palaubangka Island",
  ],
  PGL: ["Trent Lott International Airport", "Pascagoula"],
  PGM: ["Port Graham Airport", "Port Graham"],
  PGN: ["Pangia Airport", "Pangia"],
  PGO: ["Stevens Field", "Pagosa Springs"],
  PGP: ["Porto Alegre Airport", "Porto Alegro"],
  PGR: ["Kirk Field", "Paragould"],
  PGU: ["Persian Gulf International Airport", "Asalouyeh"],
  PGV: ["Pitt Greenville Airport", "Greenville"],
  PGX: ["P\u00e9rigueux-Bassillac Airport", "P\u00e9rigueux/Bassillac"],
  PGZ: [
    "Ponta Grossa Airport - Comandante Antonio Amilton Beraldo",
    "Ponta Grossa",
  ],
  PHA: ["Phan Rang Airport", "Phan Rang"],
  PHB: ["Prefeito Doutor Jo\u00e3o Silva Filho Airport", "Parna\u00edba"],
  PHC: ["Port Harcourt International Airport", "Port Harcourt"],
  PHD: ["Harry Clever Field", "New Philadelphia"],
  PHE: ["Port Hedland International Airport", "Port Hedland"],
  PHF: ["Newport News Williamsburg International Airport", "Newport News"],
  PHG: ["Port Harcourt City Airport", "Port Harcourt"],
  PHH: ["Phan Thiet Airport", "Phan Thiet"],
  PHI: ["Pinheiro Airport", "Pinheiro"],
  PHK: ["Palm Beach County Glades Airport", "Pahokee"],
  PHL: ["Philadelphia International Airport", "Philadelphia"],
  PHN: ["St Clair County International Airport", "Port Huron"],
  PHO: ["Point Hope Airport", "Point Hope"],
  PHP: ["Philip Airport", "Philip"],
  PHQ: ["The Monument Airport", "Phosphate Hill"],
  PHS: ["Phitsanulok Airport"],
  PHT: ["Henry County Airport", "Paris"],
  PHW: ["Hendrik Van Eck Airport", "Phalaborwa"],
  PHX: ["Phoenix Sky Harbor International Airport", "Phoenix"],
  PHY: ["Phetchabun Airport"],
  PIA: ["General Wayne A. Downing Peoria International Airport", "Peoria"],
  PIB: ["Hattiesburg Laurel Regional Airport", "Hattiesburg/Laurel"],
  PIC: ["Pine Cay Airport", "Pine Cay"],
  PID: ["Nassau Paradise Island Airport", "Nassau"],
  PIE: [
    "St Petersburg Clearwater International Airport",
    "St Petersburg-Clearwater",
  ],
  PIF: ["Pingtung North Airport", "Pingtung"],
  PIH: ["Pocatello Regional Airport", "Pocatello"],
  PIK: ["Glasgow Prestwick Airport", "Glasgow"],
  PIL: ["Carlos Miguel Gimenez Airport", "Pilar"],
  PIM: ["Harris County Airport", "Pine Mountain"],
  PIN: ["Parintins Airport", "Parintins"],
  PIO: [
    "Capit\u00e1n FAP Ren\u00e1n El\u00edas Olivera International Airport",
    "Pisco",
  ],
  PIP: ["Pilot Point Airport", "Pilot Point"],
  PIR: ["Pierre Regional Airport", "Pierre"],
  PIS: ["Poitiers-Biard Airport", "Poitiers/Biard"],
  PIT: ["Pittsburgh International Airport", "Pittsburgh"],
  PIU: [
    "Capit\u00e1n FAP Guillermo Concha Iberico International Airport",
    "Piura",
  ],
  PIV: ["Pirapora Airport", "Pirapora"],
  PIW: ["Pikwitonei Airport", "Pikwitonei"],
  PIX: ["Pico Airport", "Pico Island"],
  PIZ: ["Point Lay LRRS Airport", "Point Lay"],
  PJA: ["Pajala Airport"],
  PJB: ["Payson Airport", "Payson"],
  PJC: [
    "Dr Augusto Roberto Fuster International Airport",
    "Pedro Juan Caballero",
  ],
  PJG: ["Panjgur Airport", "Panjgur"],
  PJM: ["Puerto Jimenez Airport", "Puerto Jimenez"],
  PKA: ["Napaskiak Airport", "Napaskiak"],
  PKB: ["Mid Ohio Valley Regional Airport", "Parkersburg"],
  PKC: ["Yelizovo Airport", "Petropavlovsk-Kamchatsky"],
  PKD: ["Park Rapids Municipal Konshok Field", "Park Rapids"],
  PKE: ["Parkes Airport", "Parkes"],
  PKF: ["Park Falls Municipal Airport", "Park Falls"],
  PKG: ["Pulau Pangkor Airport", "Pangkor Island"],
  PKH: ["Porto Cheli Airport", "Porto Cheli"],
  PKJ: ["Playa Grande Airport", "Playa Grande"],
  PKK: ["Pakhokku Airport", "Pakhokku"],
  PKN: ["Iskandar Airport", "Pangkalanbun-Borneo Island"],
  PKO: ["Parakou Airport", "Parakou"],
  PKP: ["Puka Puka Airport"],
  PKR: ["Pokhara Airport", "Pokhara"],
  PKT: ["Port Keats Airport"],
  PKU: [
    "Sultan Syarif Kasim Ii (Simpang Tiga) Airport",
    "Pekanbaru-Sumatra Island",
  ],
  PKV: ["Pskov Airport", "Pskov"],
  PKW: ["Selebi Phikwe Airport"],
  PKY: ["Tjilik Riwut Airport", "Palangkaraya-Kalimantan Tengah"],
  PKZ: ["Pakse International Airport", "Pakse"],
  PLB: ["Clinton County Airport", "Plattsburgh"],
  PLD: ["Playa Samara/Carrillo Airport", "Carrillo"],
  PLE: ["Paiela Airport", "Paiela"],
  PLF: ["Pala Airport", "Pala"],
  PLH: ["Plymouth City Airport", "Plymouth"],
  PLJ: ["Placencia Airport", "Placencia"],
  PLK: ["M. Graham Clark Downtown Airport", "Branson / Hollister"],
  PLL: ["Ponta Pelada Airport", "Manaus"],
  PLM: ["Sultan Mahmud Badaruddin II Airport", "Palembang-Sumatra Island"],
  PLN: ["Pellston Regional Airport of Emmet County Airport", "Pellston"],
  PLO: ["Port Lincoln Airport", "Port Lincoln"],
  PLP: ["Captain Ramon Xatruch Airport", "La Palma"],
  PLQ: ["Palanga International Airport", "Palanga"],
  PLR: ["St Clair County Airport", "Pell City"],
  PLS: ["Providenciales Airport", "Providenciales Island"],
  PLT: ["Plato Airport", "Plato"],
  PLU: ["Pampulha - Carlos Drummond de Andrade Airport", "Belo Horizonte"],
  PLV: ["Suprunovka Airport", "Poltava"],
  PLW: ["Mutiara Airport", "Palu-Celebes Island"],
  PLX: ["Semipalatinsk Airport", "Semey"],
  PLY: ["Plymouth Municipal Airport", "Plymouth"],
  PLZ: ["Port Elizabeth Airport", "Port Elizabeth"],
  PMA: ["Pemba Airport", "Chake"],
  PMB: ["Pembina Municipal Airport", "Pembina"],
  PMC: ["El Tepual Airport", "Puerto Montt"],
  PMD: ["Palmdale Regional/USAF Plant 42 Airport", "Palmdale"],
  PME: ["Fleetlands Heliport", "Gosport"],
  PMF: ["Parma Airport", "Parma"],
  PMG: ["Ponta Por\u00e3 Airport", "Ponta Por\u00e3"],
  PMH: ["Greater Portsmouth Regional Airport", "Portsmouth"],
  PMI: ["Palma De Mallorca Airport", "Palma De Mallorca"],
  PMK: ["Palm Island Airport"],
  PML: ["Port Moller Airport", "Cold Bay"],
  PMM: ["Phanom Sarakham Airport", "Phanom Sarakham"],
  PMN: ["Pumani Airport", "Pumani"],
  PMO: ["Falcone\u2013Borsellino Airport", "Palermo"],
  PMP: ["Pimaga Airport", "Pimaga"],
  PMQ: ["Perito Moreno Airport", "Perito Moreno"],
  PMR: ["Palmerston North Airport"],
  PMS: ["Palmyra Airport", "Tadmur"],
  PMT: ["Paramakatoi Airport", "Paramakatoi"],
  PMV: [
    "Del Caribe Santiago Mari\u00f1o International Airport",
    "Isla Margarita",
  ],
  PMW: ["Brigadeiro Lysias Rodrigues Airport", "Palmas"],
  PMX: ["Metropolitan Airport", "Palmer"],
  PMY: ["El Tehuelche Airport", "Puerto Madryn"],
  PMZ: ["Palmar Sur Airport", "Palmar Sur"],
  PNA: ["Pamplona Airport", "Pamplona"],
  PNB: ["Porto Nacional Airport", "Porto Nacional"],
  PNC: ["Ponca City Regional Airport", "Ponca City"],
  PND: ["Punta Gorda Airport", "Punta Gorda"],
  PNE: ["Northeast Philadelphia Airport", "Philadelphia"],
  PNG: ["Paranagu\u00e1 Airport", "Paranagu\u00e1"],
  PNH: ["Phnom Penh International Airport", "Phnom Penh"],
  PNI: ["Pohnpei International Airport", "Pohnpei Island"],
  PNK: ["Supadio Airport", "Pontianak-Borneo Island"],
  PNL: ["Pantelleria Airport", "Pantelleria (TP)"],
  PNN: ["Princeton Municipal Airport", "Princeton"],
  PNP: ["Girua Airport", "Popondetta"],
  PNQ: ["Pune Airport", "Pune"],
  PNR: ["Pointe Noire Airport", "Pointe Noire"],
  PNS: ["Pensacola Regional Airport", "Pensacola"],
  PNT: ["Tte. Julio Gallardo Airport", "Puerto Natales"],
  PNU: ["Panguitch Municipal Airport", "Panguitch"],
  PNV: ["Panev\u0117\u017eys Air Base", "Panev\u0117\u017eys"],
  PNX: ["North Texas Regional Airport/Perrin Field", "Sherman/Denison"],
  PNY: ["Pondicherry Airport", "Pondicherry"],
  PNZ: ["Senador Nilo Coelho Airport", "Petrolina"],
  POA: ["Salgado Filho Airport", "Porto Alegre"],
  POB: ["Pope Field", "Fayetteville"],
  POC: ["Brackett Field", "La Verne"],
  POD: ["Podor Airport", "Podor"],
  POE: ["Polk Army Air Field", "Fort Polk"],
  POF: ["Poplar Bluff Municipal Airport", "Poplar Bluff"],
  POG: ["Port Gentil Airport", "Port Gentil"],
  POI: ["Capitan Nicolas Rojas Airport", "Potos\u00ed"],
  POJ: ["Patos de Minas Airport", "Patos De Minas"],
  POL: ["Pemba Airport", "Pemba / Porto Amelia"],
  POM: ["Port Moresby Jacksons International Airport", "Port Moresby"],
  PON: ["Popt\u00fan Airport", "Popt\u00fan"],
  POO: [
    "Po\u00e7os de Caldas - Embaixador Walther Moreira Salles Airport",
    "Po\u00e7os De Caldas",
  ],
  POP: ["Gregorio Luperon International Airport", "Puerto Plata"],
  POR: ["Pori Airport", "Pori"],
  POS: ["Piarco International Airport", "Port of Spain"],
  POT: ["Ken Jones Airport", "Ken Jones"],
  POU: ["Dutchess County Airport", "Poughkeepsie"],
  POV: ["Pre\u0161ov Air Base", "Pre\u0161ov"],
  POW: ["Portoroz Airport", "Portoroz"],
  POX: ["Pontoise - Cormeilles-en-Vexin Airport", "Cormeilles-en-Vexin"],
  POY: ["Powell Municipal Airport", "Powell"],
  POZ: ["Pozna\u0144-\u0141awica Airport", "Pozna\u0144"],
  PPA: ["Perry Lefors Field", "Pampa"],
  PPB: ["Presidente Prudente Airport", "Presidente Prudente"],
  PPC: ["Prospect Creek Airport", "Prospect Creek"],
  PPD: ["Palmas del Mar Airstrip", "Palmas del Mar"],
  PPE: ["Mar de Cort\u00e9s International Airport", "Puerto Pe\u00f1asco"],
  PPF: ["Tri-City Airport", "Parsons"],
  PPG: ["Pago Pago International Airport", "Pago Pago"],
  PPH: ["Perai Tepuy Airport"],
  PPI: ["Port Pirie Airport"],
  PPJ: ["Pulau Panjang Airport", "Jakarta"],
  PPK: ["Petropavlosk South Airport", "Petropavlosk"],
  PPL: ["Phaplu Airport", "Phaplu"],
  PPM: ["Pompano Beach Airpark", "Pompano Beach"],
  PPN: ["Guillermo Le\u00f3n Valencia Airport", "Popay\u00e1n"],
  PPP: ["Proserpine Whitsunday Coast Airport", "Proserpine"],
  PPQ: ["Paraparaumu Airport"],
  PPR: ["Pasir Pangaraan Airport", "Pasir Pengarayan-Sumatra Island"],
  PPS: ["Puerto Princesa Airport", "Puerto Princesa City"],
  PPT: ["Faa'a International Airport", "Papeete"],
  PPU: ["Hpapun Airport", "Pa Pun"],
  PPV: ["Port Protection Seaplane Base", "Port Protection"],
  PPW: ["Papa Westray Airport", "Papa Westray"],
  PPX: ["Param Airport", "Nepesi"],
  PPY: ["Pouso Alegre Airport", "Pouso Alegre"],
  PPZ: ["Puerto Paez Airport", "Puerto Paez"],
  PQC: ["Phu Quoc International Airport", "Phu Quoc Island"],
  PQD: ["Passikudah Helipad", "Batticaloa"],
  PQI: ["Northern Maine Regional Airport at Presque Isle", "Presque Isle"],
  PQM: ["Palenque International Airport"],
  PQQ: ["Port Macquarie Airport", "Port Macquarie"],
  PQS: ["Pilot Station Airport", "Pilot Station"],
  PRA: ["General Urquiza Airport", "Parana"],
  PRB: ["Paso Robles Municipal Airport", "Paso Robles"],
  PRC: ["Ernest A. Love Field", "Prescott"],
  PRD: ["Pardoo Airport", "Pardoo"],
  PRE: ["Pore Airport", "Pore"],
  PRG: ["V\u00e1clav Havel Airport Prague", "Prague"],
  PRH: ["Phrae Airport"],
  PRI: ["Praslin Airport", "Praslin Island"],
  PRK: ["Prieska Airport", "Prieska"],
  PRM: ["Portim\u00e3o Airport", "Portim\u00e3o"],
  PRN: ["Pri\u0161tina International Airport", "Prishtina"],
  PRO: ["Perry Municipal Airport", "Perry"],
  PRP: ["Propriano Airport", "Propriano"],
  PRQ: ["Termal Airport", "Presidencia Roque S\u00e1enz Pe\u00f1a"],
  PRR: ["Paruma Airport", "Paruma"],
  PRS: ["Parasi Airport", "Parasi"],
  PRU: ["Pyay Airport", "Pye"],
  PRV: ["P\u0159erov Air Base", "P\u0159erov"],
  PRW: ["Prentice Airport", "Prentice"],
  PRX: ["Cox Field", "Paris"],
  PRY: ["Wonderboom Airport", "Pretoria"],
  PRZ: ["Prineville Airport", "Prineville"],
  PSA: ["Pisa International Airport", "Pisa"],
  PSC: ["Tri Cities Airport", "Pasco"],
  PSD: ["Port Said Airport", "Port Said"],
  PSE: ["Mercedita Airport", "Ponce"],
  PSF: ["Pittsfield Municipal Airport", "Pittsfield"],
  PSG: ["Petersburg James A Johnson Airport", "Petersburg"],
  PSH: ["St. Peter-Ording Airport", "Sankt Peter-Ording"],
  PSI: ["Pasni Airport", "Pasni"],
  PSJ: ["Kasiguncu Airport", "Poso-Celebes Island"],
  PSK: ["New River Valley Airport", "Dublin"],
  PSL: ["Perth/Scone Airport", "Perth"],
  PSM: ["Portsmouth International at Pease Airport", "Portsmouth"],
  PSN: ["Palestine Municipal Airport", "Palestine"],
  PSO: ["Antonio Narino Airport", "Pasto"],
  PSP: ["Palm Springs International Airport", "Palm Springs"],
  PSQ: ["Philadelphia Seaplane Base", "Essington"],
  PSR: ["Pescara International Airport", "Pescara"],
  PSS: ["Libertador Gral D Jose De San Martin Airport", "Posadas"],
  PST: ["Preston Airport", "Preston"],
  PSU: ["Pangsuma Airport", "Putussibau-Borneo Island"],
  PSV: ["Papa Stour Airport", "Papa Stour Island"],
  PSW: ["Municipal Jos\u00e9 Figueiredo Airport", "Passos"],
  PSX: ["Palacios Municipal Airport", "Palacios"],
  PSY: ["Port Stanley Airport", "Stanley"],
  PSZ: ["Capit\u00e1n Av. Salvador Ogaya G. airport", "Puerto Su\u00e1rez"],
  PTA: ["Port Alsworth Airport", "Port Alsworth"],
  PTB: ["Dinwiddie County Airport", "Petersburg"],
  PTC: ["Port Alice Seaplane Base", "Port Alice"],
  PTD: ["Port Alexander Seaplane Base", "Port Alexander"],
  PTF: ["Malolo Lailai Island Airport", "Malolo Lailai Island"],
  PTG: ["Polokwane International Airport", "Polokwane"],
  PTH: ["Port Heiden Airport", "Port Heiden"],
  PTJ: ["Portland Airport"],
  PTK: ["Oakland County International Airport", "Pontiac"],
  PTM: ["Palmarito Airport", "Palmarito"],
  PTN: ["Harry P Williams Memorial Airport", "Patterson"],
  PTO: ["Pato Branco Airport", "Pato Branco"],
  PTP: ["Pointe-\u00e0-Pitre Le Raizet", "Pointe-\u00e0-Pitre"],
  PTQ: ["Porto de Moz Airport", "Porto De Moz"],
  PTT: ["Pratt Regional Airport", "Pratt"],
  PTU: ["Platinum Airport", "Platinum"],
  PTV: ["Porterville Municipal Airport", "Porterville"],
  PTW: ["Heritage Field", "Pottstown"],
  PTX: ["Pitalito Airport", "Pitalito"],
  PTY: ["Tocumen International Airport", "Tocumen"],
  PTZ: ["Rio Amazonas Airport", "Shell Mera"],
  PUA: ["Puas Airport", "Puas Mission"],
  PUB: ["Pueblo Memorial Airport", "Pueblo"],
  PUC: ["Carbon County Regional/Buck Davis Field", "Price"],
  PUD: ["Puerto Deseado Airport", "Puerto Deseado"],
  PUE: ["Puerto Obaldia Airport", "Puerto Obaldia"],
  PUF: ["Pau Pyr\u00e9n\u00e9es Airport", "Pau/Pyr\u00e9n\u00e9es (Uzein)"],
  PUG: ["Port Augusta Airport"],
  PUI: ["Pureni Airport", "Pureni"],
  PUJ: ["Punta Cana International Airport", "Punta Cana"],
  PUK: ["Pukarua Airport", "Pukarua"],
  PUL: ["Port of Poulsbo Marina Moorage Seaplane Base", "Poulsbo"],
  PUM: ["Pomala Airport", "Kolaka"],
  PUN: ["Punia Airport", "Punia"],
  PUP: ["Po Airport", "Po"],
  PUQ: ["Pdte. Carlos Iba\u00f1ez del Campo Airport", "Punta Arenas"],
  PUR: ["Puerto Rico Airport", "Puerto Rico/Manuripi"],
  PUS: ["Gimhae International Airport", "Busan"],
  PUT: ["Sri Sathya Sai Airport", "Puttaparthi"],
  PUU: ["Tres De Mayo Airport", "Puerto As\u00eds"],
  PUV: ["Poum Airport", "Poum"],
  PUW: ["Pullman Moscow Regional Airport", "Pullman/Moscow"],
  PUX: ["El Mirador Airport", "Puerto Varas"],
  PUY: ["Pula Airport", "Pula"],
  PUZ: ["Puerto Cabezas Airport", "Puerto Cabezas"],
  PVA: ["El Embrujo Airport", "Providencia"],
  PVC: ["Provincetown Municipal Airport", "Provincetown"],
  PVD: ["Theodore Francis Green State Airport", "Providence"],
  PVE: ["El Porvenir Airport", "El Porvenir"],
  PVF: ["Placerville Airport", "Placerville"],
  PVG: ["Shanghai Pudong International Airport", "Shanghai"],
  PVH: ["Governador Jorge Teixeira de Oliveira Airport", "Porto Velho"],
  PVI: ["Paranava\u00ed Airport", "Paranava\u00ed"],
  PVK: ["Aktion National Airport", "Preveza/Lefkada"],
  PVL: ["Pike County-Hatcher Field", "Pikeville"],
  PVO: ["Reales Tamarindos Airport", "Portoviejo"],
  PVR: [
    "Licenciado Gustavo D\u00edaz Ordaz International Airport",
    "Puerto Vallarta",
  ],
  PVS: ["Provideniya Bay Airport", "Chukotka"],
  PVU: ["Provo Municipal Airport", "Provo"],
  PVW: ["Hale County Airport", "Plainview"],
  PVZ: ["Casement Airport", "Plainesville"],
  PWA: ["Wiley Post Airport", "Oklahoma City"],
  PWD: ["Sher-Wood Airport", "Plentywood"],
  PWE: ["Pevek Airport", "Pevek"],
  PWI: ["Beles Airport", "Pawe"],
  PWK: ["Chicago Executive Airport", "Chicago/Prospect Heights/Wheeling"],
  PWL: ["Purwokerto Airport", "Purwokerto-Java Island"],
  PWM: ["Portland International Jetport Airport", "Portland"],
  PWN: ["Pitts Town Airport", "Pitts Town"],
  PWO: ["Pweto Airport", "Pweto"],
  PWQ: ["Pavlodar Airport", "Pavlodar"],
  PWR: ["Port Walter Seaplane Base", "Port Walter"],
  PWT: ["Bremerton National Airport", "Bremerton"],
  PWY: ["Ralph Wenz Field", "Pinedale"],
  PXH: ["Prominent Hill Airport", "OZ Minerals Prominent Hill Mine"],
  PXL: ["Polacca Airport", "Polacca"],
  PXM: ["Puerto Escondido International Airport", "Puerto Escondido"],
  PXO: ["Porto Santo Airport", "Vila Baleira"],
  PXR: ["Surin Airport", "Surin"],
  PXU: ["Pleiku Airport", "Pleiku"],
  PYA: ["Vel\u00e1squez Airport", "Puerto Boyac\u00e1"],
  PYB: ["Jeypore Airport", "Jeypore"],
  PYC: ["Play\u00f3n Chico Airport", "Play\u00f3n Chico"],
  PYE: ["Tongareva Airport", "Penrhyn Island"],
  PYH: ["Cacique Aramare Airport", "Puerto Ayacucho"],
  PYJ: ["Polyarny Airport", "Yakutia"],
  PYK: ["Payam International Airport", "Karaj"],
  PYL: ["Perry Island Seaplane Base", "Perry Island"],
  PYM: ["Plymouth Municipal Airport", "Plymouth"],
  PYO: ["Putumayo Airport", "Puerto Putumayo"],
  PYR: ["Andravida Air Base", "Andravida"],
  PYV: ["Yaviza Airport", "Yaviza"],
  PYX: ["Pattaya Airpark", "Pattaya"],
  PYY: ["Mae Hong Son Airport", "Mae Hong Son"],
  PZA: ["Paz De Ariporo Airport", "Paz De Ariporo"],
  PZB: ["Pietermaritzburg Airport", "Pietermaritzburg"],
  PZE: ["Penzance Heliport", "Penzance"],
  PZH: ["Zhob Airport", "Fort Sandeman"],
  PZI: ["Bao'anying Airport", "Panzhihua"],
  PZK: ["Pukapuka Island Airport", "Pukapuka Atoll"],
  PZL: ["Zulu Inyala Airport", "Phinda"],
  PZO: [
    "General Manuel Carlos Piar International Airport",
    "Puerto Ordaz-Ciudad Guayana",
  ],
  PZS: ["Maquehue Airport", "Temuco"],
  PZU: ["Port Sudan New International Airport", "Port Sudan"],
  PZY: ["Pie\u0161\u0165any Airport", "Pie\u0161\u0165any"],
  QAC: ["Castro Airport", "Castro"],
  QAQ: ["L'Aquila\u2013Preturo Airport", "L'Aquila"],
  QBC: ["Bella Coola Airport", "Bella Coola"],
  QCH: ["Colatina Airport", "Colatina"],
  QCJ: ["Botucatu - Tancredo de Almeida Neves Airport", "Botucatu"],
  QCN: ["Canela Airport", "Canela"],
  QCO: ["Col\u00f3n Airport", "Col\u00f3n"],
  QCP: ["Currais Novos Airport", "Currais Novos"],
  QCR: ["Curitibanos Airport", "Curitibanos"],
  QCU: ["Akunaq Heliport"],
  QCY: ["RAF Coningsby", "Coningsby"],
  QDB: ["Cachoeira do Sul Airport", "Cachoeira Do Sul"],
  QDC: ["Dracena Airport", "Dracena"],
  QDF: ["Conselheiro Lafaiete Airport", "Conselheiro Lafaiete"],
  QDJ: ["Tsletsi Airport", "Djelfa"],
  QDY: ["Taean Airport", "Taean"],
  QEF: ["Frankfurt-Egelsbach Airport", "Frankfurt am Main"],
  QEO: ["Bielsko-Bialo Kaniow Airfield", "Czechowice-Dziedzice"],
  QFG: ["Eqalugaarsuit Heliport", "Eqalugaarsuit"],
  QFI: ["Iginniarfik", "Iginniarfik"],
  QFO: ["Duxford Aerodrome", "Duxford"],
  QFX: ["Igaliku Heliport", "Igaliku"],
  QGA: ["Gua\u00edra Airport", "Gua\u00edra"],
  QGB: ["Limeira Airport", "Limeira"],
  QGC: ["Len\u00e7\u00f3is Paulista Airport", "Len\u00e7\u00f3is Paulista"],
  QGF: ["Montenegro Airport", "Montenegro"],
  QGP: ["Garanhuns Airport", "Garanhuns"],
  QGQ: ["Attu Heliport", "Attu"],
  QGS: ["Alagoinhas Airport", "Alagoinhas"],
  QGU: ["Gifu Airport", "Gifu"],
  QHB: ["Piracicaba Airport", "Piracicaba"],
  QHN: ["Taguatinga Airport", "Taguatinga"],
  QHP: ["Base de Avia\u00e7\u00e3o de Taubat\u00e9 Airport", "Taubat\u00e9"],
  QHR: ["Harar Meda Airport", "Debre Zeyit"],
  QHU: ["Husum-Schwesing Airport", "Husum"],
  QHV: ["Novo Hamburgo Airport", "Novo Hamburgo"],
  QID: ["M\u00e9lio Viana Airport", "Tr\u00eas Cora\u00e7\u00f5es"],
  QIG: ["Iguatu Airport", "Iguatu"],
  QIQ: ["Rio Claro Airport", "Rio Claro"],
  QIT: ["Itapetinga Airport", "Itapetinga"],
  QJB: ["Jubail Airport", "Jubail"],
  QJD: ["Jindabyne Airport"],
  QJE: ["Kitsissuarsuit Heliport", "Kitsissuarsuit"],
  QJI: ["Ikamiut Heliport", "Ikamiut"],
  QJP: ["Pocheon G 217 Airport", "Pocheon"],
  QKX: ["Kautokeino Air Base"],
  QLA: ["Lasham Airport", "Lasham"],
  QLD: ["Blida Airport"],
  QLF: ["Lahti Vesivehmaa Airport"],
  QLP: ["Sarzana-Luni Air Base", "Sarzana (SP)"],
  QLR: ["Monte Real Air Base"],
  QLS: ["Lausanne-Bl\u00e9cherette Airport", "Lausanne"],
  QLT: ["Latina Air Base", "Latina"],
  QMF: ["Mafra Airport", "Mafra"],
  QMH: ["Oum el Bouaghi airport", "Oum El Bouaghi"],
  QNC: ["Neuchatel Airport"],
  QND: ["Cenej Airport", "Novi Sad"],
  QNJ: ["Annemasse Airport", "Annemasse"],
  QNV: ["Aeroclube Airport", "Nova Igua\u00e7u"],
  QNY: ["New York Skyports Inc Seaplane Base", "New York"],
  QOA: ["Mococa Airport", "Mococa"],
  QOJ: ["S\u00e3o Borja Airport", "S\u00e3o Borja"],
  QOQ: ["Saarloq Heliport"],
  QOW: ["Sam Mbakwe International Airport", "Owerri"],
  QPA: ["Padova Airport", "Padova"],
  QPD: ["Pinar Del Rio Airport", "Pinar del Rio"],
  QPG: ["Paya Lebar Air Base"],
  QPH: ["Palapye Airport", "Palapye"],
  QPS: ["Campo Fontenelle Airport", "Pirassununga"],
  QPW: ["Kangaatsiaq Heliport", "Kangaatsiaq"],
  QQT: ["Qeqertaq Heliport", "Qeqertaq"],
  QRA: ["Rand Airport", "Johannesburg"],
  QRC: ["De La Independencia Airport", "Rancagua"],
  QRF: ["Bragado Airport", "Bragado"],
  QRM: ["Narromine Airport"],
  QRO: ["Quer\u00e9taro Intercontinental Airport", "Quer\u00e9taro"],
  QRR: ["Warren Airport"],
  QRT: ["Rieti Airport", "Rieti"],
  QRW: ["Warri Airport", "Warri"],
  QRY: ["Ikerassaarsuk Heliport", "Ikerassaarsuk"],
  QSA: ["Sabadell Airport", "Sabadell"],
  QSC: [
    "M\u00e1rio Pereira Lopes\u2013S\u00e3o Carlos Airport",
    "S\u00e3o Carlos",
  ],
  QSF: ["Ain Arnat Airport", "S\u00e9tif"],
  QSI: ["Moshi Airport", "Moshi"],
  QSN: ["San Nicolas De Bari Airport", "San Nicol\u00e1s"],
  QSR: ["Salerno Costa d'Amalfi Airport", "Salerno"],
  QSX: ["New Amsterdam Airport", "New Amsterdam"],
  QSZ: ["Yeerqiang Airport", "Shache"],
  QUB: ["Ubari Airport", "Ubari"],
  QUG: ["Chichester/Goodwood Airport", "Chichester"],
  QUN: ["Camp Page Airport (A-306)", "Chun Chon City"],
  QUO: ["Akwa Ibom International Airport", "Uyo"],
  QUY: ["RAF Wyton", "St. Ives"],
  QVR: ["Volta Redonda Airport", "Volta Redonda"],
  QWG: ["Wilgrove Air Park", "Charlotte"],
  QXH: ["Sch\u00f6nhagen Airport", "Sch\u00f6nhagen"],
  QYD: ["Oksywie Military Air Base", "Gdynia"],
  RAA: ["Rakanda Airport", "Rakanda"],
  RAB: ["Tokua Airport", "Tokua"],
  RAC: ["John H Batten Airport", "Racine"],
  RAE: ["Arar Domestic Airport", "Arar"],
  RAF: ["Rafaela Airport", "Rafaela"],
  RAG: ["Raglan Airfield"],
  RAH: ["Rafha Domestic Airport", "Rafha"],
  RAI: ["Praia International Airport", "Praia"],
  RAJ: ["Rajkot Airport", "Rajkot"],
  RAK: ["Menara Airport", "Marrakech"],
  RAL: ["Riverside Municipal Airport", "Riverside"],
  RAM: ["Ramingining Airport"],
  RAN: ["Ravenna Airport", "Ravenna"],
  RAO: ["Leite Lopes Airport", "Ribeir\u00e3o Preto"],
  RAP: ["Rapid City Regional Airport", "Rapid City"],
  RAQ: ["Sugimanuru Airport", "Raha-Muna Island"],
  RAR: ["Rarotonga International Airport", "Avarua"],
  RAS: ["Sardar-e-Jangal Airport", "Rasht"],
  RAT: ["Raduzhny Airport", "Raduzhnyi"],
  RAV: ["Cravo Norte Airport", "Cravo Norte"],
  RAW: ["Arawa Airport", "Arawa"],
  RAX: ["Oram Airport"],
  RAZ: ["Rawalakot Airport", "Rawalakot"],
  RBA: ["Rabat-Sal\u00e9 Airport", "Rabat"],
  RBB: ["Borba Airport", "Borba"],
  RBC: ["Robinvale Airport"],
  RBD: ["Dallas Executive Airport", "Dallas"],
  RBE: ["Ratanakiri Airport", "Ratanakiri"],
  RBF: ["Big Bear City Airport", "Big Bear"],
  RBG: ["Roseburg Regional Airport", "Roseburg"],
  RBI: ["Rabi Island Airport", "Rabi Island"],
  RBJ: ["Rebun Airport", "Rebun"],
  RBK: ["French Valley Airport", "Murrieta/Temecula"],
  RBL: ["Red Bluff Municipal Airport", "Red Bluff"],
  RBM: ["Straubing Airport", "Straubing"],
  RBO: ["Robor\u00e9 Airport", "Robor\u00e9"],
  RBP: ["Raba Raba Airport", "Rabaraba"],
  RBQ: ["Rurenabaque Airport", "Rurenabaque"],
  RBR: ["Pl\u00e1cido de Castro Airport", "Rio Branco"],
  RBS: ["Orbost Airport"],
  RBT: ["Marsabit Airport", "Marsabit"],
  RBU: ["Roebourne Airport", "Roebourne"],
  RBV: ["Ramata Airport", "Ramata"],
  RBW: ["Lowcountry Regional Airport", "Walterboro"],
  RBX: ["Rumbek Airport", "Rumbek"],
  RBY: ["Ruby Airport", "Ruby"],
  RCA: ["Ellsworth Air Force Base", "Rapid City"],
  RCB: ["Richards Bay Airport", "Richards Bay"],
  RCE: ["Roche Harbor Airport", "Roche Harbor"],
  RCH: ["Almirante Padilla Airport", "Riohacha"],
  RCK: ["H H Coffield Regional Airport", "Rockdale"],
  RCL: ["Redcliffe Airport", "Redcliffe"],
  RCM: ["Richmond Airport"],
  RCN: ["American River Airport", "American River"],
  RCO: ["Rochefort-Saint-Agnant (BA 721) Airport", "Rochefort/Saint-Agnant"],
  RCQ: ["Reconquista Airport", "Reconquista"],
  RCR: ["Fulton County Airport", "Rochester"],
  RCS: ["Rochester Airport", "Rochester"],
  RCT: ["Nartron Field", "Reed City"],
  RCU: ["Area De Material Airport", "Rio Cuarto"],
  RCY: ["Rum Cay Airport"],
  RDA: ["Rockhampton Downs Airport", "Rockhampton Downs"],
  RDB: ["Red Dog Airport", "Red Dog"],
  RDC: ["Reden\u00e7\u00e3o Airport", "Reden\u00e7\u00e3o"],
  RDD: ["Redding Municipal Airport", "Redding"],
  RDE: ["Merdei Airport", "Merdei-Papua Island"],
  RDG: ["Reading Regional Carl A Spaatz Field", "Reading"],
  RDM: ["Roberts Field", "Redmond"],
  RDN: ["LTS Pulau Redang Airport", "Redang"],
  RDO: ["Radom Airport", "Radom"],
  RDP: ["Kazi Nazrul Islam Airport", "Durgapur"],
  RDR: ["Grand Forks Air Force Base", "Grand Forks"],
  RDS: ["Rincon De Los Sauces Airport", "Rincon de los Sauces"],
  RDT: ["Richard Toll Airport", "Richard Toll"],
  RDU: ["Raleigh Durham International Airport", "Raleigh/Durham"],
  RDV: ["Red Devil Airport", "Red Devil"],
  RDZ: ["Rodez-Marcillac Airport", "Rodez/Marcillac"],
  REA: ["Reao Airport"],
  REB: ["Rechlin-L\u00e4rz Airport", "L\u00e4rz"],
  REC: ["Guararapes - Gilberto Freyre International Airport", "Recife"],
  RED: ["Mifflin County Airport", "Reedsville"],
  REE: ["Reese Airpark", "Lubbock"],
  REG: ["Reggio Calabria Airport", "Reggio Calabria"],
  REH: ["Rehoboth Airport", "Rehoboth Beach"],
  REI: ["Regina Airport", "Regina"],
  REL: ["Almirante Marco Andres Zar Airport", "Rawson"],
  REN: ["Orenburg Central Airport", "Orenburg"],
  REO: ["Rome State Airport", "Rome"],
  REP: ["Siem Reap International Airport", "Siem Reap"],
  REQ: ["Reko Diq Airport", "Chagai"],
  RER: ["Retalhuleu Airport", "Retalhuleu"],
  RES: ["Resistencia International Airport", "Resistencia"],
  RET: ["R\u00f8st Airport"],
  REU: ["Reus Air Base", "Reus"],
  REW: ["Rewa Airport, Chorhata, REWA", "Rewa"],
  REX: ["General Lucio Blanco International Airport", "Reynosa"],
  REY: ["Reyes Airport", "Reyes"],
  REZ: ["Resende Airport", "Resende"],
  RFA: ["Rafa\u00ef Airport", "Rafa\u00ef"],
  RFD: ["Chicago Rockford International Airport", "Chicago/Rockford"],
  RFN: ["Raufarh\u00f6fn Airport", "Raufarh\u00f6fn"],
  RFP: ["Raiatea Airport", "Uturoa"],
  RFR: ["Rio Frio / Progreso Airport", "Rio Frio / Progreso"],
  RFS: ["Rosita Airport", "La Rosita"],
  RGA: ["Hermes Quijada International Airport", "Rio Grande"],
  RGE: ["Porgera Airport", "Porgera"],
  RGH: ["Balurghat Airport", "Balurghat"],
  RGI: ["Rangiroa Airport"],
  RGK: ["Gorno-Altaysk Airport", "Gorno-Altaysk"],
  RGL: ["Piloto Civil N. Fern\u00e1ndez Airport", "Rio Gallegos"],
  RGN: ["Yangon International Airport", "Yangon"],
  RGO: ["Orang Airport", "Hoemun-ri"],
  RGR: ["Ranger Municipal Airport", "Ranger"],
  RGS: ["Burgos Airport", "Burgos"],
  RGT: ["Japura Airport", "Rengat-Sumatra Island"],
  RHA: ["Reykh\u00f3lar Airport", "Reykh\u00f3lar"],
  RHD: [
    "Termas de R\u00edo Hondo international Airport",
    "Termas de R\u00edo Hondo",
  ],
  RHE: ["Reims-Champagne (BA 112) Air Base", "Reims/Champagne"],
  RHG: ["Ruhengeri Airport", "Ruhengeri"],
  RHI: ["Rhinelander Oneida County Airport", "Rhinelander"],
  RHL: ["Roy Hill Station Airport"],
  RHN: ["Skorpion Mine Airport", "Rosh Pinah"],
  RHO: ["Diagoras Airport", "Rodes Island"],
  RHP: ["Ramechhap Airport", "Ramechhap"],
  RHT: ["Alxa Right Banner Badanjilin Airport", "Badanjilin"],
  RHV: ["Reid-Hillview Airport of Santa Clara County", "San Jose"],
  RIA: ["Santa Maria Airport", "Santa Maria"],
  RIB: ["Capit\u00e1n Av. Selin Zeitun Lopez Airport", "Riberalta"],
  RIC: ["Richmond International Airport", "Richmond"],
  RIE: ["hln", "Rice Lake"],
  RIG: ["Rio Grande Airport", "Rio Grande"],
  RIH: ["Scarlett Martinez International Airport", "R\u00edo Hato"],
  RIJ: ["Juan Simons Vela Airport", "Rioja"],
  RIL: ["Garfield County Regional Airport", "Rifle"],
  RIM: ["San Nicolas Airport", "Rodriguez de Mendoza"],
  RIN: ["Ringi Cove Airport", "Ringi Cove"],
  RIS: ["Rishiri Airport", "Rishiri"],
  RIV: ["March ARB Airport", "Riverside"],
  RIW: ["Riverton Regional Airport", "Riverton"],
  RIX: ["Riga International Airport", "Riga"],
  RIY: ["Mukalla International Airport", "Riyan"],
  RIZ: ["Rizhao Shanzihe Airport", "Rizhao"],
  RJA: ["Rajahmundry Airport", "Rajahmundry"],
  RJB: ["Rajbiraj Airport", "Rajbiraj"],
  RJH: ["Shah Mokhdum Airport", "Rajshahi"],
  RJI: ["Rajouri Airport", "Rajouri"],
  RJK: ["Rijeka Airport", "Rijeka"],
  RJL: ["Logro\u00f1o-Agoncillo Airport", "Logro\u00f1o"],
  RJN: ["Rafsanjan Airport"],
  RKA: ["Aratika Nord Airport"],
  RKD: ["Knox County Regional Airport", "Rockland"],
  RKE: ["Copenhagen Roskilde Airport", "Copenhagen"],
  RKH: ["Rock Hill - York County Airport", "Rock Hill"],
  RKI: ["Rokot Airport", "Rokot-Sumatra Island"],
  RKO: ["Rokot Airport", "Sipora Island"],
  RKP: ["Aransas County Airport", "Rockport"],
  RKS: ["Southwest Wyoming Regional Airport", "Rock Springs"],
  RKT: ["Ras Al Khaimah International Airport", "Ras Al Khaimah"],
  RKU: ["Kairuku Airport", "Yule Island"],
  RKV: ["Reykjavik Airport", "Reykjavik"],
  RKW: ["Rockwood Municipal Airport", "Rockwood"],
  RKY: ["Rokeby Airport", "Rokeby"],
  RKZ: ["Shigatse Air Base", "Xigaz\u00ea"],
  RLA: ["Rolla Downtown Airport", "Rolla"],
  RLG: ["Rostock-Laage Airport", "Rostock"],
  RLK: ["Bayannur Tianjitai Airport", "Bavannur"],
  RLO: ["Valle Del Conlara International Airport", "Merlo"],
  RLP: ["Rosella Plains Airport", "Rosella Plains"],
  RLT: ["Arlit Airport", "Arlit"],
  RMA: ["Roma Airport", "Roma"],
  RMB: ["Buraimi Airport", "Buraimi"],
  RMD: ["Basanth Nagar Airport", "Ramagundam"],
  RME: ["Griffiss International Airport", "Rome"],
  RMF: ["Marsa Alam International Airport", "Marsa Alam"],
  RMG: ["Richard B Russell Airport", "Rome"],
  RMI: ["Federico Fellini International Airport", "Rimini"],
  RMK: ["Renmark Airport"],
  RML: ["Colombo Ratmalana Airport", "Colombo"],
  RMN: ["Rumginae Airport"],
  RMP: ["Rampart Airport", "Rampart"],
  RMQ: ["Taichung Ching Chuang Kang Airport", "Taichung City"],
  RMS: ["Ramstein Air Base", "Ramstein"],
  RMT: ["Rimatara Airport", "Rimatara Island"],
  RMU: ["Regi\u00f3n de Murcia International Airport", "Corvera"],
  RMY: ["Mariposa Yosemite Airport", "Mariposa"],
  RNA: ["Ulawa Airport", "Arona"],
  RNB: ["Ronneby Airport"],
  RNC: ["Warren County Memorial Airport", "Mc Minnville"],
  RND: ["Randolph Air Force Base", "Universal City"],
  RNE: ["Roanne-Renaison Airport", "Roanne/Renaison"],
  RNI: ["Corn Island", "Corn Island"],
  RNJ: ["Yoron Airport", "Yoronjima"],
  RNL: ["Rennell/Tingoa Airport", "Rennell Island"],
  RNM: ["Qarn Alam Airport", "Ghaba"],
  RNN: ["Bornholm Airport", "R\u00f8nne"],
  RNO: ["Reno Tahoe International Airport", "Reno"],
  RNP: ["Rongelap Island Airport", "Rongelap Island"],
  RNR: ["Robinson River Airport", "Robinson River"],
  RNS: ["Rennes-Saint-Jacques Airport", "Rennes/Saint-Jacques"],
  RNT: ["Renton Municipal Airport", "Renton"],
  RNU: ["Ranau Airport", "Ranau"],
  RNZ: ["Jasper County Airport", "Rensselaer"],
  ROA: ["Roanoke\u2013Blacksburg Regional Airport", "Roanoke"],
  ROB: ["Roberts International Airport", "Monrovia"],
  ROC: ["Greater Rochester International Airport", "Rochester"],
  ROD: ["Robertson Airport", "Robertson"],
  ROF: ["Montague-Yreka Rohrer Field", "Montague"],
  ROG: ["Rogers Municipal Airport-Carter Field", "Rogers"],
  ROH: ["Robinhood Airport"],
  ROI: ["Roi Et Airport"],
  ROK: ["Rockhampton Airport", "Rockhampton"],
  ROL: ["Roosevelt Municipal Airport", "Roosevelt"],
  ROO: ["Maestro Marinho Franco Airport", "Rondon\u00f3polis"],
  ROP: ["Rota International Airport", "Rota Island"],
  ROR: ["Babelthuap Airport", "Babelthuap Island"],
  ROS: ["Islas Malvinas Airport", "Rosario"],
  ROT: ["Rotorua Regional Airport", "Rotorua"],
  ROU: ["Ruse Airport", "Ruse"],
  ROV: ["Platov International Airport", "Rostov-on-Don"],
  ROW: ["Roswell International Air Center Airport", "Roswell"],
  ROX: ["Roseau Municipal Rudy Billberg Field", "Roseau"],
  ROY: ["Rio Mayo Airport", "Rio Mayo"],
  ROZ: ["Rota Naval Station Airport", "Rota"],
  RPA: ["Rolpa Airport", "Rolpa"],
  RPB: ["Roper Bar Airport", "Roper Bar"],
  RPM: ["Ngukurr Airport"],
  RPN: ["Ben Ya'akov Airport", "Rosh Pina"],
  RPR: ["Raipur Airport", "Raipur"],
  RPV: ["Roper Valley Airport", "Roper Valley"],
  RPX: ["Roundup Airport", "Roundup"],
  RQO: ["El Reno Regional Airport", "El Reno"],
  RQW: ["Qayyarah West Airport", "Qayyarah"],
  RRE: ["Marree Airport"],
  RRG: ["Sir Charles Gaetan Duval Airport", "Port Mathurin"],
  RRK: ["Rourkela Airport"],
  RRL: ["Merrill Municipal Airport", "Merrill"],
  RRM: ["Marromeu Airport", "Marromeu"],
  RRR: ["Raroia Airport"],
  RRS: ["R\u00f8ros Airport", "R\u00f8ros"],
  RRT: ["Warroad International Memorial Airport", "Warroad"],
  RRV: ["Robinson River Airport"],
  RSA: ["Santa Rosa Airport", "Santa Rosa"],
  RSB: ["Roseberth Airport"],
  RSD: ["Rock Sound Airport", "Rock Sound"],
  RSE: ["Rose Bay Seaplane Base", "Sydney"],
  RSH: ["Russian Mission Airport", "Russian Mission"],
  RSJ: ["Rosario Seaplane Base", "Rosario"],
  RSK: ["Abresso Airport", "Ransiki-Papua Island"],
  RSL: ["Russell Municipal Airport", "Russell"],
  RSN: ["Ruston Regional Airport", "Ruston"],
  RSS: ["Damazin Airport", "Ad Damazin"],
  RST: ["Rochester International Airport", "Rochester"],
  RSU: ["Yeosu Airport", "Yeosu"],
  RSW: ["Southwest Florida International Airport", "Fort Myers"],
  RSX: ["Rouses Point Seaplane Base", "Rouses Point"],
  RTA: ["Rotuma Airport", "Rotuma"],
  RTB: ["Juan Manuel Galvez International Airport", "Roatan Island"],
  RTC: ["Ratnagiri Airport"],
  RTG: ["Frans Sales Lega Airport", "Satar Tacik-Flores Island"],
  RTI: ["David Constantijn Saudale Airport", "Ba'a - Rote Island"],
  RTL: ["Spirit Lake Municipal Airport", "Spirit Lake"],
  RTM: ["Rotterdam The Hague Airport", "Rotterdam"],
  RTN: ["Raton Municipal-Crews Field", "Raton"],
  RTP: ["Rutland Plains Airport"],
  RTS: ["Rottnest Island Airport"],
  RTW: ["Saratov Central Airport", "Saratov"],
  RTY: ["Merty Merty Airport"],
  RUA: ["Arua Airport", "Arua"],
  RUD: ["Shahroud Airport"],
  RUE: ["Rughenda Airfield", "Butembo"],
  RUF: ["Yuruf Airport", "Amgotro"],
  RUG: ["Rugao Air Base", "Rugao"],
  RUH: ["King Khaled International Airport", "Riyadh"],
  RUI: ["Sierra Blanca Regional Airport", "Ruidoso"],
  RUK: ["Rukum Chaurjahari Airport", "Rukumkot"],
  RUM: ["Rumjatar Airport", "Rumjatar"],
  RUN: ["Roland Garros Airport", "St Denis"],
  RUP: ["Rupsi India Airport"],
  RUR: ["Rurutu Airport"],
  RUS: ["Marau Airport", "Marau"],
  RUT: ["Rutland - Southern Vermont Regional Airport", "Rutland"],
  RUU: ["Ruti Airport", "Kawbenaberi"],
  RUV: ["Rubelsanto Airport", "Rubelsanto"],
  RUY: ["Cop\u00e1n Ruinas Airport", "Cop\u00e1n Ruinas"],
  RVA: ["Farafangana Airport"],
  RVC: ["River Cess Airport/Heliport", "River Cess"],
  RVD: ["General Leite de Castro Airport", "Rio Verde"],
  RVE: ["Los Colonizadores Airport", "Saravena"],
  RVH: ["Rzhevka Airport", "St. Petersburg"],
  RVI: ["Rostov-on-Don Airport", "Rostov-on-Don"],
  RVK: ["R\u00f8rvik Airport, Ryum", "R\u00f8rvik"],
  RVN: ["Rovaniemi Airport", "Rovaniemi"],
  RVO: ["Reivilo Airport", "Reivilo"],
  RVR: ["Green River Municipal Airport", "Green River"],
  RVS: ["Richard Lloyd Jones Jr Airport", "Tulsa"],
  RVT: ["Ravensthorpe Airport"],
  RVV: ["Raivavae Airport"],
  RVY: [
    "Presidente General Don Oscar D. Gestido International Airport",
    "Rivera",
  ],
  RWF: ["Redwood Falls Municipal Airport", "Redwood Falls"],
  RWI: ["Rocky Mount Wilson Regional Airport", "Rocky Mount"],
  RWL: ["Rawlins Municipal Airport/Harvey Field", "Rawlins"],
  RWN: ["Rivne International Airport", "Rivne"],
  RXE: ["Rexburg Madison County Airport", "Rexburg"],
  RXS: ["Roxas Airport", "Roxas City"],
  RYB: ["Staroselye Airport", "Rybinsk"],
  RYG: ["Moss Airport, Rygge", "Oslo"],
  RYK: ["Shaikh Zaid Airport", "Rahim Yar Khan"],
  RYN: ["Royan-M\u00e9dis Airport", "Royan/M\u00e9dis"],
  RYO: ["28 de Noviembre Airport", "Rio Turbio"],
  RZA: ["Santa Cruz Airport", "Santa Cruz"],
  RZE: ["Rzesz\u00f3w-Jasionka Airport", "Rzesz\u00f3w"],
  RZH: ["Quartz Hill Airport", "Lancaster"],
  RZN: ["Turlatovo Airport", "Ryazan"],
  RZP: ["Cesar Lim Rodriguez Airport", "Taytay Airport, Sandoval Airport"],
  RZR: ["Ramsar Airport"],
  RZS: ["Sawan Airport", "Sawan"],
  RZZ: ["Halifax County Airport", "Roanoke Rapids"],
  SAA: ["Shively Field", "Saratoga"],
  SAB: ["Juancho E. Yrausquin Airport", "Saba"],
  SAC: ["Sacramento Executive Airport", "Sacramento"],
  SAD: ["Safford Regional Airport", "Safford"],
  SAE: ["Sangir Airport", "Sangir-Simbawa Island"],
  SAF: ["Santa Fe Municipal Airport", "Santa Fe"],
  SAG: ["Shirdi Airport", "Shirdi"],
  SAH: ["Sana'a International Airport", "Sana'a"],
  SAK: ["Sau\u00f0\u00e1rkr\u00f3kur Airport", "Sau\u00f0\u00e1rkr\u00f3kur"],
  SAL: [
    "Monse\u00f1or \u00d3scar Arnulfo Romero International Airport",
    "San Salvador (San Luis Talpa)",
  ],
  SAM: ["Salamo Airport", "Salamo"],
  SAN: ["San Diego International Airport", "San Diego"],
  SAP: ["Ram\u00f3n Villeda Morales International Airport", "San Pedro Sula"],
  SAQ: ["San Andros Airport", "Andros Island"],
  SAR: ["Sparta Community Hunter Field", "Sparta"],
  SAS: ["Salton Sea Airport", "Salton City"],
  SAT: ["San Antonio International Airport", "San Antonio"],
  SAU: ["Sabu-Tardanu Airport", "Sabu-Sawu Island"],
  SAV: ["Savannah Hilton Head International Airport", "Savannah"],
  SAW: ["Sabiha G\u00f6k\u00e7en International Airport", "Istanbul"],
  SAX: ["Sambu Airport", "Boca de S\u00e1balo"],
  SAY: ["Siena-Ampugnano Airport", "Siena"],
  SAZ: ["Sasstown Airport", "Sasstown"],
  SB2: ["Fazenda Bela Vista Airport", "3 Marias"],
  SBA: ["Santa Barbara Municipal Airport", "Santa Barbara"],
  SBB: ["Santa B\u00e1rbara de Barinas Airport", "Santa B\u00e1rbara"],
  SBC: ["Selbang Airport", "Selbang"],
  SBD: ["San Bernardino International Airport", "San Bernardino"],
  SBE: ["Suabi Airport"],
  SBF: ["Sardeh Band Airport", "Sardeh Band"],
  SBG: ["Maimun Saleh Airport", "Sabang-We Island"],
  SBH: ["Gustaf III Airport", "Gustavia"],
  SBI: ["Sambailo Airport", "Koundara"],
  SBJ: ["S\u00e3o Mateus Airport", "S\u00e3o Mateus"],
  SBK: ["Saint-Brieuc-Armor Airport", "Saint-Brieuc/Armor"],
  SBL: ["Santa Ana Del Yacuma Airport", "Santa Ana del Yacuma"],
  SBM: ["Sheboygan County Memorial Airport", "Sheboygan"],
  SBN: ["South Bend Regional Airport", "South Bend"],
  SBO: ["Salina Gunnison Airport", "Salina"],
  SBP: ["San Luis County Regional Airport", "San Luis Obispo"],
  SBQ: ["Sibi Airport", "Sibi"],
  SBR: ["Saibai Island Airport", "Saibai Island"],
  SBS: ["Steamboat Springs Bob Adams Field", "Steamboat Springs"],
  SBT: ["Sabetta International Airport", "Sabetta"],
  SBU: ["Springbok Airport", "Springbok"],
  SBV: ["Sabah Airport", "Sabah"],
  SBW: ["Sibu Airport", "Sibu"],
  SBX: ["Shelby Airport", "Shelby"],
  SBY: ["Salisbury Ocean City Wicomico Regional Airport", "Salisbury"],
  SBZ: ["Sibiu International Airport", "Sibiu"],
  SCB: ["Scribner State Airport", "Scribner"],
  SCC: ["Deadhorse Airport", "Deadhorse"],
  SCD: ["Sulaco Airport", "Sulaco"],
  SCE: ["University Park Airport", "State College"],
  SCF: ["Scottsdale Airport", "Scottsdale"],
  SCG: ["Spring Creek Airport"],
  SCH: ["Schenectady County Airport", "Schenectady"],
  SCI: ["Paramillo Airport"],
  SCK: ["Stockton Metropolitan Airport", "Stockton"],
  SCL: [
    "Comodoro Arturo Merino Ben\u00edtez International Airport",
    "Santiago",
  ],
  SCM: ["Scammon Bay Airport", "Scammon Bay"],
  SCN: ["Saarbr\u00fccken Airport", "Saarbr\u00fccken"],
  SCO: ["Aktau Airport", "Aktau"],
  SCP: ["Mont-Dauphin - St-Cr\u00e9pin Airport", "Mende"],
  SCQ: ["Santiago de Compostela Airport", "Santiago de Compostela"],
  SCR: ["Scranton Municipal Airport.", "Scranton"],
  SCS: ["Scatsta Airport", "Shetland Islands"],
  SCT: ["Socotra International Airport", "Socotra Islands"],
  SCU: ["Antonio Maceo International Airport", "Santiago"],
  SCV: ["Suceava Stefan cel Mare Airport", "Suceava"],
  SCW: ["Syktyvkar Airport", "Syktyvkar"],
  SCX: ["Salina Cruz Naval Air Station", "Salina Cruz"],
  SCY: ["San Crist\u00f3bal Airport", "San Crist\u00f3bal"],
  SCZ: [
    "Santa Cruz/Graciosa Bay/Luova Airport",
    "Santa Cruz/Graciosa Bay/Luova",
  ],
  SDB: ["Langebaanweg Airport", "Langebaanweg"],
  SDC: ["Sand Creek Airport", "Sand Creek"],
  SDD: ["Lubango Airport", "Lubango"],
  SDE: [
    "Vicecomodoro Angel D. La Paz Aragon\u00e9s Airport",
    "Santiago del Estero",
  ],
  SDF: ["Louisville International Standiford Field", "Louisville"],
  SDG: ["Sanandaj Airport"],
  SDH: ["Santa Rosa de Cop\u00e1n Airport", "Santa Rosa de Cop\u00e1n"],
  SDI: ["Saidor Airport", "Saidor"],
  SDJ: ["Sendai Airport", "Sendai"],
  SDK: ["Sandakan Airport", "Sandakan"],
  SDL: [
    "Sundsvall-H\u00e4rn\u00f6sand Airport",
    "Sundsvall/ H\u00e4rn\u00f6sand",
  ],
  SDM: ["Brown Field Municipal Airport", "San Diego"],
  SDN: ["Sandane Airport (Anda)", "Sandane"],
  SDP: ["Sand Point Airport", "Sand Point"],
  SDQ: ["Las Am\u00e9ricas International Airport", "Santo Domingo"],
  SDR: ["Santander Airport", "Santander"],
  SDS: ["Sado Airport", "Sado"],
  SDT: ["Saidu Sharif Airport", "Saidu Sharif"],
  SDU: ["Santos Dumont Airport", "Rio De Janeiro"],
  SDV: ["Sde Dov Airport", "Tel Aviv"],
  SDX: ["Sedona Airport", "Sedona"],
  SDY: ["Sidney - Richland Regional Airport", "Sidney"],
  SEA: ["Seattle Tacoma International Airport", "Seattle"],
  SEB: ["Sabha Airport", "Sabha"],
  SEE: ["Gillespie Field", "San Diego/El Cajon"],
  SEF: ["Sebring Regional Airport", "Sebring"],
  SEG: ["Penn Valley Airport", "Selinsgrove"],
  SEH: ["Senggeh Airport", "Senggeh"],
  SEK: ["Srednekolymsk Airport", "Srednekolymsk"],
  SEM: ["Craig Field", "Selma"],
  SEN: ["Southend Airport", "Southend"],
  SEO: ["Seguela Airport", "Seguela"],
  SEP: ["Stephenville Clark Regional Airport", "Stephenville"],
  SEQ: ["Sungai Pakning Bengkalis Airport", "Bengkalis-Sumatra Island"],
  SER: ["Freeman Municipal Airport", "Seymour"],
  SEU: ["Seronera Airport", "Seronera"],
  SEV: ["Sievierodonetsk Airport", "Sievierodonetsk"],
  SEW: ["Siwa Oasis North Airport", "Siwa"],
  SEX: ["Sembach Air Base", "Sembach"],
  SEY: ["S\u00e9libaby Airport", "S\u00e9libaby"],
  SEZ: ["Seychelles International Airport", "Mahe Island"],
  SFA: ["Sfax Thyna International Airport", "Sfax"],
  SFB: ["Orlando Sanford International Airport", "Orlando"],
  SFC: ["St-Fran\u00e7ois Airport", "St-Fran\u00e7ois"],
  SFD: ["San Fernando De Apure Airport", "Inglaterra"],
  SFE: ["San Fernando Airport"],
  SFF: ["Felts Field", "Spokane"],
  SFG: ["L'Esp\u00e9rance Airport", "Grand Case"],
  SFH: ["San Felipe International Airport", "Mexicali"],
  SFI: ["Safi Airport", "Safi"],
  SFJ: ["Kangerlussuaq Airport", "Kangerlussuaq"],
  SFK: ["Soure Airport", "Soure"],
  SFL: ["S\u00e3o Filipe Airport", "S\u00e3o Filipe"],
  SFM: ["Sanford Seacoast Regional Airport", "Sanford"],
  SFN: ["Sauce Viejo Airport", "Santa Fe"],
  SFO: ["San Francisco International Airport", "San Francisco"],
  SFQ: ["\u015eanl\u0131urfa Airport", "\u015eanl\u0131urfa"],
  SFR: ["San Fernando Airport", "Los Angeles"],
  SFS: ["Subic Bay International Airport", "Olongapo City"],
  SFT: ["Skellefte\u00e5 Airport", "Skellefte\u00e5"],
  SFU: ["Safia Airport", "Safia"],
  SFX: ["Macagua Airport", "Ciudad Guayana"],
  SFZ: ["North Central State Airport", "Pawtucket"],
  SGA: ["Sheghnan Airport", "Sheghnan"],
  SGC: ["Surgut Airport", "Surgut"],
  SGD: ["S\u00f8nderborg Airport", "S\u00f8nderborg"],
  SGE: ["Siegerland Airport"],
  SGF: ["Springfield Branson National Airport", "Springfield"],
  SGG: ["Simanggang Airport", "Sri Aman"],
  SGH: ["Springfield-Beckley Municipal Airport", "Springfield"],
  SGI: ["Mushaf Air Base", "Sargodha"],
  SGK: ["Sengapi Airstrip", "Sengapi"],
  SGL: ["Danilo Atienza Air Base", "Cavite City"],
  SGM: ["San Ignacio Airport", "San Ignacio"],
  SGN: ["Tan Son Nhat International Airport", "Ho Chi Minh City"],
  SGO: ["St George Airport"],
  SGP: ["Shay Gap Airport", "Shay Gap"],
  SGQ: ["Sanggata/Sangkimah Airport", "Sanggata/Sangkimah"],
  SGR: ["Sugar Land Regional Airport", "Houston"],
  SGT: ["Stuttgart Municipal Airport / Carl Humphrey Field", "Stuttgart"],
  SGU: ["St George Municipal Airport", "St George"],
  SGV: ["Sierra Grande Airport", "Sierra Grande"],
  SGW: ["Saginaw Seaplane Base", "Saginaw Bay"],
  SGX: ["Songea Airport", "Songea"],
  SGY: ["Skagway Airport", "Skagway"],
  SGZ: ["Songkhla Airport"],
  SHA: ["Shanghai Hongqiao International Airport", "Shanghai"],
  SHB: ["Nakashibetsu Airport", "Nakashibetsu"],
  SHC: ["Shire Inda Selassie Airport", "Shire Indasilase"],
  SHD: [
    "Shenandoah Valley Regional Airport",
    "Staunton/Waynesboro/Harrisonburg",
  ],
  SHE: ["Taoxian Airport", "Shenyang"],
  SHF: ["Huayuan Airport", "Shihezi"],
  SHG: ["Shungnak Airport", "Shungnak"],
  SHH: ["Shishmaref Airport", "Shishmaref"],
  SHI: ["Shimojishima Airport", "Shimojishima"],
  SHJ: ["Sharjah International Airport", "Sharjah"],
  SHK: ["Sehonghong Airport", "Sehonghong"],
  SHL: ["Shillong Airport", "Shillong"],
  SHM: ["Nanki Shirahama Airport", "Shirahama"],
  SHN: ["Sanderson Field", "Shelton"],
  SHO: ["King Mswati III International Airport", "Manzini"],
  SHQ: ["Southport Airport"],
  SHR: ["Sheridan County Airport", "Sheridan"],
  SHS: ["Shashi Airport", "Shashi"],
  SHT: ["Shepparton Airport"],
  SHU: ["Smith Point Airport"],
  SHV: ["Shreveport Regional Airport", "Shreveport"],
  SHW: ["Sharurah Airport"],
  SHX: ["Shageluk Airport", "Shageluk"],
  SHY: ["Shinyanga Airport", "Shinyanga"],
  SHZ: ["Seshutes Airport", "Seshutes"],
  SIA: ["Xi'an Xiguan Airport", "Xi'an"],
  SIB: ["Sibiti Airport", "Sibiti"],
  SIC: ["San Jos\u00e9 Island Airport", "Las Perlas"],
  SID: ["Am\u00edlcar Cabral International Airport", "Espargos"],
  SIE: ["Sines Airport"],
  SIF: ["Simara Airport", "Simara"],
  SIG: ["Fernando Luis Ribas Dominicci Airport", "San Juan"],
  SIH: ["Silgadi Doti Airport", "Silgadi Doti"],
  SII: ["Sidi Ifni Xx Airport", "Sidi Ifni"],
  SIJ: ["Siglufj\u00f6r\u00f0ur Airport", "Siglufj\u00f6r\u00f0ur"],
  SIK: ["Sikeston Memorial Municipal Airport", "Sikeston"],
  SIL: ["Sila Airport", "Sila Mission"],
  SIM: ["Simbai Airport", "Simbai"],
  SIN: ["Singapore Changi Airport", "Singapore"],
  SIO: ["Smithton Airport"],
  SIP: ["Simferopol International Airport", "Simferopol"],
  SIQ: ["Dabo Airport", "Pasirkuning-Singkep Island"],
  SIR: ["Sion Airport", "Sion"],
  SIS: ["Sishen Airport", "Sishen"],
  SIT: ["Sitka Rocky Gutierrez Airport", "Sitka"],
  SIU: ["Siuna", "Siuna"],
  SIV: ["Sullivan County Airport", "Monticello"],
  SIW: ["Sibisa Airport", "Parapat-Sumatra Island"],
  SIX: ["Singleton Airport", "Singleton"],
  SIZ: ["Sissano Airport", "Sissano"],
  SJA: ["San Juan de Marcona Airport", "San Juan de Marcona"],
  SJB: ["San Joaqu\u00edn Airport", "San Joaqu\u00edn"],
  SJC: ["Norman Y. Mineta San Jose International Airport", "San Jose"],
  SJD: ["Los Cabos International Airport", "San Jos\u00e9 del Cabo"],
  SJE: ["Jorge E. Gonzalez Torres Airport", "San Jos\u00e9 Del Guaviare"],
  SJF: ["Cruz Bay Seaplane Base", "Saint John Island"],
  SJH: ["San Juan Del C\u00e9sar Airport", "San Juan Del C\u00e9sar"],
  SJI: ["San Jose Airport", "San Jose"],
  SJJ: ["Sarajevo International Airport", "Sarajevo"],
  SJK: [
    "Professor Urbano Ernesto Stumpf Airport",
    "S\u00e3o Jos\u00e9 Dos Campos",
  ],
  SJL: [
    "S\u00e3o Gabriel da Cachoeira Airport",
    "S\u00e3o Gabriel Da Cachoeira",
  ],
  SJM: ["San Juan Airport", "San Juan"],
  SJN: ["St Johns Industrial Air Park", "St Johns"],
  SJO: ["Juan Santamaria International Airport", "San Jose"],
  SJP: [
    "Prof. Eribelto Manoel Reino State Airport",
    "S\u00e3o Jos\u00e9 Do Rio Preto",
  ],
  SJQ: ["Sesheke Airport", "Sesheke"],
  SJR: ["San Juan De Uraba Airport", "San Juan De Uraba"],
  SJS: ["San Jos\u00e9 De Chiquitos Airport", "San Jos\u00e9 de Chiquitos"],
  SJT: ["San Angelo Regional Mathis Field", "San Angelo"],
  SJU: ["Luis Munoz Marin International Airport", "San Juan"],
  SJV: ["San Javier Airport", "San Javier"],
  SJW: ["Shijiazhuang Daguocun International Airport", "Shijiazhuang"],
  SJX: ["Sartaneja Airport", "Sartaneja"],
  SJY: ["Sein\u00e4joki Airport", "Sein\u00e4joki / Ilmajoki"],
  SJZ: ["S\u00e3o Jorge Airport", "Velas"],
  SKA: ["Fairchild Air Force Base", "Spokane"],
  SKB: ["Robert L. Bradshaw International Airport", "Basseterre"],
  SKC: ["Suki Airport", "Suki"],
  SKD: ["Samarkand Airport", "Samarkand"],
  SKE: ["Skien Airport", "Geiteryggen"],
  SKF: ["Lackland Air Force Base", "San Antonio"],
  SKG: ["Thessaloniki Macedonia International Airport", "Thessaloniki"],
  SKH: ["Surkhet Airport", "Surkhet"],
  SKI: ["Skikda Airport", "Skikda"],
  SKJ: ["Sitkinak Airport", "Sitkinak Island"],
  SKK: ["Shaktoolik Airport", "Shaktoolik"],
  SKM: ["Skeldon Airport", "Skeldon"],
  SKN: ["Stokmarknes Skagen Airport", "Hadsel"],
  SKO: ["Sadiq Abubakar III International Airport", "Sokoto"],
  SKP: ["Skopje Alexander the Great Airport", "Skopje"],
  SKQ: ["Sekakes Airport", "Sekakes"],
  SKR: ["Shakiso Airport", "Shakiso"],
  SKS: ["Skrydstrup Air Base", "Vojens"],
  SKT: ["Sialkot Airport", "Sialkot"],
  SKU: ["Skiros Airport", "Skiros Island"],
  SKV: ["St Catherine International Airport"],
  SKW: ["Skwentna Airport", "Skwentna"],
  SKX: ["Saransk Airport", "Saransk"],
  SKY: ["Griffing Sandusky Airport", "Sandusky"],
  SKZ: ["Sukkur Airport", "Mirpur Khas"],
  SLA: ["Martin Miguel De Guemes International Airport", "Salta"],
  SLB: ["Storm Lake Municipal Airport", "Storm Lake"],
  SLC: ["Salt Lake City International Airport", "Salt Lake City"],
  SLD: ["Slia\u010d Airport", "Slia\u010d"],
  SLE: ["Salem Municipal Airport/McNary Field", "Salem"],
  SLF: ["Sulayel Airport"],
  SLG: ["Smith Field", "Siloam Springs"],
  SLH: ["Sola Airport", "Sola"],
  SLI: ["Solwesi Airport", "Solwesi"],
  SLJ: ["Solomon Airport", "Karijini National Park"],
  SLK: ["Adirondack Regional Airport", "Saranac Lake"],
  SLL: ["Salalah Airport", "Salalah"],
  SLM: ["Salamanca Airport", "Salamanca"],
  SLN: ["Salina Municipal Airport", "Salina"],
  SLO: ["Salem Leckrone Airport", "Salem"],
  SLP: ["Ponciano Arriaga International Airport", "San Luis Potos\u00ed"],
  SLQ: ["Sleetmute Airport", "Sleetmute"],
  SLR: ["Sulphur Springs Municipal Airport", "Sulphur Springs"],
  SLS: ["Silistra Polkovnik Lambrinovo Air Base", "Silistra"],
  SLT: ["Harriet Alexander Field", "Salida"],
  SLU: ["George F. L. Charles Airport", "Castries"],
  SLV: ["Shimla Airport"],
  SLW: ["Plan De Guadalupe International Airport", "Saltillo"],
  SLX: ["Salt Cay Airport", "Salt Cay"],
  SLY: ["Salekhard Airport", "Salekhard"],
  SLZ: ["Marechal Cunha Machado International Airport", "S\u00e3o Lu\u00eds"],
  SMA: ["Santa Maria Airport", "Vila do Porto"],
  SMB: ["Franco Bianco Airport", "Cerro Sombrero"],
  SMD: ["Smith Field", "Fort Wayne"],
  SME: ["Lake Cumberland Regional Airport", "Somerset"],
  SMF: ["Sacramento International Airport", "Sacramento"],
  SMG: ["Santa Maria Airport", "Santa Mar\u00eda"],
  SMH: ["Sapmanga Airport", "Sapmanga"],
  SMI: ["Samos Airport", "Samos Island"],
  SMJ: ["Sim Airport", "Sim"],
  SMK: ["St Michael Airport", "St Michael"],
  SML: ["Stella Maris Airport", "Stella Maris"],
  SMM: ["Semporna Airport", "Semporna"],
  SMN: ["Lemhi County Airport", "Salmon"],
  SMO: ["Santa Monica Municipal Airport", "Santa Monica"],
  SMP: ["Stockholm Airport", "Stockholm"],
  SMQ: ["Sampit(Hasan) Airport", "Sampit-Borneo Island"],
  SMR: ["Sim\u00f3n Bol\u00edvar International Airport", "Santa Marta"],
  SMS: ["Sainte Marie Airport"],
  SMT: ["Adolino Bedin Regional Airport", "Sorriso"],
  SMU: ["Sheep Mountain Airport", "Sheep Mountain"],
  SMV: ["Samedan Airport"],
  SMW: ["Smara Airport", "Smara"],
  SMX: ["Santa Maria Pub/Capt G Allan Hancock Field", "Santa Maria"],
  SMY: ["Simenti Airport", "Simenti"],
  SMZ: ["Stoelmanseiland Airport", "Stoelmanseiland"],
  SNA: ["John Wayne Airport-Orange County Airport", "Santa Ana"],
  SNB: ["Snake Bay Airport"],
  SNC: ["General Ulpiano Paez Airport", "Salinas"],
  SNE: ["Pregui\u00e7a Airport", "Pregui\u00e7a"],
  SNF: ["Sub Teniente Nestor Arias Airport", "San Felipe"],
  SNG: [
    "Capit\u00e1n Av. Juan Cochamanidis S. Airport",
    "San Ignacio de Velasco",
  ],
  SNH: ["Stanthorpe Airport"],
  SNI: ["Greenville Sinoe Airport", "Greenville"],
  SNJ: ["San Julian Air Base", "Pinar Del Rio"],
  SNK: ["Winston Field", "Snyder"],
  SNL: ["Shawnee Regional Airport", "Shawnee"],
  SNM: ["San Ignacio de Moxos Airport", "San Ignacio de Moxos"],
  SNN: ["Shannon Airport", "Shannon"],
  SNO: ["Sakon Nakhon Airport"],
  SNP: ["St Paul Island Airport", "St Paul Island"],
  SNQ: ["San Quint\u00edn Military Airstrip", "Military Camp Number 2-D"],
  SNR: ["Saint-Nazaire-Montoir Airport", "Saint-Nazaire/Montoir"],
  SNS: ["Salinas Municipal Airport", "Salinas"],
  SNT: ["Las Cruces Airport", "Sabana De Torres"],
  SNU: ["Abel Santamaria Airport", "Santa Clara"],
  SNV: ["Santa Elena de Uairen Airport"],
  SNW: ["Thandwe Airport", "Thandwe"],
  SNX: ["Semnan Municipal Airport", "Semnan"],
  SNY: ["Sidney Municipal-Lloyd W Carr Field", "Sidney"],
  SNZ: ["Santa Cruz Air Force Base", "Rio de Janeiro"],
  SOA: ["S\u00f3c Tr\u0103ng Airport", "S\u00f3c Tr\u0103ng"],
  SOB: ["S\u00e1rmell\u00e9k International Airport", "S\u00e1rmell\u00e9k"],
  SOC: ["Adi Sumarmo Wiryokusumo Airport", "Sukarata(Solo)-Java Island"],
  SOD: ["Sorocaba Airport", "Sorocaba"],
  SOE: ["Souanke Airport", "Souanke"],
  SOF: ["Sofia Airport", "Sofia"],
  SOG: ["Sogndal Airport", "Sogndal"],
  SOH: ["Solita Airport", "Solita"],
  SOI: ["South Molle Island Helipad", "South Molle Island Resort"],
  SOJ: ["S\u00f8rkjosen Airport", "S\u00f8rkjosen"],
  SOK: ["Semonkong Airport", "Semonkong"],
  SOL: ["Solomon State Field", "Solomon"],
  SOM: ["San Tom\u00e9 Airport", "El Tigre"],
  SON: ["Santo Pekoa International Airport", "Luganville"],
  SOO: ["S\u00f6derhamn Airport", "S\u00f6derhamn"],
  SOP: ["Moore County Airport", "Pinehurst/Southern Pines"],
  SOQ: ["Dominique Edward Osok Airport", "Sorong-Papua Island"],
  SOR: ["Al Thaurah Airport", "Al Thaurah"],
  SOT: ["Sodankyla Airport", "Sodankyla"],
  SOU: ["Southampton Airport", "Southampton"],
  SOV: ["Seldovia Airport", "Seldovia"],
  SOW: ["Show Low Regional Airport", "Show Low"],
  SOX: ["Alberto Lleras Camargo Airport", "Sogamoso"],
  SOY: ["Stronsay Airport", "Stronsay"],
  SOZ: ["Solenzara (BA 126) Air Base", "Solenzara"],
  SPA: ["Spartanburg Downtown Memorial Airport", "Spartanburg"],
  SPB: ["Charlotte Amalie Harbor Seaplane Base", "Charlotte Amalie St Thomas"],
  SPC: ["La Palma Airport", "Sta Cruz de la Palma, La Palma Island"],
  SPD: ["Saidpur Airport", "Saidpur"],
  SPE: ["Sepulot Airport", "Sepulot"],
  SPF: ["Black Hills Airport-Clyde Ice Field", "Spearfish"],
  SPG: ["Albert Whitted Airport", "St Petersburg"],
  SPH: ["Sopu Airport", "Sopu"],
  SPI: ["Abraham Lincoln Capital Airport", "Springfield"],
  SPJ: ["Sparti Airport", "Sparti"],
  SPM: ["Spangdahlem Air Base", "Trier"],
  SPN: ["Saipan International Airport", "Saipan Island"],
  SPP: ["Menongue Airport", "Menongue"],
  SPQ: ["Catalina Air-Sea Terminal Heliport", "San Pedro"],
  SPR: ["San Pedro Airport", "San Pedro"],
  SPS: [
    "Sheppard Air Force Base-Wichita Falls Municipal Airport",
    "Wichita Falls",
  ],
  SPT: ["Sipitang Airport", "Sipitang"],
  SPU: ["Split Airport", "Split"],
  SPV: ["Sepik Plains Airport", "Sepik Plains"],
  SPW: ["Spencer Municipal Airport", "Spencer"],
  SPX: ["Houston Gulf Airport", "Houston"],
  SPY: ["San Pedro Airport"],
  SPZ: ["Springdale Municipal Airport", "Springdale"],
  SQA: ["Santa Ynez Airport", "Santa Ynez"],
  SQB: ["Santa Ana Airport", "Piedras"],
  SQC: ["Southern Cross Airport"],
  SQD: ["Shangrao Sanqingshan Airport", "Shangrao"],
  SQE: ["San Luis De Palenque Airport", "San Luis De Palenque"],
  SQF: ["Solano Airport", "Solano"],
  SQG: ["Sintang(Susilo) Airport", "Sintang-Borneo Island"],
  SQH: ["Na-San Airport", "Son-La"],
  SQI: [
    "Whiteside County Airport-Joseph H Bittorf Field",
    "Sterling/Rockfalls",
  ],
  SQJ: ["Shaxian Airport", "Sanming"],
  SQK: ["Sidi Barrani Airport", "Sidi Barrani"],
  SQL: ["San Carlos Airport", "San Carlos"],
  SQM: ["S\u00e3o Miguel do Araguaia Airport", "S\u00e3o Miguel Do Araguaia"],
  SQN: ["Emalamo Sanana Airport", "Sanana-Seram Island"],
  SQO: ["Storuman Airport"],
  SQQ: ["\u0160iauliai International Airport", "\u0160iauliai"],
  SQR: ["Soroako Airport", "Soroako-Celebes Island"],
  SQS: ["Matthew Spain Airport", "San Ignacio"],
  SQT: ["China Strait Airstrip", "Samarai Island"],
  SQU: ["Saposoa Airport", "Plaza Saposoa"],
  SQV: ["Sequim Valley Airport", "Sequim"],
  SQW: ["Skive Airport", "Skive"],
  SQX: ["S\u00e3o Miguel do Oeste Airport", "S\u00e3o Miguel Do Oeste"],
  SQY: [
    "S\u00e3o Louren\u00e7o do Sul Airport",
    "S\u00e3o Louren\u00e7o Do Sul",
  ],
  SQZ: ["RAF Scampton", "Scampton"],
  SRA: ["Santa Rosa Airport", "Santa Rosa"],
  SRB: ["Santa Rosa De Yacuma Airport", "Santa Rosa"],
  SRD: ["San Ram\u00f3n Airport", "San Ram\u00f3n / Mamor\u00e9"],
  SRE: ["Juana Azurduy De Padilla Airport", "Sucre"],
  SRF: ["Hamilton Field", "San Rafael"],
  SRG: ["Achmad Yani Airport", "Semarang-Java Island"],
  SRH: ["Sarh Airport", "Sarh"],
  SRI: ["Temindung Airport", "Samarinda"],
  SRJ: ["Capit\u00e1n Av. German Quiroga G. Airport", "San Borja"],
  SRL: ["Palo Verde Airport", "Santa Rosalia"],
  SRM: ["Sandringham Airport", "Sandringham Station"],
  SRN: ["Strahan Airport"],
  SRO: ["Santana Ramos Airport", "Santana Ramos"],
  SRP: ["Stord Airport", "Leirvik"],
  SRQ: ["Sarasota Bradenton International Airport", "Sarasota/Bradenton"],
  SRR: ["Dunwich Airport", "Stradbroke Island"],
  SRS: ["San Marcos Airport", "San Marcos"],
  SRT: ["Soroti Airport", "Soroti"],
  SRU: ["Santa Cruz Sky Park", "Scotts Valley"],
  SRV: ["Stony River 2 Airport", "Stony River"],
  SRW: ["Mid-Carolina Regional Airport", "Salisbury"],
  SRX: ["Gardabya Airport", "Sirt"],
  SRY: ["Dasht-e Naz Airport", "Sari"],
  SRZ: ["El Trompillo Airport", "Santa Cruz"],
  SSA: [
    "Deputado Luiz Eduardo Magalh\u00e3es International Airport",
    "Salvador",
  ],
  SSB: ["Christiansted Harbor Seaplane Base", "Christiansted St Croix"],
  SSC: ["Shaw Air Force Base", "Sumter"],
  SSD: ["V\u00edctor Laf\u00f3n Airport", "San Felipe"],
  SSE: ["Solapur Airport", "Solapur"],
  SSF: ["Stinson Municipal Airport", "San Antonio"],
  SSG: ["Malabo Airport", "Malabo"],
  SSH: ["Sharm El Sheikh International Airport", "Sharm el-Sheikh"],
  SSI: ["Malcolm McKinnon Airport", "Brunswick"],
  SSJ: ["Sandnessj\u00f8en Airport (Stokka)", "Alstahaug"],
  SSK: ["Sturt Creek Airport", "Sturt Creek"],
  SSL: ["Santa Rosalia Airport", "Santa Rosalia"],
  SSN: ["Seoul Air Base (K-16)", "Simgok-dong"],
  SSO: ["S\u00e3o Louren\u00e7o Airport", "S\u00e3o Louren\u00e7o"],
  SSP: ["Silver Plains Airport", "Silver Plains"],
  SSQ: ["La Sarre Airport", "La Sarre"],
  SSR: ["Sara Airport", "Pentecost Island"],
  SSS: ["Siassi Airport", "Siassi"],
  SST: ["Santa Teresita Airport", "Santa Teresita"],
  SSU: ["Greenbrier Airport", "White Sulphur Springs"],
  SSV: ["Siasi Airport", "Siasi Island"],
  SSW: ["Stuart Island Airpark", "Stuart Island"],
  SSX: ["Singita Safari Lodge Airport", "Singita Safari Lodge"],
  SSY: ["Mbanza Congo Airport", "Mbanza Congo"],
  SSZ: ["Base A\u00e9rea de Santos Airport", "Guaruj\u00e1"],
  STA: ["Stauning Airport", "Skjern / Ringk\u00f8bing"],
  STB: ["Santa B\u00e1rbara del Zulia Airport"],
  STC: ["St Cloud Regional Airport", "St Cloud"],
  STD: ["Mayor Buenaventura Vivas International Airport", "Santo Domingo"],
  STE: ["Stevens Point Municipal Airport", "Stevens Point"],
  STF: ["Stephens Island Seaplane Base", "Stephens Island"],
  STG: ["St George Airport", "St George"],
  STH: ["Strathmore Airport"],
  STI: ["Cibao International Airport", "Santiago"],
  STJ: ["Rosecrans Memorial Airport", "St Joseph"],
  STK: ["Sterling Municipal Airport", "Sterling"],
  STL: ["St Louis Lambert International Airport", "St Louis"],
  STM: ["Maestro Wilson Fonseca Airport", "Santar\u00e9m"],
  STN: ["London Stansted Airport", "London"],
  STP: ["St Paul Downtown Holman Field", "St Paul"],
  STQ: ["St Marys Municipal Airport", "St Marys"],
  STR: ["Stuttgart Airport", "Stuttgart"],
  STS: ["Charles M. Schulz Sonoma County Airport", "Santa Rosa"],
  STT: ["Cyril E. King Airport", "Charlotte Amalie, Harry S. Truman Airport"],
  STU: ["Santa Cruz Airport", "Santa Cruz"],
  STV: ["Surat Airport"],
  STW: ["Stavropol Shpakovskoye Airport", "Stavropol"],
  STX: ["Henry E Rohlsen Airport", "Christiansted"],
  STY: ["Nueva Hesperides International Airport", "Salto"],
  STZ: ["Santa Terezinha Airport", "Santa Terezinha"],
  SUA: ["Witham Field", "Stuart"],
  SUB: ["Juanda International Airport", "Surabaya"],
  SUD: ["Stroud Municipal Airport", "Stroud"],
  SUE: ["Door County Cherryland Airport", "Sturgeon Bay"],
  SUF: ["Lamezia Terme Airport", "Lamezia Terme (CZ)"],
  SUG: ["Surigao Airport", "Surigao City"],
  SUH: ["Sur Airport", "Sur"],
  SUI: ["Sukhumi Dranda Airport", "Sukhumi"],
  SUJ: ["Satu Mare Airport", "Satu Mare"],
  SUK: ["Sakkyryr Airport", "Batagay-Alyta"],
  SUL: ["Sui Airport", "Sui"],
  SUM: ["Sumter Airport", "Sumter"],
  SUN: ["Friedman Memorial Airport", "Hailey"],
  SUO: ["Sunriver Airport", "Sunriver"],
  SUP: ["Trunojoyo Airport", "Sumenep-Madura Island"],
  SUQ: ["Sucua Airport", "Sucua"],
  SUR: ["Summer Beaver Airport", "Summer Beaver"],
  SUS: ["Spirit of St Louis Airport", "St Louis"],
  SUT: ["Sumbawanga Airport", "Sumbawanga"],
  SUU: ["Travis Air Force Base", "Fairfield"],
  SUV: ["Nausori International Airport", "Nausori"],
  SUW: ["Richard I Bong Airport", "Superior"],
  SUX: ["Sioux Gateway Col. Bud Day Field", "Sioux City"],
  SUY: ["Suntar Airport", "Suntar"],
  SUZ: ["Suria Airport", "Suria"],
  SVA: ["Savoonga Airport", "Savoonga"],
  SVB: ["Sambava Airport"],
  SVC: ["Grant County Airport", "Silver City"],
  SVD: ["Argyle International Airport", "Kingstown"],
  SVE: ["Susanville Municipal Airport", "Susanville"],
  SVF: ["Sav\u00e9 Airport", "Sav\u00e9"],
  SVG: ["Stavanger Airport Sola", "Stavanger"],
  SVH: ["Statesville Regional Airport", "Statesville"],
  SVI: ["Eduardo Falla Solano Airport", "San Vicente Del Cagu\u00e1n"],
  SVJ: ["Svolv\u00e6r Helle Airport", "Svolv\u00e6r"],
  SVK: ["Silver Creek Airport", "Silver Creek"],
  SVL: ["Savonlinna Airport", "Savonlinna"],
  SVN: ["Hunter Army Air Field", "Savannah"],
  SVO: ["Sheremetyevo International Airport", "Moscow"],
  SVP: ["Kuito Airport", "Kuito"],
  SVQ: ["Sevilla Airport", "Sevilla"],
  SVS: ["Stevens Village Airport", "Stevens Village"],
  SVT: ["Savuti Airport", "Savuti"],
  SVU: ["Savusavu Airport", "Savusavu"],
  SVV: ["San Salvador de Paul Airport", "San Salvador de Paul"],
  SVW: ["Sparrevohn LRRS Airport", "Sparrevohn"],
  SVX: ["Koltsovo Airport", "Yekaterinburg"],
  SVZ: ["San Antonio Del Tachira Airport"],
  SWA: ["Jieyang Chaoshan International Airport", "Shantou"],
  SWB: ["Shaw River Airport", "Shaw River"],
  SWC: ["Stawell Airport"],
  SWD: ["Seward Airport", "Seward"],
  SWE: ["Siwea Airport", "Siwea"],
  SWF: ["Stewart International Airport", "Newburgh"],
  SWG: ["Satwag Airport", "Satwag"],
  SWH: ["Swan Hill Airport"],
  SWJ: ["Southwest Bay Airport", "Malekula Island"],
  SWM: ["Suia-Missu Airport", "Alto Boa Vista"],
  SWN: ["Sahiwal Airport", "Sahiwal"],
  SWO: ["Stillwater Regional Airport", "Stillwater"],
  SWP: ["Swakopmund Airport", "Swakopmund"],
  SWQ: ["Sumbawa Besar Airport", "Sumbawa Island"],
  SWR: ["Silur Airport", "Silur Mission"],
  SWS: ["Swansea Airport", "Swansea"],
  SWT: ["Strezhevoy Airport", "Strezhevoy"],
  SWU: ["Suwon Airport"],
  SWV: ["Shikarpur Airport", "Shikarpur"],
  SWW: ["Avenger Field", "Sweetwater"],
  SWX: ["Shakawe Airport", "Shakawe"],
  SWY: ["Sitiawan Airport", "Sitiawan"],
  SXA: ["Sialum Airport", "Sialum"],
  SXB: ["Strasbourg Airport", "Strasbourg"],
  SXE: ["West Sale Airport", "Sale"],
  SXF: ["Berlin-Sch\u00f6nefeld Airport", "Berlin"],
  SXG: ["Senanga Airport", "Senanga"],
  SXH: ["Sehulea Airport", "Sehulea"],
  SXI: ["Sirri Island Airport"],
  SXJ: ["Shanshan Airport", "Shanshan"],
  SXK: ["Saumlaki/Olilit Airport", "Saumlaki-Yamdena Island"],
  SXL: ["Sligo Airport", "Sligo"],
  SXM: ["Princess Juliana International Airport", "Saint Martin"],
  SXN: ["Sua Pan Airport", "Sowa"],
  SXO: [
    "S\u00e3o F\u00e9lix do Araguaia Airport",
    "S\u00e3o F\u00e9lix Do Araguaia",
  ],
  SXP: ["Nunam Iqua Airport", "Nunam Iqua"],
  SXQ: ["Soldotna Airport", "Soldotna"],
  SXR: ["Sheikh ul Alam Airport", "Srinagar"],
  SXS: ["Sahabat [Sahabat 16] Airport", "Sahabat"],
  SXT: ["Sungai Tiang Airport", "Taman Negara"],
  SXU: ["Soddu Airport", "Soddu"],
  SXV: ["Salem Airport", "Salem"],
  SXW: ["Sauren Airport", "Sauren"],
  SXX: ["S\u00e3o F\u00e9lix do Xingu Airport", "S\u00e3o F\u00e9lix Do Xingu"],
  SXY: ["Sidney Municipal Airport", "Sidney"],
  SXZ: ["Siirt Airport", "Siirt"],
  SYA: ["Eareckson Air Station", "Shemya"],
  SYB: ["Seal Bay Seaplane Base", "Seal Bay"],
  SYC: ["Shiringayoc Airport", "Shiringayoc"],
  SYD: ["Sydney Kingsford Smith International Airport", "Sydney"],
  SYE: ["Sadah Airport", "Sadah"],
  SYF: ["Silva Bay Seaplane Base", "Gabriola Island"],
  SYH: ["Syangboche Airport", "Namche Bazaar"],
  SYI: ["Bomar Field Shelbyville Municipal Airport", "Shelbyville"],
  SYJ: ["Sirjan Airport"],
  SYK: ["Stykkish\u00f3lmur Airport", "Stykkish\u00f3lmur"],
  SYL: ["Roberts Army Heliport", "Camp Roberts/San Miguel"],
  SYM: ["Pu'er Simao Airport", "Pu'er"],
  SYN: ["Stanton Airfield", "Stanton"],
  SYO: ["Shonai Airport", "Shonai"],
  SYP: ["Ruben Cantu Airport", "Santiago"],
  SYQ: ["Tobias Bolanos International Airport", "San Jose"],
  SYR: ["Syracuse Hancock International Airport", "Syracuse"],
  SYS: ["Saskylakh Airport", "Saskylakh"],
  SYT: ["Saint-Yan Airport", "Saint-Yan"],
  SYU: ["Warraber Island Airport", "Sue Islet"],
  SYV: ["Sylvester Airport", "Sylvester"],
  SYW: ["Sehwan Sharif Airport", "Sehwan Sharif"],
  SYX: ["Sanya Phoenix International Airport", "Sanya"],
  SYY: ["Stornoway Airport", "Stornoway"],
  SYZ: ["Shiraz Shahid Dastghaib International Airport", "Shiraz"],
  SZA: ["Soyo Airport", "Soyo"],
  SZB: ["Sultan Abdul Aziz Shah International Airport", "Subang"],
  SZE: ["Semera Airport", "Semera"],
  SZF: ["Samsun \u00c7ar\u015famba Airport", "Samsun"],
  SZG: ["Salzburg Airport", "Salzburg"],
  SZH: ["Senipah Heliport", "Senipah"],
  SZI: ["Zaysan Airport", "Zaysan"],
  SZJ: ["Siguanea Airport", "Isla de la Juventud"],
  SZK: ["Skukuza Airport", "Skukuza"],
  SZL: ["Whiteman Air Force Base", "Knob Noster"],
  SZM: ["Sesriem Airstrip"],
  SZN: ["Santa Cruz Island Airport", "Santa Barbara"],
  SZP: ["Santa Paula Airport", "Santa Paula"],
  SZQ: ["Saenz Pe\u00f1a Airport", "Presidencia Roque S\u00e1enz Pe\u00f1a"],
  SZR: ["Stara Zagora Airport", "Stara Zagora"],
  SZS: ["Ryan's Creek Aerodrome", "Oban"],
  SZT: ["San Cristobal de las Casas Airport", "San Cristobal de las Casas"],
  SZV: ["Suzhou Guangfu Airport", "Suzhou"],
  SZW: ["Schwerin Parchim Airport"],
  SZX: ["Shenzhen Bao'an International Airport", "Shenzhen"],
  SZY: ["Olsztyn-Mazury Airport", "Olsztyn"],
  SZZ: ['Szczecin-Goleni\u00f3w "Solidarno\u015b\u0107" Airport', "Goleniow"],
  TAA: ["Tarapaina Airport", "Tarapaina"],
  TAB: ["Tobago-Crown Point Airport", "Scarborough"],
  TAC: ["Daniel Z. Romualdez Airport", "Tacloban City"],
  TAD: ["Perry Stokes Airport", "Trinidad"],
  TAE: ["Daegu Airport", "Daegu"],
  TAF: ["Tafaraoui Airport"],
  TAG: ["Tagbilaran Airport", "Tagbilaran City"],
  TAH: ["Tanna Airport"],
  TAI: ["Ta'izz International Airport", "Ta'izz"],
  TAJ: ["Tadji Airport", "Aitape"],
  TAK: ["Takamatsu Airport", "Takamatsu"],
  TAL: ["Ralph M Calhoun Memorial Airport", "Tanana"],
  TAM: ["General Francisco Javier Mina International Airport", "Tampico"],
  TAN: ["Tangalooma Airport"],
  TAO: ["Liuting Airport", "Qingdao"],
  TAP: ["Tapachula International Airport", "Tapachula"],
  TAQ: ["Tarcoola Airport", "Tarcoola"],
  TAR: ['Taranto-Grottaglie "Marcello Arlotta" Airport', "Grottaglie"],
  TAS: ["Tashkent International Airport", "Tashkent"],
  TAT: ["Poprad-Tatry Airport", "Poprad"],
  TAU: ["Tauramena Airport", "Tauramena"],
  TAV: ["Tau Airport", "Tau Village"],
  TAW: ["Tacuarembo Airport", "Tacuarembo"],
  TAX: ["Taliabu Island Airport", "Tikong-Taliabu Island"],
  TAY: ["Tartu Airport", "Tartu"],
  TAZ: ["Da\u015foguz Airport", "Da\u015foguz"],
  TBA: ["Tabibuga Airport", "Tabibuga"],
  TBB: ["Dong Tac Airport", "Tuy Hoa"],
  TBC: ["Tuba City Airport", "Tuba City"],
  TBE: ["Timbunke Airport", "Timbunke"],
  TBF: ["Tabiteuea North Airport"],
  TBG: ["Tabubil Airport", "Tabubil"],
  TBH: ["Tugdan Airport", "Tablas Island"],
  TBI: ["New Bight Airport", "Cat Island"],
  TBJ: ["Tabarka 7 Novembre Airport", "Tabarka"],
  TBK: ["Timber Creek Airport"],
  TBL: ["Tableland Homestead Airport"],
  TBM: ["Tumbang Samba Airport", "Tumbang Samba-Borneo Island"],
  TBN: ["Waynesville-St. Robert Regional Forney field", "Fort Leonard Wood"],
  TBO: ["Tabora Airport", "Tabora"],
  TBP: ["Capitan FAP Pedro Canga Rodriguez Airport", "Tumbes"],
  TBQ: ["Tarabo Airport", "Tarabo"],
  TBR: ["Statesboro Bulloch County Airport", "Statesboro"],
  TBS: ["Tbilisi International Airport", "Tbilisi"],
  TBT: ["Tabatinga Airport", "Tabatinga"],
  TBU: ["Fua'amotu International Airport", "Nuku'alofa"],
  TBV: ["Tabal Airstrip", "Tabal Island"],
  TBW: ["Donskoye Airport", "Tambov"],
  TBY: ["Tshabong Airport", "Tshabong"],
  TBZ: ["Tabriz International Airport", "Tabriz"],
  TCA: ["Tennant Creek Airport", "Tennant Creek"],
  TCB: ["Treasure Cay Airport", "Treasure Cay"],
  TCC: ["Tucumcari Municipal Airport", "Tucumcari"],
  TCD: ["Tarapac\u00e1 Airport", "Tarapac\u00e1"],
  TCE: ["Tulcea Airport", "Tulcea"],
  TCG: ["Tacheng Airport", "Tacheng"],
  TCH: ["Tchibanga Airport", "Tchibanga"],
  TCJ: ["Torembi Airport", "Torembi"],
  TCK: ["Tinboli Airport", "Tinboli"],
  TCL: ["Tuscaloosa Regional Airport", "Tuscaloosa"],
  TCM: ["McChord Air Force Base", "Tacoma"],
  TCN: ["Tehuacan Airport"],
  TCO: ["La Florida Airport", "Tumaco"],
  TCP: ["Taba International Airport", "Taba"],
  TCQ: ["Coronel FAP Carlos Ciriani Santa Rosa International Airport", "Tacna"],
  TCR: ["Tuticorin Airport", "Thoothukudi"],
  TCS: ["Truth Or Consequences Municipal Airport", "Truth Or Consequences"],
  TCT: ["Takotna Airport", "Takotna"],
  TCU: ["Thaba Nchu Tar Airport", "Homeward"],
  TCW: ["Tocumwal Airport"],
  TCX: ["Tabas Airport", "Tabas"],
  TCY: ["Terrace Bay Airport", "Terrace Bay"],
  TCZ: ["Tengchong Tuofeng Airport", "Tengchong"],
  TDA: ["Trinidad Airport", "Trinidad"],
  TDB: ["Tetebedi Airport", "Tetebedi"],
  TDD: ["Teniente Av. Jorge Henrich Arauz Airport", "Trinidad"],
  TDG: ["Tandag Airport"],
  TDJ: ["Tadjoura Airport", "Tadjoura"],
  TDK: ["Taldykorgan Airport", "Taldy Kurgan"],
  TDL: ["H\u00e9roes De Malvinas Airport", "Tandil"],
  TDN: ["Theda Station Airport", "Theda Station"],
  TDO: ["Ed Carlson Memorial Field South Lewis County Airport", "Toledo"],
  TDP: ["Trompeteros Airport", "Corrientes"],
  TDR: ["Theodore Airport"],
  TDS: ["Sasereme Airport", "Sasereme"],
  TDT: ["Tanda Tula Airport", "Welverdiend"],
  TDV: ["Samangoky Airport", "Tanandava"],
  TDW: ["Tradewind Airport", "Amarillo"],
  TDX: ["Trat Airport"],
  TDZ: ["Toledo Executive Airport", "Toledo"],
  TEA: ["Tela Airport", "Tela"],
  TEB: ["Teterboro Airport", "Teterboro"],
  TEC: ["Tel\u00eamaco Borba Airport", "Tel\u00eamaco Borba"],
  TED: ["Thisted Airport", "Thisted"],
  TEE: ["Cheikh Larbi T\u00e9bessi Airport", "T\u00e9bessi"],
  TEF: ["Telfer Airport"],
  TEG: ["Tenkodogo Airport", "Tenkodogo"],
  TEI: ["Tezu Airport", "Tezu"],
  TEK: ["Tatitlek Airport", "Tatitlek"],
  TEL: ["Telupid Airport", "Telupid"],
  TEM: ["Temora Airport"],
  TEN: ["Tongren Fenghuang Airport"],
  TEO: ["Terapo Airport", "Terapo Mission"],
  TEP: ["Tep Tep Airport", "Teptep"],
  TEQ: ["Tekirda\u011f \u00c7orlu Airport", "\u00c7orlu"],
  TER: ["Lajes Airport", "Praia da Vit\u00f3ria"],
  TES: ["Tessenei Airport", "Tessenei"],
  TET: ["Chingozi Airport", "Tete"],
  TEU: ["Manapouri Airport"],
  TEV: ["Teruel Airport", "Teruel"],
  TEX: ["Telluride Regional Airport", "Telluride"],
  TEY: ["\u00deingeyri Airport", "\u00deingeyri"],
  TEZ: ["Tezpur Airport"],
  TFB: ["Tifalmin Airport", "Tifalmin"],
  TFF: ["Tef\u00e9 Airport", "Tef\u00e9"],
  TFI: ["Tufi Airport", "Tufi"],
  TFL: ["Juscelino Kubitscheck Airport", "Te\u00f3filo Otoni"],
  TFM: ["Telefomin Airport", "Telefomin"],
  TFN: ["Tenerife Norte Airport", "Tenerife Island"],
  TFS: ["Tenerife South Airport", "Tenerife Island"],
  TFT: ["Taftan Airport", "Taftan"],
  TFY: ["Tarfaya Airport", "Tarfaya"],
  TGA: ["Tengah Air Base"],
  TGC: ["Tanjung Manis Airport", "Tanjung Manis"],
  TGD: ["Podgorica Airport", "Podgorica"],
  TGE: ["Sharpe Field", "Tuskegee"],
  TGG: ["Sultan Mahmud Airport", "Kuala Terengganu"],
  TGH: ["Tongoa Airport", "Tongoa Island"],
  TGI: ["Tingo Maria Airport", "Tingo Maria"],
  TGJ: ["Tiga Airport", "Tiga"],
  TGK: ["Taganrog Yuzhny Airport", "Taganrog"],
  TGL: ["Tagula Airport", "Sudest Island"],
  TGM: [
    "Transilvania T\u00e2rgu Mure\u015f International Airport",
    "T\u00e2rgu Mure\u015f",
  ],
  TGN: ["Latrobe Valley Airport", "Morwell"],
  TGO: ["Tongliao Airport", "Tongliao"],
  TGP: ["Podkamennaya Tunguska Airport", "Bor"],
  TGQ: ["Tangar\u00e1 da Serra Airport", "Tangar\u00e1 Da Serra"],
  TGR: ["Touggourt Sidi Madhi Airport", "Touggourt"],
  TGS: ["Chokw\u00e9 Airport", "Chokw\u00e9"],
  TGT: ["Tanga Airport", "Tanga"],
  TGU: ["Toncont\u00edn International Airport", "Tegucigalpa"],
  TGV: ["Bukhovtsi Airfield", "Targovishte"],
  TGZ: ["Angel Albino Corzo International Airport", "Tuxtla Guti\u00e9rrez"],
  THA: ["Tullahoma Regional Arpt/Wm Northern Field", "Tullahoma"],
  THB: ["Thaba-Tseka Airport", "Thaba-Tseka"],
  THC: ["Tchien Airport", "Tchien"],
  THD: ["Tho Xuan Airport", "Thanh H\u00f3a"],
  THE: ["Senador Petr\u00f4nio Portela Airport", "Teresina"],
  THF: ["Berlin-Tempelhof International Airport", "Berlin"],
  THG: ["Thangool Airport", "Biloela"],
  THH: ["Taharoa Aerodrome", "Taharoa"],
  THI: ["Tichitt Airport", "Tichitt"],
  THK: ["Thakhek Airport", "Thakhek"],
  THL: ["Tachileik Airport", "Tachileik"],
  THM: ["Thompson Falls Airport", "Thompson Falls"],
  THN: ["Trollh\u00e4ttan-V\u00e4nersborg Airport", "Trollh\u00e4ttan"],
  THO: ["Thorshofn Airport", "Thorshofn"],
  THP: ["Hot Springs County Airport", "Thermopolis"],
  THQ: ["Tianshui Maijishan Airport", "Tianshui"],
  THR: ["Mehrabad International Airport", "Tehran"],
  THS: ["Sukhothai Airport"],
  THT: ["Tamchakett Airport", "Tamchakett"],
  THU: ["Thule Air Base", "Thule"],
  THV: ["York Airport", "York"],
  THW: ["Trincomalee Harbor Waterdrome", "Trincomalee"],
  THX: ["Turukhansk Airport", "Turukhansk"],
  THY: ["Thohoyandou Airport", "Thohoyandou"],
  THZ: ["Tahoua Airport", "Tahoua"],
  TIA: ["Tirana International Airport Mother Teresa", "Tirana"],
  TIB: ["Tib\u00fa Airport", "Tib\u00fa"],
  TIC: ["Tinak Airport", "Arno Atoll"],
  TID: ["Bou Chekif Airport", "Tiaret"],
  TIE: ["Tippi Airport", "Tippi"],
  TIF: ["Ta\u2019if Regional Airport", "Ta\u2019if"],
  TIG: ["Tingwon Airport", "Tingwon Island"],
  TIH: ["Tikehau Airport"],
  TII: ["Tarin Kowt Airport", "Tarin Kowt"],
  TIJ: ["General Abelardo L. Rodr\u00edguez International Airport", "Tijuana"],
  TIK: ["Tinker Air Force Base", "Oklahoma City"],
  TIL: ["Cheadle Airport", "Cheadle"],
  TIM: ["Moses Kilangin Airport", "Timika-Papua Island"],
  TIN: ["Tindouf Airport", "Tindouf"],
  TIO: ["Tilin Airport", "Tilin"],
  TIP: ["Tripoli International Airport", "Tripoli"],
  TIQ: ["Tinian International Airport", "Tinian Island"],
  TIR: ["Tirupati Airport", "Tirupati"],
  TIU: ["Timaru Airport"],
  TIV: ["Tivat Airport", "Tivat"],
  TIW: ["Tacoma Narrows Airport", "Tacoma"],
  TIX: ["Space Coast Regional Airport", "Titusville"],
  TIY: ["Tidjikja Airport", "Tidjikja"],
  TIZ: ["Tari Airport", "Tari"],
  TJA: ["Capitan Oriel Lea Plaza Airport", "Tarija"],
  TJB: ["Sei Bati Airport", "Tanjung Balai-Karinmunbesar Island"],
  TJC: ["Ticantiki Airport", "Ticantiqui"],
  TJG: ["Warukin Airport", "Tanta-Tabalong-Borneo Island"],
  TJH: ["Tajima Airport", "Tajima"],
  TJI: ["Trujillo Airport", "Trujillo"],
  TJK: ["Tokat Airport", "Tokat"],
  TJL: ["Pl\u00ednio Alarcom Airport", "Tr\u00eas Lagoas"],
  TJM: ["Roshchino International Airport", "Tyumen"],
  TJN: ["Takume Airport", "Takume"],
  TJQ: [
    "Buluh Tumbang (H A S Hanandjoeddin) Airport",
    "Tanjung Pandan-Belitung Island",
  ],
  TJS: ["Tanjung Harapan Airport", "Tanjung Selor-Borneo Island"],
  TJU: ["Kulob Airport", "Kulyab"],
  TJV: ["Tanjore Air Force Base", "Thanjavur"],
  TKA: ["Talkeetna Airport", "Talkeetna"],
  TKB: ["Tekadu Airport", "Tekadu"],
  TKC: ["Tiko Airport", "Tiko"],
  TKD: ["Takoradi Airport", "Sekondi-Takoradi"],
  TKE: ["Tenakee Seaplane Base", "Tenakee Springs"],
  TKF: ["Truckee Tahoe Airport", "Truckee"],
  TKG: ["Radin Inten II (Branti) Airport", "Bandar Lampung-Sumatra Island"],
  TKH: ["Takhli Airport"],
  TKI: ["Tokeen Seaplane Base", "Tokeen"],
  TKJ: ["Tok Junction Airport", "Tok"],
  TKK: ["Chuuk International Airport", "Weno Island"],
  TKL: ["Taku Lodge Seaplane Base", "Taku Lodge"],
  TKM: ["El Pet\u00e9n Airport", "Tikal"],
  TKN: ["Tokunoshima Airport", "Tokunoshima"],
  TKO: ["Tlokoeng Airport", "Tlokoeng"],
  TKP: ["Takapoto Airport"],
  TKQ: ["Kigoma Airport", "Kigoma"],
  TKR: ["Thakurgaon Airport", "Thakurgaon"],
  TKS: ["Tokushima Airport/JMSDF Air Base", "Tokushima"],
  TKT: ["Tak Airport"],
  TKU: ["Turku Airport", "Turku"],
  TKV: ["Tatakoto Airport", "Tatakoto"],
  TKW: ["Tekin Airport", "Tekin"],
  TKX: ["Takaroa Airport"],
  TKY: ["Turkey Creek Airport", "Turkey Creek"],
  TKZ: ["Tokoroa Airfield", "Tokoroa"],
  TLA: ["Teller Airport", "Teller"],
  TLB: ["Tarbela Dam Airport", "Tarbela"],
  TLC: ["Licenciado Adolfo Lopez Mateos International Airport", "Toluca"],
  TLD: ["Limpopo Valley Airport", "Tuli Lodge"],
  TLE: ["Toliara Airport"],
  TLF: ["Telida Airport", "Telida"],
  TLG: ["Tulaghi Heliport", "Tulaghi Island"],
  TLH: ["Tallahassee Regional Airport", "Tallahassee"],
  TLI: ["Sultan Bantilan Airport", "Toli Toli-Celebes Island"],
  TLJ: ["Tatalina LRRS Airport", "Takotna"],
  TLK: ["Talakan Airport", "Talakan Oil Field"],
  TLL: ["Lennart Meri Tallinn Airport", "Tallinn"],
  TLM: ["Zenata \u2013 Messali El Hadj Airport", "Tlemcen"],
  TLN: ["Toulon-Hy\u00e8res Airport", "Toulon/Hy\u00e8res/Le Palyvestre"],
  TLO: ["Tol Airport", "Tol"],
  TLP: ["Tumolbil Airport", "Tumolbil"],
  TLQ: ["Turpan Jiaohe Airport", "Turpan"],
  TLR: ["Mefford Field", "Tulare"],
  TLS: ["Toulouse-Blagnac Airport", "Toulouse/Blagnac"],
  TLT: ["Tuluksak Airport", "Tuluksak"],
  TLU: ["Golfo de Morrosquillo Airport", "Tol\u00fa"],
  TLV: ["Ben Gurion International Airport", "Tel Aviv"],
  TLW: ["Talasea Airport", "Talasea"],
  TLX: ["Panguilemo Airport", "Talca"],
  TLY: ["Plastun Airport", "Plastun"],
  TLZ: ["Catal\u00e3o Airport", "Catal\u00e3o"],
  TMA: ["Henry Tift Myers Airport", "Tifton"],
  TMB: ["Kendall-Tamiami Executive Airport", "Miami"],
  TMC: ["Tambolaka Airport", "Waikabubak-Sumba Island"],
  TMD: ["Timbedra Airport", "Timbedra"],
  TME: ["Gustavo Vargas Airport", "Tame"],
  TMF: ["Thimarafushi Airport", "Thimarafushi"],
  TMG: ["Tomanggong Airport", "Tomanggong"],
  TMH: ["Tanah Merah Airport", "Tanah Merah-Papua Island"],
  TMI: ["Tumling Tar Airport", "Tumling Tar"],
  TMJ: ["Termez Airport", "Termez"],
  TML: ["Tamale Airport", "Tamale"],
  TMM: ["Toamasina Airport"],
  TMN: ["Tamana Island Airport", "Tamana Island"],
  TMO: ["Tumeremo Airport"],
  TMP: ["Tampere-Pirkkala Airport", "Tampere / Pirkkala"],
  TMQ: ["Tambao Airport", "Tambao"],
  TMR: ["Aguenar \u2013 Hadj Bey Akhamok Airport", "Tamanrasset"],
  TMS: ["S\u00e3o Tom\u00e9 International Airport", "S\u00e3o Tom\u00e9"],
  TMT: ["Trombetas Airport", "Oriximin\u00e1"],
  TMU: ["Tambor Airport", "Nicoya"],
  TMW: ["Tamworth Airport", "Tamworth"],
  TMX: ["Timimoun Airport", "Timimoun"],
  TMY: ["Tiom Airport", "Tiom-Papua Island"],
  TMZ: ["Thames Aerodrome"],
  TNA: ["Yaoqiang Airport", "Jinan"],
  TNB: ["Tanah Grogot Airport", "Tanah Grogot-Borneo Island"],
  TNC: ["Tin City Long Range Radar Station Airport", "Tin City"],
  TND: ["Alberto Delgado Airport", "Trinidad"],
  TNE: ["New Tanegashima Airport", "Tanegashima"],
  TNF: ["Toussus-le-Noble Airport", "Toussus-le-Noble"],
  TNG: ["Ibn Batouta Airport", "Tangier"],
  TNH: ["Tonghua Sanyuanpu Airport", "Tonghua"],
  TNI: ["Satna Airport"],
  TNJ: [
    "Raja Haji Fisabilillah International Airport",
    "Tanjung Pinang-Bintan Island",
  ],
  TNK: ["Tununak Airport", "Tununak"],
  TNM: ["Teniente Rodolfo Marsh Martin Base", "Isla Rey Jorge"],
  TNN: ["Tainan Airport", "Tainan City"],
  TNO: ["Tamarindo Airport", "Tamarindo"],
  TNP: ["Twentynine Palms Airport", "Twentynine Palms"],
  TNQ: ["Washington Island Airstrip", "Teraina"],
  TNR: ["Ivato Airport", "Antananarivo"],
  TNS: ["Tungsten (Cantung) Airport", "Tungsten"],
  TNT: ["Dade Collier Training and Transition Airport", "Miami"],
  TNU: ["Newton Municipal Airport", "Newton"],
  TNV: ["Tabuaeran Island Airport", "Tabuaeran Island"],
  TNW: ["Jumandy Airport", "Tena"],
  TNX: ["Stung Treng Airport", "Stung Treng"],
  TNZ: ["Tosontsengel Airport", "Tosontsengel"],
  TOA: ["Zamperini Field", "Torrance"],
  TOB: ["Gamal Abdel Nasser Airport", "Tobruk"],
  TOC: ["Toccoa Airport - R.G. Letourneau Field", "Toccoa"],
  TOD: ["Pulau Tioman Airport", "Pulau Tioman"],
  TOE: ["Tozeur Nefta International Airport", "Tozeur"],
  TOF: ["Bogashevo Airport", "Tomsk"],
  TOG: ["Togiak Airport", "Togiak Village"],
  TOH: ["Torres Airstrip", "Loh/Linua"],
  TOI: ["Troy Municipal Airport at N Kenneth Campbell Field", "Troy"],
  TOJ: ["Torrej\u00f3n Airport", "Madrid"],
  TOK: ["Torokina Airport", "Torokina"],
  TOL: ["Toledo Express Airport", "Toledo"],
  TOM: ["Timbuktu Airport", "Timbuktu"],
  TON: ["Tonu Airport", "Tonu"],
  TOO: ["San Vito De Java Airport", "Coto Brus"],
  TOP: ["Philip Billard Municipal Airport", "Topeka"],
  TOQ: ["Barriles Airport", "Tocopilla"],
  TOR: ["Torrington Municipal Airport", "Torrington"],
  TOS: ["Troms\u00f8 Airport", "Troms\u00f8"],
  TOT: ["Totness Airport", "Totness"],
  TOU: ["Touho Airport", "Touho"],
  TOW: ["Toledo Airport", "Toledo"],
  TOX: ["Tobolsk Airport", "Tobolsk"],
  TOY: ["Toyama Airport", "Toyama"],
  TOZ: ["Mahana Airport", "Touba"],
  TPA: ["Tampa International Airport", "Tampa"],
  TPC: ["Tarapoa Airport", "Tarapoa"],
  TPE: ["Taiwan Taoyuan International Airport", "Taipei"],
  TPF: ["Peter O Knight Airport", "Tampa"],
  TPG: ["Taiping (Tekah) Airport", "Taiping"],
  TPH: ["Tonopah Airport", "Tonopah"],
  TPI: ["Tapini Airport", "Tapini"],
  TPJ: ["Taplejung Airport", "Taplejung"],
  TPK: ["Teuku Cut Ali Airport", "Tapak Tuan-Sumatra Island"],
  TPL: ["Draughon Miller Central Texas Regional Airport", "Temple"],
  TPN: ["Tiputini Airport", "Tiputini"],
  TPP: ["Cadete FAP Guillermo Del Castillo Paredes Airport", "Tarapoto"],
  TPQ: ["Amado Nervo National Airport", "Tepic"],
  TPR: ["Tom Price Airport", "Tom Price"],
  TPS: ["Vincenzo Florio Airport Trapani-Birgi", "Trapani (TP)"],
  TPT: ["Tapeta Airport", "Tapeta"],
  TPU: ["Tikapur Airport", "Tikapur"],
  TPX: ["Tupai Airport", "Tupai Atoll"],
  TQD: ["Al Taqaddum Air Base", "Al Habbaniyah"],
  TQL: ["Tarko-Sale Airport", "Tarko-Sale"],
  TQN: ["Talolqan Airport", "Taloqan"],
  TQP: ["Trepell Airport", "Trepell"],
  TQQ: ["Maranggo Airport", "Waha-Tomea Island"],
  TQR: ["San Domino Island Heliport", "Tremiti Islands"],
  TQS: ["Tres Esquinas Air Base", "Tres Esquinas"],
  TR7: ["Ottawa / Rockcliffe Seaplane Base", "Ottawa"],
  TRA: ["Tarama Airport", "Tarama"],
  TRB: ["Gonzalo Mej\u00eda Airport", "Turbo"],
  TRC: ["Francisco Sarabia International Airport", "Torre\u00f3n"],
  TRD: ["Trondheim Airport V\u00e6rnes", "Trondheim"],
  TRE: ["Tiree Airport", "Balemartine"],
  TRF: ["Sandefjord Airport, Torp", "Torp"],
  TRG: ["Tauranga Airport", "Tauranga"],
  TRH: ["Trona Airport", "Trona"],
  TRI: ["Tri-Cities Regional TN/VA Airport", "Bristol/Johnson/Kingsport"],
  TRJ: ["Tarakbits Airport", "Tarakbits"],
  TRK: ["Juwata Airport", "Tarakan Island"],
  TRL: ["Terrell Municipal Airport", "Terrell"],
  TRM: ["Jacqueline Cochran Regional Airport", "Palm Springs"],
  TRN: ["Turin Airport", "Torino"],
  TRO: ["Taree Airport", "Taree"],
  TRQ: ["Tarauac\u00e1 Airport", "Tarauac\u00e1"],
  TRR: ["China Bay Airport", "Trincomalee"],
  TRS: ["Trieste\u2013Friuli Venezia Giulia Airport", "Trieste"],
  TRU: [
    "Capitan FAP Carlos Martinez De Pinillos International Airport",
    "Trujillo",
  ],
  TRV: ["Trivandrum International Airport", "Thiruvananthapuram"],
  TRW: ["Bonriki International Airport", "Tarawa"],
  TRY: ["Tororo Airport", "Tororo"],
  TRZ: ["Tiruchirapally Civil Airport Airport", "Tiruchirappally"],
  TSA: ["Taipei Songshan Airport", "Taipei City"],
  TSB: ["Tsumeb Airport", "Tsumeb"],
  TSC: ["Taisha Airport", "Taisha"],
  TSD: ["Tshipise Airport", "Tshipise"],
  TSE: ["Astana International Airport", "Astana"],
  TSF: ["Treviso-Sant'Angelo Airport", "Treviso"],
  TSG: ["Tanacross Airport", "Tanacross"],
  TSH: ["Tshikapa Airport", "Tshikapa"],
  TSI: ["Tsile Tsile Airport", "Tsile Tsile"],
  TSJ: ["Tsushima Airport", "Tsushima"],
  TSK: ["Taskul Airport", "Taskul"],
  TSL: ["Tamuin Airport"],
  TSM: ["Taos Regional Airport", "Taos"],
  TSN: ["Tianjin Binhai International Airport", "Tianjin"],
  TSP: ["Tehachapi Municipal Airport", "Tehachapi"],
  TSQ: ["Torres Airport", "Torres"],
  TSR: ["Timi\u015foara Traian Vuia Airport", "Timi\u015foara"],
  TSS: ["East 34th Street Heliport", "New York"],
  TST: ["Trang Airport"],
  TSU: ["Tabiteuea South Airport", "Tabiteuea South"],
  TSV: ["Townsville Airport", "Townsville"],
  TSW: ["Tsewi Airport", "Tsewi"],
  TSX: ["Tanjung Santan Airport", "Santan-Borneo Island"],
  TSY: ["Cibeureum Airport", "Tasikmalaya-Java Island"],
  TSZ: ["Tselserleg Airport", "Tselserleg"],
  TTA: ["Tan Tan Airport", "Tan Tan"],
  TTB: ["Tortol\u00ec Airport", "Arbatax"],
  TTC: ["Las Breas Airport", "Taltal"],
  TTD: ["Portland Troutdale Airport", "Portland"],
  TTE: ["Sultan Khairun Babullah Airport", "Sango-Ternate Island"],
  TTG: ["General Enrique Mosconi Airport", "Tartagal"],
  TTH: ["Thumrait Air Base", "Thumrait"],
  TTI: ["Tetiaroa Airport", "Tetiaroa"],
  TTJ: ["Tottori Airport", "Tottori"],
  TTL: ["Turtle Island Seaplane Base", "Nanuya Levu Island"],
  TTN: ["Trenton Mercer Airport", "Trenton"],
  TTO: ["Britton Municipal Airport", "Britton"],
  TTQ: ["Aerotortuguero Airport", "Roxana"],
  TTR: ["Pongtiku Airport", "Makale"],
  TTS: ["Tsaratanana Airport", "Tsaratanana"],
  TTT: ["Taitung Airport", "Taitung City"],
  TTU: ["Saniat R'mel Airport", "T\u00e9touan"],
  TTW: ["Tissa Tank Waterdrome", "Tissamaharama"],
  TTX: ["Truscott-Mungalalu Airport", "Anjo Peninsula"],
  TUA: ["Teniente Coronel Luis a Mantilla Airport", "Tulc\u00e1n"],
  TUB: ["Tubuai Airport"],
  TUC: ["Teniente Benjamin Matienzo Airport", "San Miguel de Tucum\u00e1n"],
  TUD: ["Tambacounda Airport", "Tambacounda"],
  TUE: ["Tupile Airport", "Isla Tupile"],
  TUF: ["Tours-Val-de-Loire Airport", "Tours/Val de Loire (Loire Valley)"],
  TUG: ["Tuguegarao Airport", "Tuguegarao City"],
  TUH: ["Arnold Air Force Base", "Tullahoma"],
  TUI: ["Turaif Domestic Airport", "Turaif"],
  TUJ: ["Tum Airport", "Tum"],
  TUK: ["Turbat International Airport", "Turbat"],
  TUL: ["Tulsa International Airport", "Tulsa"],
  TUM: ["Tumut Airport"],
  TUN: ["Tunis Carthage International Airport", "Tunis"],
  TUO: ["Taupo Airport", "Taupo"],
  TUP: ["Tupelo Regional Airport", "Tupelo"],
  TUQ: ["Tougan Airport", "Tougan"],
  TUR: ["Tucuru\u00ed Airport", "Tucuru\u00ed"],
  TUS: ["Tucson International Airport", "Tucson"],
  TUT: ["Tauta Airport", "Tauta"],
  TUU: ["Tabuk Airport", "Tabuk"],
  TUV: ["Tucupita Airport", "Tucupita"],
  TUX: ["Tumbler Ridge Airport", "Tumbler Ridge"],
  TUY: ["Tulum Naval Air Station", "Tulum"],
  TUZ: ["Tucum\u00e3 Airport", "Tucum\u00e3"],
  TVA: ["Morafenobe Airport", "Morafenobe"],
  TVC: ["Cherry Capital Airport", "Traverse City"],
  TVF: ["Thief River Falls Regional Airport", "Thief River Falls"],
  TVI: ["Thomasville Regional Airport", "Thomasville"],
  TVL: ["Lake Tahoe Airport", "South Lake Tahoe"],
  TVS: ["Tangshan Sann\u00fche Airport", "Tangshan"],
  TVU: ["Matei Airport", "Matei"],
  TVY: ["Dawei Airport", "Dawei"],
  TWA: ["Twin Hills Airport", "Twin Hills"],
  TWB: ["Toowoomba Airport", "Toowoomba"],
  TWD: ["Jefferson County International Airport", "Port Townsend"],
  TWE: ["Taylor Airport", "Taylor"],
  TWF: ["Joslin Field Magic Valley Regional Airport", "Twin Falls"],
  TWH: ["Two Harbors Amphibious Terminal", "Two Harbors"],
  TWP: ["Torwood Airport", "Torwood"],
  TWT: ["Sanga Sanga Airport"],
  TWU: ["Tawau Airport", "Tawau"],
  TWY: ["Tawa Airport", "Tawa"],
  TWZ: ["Pukaki Airport", "Twitzel"],
  TXF: ["9 de Maio - Teixeira de Freitas Airport", "Teixeira De Freitas"],
  TXG: ["Taichung Airport", "Taichung City"],
  TXK: ["Texarkana Regional Webb Field", "Texarkana"],
  TXL: ["Berlin-Tegel Airport", "Berlin"],
  TXM: ["Teminabuan Airport", "Atinjoe-Papua Island"],
  TXN: ["Tunxi International Airport", "Huangshan"],
  TXR: ["Tanbar Airport", "Tanbar Station"],
  TXU: ["Tabou Airport", "Tabou"],
  TYA: ["Klokovo Airfield", "Tula"],
  TYB: ["Tibooburra Airport"],
  TYD: ["Tynda Airport", "Tynda"],
  TYE: ["Tyonek Airport", "Tyonek"],
  TYF: ["Torsby Airport"],
  TYG: ["Thylungra Airport"],
  TYL: ["Capitan Montes Airport", "Talara"],
  TYM: ["Staniel Cay Airport"],
  TYN: ["Taiyuan Wusu Airport", "Taiyuan"],
  TYP: ["Tobermorey Airport", "Tobermorey"],
  TYR: ["Tyler Pounds Regional Airport", "Tyler"],
  TYS: ["McGhee Tyson Airport", "Knoxville"],
  TYT: ["Treinta y Tres Airport", "Treinta y Tres"],
  TYZ: ["Taylor Airport", "Taylor"],
  TZA: ["Belize City Municipal Airport", "Belize City"],
  TZC: ["Tuscola Area Airport", "Caro"],
  TZL: ["Tuzla International Airport", "Tuzla"],
  TZM: ["Cupul Airport", "Tizimin"],
  TZR: ["Tasz\u00e1r Air Base", "Tasz\u00e1r"],
  TZX: ["Trabzon International Airport", "Trabzon"],
  UAB: ["\u0130ncirlik Air Base", "Adana"],
  UAC: ["San Luis R\u00edo Colorado Airport", "San Luis R\u00edo Colorado"],
  UAE: ["Mount Aue Airport"],
  UAH: ["Ua Huka Airport", "Ua Huka"],
  UAI: ["Suai Airport", "Suai"],
  UAK: ["Narsarsuaq Airport", "Narsarsuaq"],
  UAL: ["Luau Airport", "Luau"],
  UAM: ["Andersen Air Force Base", "Yigo, Mariana Island"],
  UAP: ["Ua Pou Airport", "Ua Pou"],
  UAQ: ["Domingo Faustino Sarmiento Airport", "San Juan"],
  UAR: ["Bouarfa Airport", "Bouarfa"],
  UAS: ["Buffalo Spring", "Samburu"],
  UAX: ["Uaxactun Airport", "Uaxactun"],
  UBA: ["M\u00e1rio de Almeida Franco Airport", "Uberaba"],
  UBB: ["Mabuiag Island Airport", "Mabuiag Island"],
  UBI: ["Buin Airport", "Buin"],
  UBJ: ["Yamaguchi Ube Airport", "Ube"],
  UBP: ["Ubon Ratchathani Airport", "Ubon Ratchathani"],
  UBR: ["Ubrub Airport", "Ubrub-Papua Island"],
  UBS: ["Columbus Lowndes County Airport", "Columbus"],
  UBT: ["Ubatuba Airport", "Ubatuba"],
  UBU: ["Kalumburu Airport"],
  UCA: ["Oneida County Airport", "Utica"],
  UCB: ["Ulanqab Jining Airport", "Ulanqab"],
  UCC: ["Yucca Airstrip", "Mercury"],
  UCE: ["Eunice Airport", "Eunice"],
  UCK: ["Lutsk Airport", "Lutsk"],
  UCN: ["Buchanan Airport", "Buchanan"],
  UCT: ["Ukhta Airport", "Ukhta"],
  UCY: ["Everett-Stewart Regional Airport", "Union City"],
  UCZ: ["Uchiza Airport", "Uchiza"],
  UDA: ["Undara Airport"],
  UDD: ["Bermuda Dunes Airport", "Palm Springs"],
  UDI: ["Ten. Cel. Aviador C\u00e9sar Bombonato Airport", "Uberl\u00e2ndia"],
  UDJ: ["Uzhhorod International Airport", "Uzhhorod"],
  UDN: ["Udine-Campoformido Air Base", "Udine"],
  UDR: ["Maharana Pratap Airport", "Udaipur"],
  UEE: ["Queenstown Airport"],
  UEL: ["Quelimane Airport", "Quelimane"],
  UEN: ["Urengoy Airport", "Urengoy"],
  UEO: ["Kumejima Airport"],
  UES: ["Waukesha County Airport", "Waukesha"],
  UET: ["Quetta International Airport", "Quetta"],
  UFA: ["Ufa International Airport", "Ufa"],
  UGA: ["Bulgan Airport", "Bulgan"],
  UGB: ["Ugashik Bay Airport", "Pilot Point"],
  UGC: ["Urgench Airport", "Urgench"],
  UGI: ["San Juan /Uganik/ Seaplane Base", "San Juan"],
  UGN: ["Waukegan National Airport", "Chicago/Waukegan"],
  UGO: ["Uige Airport", "Uige"],
  UGS: ["Ugashik Airport", "Ugashik"],
  UGT: ["Bulagtai Resort Airport", "Umnugobitour"],
  UGU: ["Bilogai-Sugapa Airport", "Sugapa-Papua Island"],
  UHE: ["Kunovice Airport", "Uhersk\u00e9 Hradi\u0161t\u011b"],
  UHF: ["RAF Upper Heyford", "Upper Heyford"],
  UIB: ["El Cara\u00f1o Airport", "Quibd\u00f3"],
  UIH: ["Phu Cat Airport", "Quy Nohn"],
  UII: ["Utila Airport", "Utila Island"],
  UIK: ["Ust-Ilimsk Airport", "Ust-Ilimsk"],
  UIL: ["Quillayute Airport", "Quillayute"],
  UIN: ["Quincy Regional Baldwin Field", "Quincy"],
  UIO: ["Mariscal Sucre International Airport", "Quito"],
  UIP: ["Quimper-Cornouaille Airport", "Quimper/Pluguffan"],
  UIQ: ["Quion Hill Airport", "Quion Hill"],
  UIR: ["Quirindi Airport"],
  UIT: ["Jaluit Airport", "Jabor Jaluit Atoll"],
  UIZ: ["Berz-Macomb Airport"],
  UJE: ["Ujae Atoll Airport", "Ujae Atoll"],
  UJN: ["Uljin Airport", "Bongsan-ri, Uljin"],
  UKA: ["Ukunda Airstrip", "Ukunda"],
  UKB: ["Kobe Airport", "Kobe"],
  UKG: ["Ust-Kuyga Airport", "Ust-Kuyga"],
  UKH: ["Mukhaizna Airport", "Mukhaizna Oil Field"],
  UKI: ["Ukiah Municipal Airport", "Ukiah"],
  UKK: ["Ust-Kamennogorsk Airport", "Ust Kamenogorsk"],
  UKN: ["Waukon Municipal Airport", "Waukon"],
  UKR: ["Mukeiras Airport", "Mukayras"],
  UKS: ["Belbek Airport", "Sevastopol"],
  UKT: ["Quakertown Airport", "Quakertown"],
  UKU: ["Nuku Airport", "Nuku"],
  UKX: ["Ust-Kut Airport", "Ust-Kut"],
  ULA: ["Capitan D Daniel Vazquez Airport", "San Julian"],
  ULB: ["Ul\u00e9i Airport", "Ambryn Island"],
  ULC: ["Los Cerrillos Airport", "Santiago"],
  ULD: ["Prince Mangosuthu Buthelezi Airport", "Ulundi"],
  ULE: ["Sule Airport", "Sule"],
  ULG: ["Ulgii Mongolei Airport"],
  ULH: ["Majeed Bin Abdulaziz Airport", "Al Ula"],
  ULI: ["Ulithi Airport", "Falalop Island"],
  ULK: ["Lensk Airport", "Lensk"],
  ULM: ["New Ulm Municipal Airport", "New Ulm"],
  ULN: ["Chinggis Khaan International Airport", "Ulan Bator"],
  ULO: ["Ulaangom Airport", "Ulaangom"],
  ULP: ["Quilpie Airport"],
  ULQ: ["Heriberto G\u00edl Mart\u00ednez Airport", "Tulu\u00e1"],
  ULU: ["Gulu Airport", "Gulu"],
  ULV: ["Ulyanovsk Baratayevka Airport", "Ulyanovsk"],
  ULX: ["Ulusaba Airport", "Ulusaba"],
  ULY: ["Ulyanovsk East Airport", "Ulyanovsk"],
  ULZ: ["Donoi Airport", "Uliastai"],
  UMA: ["Punta de Maisi Airport", "Maisi"],
  UMC: ["Umba Airport", "Umba"],
  UMD: ["Uummannaq Heliport", "Uummannaq"],
  UME: ["Ume\u00e5 Airport", "Ume\u00e5"],
  UMI: ["Quince Air Base", "Quince Mil"],
  UMM: ["Summit Airport", "Summit"],
  UMR: ["Woomera Airfield", "Woomera"],
  UMS: ["Ust-Maya Airport", "Ust-Maya"],
  UMT: ["Umiat Airport", "Umiat"],
  UMU: ["Umuarama Airport", "Umuarama"],
  UMY: ["Sumy Airport", "Sumy"],
  UNA: ["Hotel Transam\u00e9rica Airport", "Una"],
  UNC: ["Unguia Airport", "Arquia"],
  UND: ["Konduz Airport"],
  UNE: ["Qacha's Nek Airport", "Qacha's Nek"],
  UNG: ["Kiunga Airport", "Kiunga"],
  UNI: ["Union Island International Airport", "Union Island"],
  UNK: ["Unalakleet Airport", "Unalakleet"],
  UNN: ["Ranong Airport"],
  UNR: ["\u00d6nd\u00f6rkhaan Airport", "\u00d6nd\u00f6rkhaan"],
  UNT: ["Unst Airport", "Shetland Islands"],
  UNU: ["Dodge County Airport", "Juneau"],
  UOA: ["Mururoa Atoll Airport", "Mururoa Atoll"],
  UOL: ["Buol Airport", "Buol-Celebes Island"],
  UOS: ["Franklin County Airport", "Sewanee"],
  UOX: ["University Oxford Airport", "Oxford"],
  UPB: ["Playa Baracoa Airport", "Havana"],
  UPG: ["Hasanuddin International Airport", "Ujung Pandang-Celebes Island"],
  UPL: ["Upala Airport", "Upala"],
  UPN: ["Licenciado y General Ignacio Lopez Rayon Airport"],
  UPP: ["Upolu Airport", "Hawi"],
  UPR: ["Upiara Airport", "Upiara"],
  UPV: ["Upavon Aerodrome", "Upavon"],
  URA: ["Uralsk Airport", "Uralsk"],
  URB: ["Urubupunga Airport", "Castilho"],
  URC: ["\u00dcr\u00fcmqi Diwopu International Airport", "\u00dcr\u00fcmqi"],
  URD: ["Burg Feuerstein Airport", "Ebermannstadt"],
  URE: ["Kuressaare Airport", "Kuressaare"],
  URG: ["Rubem Berta Airport", "Uruguaiana"],
  URI: ["Uribe Airport", "Uribe"],
  URJ: ["Uray Airport", "Uray"],
  URM: ["Uriman Airport"],
  URN: ["Urgun Airport", "Urgun"],
  URO: ["Rouen Airport", "Rouen/Vall\u00e9e de Seine"],
  URR: ["Urrao Airport", "Urrao"],
  URS: ["Kursk East Airport", "Kursk"],
  URT: ["Surat Thani Airport", "Surat Thani"],
  URU: ["Uroubi Airport", "Uroubi"],
  URY: ["Gurayat Domestic Airport", "Gurayat"],
  URZ: ["Or\u016bzg\u0101n Airport", "Or\u016bzg\u0101n"],
  USA: ["Concord-Padgett Regional Airport", "Concord"],
  USH: ["Malvinas Argentinas Airport", "Ushuahia"],
  USI: ["Mabaruma Airport", "Mabaruma"],
  USK: ["Usinsk Airport", "Usinsk"],
  USL: ["Useless Loop Airport"],
  USM: ["Samui Airport", "Na Thon (Ko Samui Island)"],
  USN: ["Ulsan Airport", "Ulsan"],
  USO: ["Usino Airport", "Usino"],
  USQ: ["U\u015fak Airport", "U\u015fak"],
  USR: ["Ust-Nera Airport", "Ust-Nera"],
  USS: ["Sancti Spiritus Airport", "Sancti Spiritus"],
  UST: ["Northeast Florida Regional Airport", "St Augustine"],
  USU: ["Francisco B. Reyes Airport", "Coron"],
  UTA: ["Mutare Airport", "Mutare"],
  UTB: ["Muttaburra Airport"],
  UTC: ["Soesterberg Air Base", "Soesterberg"],
  UTD: ["Nutwood Downs Airport", "Nutwood Downs"],
  UTE: ["Bultfontein Airport", "Bultfontein"],
  UTG: ["Quthing Airport", "Quthing"],
  UTH: ["Udon Thani Airport", "Udon Thani"],
  UTI: ["Utti Air Base", "Utti / Valkeala"],
  UTK: ["Utirik Airport", "Utirik Island"],
  UTM: ["Tunica Municipal Airport", "Tunica"],
  UTN: ["Pierre Van Ryneveld Airport", "Upington"],
  UTO: ["Indian Mountain LRRS Airport", "Utopia Creek"],
  UTP: ["U-Tapao International Airport", "Rayong"],
  UTR: ["Uttaradit Airport", "Uttaradit"],
  UTS: ["Ust-Tsylma Airport", "Ust-Tsylma"],
  UTT: ["K. D. Matanzima Airport", "Mthatha"],
  UTU: ["Ustupo Airport", "Ustupo"],
  UTW: ["Queenstown Airport", "Queenstown"],
  UUA: ["Bugulma Airport", "Bugulma"],
  UUD: ["Ulan-Ude Airport (Mukhino)", "Ulan Ude"],
  UUK: ["Ugnu-Kuparuk Airport", "Kuparuk"],
  UUN: ["Baruun Urt Airport"],
  UUS: ["Yuzhno-Sakhalinsk Airport", "Yuzhno-Sakhalinsk"],
  UUU: ["Manumu Airport", "Manumu"],
  UVA: ["Garner Field", "Uvalde"],
  UVE: ["Ouv\u00e9a Airport", "Ouv\u00e9a"],
  UVF: ["Hewanorra International Airport", "Vieux Fort"],
  UVL: ["El Kharga Airport"],
  UVO: ["Uvol Airport", "Uvol"],
  UWA: ["Ware Airport", "Ware"],
  UYL: ["Nyala Airport", "Nyala"],
  UYN: ["Yulin Yuyang Airport", "Yulin"],
  UYU: ["Uyuni Airport", "Quijarro"],
  UZM: ["Hope Bay Aerodrome", "Hope Bay"],
  UZR: ["Urzhar Airport", "Urzhar"],
  UZU: ["Curuzu Cuatia Airport", "Curuzu Cuatia"],
  VAA: ["Vaasa Airport", "Vaasa"],
  VAC: ["Varrelbusch Airport", "Cloppenburg"],
  VAD: ["Moody Air Force Base", "Valdosta"],
  VAF: ["Valence-Chabeuil Airport", "Valence/Chabeuil"],
  VAG: ["Major Brigadeiro Trompowsky Airport", "Varginha"],
  VAH: ["Capit\u00e1n Av. Vidal Villagomez Toledo Airport", "Vallegrande"],
  VAI: ["Vanimo Airport", "Vanimo"],
  VAK: ["Chevak Airport", "Chevak"],
  VAL: ["Valen\u00e7a Airport", "Valen\u00e7a"],
  VAM: ["Villa Airport", "Maamigili"],
  VAN: ["Van Ferit Melen Airport", "Van"],
  VAO: ["Suavanao Airport", "Suavanao"],
  VAP: ["Rodelillo Airport", "Vi\u00f1a Del Mar"],
  VAR: ["Varna Airport", "Varna"],
  VAS: ["Sivas Nuri Demira\u011f Airport", "Sivas"],
  VAT: ["Vatomandry Airport", "Vatomandry"],
  VAU: ["Vatukoula Airport", "Vatukoula"],
  VAV: ["Vava'u International Airport", "Vava'u Island"],
  VAW: ["Vard\u00f8 Airport, Svartnes", "Vard\u00f8"],
  VBA: ["Ann Airport", "Aeng"],
  VBG: ["Vandenberg Air Force Base", "Lompoc"],
  VBP: ["Bokpyinn Airport", "Bokpyinn"],
  VBS: ["Brescia Airport", "Montichiari (BS)"],
  VBV: ["Vanua Balavu Airport", "Vanua Balavu"],
  VBY: ["Visby Airport", "Visby"],
  VCA: ["Can Tho International Airport", "Can Tho"],
  VCD: ["Victoria River Downs Airport"],
  VCE: ["Venice Marco Polo Airport", "Venice"],
  VCF: ["Valcheta Airport", "Valcheta"],
  VCH: ["Vichadero Airport", "Vichadero"],
  VCL: ["Chu Lai International Airport", "Dung Quat Bay"],
  VCP: ["Viracopos International Airport", "Campinas"],
  VCR: ["Carora Airport", "Carora"],
  VCS: ["Co Ong Airport", "Con Dao"],
  VCT: ["Victoria Regional Airport", "Victoria"],
  VCV: ["Southern California Logistics Airport", "Victorville"],
  VDA: ["Ovda International Airport", "Eilat"],
  VDB: ["Leirin Airport"],
  VDC: ["Vit\u00f3ria da Conquista Airport", "Vit\u00f3ria Da Conquista"],
  VDE: ["Hierro Airport", "El Hierro Island"],
  VDH: ["Dong Hoi Airport", "Dong Hoi"],
  VDI: ["Vidalia Regional Airport", "Vidalia"],
  VDM: ["Gobernador Castello Airport", "Viedma / Carmen de Patagones"],
  VDP: ["Valle de La Pascua Airport"],
  VDR: ["Villa Dolores Airport", "Villa Dolores"],
  VDS: ["Vads\u00f8 Airport", "Vads\u00f8"],
  VDY: ["Vijayanagar Aerodrome (JSW)"],
  VDZ: ["Valdez Pioneer Field", "Valdez"],
  VEE: ["Venetie Airport", "Venetie"],
  VEL: ["Vernal Regional Airport", "Vernal"],
  VER: ["General Heriberto Jara International Airport", "Veracruz"],
  VEV: ["Barakoma Airport", "Barakoma"],
  VEX: ["Tioga Municipal Airport", "Tioga"],
  VEY: ["Vestmannaeyjar Airport", "Vestmannaeyjar"],
  VFA: ["Victoria Falls International Airport", "Victoria Falls"],
  VGA: ["Vijayawada Airport"],
  VGD: ["Vologda Airport", "Vologda"],
  VGO: ["Vigo Airport", "Vigo"],
  VGS: ["General Villegas Airport", "General Villegas"],
  VGT: ["North Las Vegas Airport", "Las Vegas"],
  VGZ: ["Villa Garz\u00f3n Airport", "Villa Garz\u00f3n"],
  VHC: ["Saurimo Airport", "Saurimo"],
  VHM: ["Vilhelmina Airport"],
  VHN: ["Culberson County Airport", "Van Horn"],
  VHV: ["Verkhnevilyuisk Airport", "Verkhnevilyuisk"],
  VHY: ["Vichy-Charmeil Airport", "Vichy/Charmeil"],
  VHZ: ["Vahitahi Airport", "Vahitahi"],
  VIA: ["Videira Airport", "Videira"],
  VIB: ["Villa Constituci\u00f3n Airport", "Ciudad Constituci\u00f3n"],
  VIC: ["Vicenza Airport", "Vicenza"],
  VIE: ["Vienna International Airport", "Vienna"],
  VIG: ["Juan Pablo P\u00e9rez Alfonso Airport", "El Vig\u00eda"],
  VIH: ["Rolla National Airport", "Rolla/Vichy"],
  VII: ["Vinh Airport", "Vinh"],
  VIJ: ["Virgin Gorda Airport", "Spanish Town"],
  VIL: ["Dakhla Airport", "Dakhla"],
  VIN: ["Vinnytsia/Gavyryshivka Airport", "Vinnitsa"],
  VIQ: ["Viqueque Airport", "Viqueque"],
  VIR: ["Virginia Airport", "Durban"],
  VIS: ["Visalia Municipal Airport", "Visalia"],
  VIT: ["Vitoria/Foronda Airport", "Alava"],
  VIU: ["Viru Harbour Airstrip", "Viru"],
  VIV: ["Vivigani Airfield", "Vivigani"],
  VIX: ["Eurico de Aguiar Salles Airport", "Vit\u00f3ria"],
  VIY: [
    "Villacoublay-V\u00e9lizy (BA 107) Air Base",
    "Villacoublay/V\u00e9lizy",
  ],
  VJB: ["Xai-Xai Airport", "Xai-Xai"],
  VJI: ["Virginia Highlands Airport", "Abingdon"],
  VJQ: ["Gurue Airport", "Gurue"],
  VKG: ["Rach Gia Airport", "Rach Gia"],
  VKO: ["Vnukovo International Airport", "Moscow"],
  VKS: ["Vicksburg Municipal Airport", "Vicksburg"],
  VKT: ["Vorkuta Airport", "Vorkuta"],
  VLA: ["Vandalia Municipal Airport", "Vandalia"],
  VLC: ["Valencia Airport", "Valencia"],
  VLD: ["Valdosta Regional Airport", "Valdosta"],
  VLE: ["Valle Airport", "Grand Canyon"],
  VLG: ["Villa Gesell Airport", "Villa Gesell"],
  VLI: ["Bauerfield International Airport", "Port Vila"],
  VLK: ["Volgodonsk Airport"],
  VLL: ["Valladolid Airport", "Valladolid"],
  VLM: ["Teniente Coronel Rafael Pab\u00f3n Airport", "Villamontes"],
  VLN: ["Arturo Michelena International Airport", "Valencia"],
  VLP: ["Vila Rica Airport", "Vila Rica"],
  VLR: ["Vallenar Airport", "Vallenar"],
  VLS: ["Valesdir Airport", "Epi Island"],
  VLU: ["Velikiye Luki Airport", "Velikiye Luki"],
  VLV: ["Dr. Antonio Nicol\u00e1s Brice\u00f1o Airport", "Valera"],
  VLY: ["Anglesey Airport", "Angelsey"],
  VME: ["Villa Reynolds Airport", "Villa Mercedes"],
  VMI: ["Dr Juan Plate Airport", "Puerto Vallemi"],
  VMU: ["Baimuru Airport", "Baimuru"],
  VNA: ["Saravane Airport", "Saravane"],
  VNC: ["Venice Municipal Airport", "Venice"],
  VND: ["Vangaindrano Airport", "Vangaindrano"],
  VNE: ["Vannes-Meucon Airport", "Vannes/Meucon"],
  VNO: ["Vilnius International Airport", "Vilnius"],
  VNR: ["Vanrook Station Airport"],
  VNS: ["Lal Bahadur Shastri Airport", "Varanasi"],
  VNT: ["Ventspils International Airport", "Ventspils"],
  VNX: ["Vilankulo Airport", "Vilanculo"],
  VNY: ["Van Nuys Airport", "Van Nuys"],
  VOD: ["Vodochody Airport", "Vodochody"],
  VOG: ["Volgograd International Airport", "Volgograd"],
  VOH: ["Vohimarina Airport"],
  VOI: ["Voinjama Airport", "Voinjama"],
  VOK: ["Volk Field", "Camp Douglas"],
  VOL: ["Nea Anchialos Airport", "Nea Anchialos"],
  VOT: ["Votuporanga Airport", "Votuporanga"],
  VOZ: ["Voronezh International Airport", "Voronezh"],
  VPE: ["Ngjiva Pereira Airport", "Ngiva"],
  VPG: ["Vipingo Estate Airport", "Vipingo Estate"],
  VPN: ["Vopnafj\u00f6r\u00f0ur Airport", "Vopnafj\u00f6r\u00f0ur"],
  VPS: ["Destin-Ft Walton Beach Airport", "Valparaiso"],
  VPY: ["Chimoio Airport", "Chimoio"],
  VPZ: ["Porter County Municipal Airport", "Valparaiso"],
  VQQ: ["Cecil Airport", "Jacksonville"],
  VQS: ["Antonio Rivera Rodriguez Airport", "Vieques Island"],
  VRA: ["Juan Gualberto Gomez International Airport", "Varadero"],
  VRB: ["Vero Beach Regional Airport", "Vero Beach"],
  VRC: ["Virac Airport", "Virac"],
  VRE: ["Vredendal Airport", "Vredendal"],
  VRI: ["Varandey Airport", "Varandey"],
  VRK: ["Varkaus Airport", "Varkaus / Joroinen"],
  VRL: ["Vila Real Airport", "Vila Real"],
  VRN: ["Verona Villafranca Airport", "Verona"],
  VRO: ["Kawama Airport", "Matanzas"],
  VRS: ["Roy Otten Memorial Airfield", "Versailles"],
  VRU: ["Vryburg Airport", "Vyrburg"],
  VRY: ["V\u00e6r\u00f8y Heliport", "V\u00e6r\u00f8y"],
  VSA: ["Carlos Rovirosa P\u00e9rez International Airport", "Villahermosa"],
  VSE: ["Aerodromo Goncalves Lobato (Viseu Airport)", "Viseu"],
  VSF: ["Hartness State (Springfield) Airport", "Springfield"],
  VSG: ["Luhansk International Airport", "Luhansk"],
  VSK: ["Vista Field", "Kennewick"],
  VST: [
    "Stockholm V\u00e4ster\u00e5s Airport",
    "Stockholm / V\u00e4ster\u00e5s",
  ],
  VTB: ["Vitebsk Vostochny Airport", "Vitebsk"],
  VTE: ["Wattay International Airport", "Vientiane"],
  VTF: ["Vatulele Airport", "Vatulele"],
  VTG: ["Vung Tau Airport", "Vung Tau"],
  VTM: ["Nevatim Air Base", "Beersheba"],
  VTN: ["Miller Field", "Valentine"],
  VTU: ["Hermanos Ameijeiras Airport", "Las Tunas"],
  VTZ: ["Vishakhapatnam Airport", "Visakhapatnam"],
  VUP: ["Alfonso L\u00f3pez Pumarejo Airport", "Valledupar"],
  VUS: ["Velikiy Ustyug Airport", "Velikiy Ustyug"],
  VUU: ["Mvuu Camp Airport", "Liwonde National Park"],
  VVB: ["Mahanoro Airport", "Mahanoro"],
  VVC: ["Vanguardia Airport", "Villavicencio"],
  VVI: ["Viru Viru International Airport", "Santa Cruz"],
  VVK: ["V\u00e4stervik Airport", "V\u00e4stervik"],
  VVN: ["Las Malvinas/Echarate Airport", "Las Malvinas"],
  VVO: ["Vladivostok International Airport", "Vladivostok"],
  VVZ: ["Illizi Takhamalt Airport", "Illizi"],
  VXC: ["Lichinga Airport", "Lichinga"],
  VXE: ["S\u00e3o Pedro Airport", "S\u00e3o Pedro"],
  VXO: ["V\u00e4xj\u00f6 Kronoberg Airport", "V\u00e4xj\u00f6"],
  VYD: ["Vryheid Airport", "Vryheid"],
  VYI: ["Vilyuisk Airport", "Vilyuisk"],
  VYS: ["Illinois Valley Regional Airport-Walter A Duncan Field", "Peru"],
  WAA: ["Wales Airport", "Wales"],
  WAB: ["Wabag Airport", "Wabag"],
  WAC: ["Waca Airport", "Waca"],
  WAD: ["Andriamena Airport", "Andriamena"],
  WAE: ["Wadi Al Dawasir Airport"],
  WAF: ["Wana Airport", "Waana"],
  WAG: ["Wanganui Airport", "Wanganui"],
  WAH: ["Harry Stern Airport", "Wahpeton"],
  WAI: ["Ambalabe Airport", "Antsohihy"],
  WAJ: ["Wawoi Falls Airport", "Wavoi Falls"],
  WAK: ["Ankazoabo Airport", "Ankazoabo"],
  WAL: ["Wallops Flight Facility Airport", "Wallops Island"],
  WAM: ["Ambatondrazaka Airport", "Ambatondrazaka"],
  WAN: ["Waverney Airport", "Waverney"],
  WAO: ["Wabo Airport", "Wabo"],
  WAP: ["Alto Palena Airport", "Alto Palena"],
  WAQ: ["Antsalova Airport", "Antsalova"],
  WAR: ["Waris Airport", "Swach"],
  WAT: ["Waterford Airport", "Waterford"],
  WAV: ["Wave Hill Airport"],
  WAW: ["Warsaw Chopin Airport", "Warsaw"],
  WAX: ["Zwara Airport", "Zuwara"],
  WAY: ["Greene County Airport", "Waynesburg"],
  WAZ: ["Warwick Airport"],
  WBA: ["Wahai,Seram Island", "Seram Island"],
  WBB: ["Stebbins Airport", "Stebbins"],
  WBC: ["Wapolu Airport", "Wapolu"],
  WBD: ["Avaratra Airport", "Befandriana"],
  WBG: ["Schleswig Air Base", "Jagel"],
  WBM: ["Wapenamanda Airport"],
  WBO: ["Antsoa Airport", "Beroroha"],
  WBQ: ["Beaver Airport", "Beaver"],
  WBR: ["Roben Hood Airport", "Big Rapids"],
  WBU: ["Boulder Municipal Airport", "Boulder"],
  WBW: ["Wilkes Barre Wyoming Valley Airport", "Wilkes-Barre"],
  WCA: ["Gamboa Airport", "Castro"],
  WCH: ["Chait\u00e9n Airport", "Chaiten"],
  WCR: ["Chandalar Lake Airport", "Chandalar Lake"],
  WDA: ["Wadi Ain Airport", "Wadi Ain"],
  WDG: ["Enid Woodring Regional Airport", "Enid"],
  WDH: ["Hosea Kutako International Airport", "Windhoek"],
  WDI: ["Wondai Airport"],
  WDN: ["Waldronaire Airport", "East Sound"],
  WDR: ["Barrow County Airport", "Winder"],
  WDS: ["Shiyan Wudangshan Airport", "Shiyan"],
  WEA: ["Parker County Airport", "Weatherford"],
  WED: ["Wedau Airport", "Wedau"],
  WEF: ["Weifang Airport", "Weifang"],
  WEH: ["Weihai Airport", "Weihai"],
  WEI: ["Weipa Airport", "Weipa"],
  WEM: ["RAF West Malling", "West Malling"],
  WEP: ["Weam Airport", "Weam"],
  WET: ["Wagethe Airport", "Wagethe-Papua Island"],
  WEW: ["Wee Waa Airport"],
  WFB: ["Ketchikan Harbor Seaplane Base", "Ketchikan"],
  WFI: ["Fianarantsoa Airport"],
  WFK: ["Northern Aroostook Regional Airport", "Frenchville"],
  WGA: ["Wagga Wagga City Airport", "Wagga Wagga"],
  WGB: ["Bahawalnagar Airport", "Bahawalnagar"],
  WGC: ["Warangal Airport", "Warangal"],
  WGE: ["Walgett Airport"],
  WGN: ["Shaoyang Wugang Airport", "Shaoyang"],
  WGO: ["Winchester Regional Airport", "Winchester"],
  WGP: ["Umbu Mehang Kunda Airport", "Waingapu-Sumba Island"],
  WGT: ["Wangaratta Airport"],
  WGU: ["Wagau Airport", "Wagau"],
  WGY: ["Wagny Airport", "Wagny"],
  WHD: ["Hyder Seaplane Base", "Hyder"],
  WHF: ["Wadi Halfa Airport", "Wadi Halfa"],
  WHK: ["Whakatane Airport"],
  WHL: ["Welshpool Airport", "Welshpool"],
  WHO: ["Franz Josef Aerodrome"],
  WHP: ["Whiteman Airport", "Los Angeles"],
  WHS: ["Whalsay Airport", "Whalsay"],
  WHT: ["Wharton Regional Airport", "Wharton"],
  WHU: ["Wuhu Air Base", "Wuhu"],
  WIB: ["Wilbarger County Airport", "Vernon"],
  WIC: ["Wick Airport", "Wick"],
  WID: ["RAF Wildenrath", "British Armed Forces"],
  WIE: ["Wiesbaden Army Airfield", "Wiesbaden"],
  WIK: ["Waiheke Reeve Airport"],
  WIL: ["Nairobi Wilson Airport", "Nairobi"],
  WIN: ["Winton Airport"],
  WIO: ["Wilcannia Airport"],
  WIR: ["Wairoa Airport", "Wairoa"],
  WIT: ["Wittenoom Airport"],
  WIU: ["Witu Airport", "Garove Island"],
  WJA: ["Woja Airport", "Woja"],
  WJF: ["General WM J Fox Airfield", "Lancaster"],
  WJR: ["Wajir Airport", "Wajir"],
  WJU: ["Wonju/Hoengseong Air Base (K-38/K-46)", "Wonju"],
  WKA: ["Wanaka Airport"],
  WKB: ["Warracknabeal Airport"],
  WKF: ["Waterkloof Air Force Base", "Pretoria"],
  WKI: ["Hwange (Town) Airport", "Hwange"],
  WKJ: ["Wakkanai Airport", "Wakkanai"],
  WKK: ["Aleknagik / New Airport", "Aleknagik"],
  WKL: ["Waikoloa Heliport", "Waikoloa Village"],
  WKN: ["Wakunai Airport", "Wakunai"],
  WKR: ["Abaco I Walker C Airport"],
  WLA: ["Wallal Airport", "Wallal"],
  WLC: ["Walcha Airport"],
  WLD: ["Strother Field", "Winfield/Arkansas City"],
  WLE: ["Miles Airport"],
  WLG: ["Wellington International Airport", "Wellington"],
  WLH: ["Walaha Airport", "Walaha"],
  WLK: ["Selawik Airport", "Selawik"],
  WLL: ["Wollogorang Airport"],
  WLO: ["Waterloo Airport"],
  WLP: ["West Angelas Airport"],
  WLR: ["Loring Seaplane Base", "Loring"],
  WLS: ["Hihifo Airport", "Wallis Island"],
  WLW: ["Willows Glenn County Airport", "Willows"],
  WMA: ["Mandritsara Airport", "Mandritsara"],
  WMB: ["Warrnambool Airport"],
  WMC: ["Winnemucca Municipal Airport", "Winnemucca"],
  WMD: ["Mandabe Airport", "Mandabe"],
  WME: ["Mount Keith Airport"],
  WMH: ["Ozark Regional Airport", "Mountain Home"],
  WMI: ["Modlin Airport", "Warsaw"],
  WMK: ["Meyers Chuck Seaplane Base", "Meyers Chuck"],
  WML: ["Malaimbandy Airport", "Malaimbandy"],
  WMN: ["Maroantsetra Airport"],
  WMO: ["White Mountain Airport", "White Mountain"],
  WMP: ["Mampikony Airport", "Mampikony"],
  WMR: ["Mananara Nord Airport", "Mananara Nord"],
  WMT: ["Zunyi Maotai Airport", "Zunyi"],
  WMX: ["Wamena Airport", "Wamena-Papua Island"],
  WNA: ["Napakiak Airport", "Napakiak"],
  WND: ["Windarra Airport"],
  WNH: ["Wenshan Puzhehei Airport", "Wenshan"],
  WNN: ["Wunnumin Lake Airport", "Wunnumin Lake"],
  WNP: ["Naga Airport", "Naga"],
  WNR: ["Windorah Airport"],
  WNS: ["Shaheed Benazirabad Airport", "Nawabashah"],
  WNU: ["Wanuma Airport", "Wanuma"],
  WNZ: ["Wenzhou Longwan International Airport", "Wenzhou"],
  WOA: ["Wonenara Airport", "Wonenara"],
  WOE: ["Woensdrecht Air Base", "Bergen Op Zoom"],
  WOK: ["Wonken Airport", "Wonken"],
  WOL: ["Wollongong Airport"],
  WON: ["Wondoola Airport", "Wondoola"],
  WOS: ["Wonsan Kalma International Airport", "Wonsan"],
  WOT: ["Wang-an Airport", "Wang-an"],
  WOW: ["Willow Airport", "Willow"],
  WPA: ["Cabo 1\u00b0 Juan Rom\u00e1n Airport", "Puerto Aysen"],
  WPB: ["Port Berg\u00e9 Airport", "Port Berg\u00e9"],
  WPC: ["Pincher Creek Airport", "Pincher Creek"],
  WPK: ["Wrotham Park Airport"],
  WPL: ["Powell Lake Seaplane Base"],
  WPM: ["Wipim Airport", "Wipim"],
  WPO: ["North Fork Valley Airport", "Paonia"],
  WPR: ["Capitan Fuentes Martinez Airport Airport", "Porvenir"],
  WPU: ["Guardiamarina Za\u00f1artu Airport", "Puerto Williams"],
  WRA: ["Warder Airport", "Warder"],
  WRB: ["Robins Air Force Base", "Warner Robins"],
  WRE: ["Whangarei Airport"],
  WRG: ["Wrangell Airport", "Wrangell"],
  WRI: ["Mc Guire Air Force Base", "Wrightstown"],
  WRL: ["Worland Municipal Airport", "Worland"],
  WRN: ["Windarling Airport", "Windarling Mine"],
  WRO: ["Copernicus Wroc\u0142aw Airport", "Wroc\u0142aw"],
  WRT: ["Warton Airport", "Warton"],
  WRW: ["Warrawagine Airport"],
  WRY: ["Westray Airport", "Westray"],
  WRZ: ["Weerawila Airport", "Weerawila"],
  WSA: ["Wasua Airport", "Wasua"],
  WSB: ["Steamboat Bay Seaplane Base", "Steamboat Bay"],
  WSD: ["Condron Army Air Field", "White Sands"],
  WSG: ["Washington County Airport", "Washington"],
  WSH: ["Brookhaven Airport", "Shirley"],
  WSM: ["Wiseman Airport", "Wiseman"],
  WSN: ["South Naknek Nr 2 Airport", "South Naknek"],
  WSO: ["Washabo Airport", "Washabo"],
  WSP: ["Waspam Airport", "Waspam"],
  WSR: ["Wasior Airport", "Wasior-Papua Island"],
  WST: ["Westerly State Airport", "Westerly"],
  WSU: ["Wasu Airport", "Wasu"],
  WSX: ["Westsound/Wsx Seaplane Base", "Westsound"],
  WSY: ["Whitsunday Island Airport"],
  WSZ: ["Westport Airport"],
  WTA: ["Tambohorano Airport", "Tambohorano"],
  WTB: ["Toowoomba Wellcamp Airport", "Toowoomba"],
  WTD: ["West End Airport", "West End"],
  WTE: ["Wotje Atoll Airport", "Wotje Atoll"],
  WTK: ["Noatak Airport", "Noatak"],
  WTL: ["Tuntutuliak Airport", "Tuntutuliak"],
  WTN: ["RAF Waddington", "Waddington"],
  WTO: ["Wotho Island Airport", "Wotho Island"],
  WTP: ["Woitape Airport", "Fatima Mission"],
  WTR: ["Whiteriver Airport", "Whiteriver"],
  WTS: ["Tsiroanomandidy Airport", "Tsiroanomandidy"],
  WTT: ["Wantoat Airport", "Wantoat"],
  WTZ: ["Whitianga Airport"],
  WUA: ["Wuhai Airport", "Wuhai"],
  WUD: ["Wudinna Airport"],
  WUG: ["Wau Airport", "Wau"],
  WUH: ["Wuhan Tianhe International Airport", "Wuhan"],
  WUI: ["Murrin Murrin Airport", "Murrin Murrin"],
  WUM: ["Wasum Airport", "Wasum"],
  WUN: ["Wiluna Airport"],
  WUS: ["Nanping Wuyishan Airport", "Wuyishan"],
  WUT: ["Xinzhou Wutaishan Airport", "Xinzhou"],
  WUU: ["Wau Airport", "Wau"],
  WUV: ["Wuvulu Island Airport", "Wuvulu Island"],
  WUX: ["Sunan Shuofang International Airport", "Wuxi"],
  WUZ: ["Wuzhou Changzhoudao Airport", "Wuzhou"],
  WVB: ["Walvis Bay Airport", "Walvis Bay"],
  WVI: ["Watsonville Municipal Airport", "Watsonville"],
  WVK: ["Manakara Airport"],
  WVL: ["Waterville Robert Lafleur Airport", "Waterville"],
  WVN: ["Wilhelmshaven-Mariensiel Airport", "Wilhelmshaven"],
  WWA: ["Wasilla Airport", "Wasilla"],
  WWD: ["Cape May County Airport", "Wildwood"],
  WWI: ["Woodie Woodie Airport", "Woodie Woodie"],
  WWK: ["Wewak International Airport", "Wewak"],
  WWP: ["Whale Pass Seaplane Float Harbor Facility", "Whale Pass"],
  WWR: ["West Woodward Airport", "Woodward"],
  WWT: ["Newtok Airport", "Newtok"],
  WWY: ["West Wyalong Airport", "West Wyalong"],
  WXN: ["Wanxian Airport", "Wanxian"],
  WYA: ["Whyalla Airport", "Whyalla"],
  WYB: ["Yes Bay Lodge Seaplane Base", "Yes Bay"],
  WYE: ["Yengema Airport", "Yengema"],
  WYN: ["Wyndham Airport"],
  WYS: ["Yellowstone Airport", "West Yellowstone"],
  WZQ: ["Urad Middle Banner", "Urad Middle Banner"],
  XAB: ["Abbeville", "Abbeville"],
  XAL: ["\u00c1lamos Airport", "\u00c1lamos"],
  XAP: ["Serafin Enoss Bertaso Airport", "Chapec\u00f3"],
  XAR: ["Aribinda Airport", "Aribinda"],
  XAU: ["Sa\u00fal Airport", "Sa\u00fal"],
  XBB: ["Blubber Bay Seaplane Base", "Blubber Bay"],
  XBE: ["Bearskin Lake Airport", "Bearskin Lake"],
  XBG: ["Bogande Airport", "Bogande"],
  XBJ: ["Birjand Airport", "Birjand"],
  XBN: ["Biniguni Airport", "Biniguni"],
  XBO: ["Boulsa Airport", "Boulsa"],
  XBR: [
    "Brockville - Thousand Islands Regional Tackaberry Airport",
    "Brockville",
  ],
  XBX: ["Bernay \u2013 St Martin Airport", "Creil"],
  XCH: ["Christmas Island Airport", "Christmas Island"],
  XCL: ["Cluff Lake Airport", "Cluff Lake"],
  XCM: ["Chatham Kent Airport", "Chatham-Kent"],
  XCO: ["Colac Airport"],
  XCR: ["Ch\u00e2lons-Vatry Airport", "Vatry"],
  XCZ: [
    "Charleville-M\u00e9zi\u00e8res Airport",
    "Charleville-M\u00e9zi\u00e8res",
  ],
  XDA: ["Dax Seyresse Airport", "Perigueux"],
  XDE: ["Diebougou Airport", "Diebougou"],
  XDJ: ["Djibo Airport", "Djibo"],
  XEN: ["Xingcheng Air Base"],
  XES: ["Grand Geneva Resort Airport", "Lake Geneva"],
  XFN: ["Xiangyang Liuji Airport", "Xiangfan"],
  XFW: ["Hamburg-Finkenwerder Airport", "Hamburg"],
  XGA: ["Gaoua Airport", "Gaoua"],
  XGG: ["Gorom-Gorom Airport", "Gorom-Gorom"],
  XGN: ["Xangongo Airport", "Xangongo"],
  XGR: ["Kangiqsualujjuaq (Georges River) Airport", "Kangiqsualujjuaq"],
  XIC: ["Xichang Qingshan Airport", "Xichang"],
  XIE: ["Xienglom Airport", "Xienglom"],
  XIG: ["Xinguara Municipal Airport", "Xinguara"],
  XIJ: ["Ahmed Al Jaber Air Base", "Ahmed Al Jaber AB"],
  XIL: ["Xilinhot Airport", "Xilinhot"],
  XIN: ["Xingning Airport", "Xingning"],
  XIQ: ["Ilimanaq Heliport", "Ilimanaq"],
  XIY: ["Xi'an Xianyang International Airport", "Xi'an"],
  XJD: ["Al Udeid Air Base", "Ar Rayyan"],
  XJM: ["Mangla Airport", "Mangla"],
  XKA: ["Kantchari Airport", "Kantchari"],
  XKH: ["Xieng Khouang Airport", "Xieng Khouang"],
  XKS: ["Kasabonika Airport", "Kasabonika"],
  XKY: ["Kaya Airport", "Kaya"],
  XLB: ["Lac Brochet Airport", "Lac Brochet"],
  XLO: ["Long Xuy\u00ean Airport", "Long Xuy\u00ean"],
  XLS: ["Saint Louis Airport", "Saint Louis"],
  XLU: ["Leo Airport", "Leo"],
  XLW: ["Lemwerder Airport", "Lemwerder"],
  XMA: ["Maramag Airport", "Maramag"],
  XMC: ["Mallacoota Airport"],
  XMD: ["Madison Municipal Airport", "Madison"],
  XME: ["Maubeuge-\u00c9lesmes Airport", "Maubeuge/\u00c9lesmes"],
  XMG: ["Mahendranagar Airport", "Mahendranagar"],
  XMH: ["Manihi Airport"],
  XMI: ["Masasi Airport", "Masasi"],
  XML: ["Minlaton Airport"],
  XMN: ["Xiamen Gaoqi International Airport", "Xiamen"],
  XMP: ["Macmillan Pass Airport", "Macmillan Pass"],
  XMS: ["Coronel E Carvajal Airport", "Macas"],
  XMW: ["Montauban Airport", "Montauban"],
  XMY: ["Yam Island Airport", "Yam Island"],
  XNA: [
    "Northwest Arkansas Regional Airport",
    "Fayetteville/Springdale/Rogers",
  ],
  XNG: ["Qu\u1ea3ng Ng\u00e3i Airfield", "Qu\u1ea3ng Ng\u00e3i"],
  XNN: ["Xining Caojiabu Airport", "Xining"],
  XNT: ["Xingtai Dalian Airport", "Xingtai"],
  XNU: ["Nouna Airport", "Nouna"],
  XOG: ["Orange-Caritat (BA 115) Air Base", "Orange/Caritat"],
  XPA: ["Pama Airport", "Pama"],
  XPK: ["Pukatawagan Airport", "Pukatawagan"],
  XPL: ["Coronel Enrique Soto Cano Air Base", "Comayagua"],
  XPP: ["Poplar River Airport", "Poplar River"],
  XPR: ["Pine Ridge Airport", "Pine Ridge"],
  XQC: ["Joint Base Balad", "Balad"],
  XQP: ["Quepos Managua Airport", "Quepos"],
  XQU: ["Qualicum Beach Airport", "Qualicum Beach"],
  XRH: ["RAAF Base Richmond", "Richmond"],
  XRR: ["Ross River Airport", "Ross River"],
  XRY: ["Jerez Airport", "Jerez de la Forntera"],
  XSB: ["Sir Bani Yas Airport"],
  XSC: ["South Caicos Airport"],
  XSD: ["Tonopah Test Range Airport", "Tonopah"],
  XSE: ["Sebba Airport", "Sebba"],
  XSI: ["South Indian Lake Airport", "South Indian Lake"],
  XSO: ["Siocon Airport"],
  XSP: ["Seletar Airport", "Seletar"],
  XTG: ["Thargomindah Airport"],
  XTL: ["Tadoule Lake Airport", "Tadoule Lake"],
  XTO: ["Taroom Airport"],
  XTR: ["Tara Airport"],
  XUZ: ["Xuzhou Guanyin Airport", "Xuzhou"],
  XVL: ["Vinh Long Airfield", "Vinh Long"],
  XVS: ["Valenciennes-Denain Airport", "Valenciennes/Denain"],
  XXN: ["Riyadh Air Base", "Riyadh"],
  XYA: ["Yandina Airport", "Yandina"],
  XYE: ["Ye Airport", "Ye"],
  XYR: ["Edwaki Airport", "Yellow River Mission"],
  XZA: ["Zabr\u00e9 Airport", "Zabr\u00e9"],
  YAA: ["Anahim Lake Airport", "Anahim Lake"],
  YAB: ["Arctic Bay Airport", "Arctic Bay"],
  YAC: ["Cat Lake Airport", "Cat Lake"],
  YAD: ["Moose Lake Airport"],
  YAG: ["Fort Frances Municipal Airport", "Fort Frances"],
  YAH: ["La Grande-4 Airport", "La Grande-4"],
  YAI: ["Gral. Bernardo O\u00b4Higgins Airport", "Chillan"],
  YAJ: ["Lyall Harbour Seaplane Base", "Saturna Island"],
  YAK: ["Yakutat Airport", "Yakutat"],
  YAL: ["Alert Bay Airport", "Alert Bay"],
  YAM: ["Sault Ste Marie Airport", "Sault Ste Marie"],
  YAN: ["Yangambi Airport", "Yangambi"],
  YAO: ["Yaound\u00e9 Airport", "Yaound\u00e9"],
  YAP: ["Yap International Airport", "Yap Island"],
  YAQ: ["Maple Bay Seaplane Base", "Maple Bay"],
  YAR: ["La Grande-3 Airport", "La Grande-3"],
  YAS: ["Yasawa Island Airport", "Yasawa Island"],
  YAT: ["Attawapiskat Airport", "Attawapiskat"],
  YAU: ["Donaldson Airport", "Kattiniq"],
  YAV: ["Mayne Island Seaplane Base", "Miners Bay"],
  YAX: ["Wapekeka Airport", "Angling Lake"],
  YAY: ["St. Anthony Airport", "St. Anthony"],
  YAZ: ["Tofino / Long Beach Airport", "Tofino"],
  YBA: ["Banff Airport", "Banff"],
  YBB: ["Kugaaruk Airport", "Kugaaruk"],
  YBC: ["Baie Comeau Airport", "Baie-Comeau"],
  YBE: ["Uranium City Airport", "Uranium City"],
  YBF: ["Bamfield Seaplane Base", "Bamfield"],
  YBG: ["CFB Bagotville", "Bagotville"],
  YBH: ["Bull Harbour Water Aerodrome", "Bull Harbour"],
  YBI: ["Black Tickle Airport", "Black Tickle"],
  YBJ: ["Baie-Johan-Beetz Water Aerodrome", "Baie-Johan-Beetz"],
  YBK: ["Baker Lake Airport", "Baker Lake"],
  YBL: ["Campbell River Airport", "Campbell River"],
  YBM: ["Bronson Creek Airport", "Bronson Creek"],
  YBO: ["Bob Quinn Lake Airport", "Bob Quinn Lake"],
  YBP: ["Yibin Caiba Airport", "Yibin"],
  YBQ: ["Telegraph Harbour Seaplane Base", "Thetis Island"],
  YBR: ["Brandon Municipal Airport", "Brandon"],
  YBS: ["Opapimiskan Lake Airport", "Opapimiskan Lake"],
  YBT: ["Brochet Airport", "Brochet"],
  YBV: ["Berens River Airport", "Berens River"],
  YBW: ["Bedwell Harbour Seaplane Base", "Bedwell Harbour"],
  YBX: ["Lourdes de Blanc Sablon Airport", "Lourdes-De-Blanc-Sablon"],
  YBY: ["Bonnyville Airport", "Bonnyville"],
  YCA: ["Courtenay Airpark", "Courtenay"],
  YCB: ["Cambridge Bay Airport", "Cambridge Bay"],
  YCC: ["Cornwall Regional Airport", "Cornwall"],
  YCD: ["Nanaimo Airport", "Nanaimo"],
  YCE: ["Centralia / James T. Field Memorial Aerodrome", "Centralia"],
  YCF: ["Cortes Bay Water Aerodrome", "Cortes Bay"],
  YCG: ["Castlegar/West Kootenay Regional Airport", "Castlegar"],
  YCH: ["Miramichi Airport", "Miramichi"],
  YCK: ["Tommy Kochon Airport", "Colville Lake"],
  YCL: ["Charlo Airport", "Charlo"],
  YCM: ["Niagara District Airport", "St Catharines"],
  YCN: ["Cochrane Airport", "Cochrane"],
  YCO: ["Kugluktuk Airport", "Kugluktuk"],
  YCQ: ["Chetwynd Airport", "Chetwynd"],
  YCR: ["Cross Lake (Charlie Sinclair Memorial) Airport", "Cross Lake"],
  YCS: ["Chesterfield Inlet Airport", "Chesterfield Inlet"],
  YCT: ["Coronation Airport", "Coronation"],
  YCU: ["Yuncheng Guangong Airport", "Yuncheng"],
  YCV: ["Cartierville Airport", "Montreal"],
  YCW: ["Chilliwack Airport", "Chilliwack"],
  YCY: ["Clyde River Airport", "Clyde River"],
  YCZ: ["Fairmont Hot Springs Airport", "Fairmont Hot Springs"],
  YDA: ["Dawson City Airport", "Dawson City"],
  YDB: ["Burwash Airport", "Burwash"],
  YDC: ["Drayton Valley Industrial Airport", "Drayton Valley"],
  YDE: ["Paradise River Airport"],
  YDF: ["Deer Lake Airport", "Deer Lake"],
  YDG: ["Digby / Annapolis Regional Airport", "Digby"],
  YDJ: ["Hatchet Lake Airport", "Hatchet Lake"],
  YDL: ["Dease Lake Airport", "Dease Lake"],
  YDN: ["Dauphin Barker Airport", "Dauphin"],
  YDO: ["Dolbeau St Felicien Airport", "Dolbeau-St-F\u00e9licien"],
  YDP: ["Nain Airport", "Nain"],
  YDQ: ["Dawson Creek Airport", "Dawson Creek"],
  YDT: ["Boundary Bay Airport", "Vancouver"],
  YDU: ["Kasba Lake Airport", "Kasba Lake"],
  YDV: ["Bloodvein River Airport", "Bloodvein River"],
  YDW: ["North of Sixty Airport", "Obre Lake"],
  YEB: ["Bar River Airport", "Bar River"],
  YEC: ["Yecheon Airbase", "Yecheon-ri"],
  YEG: ["Edmonton International Airport", "Edmonton"],
  YEH: ["Asaloyeh Airport", "Asaloyeh"],
  YEI: ["Bursa Yeni\u015fehir Airport", "Bursa"],
  YEK: ["Arviat Airport", "Arviat"],
  YEL: ["Elliot Lake Municipal Airport", "Elliot Lake"],
  YEM: ["Manitoulin East Municipal Airport", "Manitowaning"],
  YEN: ["Estevan Airport", "Estevan"],
  YEO: ["RNAS Yeovilton", "Yeovil"],
  YEQ: ["Yenkis(Yankisa) Airport"],
  YER: ["Fort Severn Airport", "Fort Severn"],
  YES: ["Yasouj Airport", "Yasouj"],
  YET: ["Edson Airport", "Edson"],
  YEU: ["Eureka Airport", "Eureka"],
  YEV: ["Inuvik Mike Zubko Airport", "Inuvik"],
  YEY: ["Amos/Magny Airport", "Amos"],
  YFA: ["Fort Albany Airport", "Fort Albany"],
  YFB: ["Iqaluit Airport", "Iqaluit"],
  YFC: ["Fredericton Airport", "Fredericton"],
  YFE: ["Forestville Airport", "Forestville"],
  YFG: ["Fontanges Airport", "Fontanges"],
  YFH: ["Fort Hope Airport", "Fort Hope"],
  YFI: ["Fort Mackay / Firebag", "Suncor Energy Site"],
  YFJ: ["Wekwe\u00e8t\u00ec Airport", "Wekwe\u00e8t\u00ec"],
  YFL: ["Fort Reliance Seaplane Base", "Fort Reliance"],
  YFO: ["Flin Flon Airport", "Flin Flon"],
  YFR: ["Fort Resolution Airport", "Fort Resolution"],
  YFS: ["Fort Simpson Airport", "Fort Simpson"],
  YFX: ["St. Lewis (Fox Harbour) Airport", "St. Lewis"],
  YGA: ["Lac Gagnon Seaplane Base", "Duhamel"],
  YGB: ["Texada Gillies Bay Airport", "Texada"],
  YGC: ["Grande Cache Airport", "Grande Cache"],
  YGE: ["Gorge Harbour Seaplane Base", "Gorge Harbour"],
  YGG: ["Ganges Seaplane Base", "Salt Spring Island"],
  YGH: ["Fort Good Hope Airport", "Fort Good Hope"],
  YGJ: ["Miho Yonago Airport", "Yonago"],
  YGK: ["Kingston Norman Rogers Airport", "Kingston"],
  YGL: ["La Grande Rivi\u00e8re Airport", "La Grande Rivi\u00e8re"],
  YGM: ["Gimli Industrial Park Airport", "Gimli"],
  YGN: ["Greenway Sound Seaplane Base", "Broughton Island"],
  YGO: ["Gods Lake Narrows Airport", "Gods Lake Narrows"],
  YGP: ["Gasp\u00e9 (Michel-Pouliot) Airport", "Gasp\u00e9"],
  YGQ: ["Geraldton Greenstone Regional Airport", "Geraldton"],
  YGR: ["\u00celes-de-la-Madeleine Airport", "\u00celes-de-la-Madeleine"],
  YGT: ["Igloolik Airport", "Igloolik"],
  YGV: ["Havre St Pierre Airport", "Havre St-Pierre"],
  YGW: ["Kuujjuarapik Airport", "Kuujjuarapik"],
  YGX: ["Gillam Airport", "Gillam"],
  YGY: ["Deception Airport"],
  YGZ: ["Grise Fiord Airport", "Grise Fiord"],
  YHA: ["Port Hope Simpson Airport", "Port Hope Simpson"],
  YHB: ["Hudson Bay Airport", "Hudson Bay"],
  YHD: ["Dryden Regional Airport", "Dryden"],
  YHE: ["Hope Airport", "Hope"],
  YHF: ["Hearst Ren\u00e9 Fontaine Municipal Airport", "Hearst"],
  YHH: ["Campbell River Seaplane Base", "Campbell River"],
  YHI: ["Ulukhaktok Holman Airport", "Ulukhaktok"],
  YHK: ["Gjoa Haven Airport", "Gjoa Haven"],
  YHM: ["John C. Munro Hamilton International Airport", "Hamilton"],
  YHN: ["Hornepayne Municipal Airport", "Hornepayne"],
  YHO: ["Hopedale Airport", "Hopedale"],
  YHP: ["Poplar Hill Airport", "Poplar Hill"],
  YHR: ["Chevery Airport", "Chevery"],
  YHS: ["Sechelt-Gibsons Airport", "Sechelt"],
  YHT: ["Haines Junction Airport", "Haines Junction"],
  YHU: ["Montr\u00e9al / Saint-Hubert Airport", "Montr\u00e9al"],
  YHY: ["Hay River / Merlyn Carter Airport", "Hay River"],
  YHZ: ["Halifax / Stanfield International Airport", "Halifax"],
  YIB: ["Atikokan Municipal Airport", "Atikokan"],
  YIC: ["Yichun Mingyueshan Airport", "Yichun"],
  YIE: ["Arxan Yi'ershi Airport", "Arxan"],
  YIF: ["St Augustin Airport", "St-Augustin"],
  YIG: ["Big Bay Seaplane Base", "Stuart Island"],
  YIH: ["Yichang Sanxia Airport", "Yichang"],
  YIK: ["Ivujivik Airport", "Ivujivik"],
  YIN: ["Yining Airport", "Yining"],
  YIO: ["Pond Inlet Airport", "Pond Inlet"],
  YIP: ["Willow Run Airport", "Detroit"],
  YIV: ["Island Lake Airport", "Island Lake"],
  YIW: ["Yiwu Airport", "Yiwu"],
  YJA: ["Jasper Airport", "Jasper"],
  YJF: ["Fort Liard Airport", "Fort Liard"],
  YJN: ["St Jean Airport", "St Jean"],
  YJP: ["Hinton/Jasper-Hinton Airport", "Hinton"],
  YJS: ["Samjiy\u014fn Airport", "Samjiy\u014fn"],
  YJT: ["Stephenville Airport", "Stephenville"],
  YKA: ["Kamloops Airport", "Kamloops"],
  YKC: ["Collins Bay Airport", "Collins Bay"],
  YKD: ["Kincardine Municipal Airport", "Kincardine"],
  YKE: ["Knee Lake Airport", "Knee Lake"],
  YKF: ["Waterloo Airport", "Kitchener"],
  YKG: ["Kangirsuk Airport", "Kangirsuk"],
  YKH: ["Yingkou Lanqi Airport", "Yingkou"],
  YKJ: ["Key Lake Airport", "Key Lake"],
  YKK: ["Kitkatla Seaplane Base", "Kitkatla"],
  YKL: ["Schefferville Airport", "Schefferville"],
  YKM: ["Yakima Air Terminal McAllister Field", "Yakima"],
  YKN: ["Chan Gurney Municipal Airport", "Yankton"],
  YKO: ["Hakkari Y\u00fcksekova Airport", "Hakkari"],
  YKQ: ["Waskaganish Airport", "Waskaganish"],
  YKS: ["Yakutsk Airport", "Yakutsk"],
  YKT: ["Klemtu Water Aerodrome", "Klemtu"],
  YKU: ["Chisasibi Airport", "Chisasibi"],
  YKX: ["Kirkland Lake Airport", "Kirkland Lake"],
  YKY: ["Kindersley Airport", "Kindersley"],
  YKZ: ["Buttonville Municipal Airport", "Toronto"],
  YLB: ["Lac La Biche Airport", "Lac La Biche"],
  YLC: ["Kimmirut Airport", "Kimmirut"],
  YLD: ["Chapleau Airport", "Chapleau"],
  YLE: ["What\u00ec Airport", "What\u00ec"],
  YLG: ["Yalgoo Airport", "Yalgoo"],
  YLH: ["Lansdowne House Airport", "Lansdowne House"],
  YLI: ["Ylivieska Airfield"],
  YLJ: ["Meadow Lake Airport", "Meadow Lake"],
  YLK: ["Barrie-Orillia (Lake Simcoe Regional Airport)", "Barrie-Orillia"],
  YLL: ["Lloydminster Airport", "Lloydminster"],
  YLM: ["Clinton Creek Airport", "Clinton Creek Mine"],
  YLP: ["Mingan Airport", "Mingan"],
  YLQ: ["La Tuque Airport", "La Tuque"],
  YLR: ["Leaf Rapids Airport", "Leaf Rapids"],
  YLS: ["Lebel-sur-Quevillon Airport", "Lebel-sur-Qu\u00e9villon"],
  YLT: ["Alert Airport", "Alert"],
  YLV: ["Yevlakh Airport", "Yevlakh"],
  YLW: ["Kelowna International Airport", "Kelowna"],
  YLY: ["Langley Airport", "Langley"],
  YMA: ["Mayo Airport", "Mayo"],
  YMB: ["Merritt Airport", "Merritt"],
  YMD: ["Mould Bay Airport", "Prince Patrick Is."],
  YME: ["Matane Airport", "Matane"],
  YMG: ["Manitouwadge Airport", "Manitouwadge"],
  YMH: ["Mary's Harbour Airport", "Mary's Harbour"],
  YMJ: ["Moose Jaw Air Vice Marshal C. M. McEwen Airport", "Moose Jaw"],
  YMK: ["Mys Kamenny Airport", "Mys Kamennyi"],
  YML: ["Charlevoix Airport", "Charlevoix"],
  YMM: ["Fort McMurray Airport", "Fort McMurray"],
  YMN: ["Makkovik Airport", "Makkovik"],
  YMO: ["Moosonee Airport", "Moosonee"],
  YMP: ["Port Mcneill Airport", "Port Mcneill"],
  YMS: ["Moises Benzaquen Rengifo Airport", "Yurimaguas"],
  YMT: ["Chapais Airport", "Chibougamau"],
  YMU: ["Mansons Landing Seaplane Base"],
  YMV: ["Mary River Aerodrome"],
  YMW: ["Maniwaki Airport", "Maniwaki"],
  YMX: ["Montreal International (Mirabel) Airport", "Montr\u00e9al"],
  YNA: ["Natashquan Airport", "Natashquan"],
  YNB: ["Prince Abdulmohsin Bin Abdulaziz Airport", "Yanbu"],
  YNC: ["Wemindji Airport", "Wemindji"],
  YND: ["Ottawa / Gatineau Airport", "Gatineau"],
  YNE: ["Norway House Airport", "Norway House"],
  YNG: ["Youngstown Warren Regional Airport", "Youngstown/Warren"],
  YNH: ["Hudsons Hope Airport", "Hudson's Hope"],
  YNJ: ["Yanji Chaoyangchuan Airport", "Yanji"],
  YNL: ["Points North Landing Airport", "Points North Landing"],
  YNM: ["Matagami Airport", "Matagami"],
  YNO: ["North Spirit Lake Airport", "North Spirit Lake"],
  YNP: ["Natuashish Airport", "Natuashish"],
  YNS: ["Nemiscau Airport", "Nemiscau"],
  YNT: ["Yantai Laishan Airport", "Yantai"],
  YNX: ["Snap Lake Airport", "Snap Lake Mine"],
  YNY: ["Yangyang International Airport", "Gonghang-ro"],
  YNZ: ["Yancheng Airport", "Yancheng"],
  YOA: ["Ekati Airport", "Ekati"],
  YOC: ["Old Crow Airport", "Old Crow"],
  YOD: ["CFB Cold Lake", "Cold Lake"],
  YOE: ["Donnelly Airport", "Donnelly"],
  YOG: ["Ogoki Post Airport", "Ogoki Post"],
  YOH: ["Oxford House Airport", "Oxford House"],
  YOI: ["Opinaca Aerodrome", "\u00c9l\u00e9onore Mine"],
  YOJ: ["High Level Airport", "High Level"],
  YOL: ["Yola Airport", "Yola"],
  YON: ["Yongphulla Airport", "Tashigang"],
  YOO: ["Toronto/Oshawa Executive Airport", "Oshawa"],
  YOP: ["Rainbow Lake Airport", "Rainbow Lake"],
  YOS: ["Owen Sound / Billy Bishop Regional Airport", "Owen Sound"],
  YOT: ["Yotvata Airfield", "Yotvata"],
  YOW: ["Ottawa Macdonald-Cartier International Airport", "Ottawa"],
  YOY: ["C J.H.L.(Joe) Lecomte) Heliport", "Valcartier"],
  YPA: ["Prince Albert Glass Field", "Prince Albert"],
  YPB: ["Alberni Valley Regional Airport", "Port Alberni"],
  YPC: ["Paulatuk (Nora Aliqatchialuk Ruben) Airport", "Paulatuk"],
  YPD: ["Parry Sound Area Municipal Airport", "Parry Sound"],
  YPE: ["Peace River Airport", "Peace River"],
  YPG: ["Southport Airport", "Portage la Prairie"],
  YPH: ["Inukjuak Airport", "Inukjuak"],
  YPI: ["Port Simpson Seaplane Base", "Lax Kw'alaams"],
  YPJ: ["Aupaluk Airport", "Aupaluk"],
  YPL: ["Pickle Lake Airport", "Pickle Lake"],
  YPM: ["Pikangikum Airport", "Pikangikum"],
  YPN: ["Port Menier Airport", "Port-Menier"],
  YPO: ["Peawanuck Airport", "Peawanuck"],
  YPQ: ["Peterborough Airport", "Peterborough"],
  YPR: ["Prince Rupert Airport", "Prince Rupert"],
  YPS: ["Port Hawkesbury Airport", "Port Hawkesbury"],
  YPT: ["Pender Harbour Seaplane Base"],
  YPW: ["Powell River Airport", "Powell River"],
  YPX: ["Puvirnituq Airport", "Puvirnituq"],
  YPY: ["Fort Chipewyan Airport", "Fort Chipewyan"],
  YPZ: ["Burns Lake Airport", "Burns Lake"],
  YQA: ["Muskoka Airport", "Muskoka"],
  YQB: ["Quebec Jean Lesage International Airport", "Quebec"],
  YQC: ["Quaqtaq Airport", "Quaqtaq"],
  YQD: ["The Pas Airport", "The Pas"],
  YQF: ["Red Deer Regional Airport", "Red Deer"],
  YQG: ["Windsor Airport", "Windsor"],
  YQH: ["Watson Lake Airport", "Watson Lake"],
  YQI: ["Yarmouth Airport", "Yarmouth"],
  YQJ: ["April Point Seaplane Base", "Quadra Island"],
  YQK: ["Kenora Airport", "Kenora"],
  YQL: ["Lethbridge County Airport", "Lethbridge"],
  YQM: ["Greater Moncton International Airport", "Moncton"],
  YQN: ["Nakina Airport", "Nakina"],
  YQQ: ["Comox Airport", "Comox"],
  YQR: ["Regina International Airport", "Regina"],
  YQS: ["St Thomas Municipal Airport", "St Thomas"],
  YQT: ["Thunder Bay Airport", "Thunder Bay"],
  YQU: ["Grande Prairie Airport", "Grande Prairie"],
  YQV: ["Yorkton Municipal Airport", "Yorkton"],
  YQW: ["North Battleford Airport", "North Battleford"],
  YQX: ["Gander International Airport", "Gander"],
  YQY: ["Sydney / J.A. Douglas McCurdy Airport", "Sydney"],
  YQZ: ["Quesnel Airport", "Quesnel"],
  YRA: ["Rae Lakes Airport", "Gam\u00e8t\u00ec"],
  YRB: ["Resolute Bay Airport", "Resolute Bay"],
  YRC: ["Refuge Cove Seaplane Base", "Desolation Sound"],
  YRD: ["Dean River Airport", "Kimsquit Valley"],
  YRF: ["Cartwright Airport", "Cartwright"],
  YRG: ["Rigolet Airport", "Rigolet"],
  YRI: ["Rivi\u00e8re-du-Loup Airport", "Rivi\u00e8re-du-Loup"],
  YRJ: ["Roberval Airport", "Roberval"],
  YRL: ["Red Lake Airport", "Red Lake"],
  YRM: ["Rocky Mountain House Airport", "Rocky Mountain House"],
  YRN: ["Rivers Inlet Seaplane Base", "Rivers Inlet"],
  YRO: ["Ottawa / Rockcliffe Airport", "Ottawa"],
  YRQ: ["Trois-Rivi\u00e8res Airport", "Trois-Rivi\u00e8res"],
  YRR: ["Stuart Island Airstrip", "Big Bay"],
  YRS: ["Red Sucker Lake Airport", "Red Sucker Lake"],
  YRT: ["Rankin Inlet Airport", "Rankin Inlet"],
  YRV: ["Revelstoke Airport", "Revelstoke"],
  YSA: ["Sable Island Landing Strip", "Sable Island"],
  YSB: ["Sudbury Airport", "Sudbury"],
  YSC: ["Sherbrooke Airport", "Sherbrooke"],
  YSD: ["Suffield Heliport", "Suffield"],
  YSE: ["Squamish Airport", "Squamish"],
  YSF: ["Stony Rapids Airport", "Stony Rapids"],
  YSG: ["Lutselk'e Airport", "Lutselk'e"],
  YSH: ["Smiths Falls-Montague (Russ Beach) Airport", "Smiths Falls"],
  YSI: [
    "Parry Sound/Frying Pan Island-Sans Souci Seaplane Base",
    "Frying Pan Island",
  ],
  YSJ: ["Saint John Airport", "Saint John"],
  YSK: ["Sanikiluaq Airport", "Sanikiluaq"],
  YSL: ["St Leonard Airport", "St Leonard"],
  YSM: ["Fort Smith Airport", "Fort Smith"],
  YSN: ["Shuswap Regional Airport", "Salmon Arm"],
  YSO: ["Postville Airport", "Postville"],
  YSP: ["Marathon Airport", "Marathon"],
  YSQ: ["Songyuan Chaganhu Airport", "Qian Gorlos Mongol Autonomous County"],
  YST: ["St. Theresa Point Airport", "St. Theresa Point"],
  YSU: ["Summerside Airport", "Summerside"],
  YSV: ["Saglek Airport", "RCAF Station Saglek"],
  YSX: ["Bella Bella/Shearwater Seaplane Base", "Bella Bella"],
  YSY: [
    "Sachs Harbour (David Nasogaluak Jr. Saaryuaq) Airport",
    "Sachs Harbour",
  ],
  YTA: ["Pembroke Airport", "Pembroke"],
  YTB: ["Hartley Bay Seaplane Base", "Hartley Bay"],
  YTD: ["Thicket Portage Airport", "Thicket Portage"],
  YTE: ["Cape Dorset Airport", "Cape Dorset"],
  YTF: ["Alma Airport", "Alma"],
  YTG: ["Sullivan Bay Seaplane Base", "Sullivan Bay"],
  YTH: ["Thompson Airport", "Thompson"],
  YTJ: ["Terrace Bay Airport", "Terrace Bay, ON"],
  YTL: ["Big Trout Lake Airport", "Big Trout Lake"],
  YTM: [
    "La Macaza / Mont-Tremblant International Inc Airport",
    "Rivi\u00e8re Rouge",
  ],
  YTP: ["Tofino Harbour Seaplane Base", "Tofino Harbour"],
  YTQ: ["Tasiujaq Airport", "Tasiujaq"],
  YTR: ["CFB Trenton", "Trenton"],
  YTS: ["Timmins/Victor M. Power", "Timmins"],
  YTT: ["Tisdale Airport", "Tisdale"],
  YTU: ["Tasu Water Aerodrome", "Tasu"],
  YTX: ["Telegraph Creek Airport", "Telegraph Creek"],
  YTY: ["Yangzhou Taizhou Airport", "Yangzhou and Taizhou"],
  YTZ: ["Billy Bishop Toronto City Centre Airport", "Toronto"],
  YUA: ["Yuanmou Air Base", "Yuanmou"],
  YUB: ["Tuktoyaktuk Airport", "Tuktoyaktuk"],
  YUD: ["Umiujaq Airport", "Umiujaq"],
  YUE: ["Yuendumu Airport"],
  YUJ: ["Lady Franklin Point Airport"],
  YUL: [
    "Montreal / Pierre Elliott Trudeau International Airport",
    "Montr\u00e9al",
  ],
  YUM: ["Yuma MCAS/Yuma International Airport", "Yuma"],
  YUS: ["Yushu Batang Airport", "Yushu"],
  YUT: ["Repulse Bay Airport", "Repulse Bay"],
  YUX: ["Hall Beach Airport", "Hall Beach"],
  YUY: ["Rouyn Noranda Airport", "Rouyn-Noranda"],
  YVA: ["Iconi Airport", "Moroni"],
  YVB: ["Bonaventure Airport", "Bonaventure"],
  YVC: ["La Ronge Airport", "La Ronge"],
  YVD: ["Yeva Airport", "Yeva"],
  YVE: ["Vernon Airport", "Vernon"],
  YVG: ["Vermilion Airport", "Vermilion"],
  YVM: ["Qikiqtarjuaq Airport", "Qikiqtarjuaq"],
  YVO: ["Val-d'Or Airport", "Val-d'Or"],
  YVP: ["Kuujjuaq Airport", "Kuujjuaq"],
  YVQ: ["Norman Wells Airport", "Norman Wells"],
  YVR: ["Vancouver International Airport", "Vancouver"],
  YVT: ["Buffalo Narrows Airport", "Buffalo Narrows"],
  YVV: ["Wiarton Airport", "Wiarton"],
  YVZ: ["Deer Lake Airport", "Deer Lake"],
  YWA: ["Petawawa Airport", "Petawawa"],
  YWB: ["Kangiqsujuaq (Wakeham Bay) Airport", "Kangiqsujuaq"],
  YWG: [
    "Winnipeg / James Armstrong Richardson International Airport",
    "Winnipeg",
  ],
  YWH: ["Victoria Harbour Seaplane Base", "Victoria"],
  YWJ: ["D\u00e9line Airport", "D\u00e9line"],
  YWK: ["Wabush Airport", "Wabush"],
  YWL: ["Williams Lake Airport", "Williams Lake"],
  YWM: ["Williams Harbour Airport", "Williams Harbour"],
  YWN: ["Winisk Airport", "Winisk"],
  YWO: ["Lupin Airport", "Lupin Mine"],
  YWP: ["Webequie Airport", "Webequie"],
  YWQ: ["Chute-Des-Passes/Lac Margane Seaplane Base", "Chutes-des-Passes"],
  YWR: ["White River Seaplane Base", "White River"],
  YWS: ["Whistler/Green Lake Water Aerodrome", "Whistler"],
  YWY: ["Wrigley Airport", "Wrigley"],
  YXC: ["Cranbrook/Canadian Rockies International Airport", "Cranbrook"],
  YXD: ["Edmonton City Centre (Blatchford Field) Airport", "Edmonton"],
  YXE: ["Saskatoon John G. Diefenbaker International Airport", "Saskatoon"],
  YXH: ["Medicine Hat Airport", "Medicine Hat"],
  YXI: ["Bonnechere Airport", "Killaloe"],
  YXJ: ["Fort St John Airport", "Fort St.John"],
  YXK: ["Rimouski Airport", "Rimouski"],
  YXL: ["Sioux Lookout Airport", "Sioux Lookout"],
  YXN: ["Whale Cove Airport", "Whale Cove"],
  YXP: ["Pangnirtung Airport", "Pangnirtung"],
  YXQ: ["Beaver Creek Airport", "Beaver Creek"],
  YXR: ["Earlton (Timiskaming Regional) Airport", "Earlton"],
  YXS: ["Prince George Airport", "Prince George"],
  YXT: ["Northwest Regional Airport Terrace-Kitimat", "Terrace"],
  YXU: ["London Airport", "London"],
  YXX: ["Abbotsford Airport", "Abbotsford"],
  YXY: ["Whitehorse / Erik Nielsen International Airport", "Whitehorse"],
  YXZ: ["Wawa Airport", "Wawa"],
  YYA: ["Sanhe Airport", "Yueyang"],
  YYB: ["North Bay Jack Garland Airport", "North Bay"],
  YYC: ["Calgary International Airport", "Calgary"],
  YYD: ["Smithers Airport", "Smithers"],
  YYE: ["Fort Nelson Airport", "Fort Nelson"],
  YYF: ["Penticton Airport", "Penticton"],
  YYG: ["Charlottetown Airport", "Charlottetown"],
  YYH: ["Taloyoak Airport", "Taloyoak"],
  YYI: ["Rivers Airport"],
  YYJ: ["Victoria International Airport", "Victoria"],
  YYL: ["Lynn Lake Airport", "Lynn Lake"],
  YYM: ["Cowley Airport", "Cowley"],
  YYN: ["Swift Current Airport", "Swift Current"],
  YYQ: ["Churchill Airport", "Churchill"],
  YYR: ["Goose Bay Airport", "Goose Bay"],
  YYS: ["Blissville Airport", "Blissville"],
  YYT: ["St. John's International Airport", "St. John's"],
  YYU: ["Kapuskasing Airport", "Kapuskasing"],
  YYW: ["Armstrong Airport", "Armstrong"],
  YYY: ["Mont Joli Airport", "Mont-Joli"],
  YYZ: ["Lester B. Pearson International Airport", "Toronto"],
  YZA: ["Cache Creek-Ashcroft Regional Airport", "Cache Creek"],
  YZD: ["Downsview Airport", "Toronto"],
  YZE: ["Gore Bay Manitoulin Airport", "Gore Bay"],
  YZF: ["Yellowknife Airport", "Yellowknife"],
  YZG: ["Salluit Airport", "Salluit"],
  YZH: ["Slave Lake Airport", "Slave Lake"],
  YZP: ["Sandspit Airport", "Sandspit"],
  YZR: ["Chris Hadfield Airport", "Sarnia"],
  YZS: ["Coral Harbour Airport", "Coral Harbour"],
  YZT: ["Port Hardy Airport", "Port Hardy"],
  YZU: ["Whitecourt Airport", "Whitecourt"],
  YZV: ["Sept-\u00celes Airport", "Sept-\u00celes"],
  YZW: ["Teslin Airport", "Teslin"],
  YZX: ["CFB Greenwood", "Greenwood"],
  YZY: ["Zhangye Ganzhou Airport", "Zhangye"],
  YZZ: ["Trail Airport", "Trail"],
  ZAA: ["Alice Arm/Silver City Seaplane Base"],
  ZAC: ["York Landing Airport", "York Landing"],
  ZAD: ["Zadar Airport", "Zemunik (Zadar)"],
  ZAG: ["Zagreb Airport", "Zagreb"],
  ZAH: ["Zahedan International Airport", "Zahedan"],
  ZAJ: ["Zaranj Airport", "Zaranj"],
  ZAL: ["Pichoy Airport", "Valdivia"],
  ZAM: ["Zamboanga International Airport", "Zamboanga City"],
  ZAO: ["Cahors-Lalbenque Airport", "Cahors/Lalbenque"],
  ZAR: ["Zaria Airport", "Zaria"],
  ZAT: ["Zhaotong Airport", "Zhaotong"],
  ZAZ: ["Zaragoza Air Base", "Zaragoza"],
  ZBE: ["Zabreh Ostrava Airport", "Zabreh"],
  ZBF: ["Bathurst Airport", "Bathurst"],
  ZBK: ["\u017dabljak Airport", "\u017dabljak Airport"],
  ZBM: ["Bromont (Roland Desourdy) Airport", "Bromont"],
  ZBO: ["Bowen Airport"],
  ZBR: ["Konarak Airport", "Chabahar"],
  ZBY: ["Sayaboury Airport", "Sainyabuli"],
  ZCL: ["General Leobardo C. Ruiz International Airport", "Zacatecas"],
  ZCN: ["Celle Airport"],
  ZCO: ["La Araucan\u00eda Airport", "Temuco"],
  ZDY: ["Delma Airport", "Delma Island"],
  ZEC: ["Secunda Airport", "Secunda"],
  ZEG: ["Senggo Airport", "Senggo-Papua Island"],
  ZEL: ["Bella Bella (Campbell Island) Airport", "Bella Bella"],
  ZEM: ["Eastmain River Airport", "Eastmain River"],
  ZEN: ["Zenag Airport", "Zenag"],
  ZER: ["Ziro Airport", "Ziro"],
  ZFA: ["Faro Airport", "Faro"],
  ZFD: ["Fond-Du-Lac Airport", "Fond-Du-Lac"],
  ZFM: ["Fort Mcpherson Airport", "Fort Mcpherson"],
  ZFN: ["Tulita Airport", "Tulita"],
  ZFU: ["Unifly Heliport", "Aruj\u00e1"],
  ZFW: ["Fairview Airport", "Fairview"],
  ZGF: ["Grand Forks Airport", "Grand Forks"],
  ZGI: ["Gods River Airport", "Gods River"],
  ZGL: ["South Galway Airport"],
  ZGM: ["Ngoma Airport", "Ngoma"],
  ZGR: ["Little Grand Rapids Airport", "Little Grand Rapids"],
  ZGS: ["La Romaine Airport", "La Romaine"],
  ZGU: ["Gaua Island Airport", "Gaua Island"],
  ZHA: ["Zhanjiang Airport", "Zhanjiang"],
  ZHI: ["Grenchen Airport", "Grenchen"],
  ZHM: ["Shamshernagar Airport", "Shamshernagar"],
  ZHP: ["High Prairie Airport", "High Prairie"],
  ZHY: ["Zhongwei Shapotou Airport", "Zhongwei"],
  ZIA: ["Zhukovsky International Airport", "Moscow"],
  ZIC: ["Victoria Airport", "Victoria"],
  ZIG: ["Ziguinchor Airport", "Ziguinchor"],
  ZIH: ["Ixtapa Zihuatanejo International Airport", "Ixtapa"],
  ZIN: ["Interlaken Air Base"],
  ZIS: ["Alzintan Airport", "Zintan"],
  ZIX: ["Zhigansk Airport", "Zhigansk"],
  ZIZ: ["Zamzama Heliport", "Zamzama Gas Field"],
  ZJG: ["Jenpeg Airport", "Jenpeg"],
  ZJI: ["Locarno Airport", "Locarno"],
  ZJN: ["Swan River Airport", "Swan River"],
  ZKB: ["Kasaba Bay Airport", "Kasaba Bay"],
  ZKE: ["Kashechewan Airport", "Kashechewan"],
  ZKG: ["Kegaska Airport", "Kegaska"],
  ZKL: ["Zigong Airport", "Zigong"],
  ZKM: ["Sette Cama Airport", "Sette Cama"],
  ZKP: ["Zyryanka Airport", "Zyryanka"],
  ZLG: ["La G\u00fcera Airport", "La G\u00fcera"],
  ZLO: ["Playa De Oro International Airport", "Manzanillo"],
  ZLR: ["Municipal de Linares Airport", "Linares"],
  ZLT: ["La Tabati\u00e8re Airport", "La Tabati\u00e8re"],
  ZLX: ["Zalingei Airport", "Zalingei"],
  ZMD: ["Sena Madureira Airport", "Sena Madureira"],
  ZMG: ['Magdeburg "City" Airport', "Magdeburg"],
  ZMH: ["South Cariboo Region / 108 Mile Airport", "108 Mile"],
  ZMM: ["Zamora Airport"],
  ZMT: ["Masset Airport", "Masset"],
  ZNA: ["Nanaimo Harbour Water Airport", "Nanaimo"],
  ZNC: ["Nyac Airport", "Nyac"],
  ZND: ["Zinder Airport", "Zinder"],
  ZNE: ["Newman Airport", "Newman"],
  ZNZ: ["Abeid Amani Karume International Airport", "Zanzibar"],
  ZOF: ["Ocean Falls Seaplane Base", "Ocean Falls"],
  ZOS: ["Ca\u00f1al Bajo Carlos - Hott Siebert Airport", "Osorno"],
  ZPB: ["Sachigo Lake Airport", "Sachigo Lake"],
  ZPC: ["Puc\u00f3n Airport", "Pucon"],
  ZPH: ["Zephyrhills Municipal Airport", "Zephyrhills"],
  ZPO: ["Pinehouse Lake Airport", "Pinehouse Lake"],
  ZQL: ["Donaueschingen-Villingen Airport", "Donaueschingen"],
  ZQN: ["Queenstown International Airport", "Queenstown"],
  ZQW: ["Zweibr\u00fccken Airport", "Zweibr\u00fccken"],
  ZQZ: ["Zhangjiakou Ningyuan Airport", "Zhangjiakou"],
  ZRH: ["Z\u00fcrich Airport", "Zurich"],
  ZRI: ["Serui Airport", "Serui-Japen Island"],
  ZRJ: ["Round Lake (Weagamow Lake) Airport", "Round Lake"],
  ZRM: ["Sarmi Airport", "Sarmi-Papua Island"],
  ZRZ: ["Zaraza Airport", "Zaraza"],
  ZSA: ["San Salvador Airport", "San Salvador"],
  ZSE: ["Pierrefonds Airport", "St Pierre"],
  ZSJ: ["Sandy Lake Airport", "Sandy Lake"],
  ZSP: ["St. Paul Airport", "St. Paul"],
  ZSS: ["Sassandra Airport", "Sassandra"],
  ZST: ["Stewart Airport", "Stewart"],
  ZSW: ["Prince Rupert/Seal Cove Seaplane Base"],
  ZTA: ["Tureia Airport"],
  ZTB: ["T\u00eate-\u00e0-la-Baleine Airport", "T\u00eate-\u00e0-la-Baleine"],
  ZTH: [
    'Zakynthos International Airport "Dionysios Solomos"',
    "Zakynthos Island",
  ],
  ZTM: ["Shamattawa Airport", "Shamattawa"],
  ZTR: ["Zhytomyr Airport"],
  ZTS: ["Tahsis Seaplane Base", "Tahsis"],
  ZTU: ["Zaqatala International Airport", "Zaqatala"],
  ZUC: ["Ignace Municipal Airport", "Ignace"],
  ZUD: ["Pupelde Airport", "Ancud"],
  ZUH: ["Zhuhai Jinwan Airport", "Zhuhai"],
  ZUL: ["Zilfi Airport", "Zilfi"],
  ZUM: ["Churchill Falls Airport", "Churchill Falls"],
  ZVA: ["Miandrivazo Airport"],
  ZVG: ["Springvale Airport", "Springvale"],
  ZVK: ["Savannakhet Airport"],
  ZWA: ["Andapa Airport"],
  ZWL: ["Wollaston Lake Airport", "Wollaston Lake"],
  ZXT: ["Zabrat Airport", "Baku"],
  ZYI: ["Zunyi Xinzhou Airport", "Zunyi"],
  ZYL: ["Osmany International Airport", "Sylhet"],
  ZZO: ["Zonalnoye Airport", "Tymovskoye"],
  ZZU: ["Mzuzu Airport", "Mzuzu"],
  ZZV: ["Zanesville Municipal Airport", "Zanesville"],
};

const AIRPORT_CODES = new Map(Object.entries(RAW_AIRPORT_CODES));
